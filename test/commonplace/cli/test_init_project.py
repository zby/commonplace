...
def test_init_project_seeds_scaffold_files(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "kb" / "types" / "instruction.template.md").is_file()
    assert (tmp_path / "kb" / "types" / "instruction.instructions.md").is_file()
    assert (tmp_path / "kb" / "types" / "instruction.schema.yaml").is_file()
    assert (tmp_path / "kb" / "reports" / "collection-topology.md").is_file()
