...
def test_list_collection_note_paths_skips_nested_repos_and_type_dirs(tmp_path: Path) -> None:
    ...
    kept = write(collection / "kept.md")
    nested = write(collection / "vendor" / "repo" / "ignored.md")
    (nested.parent / ".git").mkdir()
    template = write(collection / "types" / "note.template.md")
    nested_template = write(collection / "definitions" / "types" / "definition.template.md")

    discovered = project_paths.list_collection_note_paths(collection)
...
def test_resolve_note_raises_on_ambiguous_match(tmp_path: Path) -> None:
    ...
    write(tmp_path / "kb" / "reports" / "sample.md")
