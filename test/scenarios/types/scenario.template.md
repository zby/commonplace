...
- **overhead** — read only because the work is inside this framework (AGENTS.md, the collection's COLLECTION.md, a type-spec, a skill body, a dir-index / area index, a validate run). Give a real path so the harness can measure its bytes.
...
- **spared** — content this fork does *not* load because the framework gave it a cheaper surface (a dir-index line instead of a note body, a frontloaded answer, a distilled artifact). Record as a credit, not a load.
