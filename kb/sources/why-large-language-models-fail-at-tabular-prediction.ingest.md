...
The explanation study adds behavioral evidence for [the difference between legibility and faithfulness](../notes/revision-guided-by-rationale-needs-faithfulness-not-just-legibility.md), but it is weaker than the causal intervention in [Turpin et al.](language-models-dont-always-say-what-they-think.ingest.md): it compares an executable stated rule with observed task behavior rather than identifying which factor caused a prediction. Relative to [Paulsen's effective-context-window study](paulsen-maximum-effective-context-window-mecw.ingest.md) and [ConvexBench](convexbench-can-llms-recognize-convex-functions.ingest.md), this source isolates a different load variable. The three sources together distinguish token volume, dependency depth, and multi-coordinate integration, without yet establishing that these are independent internal mechanisms.
