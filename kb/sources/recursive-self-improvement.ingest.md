...
On the source's account, improvement comes from an agent proposing changes to code, algorithms, skills, model behavior, or architecture; evaluating those changes through game feedback, tests, reward signals, or performance metrics; and retaining candidates that score better. That account maps most directly to proposal selection and operative retention. It does not by itself establish [conjectural learning](../notes/definitions/conjectural-learning.md): the article does not require a formulated theory whose content guides decisions and receives formulated criticism, and several examples instead fit non-localized weight fitting or retention of successful work products.
