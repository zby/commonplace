...
- [Treat continual learning as representational-form coevolution](../notes/treat-continual-learning-as-representational-form-coevolution.md) — **evidence**; the compact-patch-to-prior framing and the industry pattern of using the text layer as supervision for weight updates.
- [trace-learning-techniques-in-related-systems](../agent-memory-systems/trace-learning-techniques-in-related-systems.md) — **evidence**; cites Reflexion, Trace, GEPA, Meta-Harness, etc. — the same systems the survey catalogues.
...
3. **Kolmogorov / description-length justification for text sample-efficiency** -- "good text updates are compact patches to a pretrained world prior" supplies an information-theoretic account of why small text-layer updates can efficiently reshape behavior without immediately changing weights. [just-a-reference]
...
7. **"Architecture research" / design-space call** -- the observation that instruction hierarchies, DSPy programs, agent skills, harnesses, and memory systems are points in one under-described design space we lack vocabulary for. Parallels (at the meta-level) the KB's `agent-orchestration-occupies-a-multi-dimensional-design-space` and the agent-memory survey's purpose; the connect report routed this to the trace-learning survey rather than the orchestration note. [just-a-reference]
