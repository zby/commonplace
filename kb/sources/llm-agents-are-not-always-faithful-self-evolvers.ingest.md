...
The KB already integrates this paper deeply — the companion connect report found a **reverse-edge-only** result (the snapshot authors no outbound edges; the signal is which notes should point *at* it). Three notes already cite the paper by bare arXiv URL and should upgrade to labelled local `evidenced-by` edges: [distillation](../notes/definitions/distillation.md) (uses it as the warning case that compressed experience can stay plausible yet lose behavioral influence), [evaluate-memory-by-effects](../notes/agent-memory-requirements/evaluate-memory-by-effects.md) (direct support for "evaluate by effects, not existence" plus the WITH/WITHOUT method), and [activate-behavior-changing-memory](../notes/agent-memory-requirements/activate-behavior-changing-memory.md) (its Behavioral Faithfulness section already names this paper). Three new `evidenced-by` candidates: [knowledge-storage-does-not-imply-contextual-activation](../notes/knowledge-storage-does-not-imply-contextual-activation.md) (the §5.2 IG result is a clean instance of "context-to-action failure" — knowledge visible in context but not integrated — and is the strongest new fit), [continual-learning-open-problem-is-behaviour-not-knowledge](../notes/continual-learning-requires-governing-behaviour-changing-writes.md) (accumulation succeeds but behaviour-change does not), and a weaker [evolving-understanding-needs-re-distillation-not-composition](../notes/evolving-understanding-needs-holistic-rewrite-not-composition.md). Two cross-collection edges were deferred to a future `agent-memory-systems`-scoped run: the reviews of [Dynamic Cheatsheet](../agent-memory-systems/reviews/dynamic-cheatsheet.md) and [Agent Workflow Memory](../agent-memory-systems/reviews/agent-workflow-memory.md), both systems this paper benchmarks and finds unfaithful to condensed experience. Net: this source is the KB's central empirical anchor for the distillation/activation/evaluate-by-effects cluster — v3 reconfirms it at larger backbone scale.
