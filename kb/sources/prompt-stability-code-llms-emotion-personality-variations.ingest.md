...
- [bitter-lesson-boundary](../notes/fixed-artifacts-split-into-exact-specs-and-proxy-theories.md) — **extends**: the non-monotonic scaling finding (smaller Qwen-1.5B more stable than larger models) provides evidence for the core distinction. Stability may not follow the same scaling patterns as performance, suggesting it is closer to requiring explicit optimization rather than emerging from scale.
