...
This mechanism partly maps to Commonplace's [conjectural learning](../notes/definitions/conjectural-learning.md). AERA's localized executable models have consequences that experience can contradict and parts that can be retained or removed. Reliability revision and deletion revise an existing model set, while induction of a new model from observation may instead add knowledge rather than refine a prior theory. The paper shows that this mixed process produced operative behavior in one constrained setting. It does not isolate which mechanism caused the performance, compare against an alternative learner, or show repeated improvement of the learning process itself.
