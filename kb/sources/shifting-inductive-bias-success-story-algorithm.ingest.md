...
In Commonplace terms, the policy is an operative retained result, and SSA supplies reject-capable evaluation. The evidence supports adaptation of search bias and, in IS, an implemented path for adapting the modification process. It is not a [theory builder](../notes/definitions/theory-builder.md). Individually editable policy probabilities provide repair locations, but a probability states nothing that criticism could argue with, so condition 1 fails for the retained policy, although the programs it generates are written in a formal language. The learning signal scores behavior by reward rate rather than testing consequences derived from a stated theory, so rollback is trial and error and condition 3 fails. The retained policy carries across tasks, but the success history records rewards, not criticism, and is not demonstrated explanatory knowledge about why each change worked.
