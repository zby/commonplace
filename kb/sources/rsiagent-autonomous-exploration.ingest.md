...
Several reported episodes meet the [theory-builder](../notes/definitions/theory-builder.md) conditions: a stated interpretation in memory (condition 1) guides action (condition 2), criticism bears on its content (condition 3), and the revised account is taken up on a later, different task (condition 4). T049's actor initially treats preservation requirements as forbidding box movement; its failure record identifies that interpretation, a synthetic repair tests a revised procedure, and a later target run consults the retained records. T085's audio practices expose a rendering discrepancy, prompt revision of resampling advice, and precede reuse of that setting. Practice tasks differ from the targets, so retention crosses problems, not only rounds on one. Membership is supported at the process level. Learning is a separate claim: the historical score gains support improved performance on the practiced targets, but do not isolate how much was caused by the retained explanations rather than extra attempts, concrete solutions, or other retained procedures.
