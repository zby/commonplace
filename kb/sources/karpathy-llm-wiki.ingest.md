...
- **The index-centric design inherits a maintenance burden.** The gist explicitly makes `index.md` load-bearing for navigation. It therefore inherits the failure mode in [stale indexes reduce discovery when they suppress fallback search](../notes/stale-indexes-reduce-discovery-when-they-suppress-fallback-search.md): if the drifting map still appears exhaustive, it can close a more current discovery route rather than merely omit items.
