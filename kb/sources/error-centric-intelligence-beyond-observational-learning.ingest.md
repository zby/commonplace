...
The paper is a conceptual comparator for [learning by theory refinement](../notes/definitions/learning-by-theory-refinement.md): both require criticism to affect retained knowledge, but Thomas emphasizes changing what a learner can represent and test, while the KB specifies a recurrent causal path through an addressable theory. Its unreachable-error question sharpens [learning inside a fixed decomposition](../notes/learning-inside-a-fixed-decomposition-inherits-its-mistakes.md) by naming intervention semantics and invariance assumptions as possible revision targets. Neither connection supplies measured evidence that the proposed loop works.
...
This partly maps onto [theory refinement](../notes/definitions/theory-refinement.md). Revising separately editable mechanisms against failures fits the operation; introducing an entirely new representational apparatus may exceed a local repair while remaining part of the broader inquiry. Thomas's explanatory knowledge has stronger conditions than the KB's tentative theory: it must concern mechanisms and support counterfactual or interventional reasoning. The source's broad definition of learning also permits durable internal state without the KB's requirement for an addressable theory outside model weights. Neither term should be substituted for the other.
...
Review [learning by theory refinement](../notes/definitions/learning-by-theory-refinement.md) against a single explicit question: does the paradigm require its representation and criticism machinery to be revisable, or is that a separate capability to assess for each theory builder?
