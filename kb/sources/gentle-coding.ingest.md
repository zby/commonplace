...
This report supersedes the prior 2026-06-05 ingest, which covered only the June README. It now covers three repository documents read together as one source (the Gentle-Coding repo): the refreshed README, the original Proof-of-Concept report (previously folded into the README as an "old section", now split out), and the RESEARCH.md literature index (new).

## Classification

The README (`gentle-coding.md`) is `code-repository` but behaves as a practitioner report plus prompt-pattern proposal. The PoC (`gentle-coding-proof-of-concept.md`) is `practitioner-report` — a small, self-run empirical study. RESEARCH.md (`gentle-coding-research.md`) is `code-repository`, functioning as an annotated bibliography. The load-bearing claims live in the README and the PoC; RESEARCH.md is a citation list.
