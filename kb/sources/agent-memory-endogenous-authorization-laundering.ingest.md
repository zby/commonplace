...
snapshot_sha256: 1bb64f3dfecbf436bf896230a1c9a21f8d11306fcbc130b713fcd66ecb096c1b
type: ingest-report
...
The mechanism is fixed-model adaptation through persistent state: a writer updates its account of permissions as new history arrives, and that account changes later action. Against the [theory-builder](../notes/definitions/theory-builder.md) conditions, the account is stated in free text or typed records (condition 1), and the exact-state intervention establishes that later action depends on what it says (condition 2). The profile is kept and consumed on later, different requests (condition 4). Criticism (condition 3) fails on the described design, and it decides the verdict: writers fold each new block into prior memory, and reviewers choose among candidate memories by fit. Neither process attempts to refute what a stored grant says, and the tested incremental trajectories show no self-repair. Replacing state with the experimenter's oracle is an intervention from outside the [boundary](../notes/definitions/theory-builder.md#boundary), not the system's own criticism. No learning claim arises: the paper measures fidelity loss during adaptation, not improved capacity.
