...
Against the [theory-builder](../notes/definitions/theory-builder.md) conditions, the harness code is localized content (condition 1), and inner runs consume it (condition 2). The outer agent states reasons for its rewrites, including diagnosed failures, and its evaluator repair names a specific flaw. That shows content-directed criticism in selected cases, although acceptance itself is argmax selection on the grade over whole variants; condition 3 is shown only at that trace strength. Condition 4 decides the verdict. The main run revises one harness against one fixed selection suite and grade, so its 99 steps are one pass of error elimination on one problem. The held-out benchmarks evaluate frozen checkpoints without further criticism or revision. The ignition test reuses an evolved agent as the improver on the same optimization problem from a fresh start, and the paper leaves promotion to the outer loop as a proposed rule, not a performed step. Nothing the run keeps is taken up on a later, different problem by the system's own process, so the system is outside. The capacity gains are a separate learning claim. The inactive selection penalty also shows that a retained rationale does not establish that the rationale governs the successful behavior.
