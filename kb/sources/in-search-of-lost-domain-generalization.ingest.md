...
5. **An unverified selection process makes an outcome check look like a process check** -- The DG literature abstracted rule-level claims ("this algorithm generalizes") from outcome checks (reported accuracy) while the process producing them went unverified; declaring the process dissolved the rule. That is [an outcome check licenses replay; a rule needs the process verified](../notes/checked-outcome-licenses-episode-retention-not-abstraction.md) restated as a benchmarking requirement, and the paper is a large-scale worked instance of it. [experiment]
...
- [Learning by learning by theory refinement may improve sample efficiency under structured shifts](../notes/learning-by-theory-refinement-may-improve-sample-efficiency.md) — is-evidence-for: this paper is the shift-difficulty caveat that paragraph already rests on
