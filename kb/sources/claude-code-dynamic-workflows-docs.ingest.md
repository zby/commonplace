...
Write the note flagged in the connect report's synthesis opportunity: **a structured-claim note framing dynamic workflows as the saveable, script-authored counterpoint to RLM's ephemeral orchestrators** — the deployed realization of the promote-recurring-`select` / keep-run-state-ephemeral split that `orchestration-strategies-and-run-state-have-opposite-persistence.md` predicts but currently lacks a real example for. Draft it from both dynamic-workflow snapshots (this docs capture and the X-article harness capture), cite this snapshot and the RLM/chat-history notes as `evidenced-by`, and add the `compares-with` cross-reference between the two snapshots at that time. If a full note is not yet warranted, the minimal alternative is a log entry plus the reverse-edge `evidenced-by` citations into the RLM/persistence cluster.
