...
**Productive tensions.** The source partially contradicts [symbolic-scheduling-over-bounded-llm-calls](../notes/bounded-context-orchestration-model.md) — predicting fixed schedulers will be dissolved — though the resolution may be that the model survives while specific implementations don't. It partially contradicts [methodology-enforcement-is-constraining](../notes/methodology-enforcement-is-constraining.md) by predicting baked-in workflows become less useful with continual learning, though the source's own surviving elements (filesystem, spawning) are themselves structural constraints.
