...
In Commonplace's terms, the reported training is parametric learning, and the predictive component is not a [theory builder](../notes/definitions/theory-builder.md). Its theory is encoder, projector, and predictor weights. Factors can be read and masked, but no factor says anything identifiable, so condition 1 fails; gradient training also fails condition 3. At the broader researcher–model–experiment boundary, the biological case shows a stated hypothesis nominated and tested externally. One test of a stated consequence does not by itself show a working process of criticism. The paper does not document a revision that followed or a next conjecture shaped by the result, so conditions 3 and 4 at that boundary remain unestablished, not failed.
