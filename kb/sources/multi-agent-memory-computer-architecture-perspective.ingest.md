...
- [three-space-agent-memory-maps-to-tulving-taxonomy](../notes/three-space-agent-memory-echoes-tulvings-taxonomy-but-the-analogy-may-be-decorative.md) — **extends**: Two independent decompositions of agent memory from different traditions (cognitive science vs. computer architecture). Together they suggest a two-axis taxonomy: content type (what kind of knowledge) x hierarchy level (access speed/capacity).
