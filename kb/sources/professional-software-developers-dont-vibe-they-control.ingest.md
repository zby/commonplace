---
description: Empirical study (N=112) finding experienced developers control AI agents through SE practices, not vibe coding -- grounds stabilisation, underspecification, and programming-practices-transfer arguments
...
- **[stabilisation](../notes/stabilisation.md)** -- *exemplifies*: developer control strategies (user rules, plan files, testing, version control) map directly onto the stabilisation spectrum from partial narrowing to full commitment.
- **[methodology-enforcement-is-stabilisation](../notes/methodology-enforcement-is-stabilisation.md)** -- *exemplifies*: Table 2 shows an enforcement gradient in practice -- instruction-layer rules through structured prompts to verification hooks.
...
- **[stabilisation-during-deployment-is-continuous-learning](../notes/stabilisation-during-deployment-is-continuous-learning.md)** -- *exemplifies*: developer control strategies are empirical evidence that stabilisation during deployment constitutes continuous learning.
...
3. **Developer control strategies as empirical stabilisation taxonomy** -- Table 2 documents the specific mechanisms (plan files, context files, user rules, iterative prompting, testing, version control, code review) with participant-level detail. These map onto the stabilisation spectrum but have not been systematically aligned. A synthesis note could argue that developers independently rediscover stabilisation. [deep-dive]
...
- **Strategies described, not evaluated.** Plan files, user rules, and context files are documented as things developers do, but the paper doesn't assess which strategies are more effective than others, or under what conditions. The KB's stabilisation spectrum offers a theoretical ordering, but this paper provides no evidence for or against that ordering. See [stabilisation](../notes/stabilisation.md) for the theoretical framework that would need empirical testing.
...
Write a note titled "Experienced developers independently rediscover the stabilisation gradient" connecting to [stabilisation](../notes/stabilisation.md), [methodology-enforcement-is-stabilisation](../notes/methodology-enforcement-is-stabilisation.md), and [programming-practices-apply-to-prompting](../notes/programming-practices-apply-to-prompting.md). The note would argue: the paper's Table 2 documents developers progressing from user rules (convention) through plan files (structured artifact) to testing and version control (commitment/verification) -- which is the stabilisation spectrum observed in the wild. The evidence is behavioral, not self-reported taxonomy, which makes it stronger than if developers were merely naming the concepts. The note should also flag the key limitation: the ordering is inferred from cross-sectional observation, not from longitudinal tracking of individual developers' strategy evolution.
