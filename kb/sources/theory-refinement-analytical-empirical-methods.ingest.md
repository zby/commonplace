...
Update [Conjectural learning has distinct precedents](../notes/theory-building-has-distinct-precedents.md) to replace its direct DOI citation with a `(snapshot required)` link to this ingest and expand the EITHER case with the proof-guided repair mechanism, fixed Horn update space, and absence of reflective or later-learning evidence.
