...
snapshot_sha256: 01c84e201aa23e93b67e985ec28024e977dedfc49520b7c42fad6a703b0f99b6
ingested: "2026-09-25"
...
For Commonplace, the key distinction is between improving execution of knowledge and criticizing that knowledge. In the ACT/GRAPES model, productions are formally stated rules (condition 1), fire through what their conditions say (condition 2), and persist to later problems (condition 4). Compilation and strengthening fail criticism (condition 3), which decides the verdict for them: composition packs a successful trace into a shorter rule, and strengthening adjusts a numeric value, so neither aims at what a production says. A learner changed only by these operations is not a [theory builder](../notes/definitions/theory-builder.md). The report's revised account of induction is closer to condition 3. It assigns generalization and discrimination to strategic problem solving using semantic knowledge, rather than to automatic comparison of rule syntax. Conscious rule formulation and correction are relevant evidence for human learners, but the report does not show criticism of stated rules as a working process for each learner, so for the full learner condition 3 stays unestablished. Learning, as faster performance on practiced tasks, is reported and attributed to compilation, not to criticism.
