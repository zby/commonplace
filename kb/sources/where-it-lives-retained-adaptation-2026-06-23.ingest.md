...
**Post-submission framing delta (September 2026).** The position has since acquired a framing the paper does not contain: it is now introduced as a small informal ontology that classifies retained artifacts by their role in a pattern of use — a relational property of the artifact-in-use, not an intrinsic property of the stored object — with prior storage-first practice characterized as a pragmatic default inherited from the software component list (component names became concept names). The paper as submitted argues the four-field vocabulary without this ontological framing, so cite it for the vocabulary and the corpus argument, not for the framing. The framing currently lives only in the ASISAS talk preparation in the workshop layer; if it survives the talk, it is a candidate note (relational classification by consumption path).
