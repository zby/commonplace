...
The May 2026 ingest of the pre-corpus draft (since removed; this report is the sole tracked ingest of the paper) had one main follow-up, and it has already been promoted: the sovereignty refinement now exists as [the risk-triad note](../notes/the-four-field-record-exposes-an-efficiency-security-and-sovereignty.md). The live connection is no longer "write the sovereignty note"; it is "make the living matrix synthesis carry the within-substrate evidence that the paper now exposes." Reverse note-to-paper links should still wait until the paper is accepted or otherwise externally citable; a self-authored draft should not be cited through an independent `evidenced-by` edge from notes it came from.

**Post-submission deltas (September 2026).** Three developments postdate the submitted paper; cite it for the four-field vocabulary and the corpus argument, not for these. First, the category the paper calls "prose" has been renamed **natural-language**, because the form's boundary is interpretive rather than editorial — fragments, facts, and structured records qualify without being expository prose. Second, the three representational forms are no longer stipulated as a list but derived from two axes — whether a defined consumer assigns the consequences, and whether a localized unit carries the content — with the fourth cell (assigned consequences, non-localized) unoccupied rather than impossible, and the read/test/probe inspection rule falling out of the axes. Both are registered in [the representational-form definition](../notes/definitions/representational-form.md), which the paper's terminology now lags. Third, the position is now introduced as a small informal ontology that classifies retained artifacts by their role in a pattern of use — a relational property of the artifact-in-use, not an intrinsic property of the stored object — with prior storage-first practice characterized as a pragmatic default inherited from the software component list (component names became concept names). Unlike the first two, this framing has no durable home yet: it lives only in the ASISAS talk preparation in the workshop layer, and if it survives the talk it is a candidate note (relational classification by consumption path).
...
4. **Old sovereignty follow-up is closed** -- The removed May 2026 ingest's recommended action was to mine the sovereignty axis back into notes. That has happened in [the risk-triad note](../notes/the-four-field-record-exposes-an-efficiency-security-and-sovereignty.md), so any current action should not repeat it. [quick-win]
