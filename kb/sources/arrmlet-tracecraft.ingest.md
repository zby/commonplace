...
**Index membership:** [related-systems-index](../notes/related-systems/related-systems-index.md) — tracecraft would be the first entry focused purely on coordination infrastructure rather than memory/knowledge management.
...
**Race condition in claiming.** The `claim` primitive appears to check whether a step is already claimed and then write a claim file. On S3, this is not atomic. Two agents calling `claim design` simultaneously could both succeed, violating the isolation guarantee that is the system's primary coordination mechanism. The README does not acknowledge this. The [coordination-guarantees note](../notes/agent-orchestration-needs-coordination-guarantees-not-just-coordination-channels.md) predicts exactly this class of failure.
