...
The mapping to [conjectural learning](../notes/definitions/conjectural-learning.md) is partial. Programs have executable consequences and editable parts, and the framework permits copy-editing. The demonstrated transfer searches for a new task-solving program using retained code as bias; it does not show retained code open to criticism of what it says. A successful program is frozen and later consumed as search bias, and candidates are selected by verification outcome without a formulated reason for failure bearing on their content. Addressability and self-directed search changes alone do not establish conjectural learning.
