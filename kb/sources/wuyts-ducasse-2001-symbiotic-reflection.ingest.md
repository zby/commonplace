...
- **Claim (paraphrase):** In the paper's SOUL/Smalltalk symbiotic-reflection design, a logic language and its object-oriented implementation language can reason about and act on each other; an explicit upping/downing transfer schema maps Smalltalk objects to SOUL terms and terms back to objects so the two levels can manipulate one another's entities.
  - **Source extract (verbatim):** New to this approach is that any element of the implementation language can be reasoned about and acted upon (not only the self representation), and that both languages are of different paradigms. Moreover, every language implementer that is faced with the problem of allowing the base language to access the underlying meta-language has to solve the problem of enabling entity transfer between both worlds. We propose a uniform schema, called upping/downing, to this problem that avoid explicit wrapping or typechecking.
  ...
  - **Source extract (verbatim):** Enabling the access and manipulation of down level structure (the object-oriented programming language) from the up level (the logic programming language) in a uniﬁed way is possible by following the simple transfer rule: upping a down entity should return an upped entity and downing an upped entity should return a down entity. Applied to SOUL, this rule reads: upping an object should return a term and downing a term should return an object.
  - **Source location:** §3.2, “The Upping/Downing Schema” (printed p. 7; PDF p. 7)
  - **Scope:** The mechanism and bidirectional capabilities are demonstrated for SOUL, a logic language, and Smalltalk, the object-oriented language in which it is implemented.
  - **Confidence:** High for the authors' architecture and transfer rule; the proposition is bounded to their symbiotic-reflection construction.
  - **Limitation:** Bidirectional reasoning and action depend on the full reflective integration, not entity transfer alone. The paper does not establish that any pair of representational forms gains reflective coverage merely by exchanging entities.
...
Use this snapshot as the primary early-source anchor for [Cross-representational reflection](../notes/reflective-coverage-is-graded-across-representational-forms.md), explicitly testing each retained claim against causal connection, mutual access and action, entity transfer, and the stated system boundary.
