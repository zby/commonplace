...
Against the [theory-builder definition](../notes/definitions/theory-builder.md), candidate programs meet condition 1, and within a repair episode execution results guide the next edit (condition 2). Code repair meets condition 3 within an episode: execution exposes a failure in what a program does, and a model edits that program. Pure selection uses conjectures without criticizing their content, and extraction recovery changes their usable expression. Condition 4 fails and decides the negative verdict: each task's operators run on that task alone, so repair rounds are one pass of error elimination, and nothing stated is retained and taken up on a later task. The offline fitted verifiers and routers carry across tasks, but as fitted models they fail condition 1. Frozen generator weights alone would not preclude a builder working through external artifacts; such retention is untested here. Accordingly, this is an evaluation of local search and correction operations, not of a theory builder. The paper also does not demonstrate improved future capacity; that learning claim stays separate.
