...
snapshot_sha256: cec54a6e885b2f72fc3abc9ffc0469dc5765648f03642dc5f10ef2d666e9b1a2
...
Against the [theory-builder definition](../notes/definitions/theory-builder.md), candidate programs meet condition 1, and within a repair episode execution results guide the next edit (condition 2). Code repair meets condition 3 within an episode: execution exposes a failure in what a program does, and a model edits that program. Pure selection uses conjectures without criticizing their content, and extraction recovery changes their usable expression. Condition 4 (iteration) is met by repair: the execution failure is kept and shapes the next edit, and the edited program is executed again. The repair configurations are therefore inside the term at a low persistence grade, the rounds of one repair episode on one task; nothing stated carries to a later task. Pure selection and extraction recovery are outside, decided by condition 3. The offline fitted verifiers and routers carry across tasks, but as fitted models they fail condition 1. Frozen generator weights alone would not preclude a builder working through external artifacts at a higher grade; such retention is untested here. The paper also does not demonstrate improved future capacity; that learning claim stays separate.

The important constraint is the [effective update space](../notes/learning-inside-a-fixed-decomposition-inherits-its-mistakes.md). Selectors operate on supplied candidate pools; repair methods draw from the same frozen generator under a bounded call budget; public tests and hand-designed probes constrain what errors can guide deployment. The nulls show limits of the tested compound configurations, not that all useful distinctions are mathematically absent from the prompt and code. M1 varies one consequential boundary—the conversion from raw response to runnable program—and succeeds especially where the standard extractor loses many candidates. This supports measuring representation losses separately from reasoning failures. It does not establish that this decomposition is best for learning, or require a revision of Commonplace's theory-builder definition.
