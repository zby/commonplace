...
The hypothesis–test–revision relations partially map onto [theory refinement](../notes/definitions/theory-refinement.md). Explicit hypotheses can be criticizable objects, and a contradiction followed by a revised hypothesis can document an episode of error correction. The graph alone does not establish separately editable theoretical premises, preservation of prior useful consequences, or later reuse of a retained revision. Absence of those measurements leaves the recurrent learning claim open. This source contributes diagnostic questions for the theory-refinement account, not evidence that the full operation is absent from all scientific agents.
