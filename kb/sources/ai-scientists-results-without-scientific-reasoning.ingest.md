...
The hypothesis–test–revision relations partially map onto the [theory-builder definition](../notes/definitions/theory-builder.md). Explicit hypotheses meet condition 1. Condition 2 is unestablished: the graph alone does not establish that hypothesis content caused later decisions. Condition 3 is partly shown within episodes: a contradiction followed by a revised hypothesis documents attempted refutation of a stated claim, although the annotated rate is low and the annotations carry calibration doubts. Condition 4 (iteration) is met where a revision occurs: the contradiction stays in the conversation and the revised hypothesis responds to it. Persistence is at the lowest grade, within one episode; nothing crosses tasks, and injected prefixes come from the same task. The arrangement stays unclassified, and condition 2 decides: without evidence that hypothesis content drives later decisions, the stated hypotheses may be commentary. Improved capacity for future action is a separate learning claim the source does not test, and finer addressability or preservation of prior useful consequences is not established either. This source contributes diagnostic questions for the theory-builder account, not evidence that the full arrangement is absent from all scientific agents.
