...
Ashby is the cybernetic **precedent** for the second loop — an adaptive system that changes its own organization when ordinary regulation fails, through a loop distinct from the one that regulates. The two-loop structure is exactly the distinction between operating a system and modifying it that the [self-improving system](../notes/definitions/self-improving-system.md) definition builds on. Under that definition the Homeostat classifies as **non-reflective, direct viability-driven self-improvement**. It is not an instance of the [proposal-selection improvement loop](../notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md): the machine's one evidence-responsive transition — out of limits, jump — both discards the incumbent configuration and produces its successor, so rejection is not an operation distinct from generation, and persistence is equilibrium rather than a verdict. The loop's functions collapse into a single trigger, which is that note's exclusion criterion.

The Homeostat does admit a functional variation–selection–retention reading — configurations vary, viability determines whether variation continues, the survivor persists — but that mapping onto the loop vocabulary is an analyst's **reconstruction**, and it marks the *floor* rather than the archetype: variation is a random draw, the viability boundary is one-bit and ranks nothing, and retention is equilibrium — the absence of a force to change further. Ashby did not decompose adaptation into generator and oracle, and translating him as though he had would credit him with distinctions he explicitly did without.

Two things follow, and the second is the load-bearing one. Ultrastability shows how *little* machinery adaptation requires, and thereby what the extra machinery of a search-and-evaluate architecture is actually buying. And it shows what a **self-representation** buys, which is the line between this and a *reflective* [self-improving system](../notes/definitions/self-improving-system.md). On the broad definition the Homeostat is itself a self-improving system — its own organization changes under direct viability-driven adaptation, with no human anywhere — occupying the category's minimal, non-reflective, fully autonomous corner.

The Homeostat genuinely retains: the surviving configuration persists and steers later behavior, which makes the retention operative. But it is non-cumulative under [the informational criterion that counts only dependence through the retained result](../notes/accumulation-counts-dependence-through-the-retained-result.md). Once a violation triggers reorganization, the new values depend on the next random-table entries, not on the retained setting; holding the violation and table position fixed, substituting another incumbent leaves the successor unchanged. The failure signal triggers replacement but carries no retained information about earlier successful settings into it, and advancing through a random table records trial count rather than what worked. The process can therefore retain without accumulating. What it retains is also an opaque parameter setting rather than a representation: nothing in the mechanism can read that setting, say why it was good, criticize it, or carry it to the next problem. Ashby is the clean demonstration that adaptation can retain without accumulating, while [routing an improvement pathway through a self-representation makes retention addressable](../notes/reflection-buys-addressability.md).

The gap is most instructive at retention. Ashby's retention criterion is *negative* — a configuration survives because it stops the reorganization, not because an evaluator endorsed it. That is a real alternative to oracle-gated retention, and it is worth holding next to the [oracle-strength spectrum](../notes/oracle-strength-spectrum.md), which grades evaluators but does not contemplate an architecture with no evaluator at all. It also contrasts instructively with the [Gödel machine](../notes/goedel-machines-are-a-proof-governed-case-of-self-modification.md), the opposite extreme: retention licensed only by proof. Ashby retains anything that stops hurting; Schmidhuber retains only what is proved to help. The contrast is architectural, not a difference of gate strength: the Gödel machine runs an explicit proof-mediated proposal-selection loop, while the Homeostat has no gate at all.
