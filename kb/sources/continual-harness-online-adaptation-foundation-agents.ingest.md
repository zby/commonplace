...
6. **Direct adoption is the main governance gap** -- Refiner edits become operative on the next step without a candidate comparison, regression gate, or explicit rollback rule, and the observed create-and-forget tail shows that live revision alone does not supply selection or retirement. A deploy-time harness loop needs a separate answer for when an edit earns persistence. [experiment]
