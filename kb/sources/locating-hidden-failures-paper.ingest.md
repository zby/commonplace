...
In Commonplace terms, this is parametric learning followed by application of the learned verifier. It is not demonstrated [conjectural learning](../notes/definitions/conjectural-learning.md): the report identifies no formulated theory criticized for what it says with a resulting improvement in future capacity. The acting agent's weights remain unchanged during candidate selection, and choosing among finished runs does not establish that the agent subsequently learns from its own mistakes. The paper presents that further loop as a prospect.
