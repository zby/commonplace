...
4. **The source supplies context-bound support for an earned-complexity boundary.** The author uses a file wiki for everyday per-project research and reserves graph memory for high-precision mining across larger corpora. That is compatible with [files beating databases early](../notes/files-not-database.md), but it does not establish the sharper structural trigger in [churning many-to-many edge state](../notes/many-to-many-edge-state-is-where-files-yield-to-a-database.md). [just-a-reference]
