...
Our interpretation is that the platform, with its agents and human auditors, is a [theory builder](../notes/definitions/theory-builder.md). Theorem statements and proof-sketches are stated in Lean (condition 1), and imported statements guide later proof work (condition 2). The Gotsman–Linial episode shows criticism aimed at what a statement says: a disproof, then a replacement that adds the missing boundary condition (condition 3). Proved theorems enter a shared library that the paper says accumulates across missions and is available to any later proof-sketch in any mission, so retained results are taken up on later, different problems (condition 4, shown by design and reported reuse). The theorem's explicit hypotheses make the repair [addressable](../notes/definitions/addressable-theory.md). Immutability of an individual stored statement does not prevent criticism or replacement by a new statement. Learning is a separate claim: the paper does not establish improvement in later mission performance, nor isolate the benefit of retaining criticism from that of simply adding a usable lemma.
