...
This supplies a plausible partial mechanism for [conjectural learning](../notes/definitions/conjectural-learning.md): formulated claims guide action and can be corrected against evidence. The benchmark gains establish improvement for the compound controller; they do not isolate criticism of claims as the cause of improved later capacity. The correction example and extraction audit do not close that causal gap. Within-task duration alone is no reason to exclude the mechanism, but neither cross-task retention nor persistent theory development is demonstrated.

The mutable graph and outline sit inside a fixed entity/concept representation, extraction policy, search-chain rules and writing interface. As [learning inside a fixed decomposition](../notes/learning-inside-a-fixed-decomposition-inherits-its-mistakes.md) predicts, successful adaptation inside those choices does not establish their general adequacy. Outline-driven queries provide an additional route beyond the current graph; consequently, graph omissions alone do not establish that a topic is unreachable by the whole system. The source supports investigating editable research state, while leaving the conjectural-learning classification and broader representation choice unsettled.
