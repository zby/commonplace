...
- [Codification](../notes/definitions/codification.md) (exemplifies) — The cognify phase converts natural-language documents into structured graph triplets with Pydantic schemas shaping the extraction target, crossing into a symbolic artifact with formal structure.

- [three-space-agent-memory-maps-to-tulving-taxonomy](../notes/three-space-agent-memory-echoes-tulvings-taxonomy-but-the-analogy.md) (grounds) — Cognee partially maps to the three-space model (cognify produces semantic knowledge, temporal_cognify edges toward episodic) but has no structural separation between memory types.

- [flat-memory-predicts-specific-cross-contamination-failures-that-are-empirically-testable](../notes/flat-memory-predicts-specific-cross-contamination-failures-that-are.md) (exemplifies) — Concrete system with no three-space separation, a candidate for observing predicted failure modes (search pollution, identity scatter) as data volume grows.
...
- [Agent memory needs discoverable, composable, trusted knowledge under bounded context](../notes/agent-memory-needs-discoverable-composable-trusted-knowledge-under.md) (contrasts) — Knowledge is discoverable (search types, embeddings) but not composable in the KB sense (no link semantics, no claim structure, no resolution-switching) and only structurally trustworthy (schema-validated but not epistemically grounded).
