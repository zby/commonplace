...
Relative to the [theory-builder definition](../notes/definitions/theory-builder.md), EEC expectations are stated in a named formalism and are meant to guide action, so conditions 1 and 2 are plausible on the authors' description, though the capture does not show either in operation. Condition 4 (iteration) is unestablished: the abstract says expectations are updated, not that updates follow from criticism and shape the next conjecture; retaining expectations for reuse bears on the persistence grade, not on membership. Condition 3 decides: membership needs a working process that criticizes what an expectation says, and the abstract describes updating without showing such criticism, so the condition is unestablished rather than failed. Whether the process improves future capacity is a separate learning claim the abstract also leaves open. Candidate repair locations and separately editable parts concern finer [addressability](../notes/definitions/theory-builder.md#addressability), a design commitment above condition 1's minimum, not membership. Our epistemic reading therefore separates extracting reusable consequences from an internal model from correcting that model's mistaken claims. Simulation can expose consequences of assumptions without independently testing their truth. The Popperian name alone does not resolve which operation occurs here.

The explicit proposed changes are to expectations. The abstract does not say whether the simulator, EEC vocabulary, admissible actions, or test criteria can change. Following [learning inside a fixed decomposition inherits its mistakes](../notes/learning-inside-a-fixed-decomposition-inherits-its-mistakes.md), this leaves the effective update space unknown; it does not demonstrate that these choices are fixed or revisable. Retaining expectations suggests a possible learning path without changing model weights, but no fixed-weight model condition is reported. Finally, the authors' reflective-reasoning claim does not establish the [reflective qualifier](../notes/definitions/theory-builder.md#qualifiers): the abstract describes action-consequence knowledge, without showing revision of a causally connected representation of the agent's own organization. This source identifies a relevant architecture to investigate, rather than requiring a change to our definitions.
...
Obtain and ingest the full paper as a distinct observation to resolve whether independent environmental errors revise retained expectations or the simulator, for comparison with condition 3 of the [theory-builder definition](../notes/definitions/theory-builder.md).
...
The same authors presented a follow-up, *Popperian Expectations for One-Shot Adaptation in Dynamic Environments*, at the ACSOS 2026 workshops ([conference listing](https://2026.acsos.org/details/acsos-2026-workshops/16/Popperian-Expectations-for-One-Shot-Adaptation-in-Dynamic-Environments)). The listing, the [SISSY programme](https://sissy-workshop.github.io/), and [author news](https://www.trustworthyai.ca/news/john-ronika-nathan-and-peter-present-at-acsos-2026/) confirm title, authorship, and presentation only. The public programme repository had no PDF, and Crossref resolved no exact record. No mechanism or outcome claim is assessable from that record, so it has no ingest; a full-text capture would be a distinct observation.
