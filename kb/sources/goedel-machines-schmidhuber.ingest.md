...
2. **Meta-level collapse answers a standing open question, in principle.** [A proposal-selection improvement loop requires search, evaluation, and operative retention](../notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md) asks whether a system can represent its own search and evaluation processes as modifiable aspects. Section 4.3 answers yes for this construction and supplies the mechanism: a target-theorem proof necessarily quantifies over the self-changes the present one enables, so the regress closes at the acceptance criterion rather than in a tower of meta-levels (printed p. 12; PDF p. 13). The answer is formal, not demonstrated — no Gödel machine has been built. [deep-dive]
3. **Gated goal change is a hard-oracle answer to goal drift.** The utility function and target theorem are themselves rewritable, but only on a proof of benefit *under the current* utility function (printed p. 16; PDF p. 17; FAQ 7, printed p. 23; PDF p. 24). For [a methodology governs its own extension only as far as it settles the meta-decisions it raises](../notes/a-methodology-governs-its-own-extension-only-as-far-as-it-settles.md) this is an existence proof that a system's own acceptance rule can govern rewrites of that acceptance rule — and it names the price. [experiment]
...
- [A proposal-selection improvement loop requires search, evaluation, and operative retention](../notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md) — is-evidence-for: the Gödel machine instantiates search (proof-technique enumeration), evaluation (target-theorem proof), and retention (`check()` → `switchprog`) separably and formally, and its meta-level collapse bears on the note's open question
- [Reflective system](../notes/definitions/reflective-system.md) — is-evidence-for: the maximal-intercession case, and the system that shows provable self-improvement is additive to reflection rather than implied by it
- [Oracle strength spectrum](../notes/oracle-strength-spectrum.md) — is-evidence-for: a proof-of-utility oracle is the limit case at the hard end, and shows a search guarantee (no local maxima) is a distinct dividend of oracle strength
- [The boundary of automation is the boundary of verification](../notes/the-boundary-of-automation-is-the-boundary-of-verification.md) — is-evidence-for: a constructive rather than observational route to the claim — self-modification is licensed by verification and cannot proceed without it
- [A methodology governs its own extension only as far as it settles the meta-decisions it raises](../notes/a-methodology-governs-its-own-extension-only-as-far-as-it-settles.md) — is-evidence-for: gated goal change (rewrite `u` only if provably better under the old `u`) is an existence proof of the fully closed extreme, with its price made explicit
- [Behavioral authority](../notes/definitions/behavioral-authority.md) — is-evidence-for: consumer, channel, and force are wired by construction, giving a negative control for the open-loop failure mode
