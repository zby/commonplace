...
- [first-principles reasoning selects for explanatory-reach over adaptive fit](../notes/first-principles-reasoning-selects-for-explanatory-reach-over.md) — evidence: the four-part negative test's criticizability criterion gets its first behavioural correlate
- [mechanistic constraints make Popperian KB recommendations actionable](../notes/mechanistic-constraints-make-popperian-kb-recommendations-actionable.md) — evidence: bears on the note's premise that criticism must be structural because it will not happen ambiently
- [discovery lifecycle](../notes/definitions/discovery-lifecycle.md) — evidence: first LLM-side measurement of the consequence-derivation to test transition
- [reach-assessment](../notes/definitions/reach-assessment.md) — evidence: the prose route to reach judgment varies measurably across models
- [conjecture is seeing the particular as an instance of the general](../notes/conjecture-is-seeing-the-particular-as-an-instance-of-the-general.md) — evidence: turn-level data on how the posit-and-recognize loop breaks
