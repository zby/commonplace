...
**Moderate connections:** specification-level-separation (functional/technical split maps to the spec-level separation framework), decomposition-heuristics-for-bounded-context-scheduling (task breakdown follows bounded-context scheduling rules), programming-practices-apply-to-prompting (SDD systematizes SE practices for agent instruction), [Selecting an LLM output fixes a result, not its interpretation](../notes/selecting-an-llm-output-fixes-a-result-not-its-interpretation.md) (sharpens — a checked-in spec or plan fixes which version guides implementation without making its language unambiguous), [spec-mining-as-codification](../notes/spec-mining-as-codification.md) (extends — SDD names how to constrain upfront, but not how recurring discoveries later harden into deterministic helpers, checks, or generated paths), writing-styles-as-underspecification-management (spec format is a prescriptive writing style for maximal constraint), legal-drafting parallel (Given/When/Then is contract-like), and two sources — professional-software-developers-dont-vibe-they-control (empirical backing) and harness-engineering (complementary production evidence).
