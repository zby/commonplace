...
snapshot_sha256: 37818993dacf91b7a45cedda36be797eeaf692b4efd1118fc647c11577125b13
ingested: "2026-08-24"
...
The source is a primary account of the human proximal-cue/distal-content structure used in the KB's navigation cluster. [Agents navigate by deciding what to read next](../notes/agents-navigate-by-deciding-what-to-read-next.md) now cites it as a narrower human analogue and separately states the local follow/skip abstraction, LLM transfer, and token/tool-call cost boundary. The chapter supports cue-based source selection, but it does not establish that follow/skip is the fundamental unit of navigation or compose cue quality and interaction cost into the target's pointer-level trade-off. Its strongest additional role is to ground and bound the human side of [linking theory](../notes/linking-theory.md): the chapter supplies a value-over-interaction-cost tendency, an activation-plus-logit operationalization, and a protocol fit, while the note changes the priced resource to bounded LLM context. The source also sharpens [link-following and search](../notes/link-following-and-search-impose-different-metadata-requirements.md) by giving a causal direction to its two-mode split, and it supplies a bounded human case of cue-based candidate discovery for [the end-to-end knowledge-access synthesis](../notes/knowledge-access-architecture-must-be-evaluated-end-to-end.md). It does not establish that synthesis's LLM-agent transfer or whole-path conclusion. Two notes gain captured prior work in place of recalled prior work: [information value is observer-relative](../notes/information-value-is-observer-relative.md), whose utility term is goal-conditioned by construction here, and [soft-bound traditions](../notes/soft-bound-traditions-as-sources-for-context-engineering-strategies.md), whose twelve-row table omits the tradition that fits its own (processor, bound, mechanism) schema most directly. The countervailing role is a limitation: the population is 14 human students on the 1998–2003 Web paying wall-clock time, so every edge into an LLM-agent claim needs the transfer argument that [human-LLM differences are load-bearing](../notes/human-llm-differences-are-load-bearing-for-knowledge-system-design.md) demands, and this source cannot supply it.
...
Use this ingest as the bounded human baseline when adjudicating [link-following and search](../notes/link-following-and-search-impose-different-metadata-requirements.md). If the mode-switching use survives, retain its exact source passage or mark the route `(snapshot required)`. That note's document-surrogate and search-result claims still need their separately assigned sources before its artifact disposition can be decided.
...
- [Agents navigate by deciding what to read next](../notes/agents-navigate-by-deciding-what-to-read-next.md) — is-evidence-for: supplies the narrower human cue-selection analogue; the target adds the LLM transfer and context-cost boundary
...
- [Knowledge-access architecture must be evaluated end to end, not by retrieval alone](../notes/knowledge-access-architecture-must-be-evaluated-end-to-end.md) — is-evidence-for: supplies a bounded human case of cue-based candidate discovery, not the whole-path agent synthesis
