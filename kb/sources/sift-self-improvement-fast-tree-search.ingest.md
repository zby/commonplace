...
There is a plausible [conjectural-learning](../notes/definitions/conjectural-learning.md) pathway beyond outcome-only selection. The judge is instructed to explain its ranking first, and reported explanations criticize particular implementation commitments: a verifier disabled by default, or a tool that claims persistent shell state while starting a fresh shell on each call. Those criticisms bear on what the program says and can affect which branches receive work. The paper also reports retained prompt and tool revisions that improve later task behavior. However, it does not isolate the contribution of formulated diagnoses or criticism from proposing, scoring, and selecting code variants. The evidence supports a mechanism compatible with conjectural learning, without establishing that a particular diagnosis caused the measured gain.
