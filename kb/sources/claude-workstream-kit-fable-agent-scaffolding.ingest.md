...
- **Claim (paraphrase):** In the author's Claude Workstream Kit account, conversation history, generic agent memory, and session-local task tracking do not carry the project-scoped live-work ledger; the kit instead stores that state in two git-versioned files with an explicit resume pointer and closure criteria.
  ...
  - **Scope:** One tool author's design account for multi-session Claude Code project work and the announced Workstream Kit; the comparison is among the specific storage surfaces named in the post.
  - **Confidence:** High that the post makes this design distinction and specifies these two files; confidence in comparative operational benefit is limited to the author's report.
  - **Limitation:** The post does not compare against every retrospective-memory or chat-history design, and its implementation and lifecycle guarantees require separate code-grounded inspection.

- **Claim (paraphrase):** In the author's Fable rebuild, principle-level instruction following made multi-phase checklists, compliance scripts, and rule-synchronization layers unnecessary, while the retained kit still used cited-evidence closure and human-authority constraints.
  ...
  - **Scope:** A first-person comparison of one predecessor system with a Fable-oriented rebuild plus self-reported acceptance tests for that kit.
  - **Confidence:** High that the author reports the artifact removals, replacement rule, and authority constraints; moderate for the behavioral result because no protocol or comparative measurements are supplied in the post.
  - **Limitation:** The report does not hold task difficulty, model generation, or deployment conditions constant, and it does not establish that the retained controls are universally necessary or that over-prescription generally degrades stronger models.
