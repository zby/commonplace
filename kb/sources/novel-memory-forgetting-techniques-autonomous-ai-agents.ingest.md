...
**Fixed budget assumption.** The framework assumes a fixed memory budget. The KB's [soft-degradation note](../notes/soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md) argues that usable context varies by task complexity. A fixed-budget forgetting framework may be over-pruning for simple tasks and under-pruning for complex ones. The paper does not address adaptive budget sizing.
