...
snapshot_sha256: 9889856286d41366c99216d4c946d3113e0cf185b624c46ca483f31be326e27b
type: ingest-report
...
Against the [theory-builder definition](../notes/definitions/theory-builder.md), at the whole development-system boundary, conditions 1 and 2 are met: proposed explanations of overhead and harness changes are stated, and they guide experiments and agent behavior. Condition 3 is met as reported. In the Action Fusion lineage, a proposed explanation of avoidable round trips guides experiments, and unreliable prompt-only triggering, a finding against the prompt's content, motivates an explicit tool-schema change. That is stronger than outcome-only variant selection. Condition 4 (iteration) is met: the prompt-triggering finding is kept with the candidate's evidence and shapes the next round, the tool-schema change, which is then tested again. The development system is therefore inside the term. The campaign addresses one problem, making the Pi harness cheaper within capability tolerances, so the persistence grade reached is across the runs of one campaign on one task. The four retained mechanisms are then used unchanged on evaluation tasks, which reuses a frozen product and ends the builder. Search-lineage code is discarded, and taking the improved harness up as the starting point of successor discovery remains future work. As a separate learning claim, the reported cost improvements support increased efficiency capacity, subject to the observed capability losses. The paper does not isolate the benefit of retaining explanations or criticism separately from retaining executable changes.
