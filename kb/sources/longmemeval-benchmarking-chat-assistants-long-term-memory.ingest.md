...
The source's adaptive mechanism is online memory-state construction rather than theory refinement. Interaction sessions and timestamps are converted into retrievable key-value items; learned extractors may derive facts and dated events, a retriever maps the current question to stored items, and a reader maps the selected history to an answer. This is accumulation plus learned transformation and selection. It does not revise an explicit, criticizable theory in response to failed cases, so it does not meet the KB's definition of [theory refinement](../notes/definitions/theory-refinement.md).
