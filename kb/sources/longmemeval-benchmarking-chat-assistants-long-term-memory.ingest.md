...
The source's adaptive mechanism is online memory-state construction rather than conjectural learning. Interaction sessions and timestamps are converted into retrievable key-value items; learned extractors may derive facts and dated events, a retriever maps the current question to stored items, and a reader maps the selected history to an answer. This is accumulation plus learned transformation and selection. The paper describes no formulated theory that guides decisions and is open to criticism of what it says, so it does not establish the KB's [conjectural learning](../notes/definitions/conjectural-learning.md).
