...
snapshot_sha256: 029a9e567738945c2c4b4dddb1e2edd4ecd47352f3761dbe01ec74056d2520e1
ingested: "2026-09-17"
...
The source's adaptive mechanism is online memory-state construction. Interaction sessions and timestamps are converted into retrievable key-value items; learned extractors may derive facts and dated events, a retriever maps the current question to stored items, and a reader maps the selected history to an answer. This is accumulation plus learned transformation and selection, and it is not a [theory builder](../notes/definitions/theory-builder.md). Stored items are text (condition 1), though raw sessions are input records and only extracted facts are stated claims. The reader's answers depend on retrieved content, as the oracle-retrieval comparisons show (condition 2). Items persist and are taken up by later questions (condition 4), but for raw sessions this is retention of input records, which does not count. Criticism (condition 3) decides the verdict: the pipeline indexes, retrieves, and reads, and no process attempts to refute what a stored item says. The reported gains compare fixed configurations; they are not a learning claim about the system.
