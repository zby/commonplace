...
Against the [theory-builder definition](../notes/definitions/theory-builder.md), conditions 1 and 2 are met: skills and pattern pages are stated text, and skills guide the solver while pages guide proposals. Condition 3 is met for skills: procedural rules are revised when later cases count against them. The ALFWorld case study traces rejection of abstract guidance, acceptance of a concrete anti-loop rule, and a later edit for another loop variant. Applying a diagnosis to propose a skill is not itself criticism of that diagnosis, and the paper does not show wiki explanations facing their own discriminating tests. That the pages can be edited separately is a matter of [addressability](../notes/definitions/theory-builder.md#addressability), not of membership. Condition 4 (iteration) is met: the wiki keeps accepted and rejected interventions, and later proposals start from them, as the successive ALFWorld loop edits show. The evaluated arrangement is therefore inside the term. The wiki persists across iterations of one evolution run for one benchmark, so the persistence grade reached is across rounds of one run; test-set and cross-model use reuse the frozen skills, which ends the builder. The paper does not show the wiki or its intervention history taken up in later work on a different benchmark. As a separate learning claim, the aggregate wiki ablation supports the usefulness of the diagnostic process, without establishing the causal accuracy of each retained explanation.
