...
**Additional grounding:** The paper's Flanker Task parallel grounds [human-writing-structures-transfer-to-llms](../notes/human-writing-structures-transfer-to-llms-because-failure-modes-overlap.md) with controlled evidence for human-LLM failure mode overlap. The PRM-guided tree search exemplifies the amplification mechanism in [error-correction-works-above-chance-oracles](../notes/error-correction-works-above-chance-oracles-with-decorrelated-checks.md). The flat-context distractor injection demonstrates the cost described in [llm-context-is-composed-without-scoping](../notes/llm-context-is-composed-without-scoping.md).

**Source complements:** The Lampinen content-effects study, the LLM web agents benchmark, ConvexBench, and Paulsen MECW each address a different axis of context degradation. Together with GSM-DC, they form a converging body of evidence for the soft-bound model.

**Synthesis opportunity (flagged by /connect):** Three independent distraction axes — content bias (Lampinen), context noise (GSM-DC), and compositional depth (ConvexBench) — with GSM-DC's Finding II showing axes 2 and 3 interact multiplicatively. A synthesis note mapping these axes, their interactions, and which interventions address which axis would consolidate substantial evidence.
...
- **Template-based language only.** GSM-DC problems are generated from templated natural language. Real-world irrelevant context is semantically richer, more varied, and often partially relevant rather than cleanly irrelevant. The power-law scaling may not hold when distractors share semantic content with the target problem — the harder and more realistic case. Lampinen's [content-effects work](./language-models-like-humans-show-content-effects-on-reasoning-tasks.md) suggests semantic similarity between distractors and the problem changes the degradation mechanism fundamentally.

- **Math-only domain.** All findings are established on grade-school math. No evidence that the power-law scaling, dual-channel degradation, or training interventions transfer to reasoning tasks in other domains (code, planning, argumentation). The [explanatory reach](../notes/first-principles-reasoning-selects-for-explanatory-reach-over-adaptive-fit.md) of the findings depends on whether the mechanisms (attention dilution, path selection corruption) are domain-general or exploit math-specific model behaviors.
...
Write a note titled "Irrelevant context degrades LLM reasoning through at least three independent axes" connecting to [agent-context-is-constrained-by-soft-degradation-not-hard-token-limits.md](../notes/agent-context-is-constrained-by-soft-degradation-not-hard-token-limits.md), [human-writing-structures-transfer-to-llms-because-failure-modes-overlap.md](../notes/human-writing-structures-transfer-to-llms-because-failure-modes-overlap.md), and [process-structure-and-output-structure-are-independent-levers.md](../notes/process-structure-and-output-structure-are-independent-levers.md) — it would synthesize three empirical pillars (GSM-DC context noise, Lampinen content bias, ConvexBench compositional depth) into a structured claim that distraction is not a single phenomenon but at least three independent degradation mechanisms requiring different mitigations. This addresses the synthesis opportunity flagged by `/connect` and would upgrade the soft-degradation note from a theoretical claim to an empirically-grounded multi-axis model.
