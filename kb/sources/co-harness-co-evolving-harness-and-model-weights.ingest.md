...
- **Claim (paraphrase):** Co-Harness alternates validated edits to a harness comprising prompts, tool schemas, skills, middleware, and memory with model fine-tuning on verified trajectories generated under the improved harness; the updated model then enters the next harness round. Its core experiment combines one HarnessCritic pass and one SFT pass in each of two rounds, while the Round 0 baseline already has an evolved harness but no SFT.
  - **Source extract (verbatim):** Co-Harness alternates between harness optimization and model optimization. An LLM-based HarnessCritic analyzes failed trajectories, identifies harness-level failure modes, and proposes validated local updates. The model is then fine-tuned on high-quality trajectories generated by the improved harness, distilling effective scaffolding into model parameters.
  - **Source location:** Abstract
  - **Source extract (verbatim):** where P denotes prompt and instruction templates, T tool definitions and interface schemas, S reusable skills or modules, Mid middleware, and M long-term memory policy.
  - **Source location:** Section 3.1, Definition 3.1
  - **Source extract (verbatim):** After validation, the accepted configuration is denoted ϕ⋆t . Running θt under ϕ⋆t then yields a trajectory set Dt , which becomes the training signal for the next model update. The resulting model θt+1 is fed back into the next Co-Harness round, initialized from ϕt+1 ← ϕ⋆t .
  - **Source location:** Section 3.2, "Dual-Loop Co-Evolution Architecture"
  - **Source extract (verbatim):** For the dual-loop experiment, we run T = 2 full co-evolution rounds (Round 1 and Round 2), each consisting of one Co-Harness Loop pass (HarnessCritic with K = 5 inner iterations) followed by one Model Alignment Loop pass (SFT on the collected trajectory set). Round 0 (Baseline) applies one HarnessCritic pass to obtain ϕ∗0 but performs no SFT, and serves as the starting point for measuring compounding gains.
  - **Source location:** Section 4.1, "Co-evolution rounds"
  - **Scope:** The proposed batch dual-loop and its two-round experiment on Qwen3-8B and Qwen3-32B in tool-integrated mathematical reasoning; the harness definition spans natural-language and symbolic components plus memory policy.
  - **Confidence:** High for the stated architecture, mutable harness dimensions, recurrence, and experiment schedule because the methods specify them directly.
  - **Limitation:** The evaluated harness spans prompts and skills as well as code, tools, and middleware, so it is not a clean distributed-parametric-plus-symbolic pairing. The round comparison jointly adds SFT data and additional HarnessCritic search and has no arm that isolates recurrent cross-form coupling from either contribution.
