...
In Commonplace's [theory-refinement](../notes/definitions/theory-refinement.md) terms, revising an existing graph is a plausible mixed-form case: procedural commitments are addressable, execution outcomes expose failures, and individual commitments can be edited while retaining others. The mapping is strongest for expert-initialized revision and later rounds after a scratch-built graph exists; the initial construction from a minimal skeleton is not itself revision of a substantive prior theory. Runtime guidance applies the retained procedure rather than changing it. Formal graph checks compute structural properties, while models interpret prose conditions and advice. Task outcomes therefore assess the combined graph-and-consumer configuration, without uniquely identifying which commitment was wrong.
