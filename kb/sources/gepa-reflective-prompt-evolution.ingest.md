...
Some resulting prompts can function as theories under Commonplace's definition of [theory refinement](../notes/definitions/theory-refinement.md): they contain explicit task rules whose behavioral consequences can fail on cases, expose separately readable candidate repair locations, and can be edited while preserving other parts. This classification applies to the rule-bearing prompt, not automatically to every reflection or prompt rewrite. GEPA demonstrates a causal consumption path from retained prompt to later behavior and selection evidence for revised prompts. It does not independently read back the prompt's propositions, test the faithfulness of the diagnoses, or distinguish a true task explanation from a useful control policy or benchmark-specific heuristic.
