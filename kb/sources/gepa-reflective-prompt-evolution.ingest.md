...
Some resulting prompts can function as operative formulated theories: they contain explicit task rules whose behavioral consequences can fail on cases and causally shape later behavior. Their separately readable repair locations also make them [addressable](../notes/definitions/addressable-theory.md). GEPA supplies selection evidence for improved revised prompts, but it does not establish that the reflection criticizes what those rules say rather than proposing variants that score-based selection retains; that distinction separates [conjectural learning](../notes/definitions/conjectural-learning.md) from black-box optimization. This classification applies to the rule-bearing prompt, not automatically to every reflection or prompt rewrite. The paper also does not independently read back the prompt's propositions, test the faithfulness of the diagnoses, or distinguish a true task explanation from a useful control policy or benchmark-specific heuristic.
