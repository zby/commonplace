...
The connect report places this source in the KB's continual-learning, substrate, and oracle-construction cluster. It **contrasts** with [Treat continual learning as substrate coevolution](../notes/treat-continual-learning-as-representational-form-coevolution.md) because it is a clean opaque-substrate loop: identify unknowns, search for facts, train weights. It **qualifies** [Continual learning's open problem is behaviour, not knowledge](../notes/continual-learning-open-problem-is-behaviour-not-knowledge.md) because the paper's successful training would be durable capacity change, but the KB's criterion is durability rather than weights. It **extends** [Automating KB learning is an open problem](../notes/automating-kb-learning-is-an-open-problem.md) by offering a concrete "what to learn" operator, while also showing why factual hallucination scoring does not directly solve judgment-heavy KB mutations. The closest oracle comparison is [Memory management policy is learnable but oracle-dependent](../notes/memory-management-policy-is-learnable-but-oracle-dependent.md): AgeMem uses task completion as the oracle; this paper uses hallucination/self-checking as an unknown detector and external search as supervision. The source also connects to [Oracle strength spectrum](../notes/oracle-strength-spectrum.md), [The boundary of automation is the boundary of verification](../notes/the-boundary-of-automation-is-the-boundary-of-verification.md), and Letta's [Continual Learning in Token Space](./continual-learning-in-token-space.md).
