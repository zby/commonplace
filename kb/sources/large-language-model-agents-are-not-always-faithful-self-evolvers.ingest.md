...
## Connections Found
`/connect` found two strong fit clusters. In learning theory, the source connects to [distillation](../notes/definitions/distillation.md) because condensed experience is a distilled artifact whose failure mode is now empirical rather than hypothetical, and to [deploy-time learning](../notes/deploy-time-learning-is-the-missing-middle.md) because the paper studies symbolic cross-session adaptation but shows that artifact presence is not enough — artifacts must remain behaviorally active. In the trace-derived / related-systems cluster, it connects to [trace-derived learning techniques in related systems](../agent-memory-systems/trace-derived-learning-techniques-in-related-systems.md) by adding a missing evaluation axis (faithfulness at inference time), to [the fundamental split in agent memory is not storage format but who decides what to remember](../agent-memory-systems/agentic-memory-systems-comparative-review.md) by suggesting a new “use-time faithfulness” dimension for memory systems, and directly grounds the nearby framework notes [ExpeL](../agent-memory-systems/reviews/expel.md) and [Dynamic Cheatsheet](../agent-memory-systems/reviews/dynamic-cheatsheet.md), which the paper evaluates explicitly.
...
2. [quick-win] Treat **behavioral faithfulness** as a missing quality criterion for [distillation](../notes/definitions/distillation.md): a summary is not good just because it is concise or semantically plausible; it must preserve enough structure to steer later behavior.
3. [deep-dive] Extend [the fundamental split in agent memory is not storage format but who decides what to remember](../agent-memory-systems/agentic-memory-systems-comparative-review.md) with a seventh dimension: **use-time faithfulness**. This paper shows that storage unit, agency, and link structure are still incomplete without asking whether the agent actually depends on the stored artifact.
...
Write a note titled **“Behavioral faithfulness is the missing evaluation criterion for distillation”** connecting to [distillation](../notes/definitions/distillation.md) and [deploy-time learning](../notes/deploy-time-learning-is-the-missing-middle.md) — it would argue that compressed artifacts should be judged by whether perturbing them changes downstream behavior, not just by semantic plausibility or task score.
