...
**The "model eats scaffolding" prediction is unfalsifiable as stated.** "In two years, most basic skills will be one-liners" — there's no definition of "basic" that would let you evaluate this prediction. The [bitter-lesson-boundary](../notes/fixed-artifacts-split-into-exact-specs-and-proxy-theories.md) note provides the theoretical framework for identifying WHICH scaffolding gets eaten (vision-feature regime) versus which persists (calculator regime), but the author conflates the two.
