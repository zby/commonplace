...
snapshot_sha256: 9be5208861713369d27872261c4d680dc3e624075b2b0521c561e62f010c5459
...
This is not a [theory builder](../notes/definitions/theory-builder.md), though it shares its retention structure. The policy has editable components and a causal role in future learning, and SSA's stack keeps surviving changes across the lifetime. But a probability entry is an edit location, not a statement: the policy is a disposition whose content is not localized, so condition 1 fails. SSA keeps or undoes changes by reward rate, with no stated reason bearing on what a change says; that is trial and error, and condition 3 fails. The stack records reward totals, not criticism. The paper supplies a route to learned learning behavior; calling every editable policy a theory would erase that distinction.
...
SSA's retention evidence comes from a single irreversible history. Environmental changes, delayed consequences, and chance can affect measured reward rates; rollback cannot provide the counterfactual trajectory without a modification. The source acknowledges that no optimality or improvement-speed guarantee follows and that environments with persistently decreasing reward may leave only the trivial empty success history. Repeated re-evaluation is useful correction machinery, not proof that surviving changes caused progress. The [success-story algorithm paper](./shifting-inductive-bias-success-story-algorithm.ingest.md) uses the same SSA machinery with overlapping authors, so the two papers are not independent confirmations of one result: this one concerns learning the modification procedure, that one assessing continued retention.
