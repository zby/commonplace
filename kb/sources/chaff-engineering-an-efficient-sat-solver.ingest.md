...
In Commonplace terms this is adaptation of search control through operative retained results, not [conjectural learning](../notes/definitions/conjectural-learning.md). A conflict clause is formulated symbolic content and is operative through what it says. However, it is a sound consequence of the formula, so it is never criticized or revised as a tentative theory; deletion removes it for memory reasons, not because it was found wrong. VSIDS counters are not formulated theories at all. The case is therefore useful as a contrast: it shows retained failure explanations steering search when those explanations are guaranteed correct, which is the condition fallible natural-language explanations lack.
