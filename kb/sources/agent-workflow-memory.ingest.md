...
Update [Trace-derived learning techniques in related systems](../agent-memory-systems/trace-derived-learning-techniques-in-related-systems.md): in the AWM section, add a short paragraph from this paper's ablations saying that abstract workflows beat concrete exemplars on Mind2Web, rule induction nearly matches LM induction on WebArena, online induction helps most under domain shift, and offline-plus-online workflows interfere rather than compose cleanly. This would strengthen the survey's maintenance/oracle argument without duplicating the existing code review.
