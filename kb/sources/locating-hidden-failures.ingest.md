...
In Commonplace terms, training Scout is parametric learning, and applying it to select a run uses the learned evaluator. Neither establishes [conjectural learning](../notes/definitions/conjectural-learning.md): the capture identifies no formulated theory criticized for what it says with a resulting improvement in future capacity. The closing aspiration that agents learn from their mistakes goes beyond the demonstrated selection result. Localization could supply a repair signal, but no persistent change in the acting agent is shown here.
