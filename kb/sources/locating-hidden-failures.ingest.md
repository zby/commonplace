...
In Commonplace terms, training Scout is parametric learning, and applying it to select a run uses the learned evaluator. Neither makes the arrangement a [theory builder](../notes/definitions/theory-builder.md). Scout's learned judgment lives in weights, so condition 1 fails for it, and its training is not criticism of stated content (condition 3). Its localizations are stated criticisms, but of trajectory steps, not of a stated theory. They serve only to choose among finished runs, and the capture reports the acting agent unchanged, so no next conjecture takes the localization up. Criticism that feeds nothing fails condition 4 as reported, and no persistence grade is reached. The closing aspiration that agents learn from their mistakes goes beyond the demonstrated selection result. Localization could supply a repair signal to a system that retained and consumed it.
