...
- **[Selecting an LLM output fixes a result, not its interpretation](../notes/selecting-an-llm-output-fixes-a-result-not-its-interpretation.md)** — contrasts: Evans narrows the generator by adopting a frozen taxonomy, whereas selecting one output fixes only result identity unless a separate semantic decision also narrows the available readings.
