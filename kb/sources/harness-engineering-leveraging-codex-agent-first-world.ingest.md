...
snapshot_sha256: c381abf662a98ecfc76d86ac1cdb39ef2e974552b591546fb5b6eb17e4b167a2
ingested: "2026-03-09"
...
3. **[deploy-time-learning-the-missing-middle](../notes/retained-artifacts-enable-persistent-deployment-time-adaptation.md)** — exemplifies: "Good harnesses compound" is deploy-time learning in practitioner language. Each constraint is a repo artifact that makes future work more reliable.
...
2. **Maintenance capacity at agent-generation scale**: Background cleanup agents scan for stale documentation, constraint violations, and pattern deviations, opening small refactoring PRs (most auto-merged). "Garbage collection for code quality." The original ingest inferred proportional cleanup-to-generation throughput; the completed [maintenance-capacity note](../notes/maintenance-capacity-must-match-harmful-artifact-inflow.md) retains the capacity pressure but identifies risk-weighted harmful inflow, not gross generation, as the governing load. [captured]
...
Resolved as [Maintenance capacity must match harmful-artifact inflow](../notes/maintenance-capacity-must-match-harmful-artifact-inflow.md). The completed note uses Lopopolo's transition from “20% Fridays” to background cleanup agents as a bounded witness, but rejects the ingest's stronger proportional-to-gross-generation inference. It instead argues that prevention, containment, detection, and repair capacity must keep pace with risk-weighted harmful retained-artifact inflow; cadence is one capacity lever rather than a universal requirement.
