...
Write a note titled `Filesystem-native tool use is a distinct long-context strategy, not just a storage choice` connecting to [Context efficiency is the central design concern in agent systems](../notes/context-efficiency-is-the-central-design-concern-in-agent-systems.md), [Tool loop](../notes/tool-loop-index.md), [The fundamental split in agent memory is not storage format but who decides what to remember](../agent-memory-systems/agentic-memory-systems-comparative-review.md), and [Process structure and output structure are independent levers](../notes/process-structure-and-output-structure-are-independent-levers.md) — it would argue that file systems matter here because they expose deterministic operations that let agents convert semantic overflow into executable process structure.
