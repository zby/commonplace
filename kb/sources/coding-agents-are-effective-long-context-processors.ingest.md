...
2. [quick-win] Add this paper as empirical grounding in [Context efficiency is the central design concern in agent systems](../notes/context-efficiency-is-the-central-design-concern-in-agent-systems.md) and [Tool loop](../tags/tool-loop-README.md); those notes currently rely more on theory and practitioner evidence than on benchmark comparisons of file-native execution vs full-context or RAG baselines.
...
Write a note titled `Filesystem-native tool use is a distinct long-context strategy, not just a storage choice` connecting to [Context efficiency is the central design concern in agent systems](../notes/context-efficiency-is-the-central-design-concern-in-agent-systems.md), [Tool loop](../tags/tool-loop-README.md), [The fundamental split in agent memory is not storage format but who decides what to remember](../agent-memory-systems/agentic-memory-systems-comparative-review.md), and [Process structure and output structure are independent levers](../notes/process-structure-and-output-structure-are-independent-levers.md) — it would argue that file systems matter here because they expose deterministic operations that let agents convert semantic overflow into executable process structure.
