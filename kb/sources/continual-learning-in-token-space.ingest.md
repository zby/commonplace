...
It most strongly **extends** [Continual learning's open problem is behaviour, not knowledge](../notes/continual-learning-open-problem-is-behaviour-not-knowledge.md): the source makes the same anti-weight-monopoly move, but with a narrower substrate proposal, learned context. In repo terms, it covers one branch of the non-weight side but not the whole space. It also **extends** [Deploy-time learning: The Missing Middle](../notes/retained-artifacts-enable-persistent-deployment-time-adaptation.md) by presenting token-space memory as an inspectable adaptation layer that sits alongside opaque weight updates, while adding a bridge back toward training through tokens-to-weights distillation.
