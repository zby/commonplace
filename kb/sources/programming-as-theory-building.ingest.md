...
- [Naur binds program theory to humans through the premise that machines only follow rules](../notes/naur-binds-theory-to-humans-via-premise-that-machines-follow-rules.md) — separates the essay's argued inexpressibility claim from its human-binding conclusion, locates the unargued machines-follow-rules premise between them, and turns the transfer cases into conditions on a text-plus-interpreter composite
