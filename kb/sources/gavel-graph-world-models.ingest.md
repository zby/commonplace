...
In Commonplace terms, the action model provides addressable premises and localized consequences, but GAVEL uses those premises to revise plans; it does not demonstrate revision of the premises themselves. The plan-feedback loop is compatible with a local [conjectural-learning](../notes/definitions/conjectural-learning.md) interpretation if plans are treated as operative proposed solutions and structured failures as criticism. Higher task success supports the usefulness of correction, but does not by itself establish that the system learns a reusable planning theory. Lack of cross-task retention does not exclude local learning; it limits the persistence claim. Likewise, changing location beliefs demonstrates observation-conditioned adaptation without establishing theory revision merely because probabilities changed.
