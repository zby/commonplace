...
snapshot_sha256: dfa2916f7043cdc027d93d1a16ab7f227689c29d23c6cceb38b51156f99086ef
...
Against the four conditions of a [theory builder](../notes/definitions/theory-builder.md), GAVEL's plan-repair loop is a theory builder at a low persistence grade. Plans and the action model are stated units (condition 1), and plans direct execution (condition 2). Simulation against the action model yields structured failures that name the violated precondition, which is criticism aimed at what a plan says (condition 3). The action model's own premises are used but never criticized or revised; it is fixed background for criticizing plans and does not keep the plans outside. Condition 4 is met within one instruction's execution: a plan revised after a named precondition failure is a new conjecture, simulated again against the action model. Persistence stays at the grade of rounds within one run: neither revised plans nor their failure records are carried to a later instruction, and learning a location prior across runs is future work. Changing location probabilities is observation-conditioned adaptation of numbers, not revision of a stated theory. Higher task success supports the usefulness of correction, which is a separate claim from membership.
