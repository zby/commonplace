...
- [Structured-prompt gains do not establish training-distribution selection](../notes/structured-prompt-gains-do-not-establish-distribution-selection.md) -- **contrasts**: The paper measures an architectural induction bias during learning, while the note warns that an inference-time prompt gain cannot identify a training-distribution mechanism. The learning result can motivate an analogy, but it does not bridge that causal gap.
...
- [from-entropy-to-epiplexity-rethinking-information-computationally-bounded.ingest](./from-entropy-to-epiplexity-rethinking-information-computational.ingest.md) -- **extends**: The epiplexity paper provides the theoretical framework (observer-relative information extraction) that explains the induction bias paper's results. Different architectures are different computational bounds extracting different amounts of structure from the same data.

**Synthesis opportunity:** "Architectural constraints as a form of constraining" -- constraint-as-learning-accelerator operates at architecture level (this paper), artifact level (constraining), and prompt level, while [structured-prompt gains do not establish training-distribution selection](../notes/structured-prompt-gains-do-not-establish-distribution-selection.md) prevents treating an observed prompt effect as evidence that all three levels share one mechanism. A synthesis note could formalize this ladder and its evidential boundaries.
...
Update [bitter-lesson-boundary](../notes/exact-implementation-does-not-validate-a-requirement.md): add this paper as grounding evidence in the Relevant Notes section, with a relationship like "grounds: 190k-run empirical study demonstrating that architectural constraint (recurrent induction bias) permanently outperforms the general method (transformer) on calculator-class state tracking, with destructive interference (kappa = 0.28) showing more diverse data actively hurts the general method." The note currently has only one entry in its Relevant Notes section and urgently needs empirical grounding for its central claim.
