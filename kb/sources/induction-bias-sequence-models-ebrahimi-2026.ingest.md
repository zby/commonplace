...
Update [bitter-lesson-boundary](../notes/fixed-artifacts-split-into-exact-specs-and-proxy-theories.md): add this paper as grounding evidence in the Relevant Notes section, with a relationship like "grounds: 190k-run empirical study demonstrating that architectural constraint (recurrent induction bias) permanently outperforms the general method (transformer) on calculator-class state tracking, with destructive interference (kappa = 0.28) showing more diverse data actively hurts the general method." The note currently has only one entry in its Relevant Notes section and urgently needs empirical grounding for its central claim.
