...
- **Claim (paraphrase):** λ-RLM retains RLM's prompt-as-environment REPL but replaces open-ended model-authored control code with a typed library of deterministic, pre-verified combinators; a deterministic planner instantiates a fixed recursive combinator chain, while neural inference is bounded and localized to task selection, leaf work, and explicitly specified synthesis steps.
  ...
  - **Scope:** The λ-RLM architecture and compact combinator library presented in the paper for long-context reasoning; the paper evaluates four task families and states formal properties for its recursive executor under explicit regularity and decomposition assumptions.
  - **Confidence:** High for the described architecture and algorithm; formal guarantees are confidence-bounded by the paper's assumptions, and “pre-verified” is the authors' characterization rather than an independent audit performed here.
  - **Limitation:** The paper says the library is extensible rather than unique or exhaustive. Neural work is not literally confined to leaf calls: the pipeline uses a bounded task-type selection call and some plans include explicit neural synthesis. The planner is a deterministic non-neural routine, not an LLM freely selecting an arbitrary chain.

- **Claim (paraphrase):** The paper's complete λ-RLM execution pipeline initializes REPL state for an input prompt, performs bounded task-type selection and deterministic planning, executes one pre-built recursive program, and returns the resulting response.
  ...
  - **Scope:** One end-to-end execution of the paper's λ-RLM algorithm for one input prompt, from REPL initialization through response return.
  - **Confidence:** High for the sequence and one-shot execution described by the algorithm.
  - **Limitation:** The paper does not specify the lifecycle of REPL state or the generated execution plan after the response returns. It therefore does not establish that state is discarded per task, persisted across tasks, or promoted into a durable reusable artifact.
