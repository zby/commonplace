...
On the paper's terms, EURISKO learns by proposing new concepts or heuristics through transformations of existing units, testing or using them where possible, changing their worth from outcomes, and retaining useful variants. Some retained rules describe how to generate or evaluate other rules, and some changes introduce new slot types, so the editable object can include parts of the learner's own operating representation. Against the [theory-builder](../notes/definitions/theory-builder.md) conditions, the frame units are localized content (condition 1), and heuristics guide the agenda through what they say (condition 2). Retained heuristics are taken up on later, different problems: when the 1982 tournament rules changed, most general heuristics from 1981 still applied to the new design task (condition 4). Condition 3 depends on the [boundary](../notes/definitions/theory-builder.md#boundary). Addressable heuristic parts receive empirical tests and local edits, but much of the machine's own retention runs on worth values and interestingness, which rank units without a stated reason bearing on what they say. Lenat's culling and correction of particular heuristics is criticism aimed at their content. With Lenat inside the boundary, the system is a partial theory builder; EURISKO alone shows condition 3 only in part. Its rules about rules make it a candidate for the reflective qualifier. The paper supports reflective proposal, evaluation, and retention more strongly than reliable improvement, which is a separate claim.
