...
On the paper's terms, EURISKO learns by proposing new concepts or heuristics through transformations of existing units, testing or using them where possible, changing their worth from outcomes, and retaining useful variants. Some retained rules describe how to generate or evaluate other rules, and some changes introduce new slot types, so the editable object can include parts of the learner's own operating representation. This is only partly [theory refinement](../notes/definitions/theory-refinement.md): addressable heuristic parts receive empirical criticism and local edits, but many changes generate new rules or concepts without revising a theory whose case-level consequences are independently checked. The paper more strongly supports reflective proposal, evaluation, and retention than reliable reflective theory refinement.
