...
Relative to [conjectural learning](../notes/definitions/conjectural-learning.md), the important contribution is a designed, inspectable correction surface inside a neural computation. The experiments replace instance-specific estimates and rerun a fixed predictor; they do not show formulated criticism changing future capacity. Training changes durable parameters, but the reported test-time corrections do not update them. Section 8 explicitly leaves learning from interventions as future work. The paper therefore qualifies any substrate-wide claim that numerical representations cannot expose selective edit targets, while leaving the stronger claim of persistent commitment-level revision unestablished.
