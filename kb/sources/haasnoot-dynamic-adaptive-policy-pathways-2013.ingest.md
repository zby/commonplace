...
The paper is a technical anchor for staged commitment in the current KB. It supports [An author should fix what the executor can't determine, not what it will](../notes/fix-what-the-executor-cant-determine-not-what-it-will.md) by showing how stable objectives and near-term actions can coexist with later choices conditioned on monitored evidence. It also supplies bounded evidence for the adaptive-planning consequence in [Codifying predictable choices leaves agents with less predictable work](../notes/codifying-predictable-choices-leaves-agents-with-less-predictable-work.md), not for that note's broader account of which choices systems codify.
