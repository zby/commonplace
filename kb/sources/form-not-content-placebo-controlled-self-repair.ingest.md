...
snapshot_sha256: a54f5d650592a892f6bac173ab640466479611ce32a2086d283ea62ec0178086
...
The Popperian commitment is explicit: failure under an external execution oracle can refute a program relative to that oracle, while passing remains provisional. The ontology, six actions, fitted severity, low-rank adapter, promotion thresholds, and public/hidden test division are implementation choices. They do not follow uniquely from that epistemic commitment. Against the [theory-builder definition](../notes/definitions/theory-builder.md), a generated program meets condition 1 and can supply an [addressable](../notes/definitions/addressable-theory.md) candidate for repair. Execution against tests is attempted refutation of what the program says, and error messages are stated, so condition 3 is met per unit. Condition 2 for the criticisms is unestablished: the content controls detected no difference between genuine error content and placebo text of the same form, and non-detection does not establish absence. Within each unit, the repaired program is kept and re-tested on the cases that refuted its predecessor, which is iteration in form (condition 4). Whether the error content, rather than the bare fact of failure, shapes the repair is the question condition 2 leaves open, so condition 2 decides: the repair loop is not shown to be a theory builder. Persistence reaches at most the grade of rounds within one run on one unit; what crosses units is fitted controller state and adapter weights, which are numeric and fail condition 1. The author's research record, which keeps the withdrawn allocator claim with the reason, shows retained criticism in the research practice, not in the repair system. The paper does not establish improved capacity for future action; that learning claim stays separate and, for these channels, unconfirmed.
