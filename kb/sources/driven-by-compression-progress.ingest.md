...
This is broader than Commonplace's [conjectural learning](../notes/definitions/conjectural-learning.md). Improving a neural predictor by fitting can satisfy the source's mechanism without a formulated theory or criticism of what it says. Revising an explicit predictive program could overlap with conjectural learning if such criticism changes future capacity, but the compression objective does not require that operation. The source adds an explicit account of how learning can select its own future evidence; it does not supply a diagnosis procedure for repairing a KB theory.
