...
The [competing-causal-theories note](../notes/competing-causal-theories-can-guide-distinguishing-experiments.md) connects directly to Section 3.4's account of opposing programs proposing algorithmic experiments, betting on conflicting predictions, and executing the experiments to settle accepted bets. This is a concrete precedent for using disagreement to select tests within compression-driven exploration. Our causal note applies that strategy to observationally equivalent mechanisms; the general compression-progress objective does not require retaining explicit causal rivals.
...
This is broader than a [theory builder](../notes/definitions/theory-builder.md) and does not require one. A neural predictor improved by fitting need not state localized theories or criticisms of their content. Section 3.4, however, supplies explicit opposing predictions and tests of their consequences, so the paper cannot be characterized as merely scoring passive fits. Its reward-based updates do not by themselves establish that stated criticisms are retained and used as the definition requires; that mapping needs implementation-level evidence. Compression progress alone neither establishes nor excludes a theory builder.

Section 1.2 preserves the history, and Appendix A.1 describes reconstructing it from a predictor and its errors. We read this as compatible with using a tentative theory to encode evidence while preserving observations for later revision. The machinery is not derived from observations alone: Appendix A.8 supplies an environment class and a weighted prior; Appendix A.9's self-modifying system starts with software, a proof searcher, axioms, and a utility function.
...
Compression progress supplies an objective, but its practical adequacy depends on the supplied representations, assumptions, and methods for proposing and testing models. The paper gives candidate machinery, including Section 3.4's competing predictions and experiments; its universal constructions do not remove those dependencies or establish feasible causal discovery in general. Our [constructed A/B example](../notes/competing-causal-theories-can-guide-distinguishing-experiments.md) shows why identical passive evidence cannot identify the mechanism. It does not show that the paper's active learners cannot acquire distinguishing evidence. The question for a concrete implementation is whether its models and search procedure can find useful hypotheses and feasible tests within its resource limits.
