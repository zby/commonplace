...
Both programs are worked examples of the decomposition lemma in [Any symbolic program with LLM calls is a select/call program](../notes/any-symbolic-program-with-llm-calls-is-a-select-call-program.md). Their inter-call computation is symbolic, so their recursion stacks, control locations, pending work, and partial results can be reified as machine state `K`. `select(K)` can then run until the next `ask` prompt, and `K + r` can record the result and resume the computation. Semantic quicksort is the clearest witness because it changes an ordinary recursive algorithm only by substituting an LLM call for its comparison operator. The lemma preserves the program's behavior, not its correctness: an inconsistent or non-transitive model comparator remains inconsistent after conversion.
