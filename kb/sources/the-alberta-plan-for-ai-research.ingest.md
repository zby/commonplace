...
Oak is a design-space counterpoint to [Learning inside a fixed decomposition inherits its mistakes](../notes/learning-inside-a-fixed-decomposition-inherits-its-mistakes.md): it progressively moves features, subtasks, options, and option models into a utility-assessed replacement loop while leaving the reward objective, signal interface, and temporal discipline outside that effective update space. Compared with [Ashby's violation-triggered reorganization](ashby-design-for-a-brain-1960.ingest.md) and [GBrain's scheduled dream cycle](../agentic-systems/reviews/gbrain.md), the Plan's per-step discipline helps separate the choice to adapt continually from the choice of when adaptation runs.
