...
- **Claim (paraphrase):** The study crossed three dataset–hypothesis tasks, four base LLMs, and five analyst personas in about 5,000 autonomous runs; after AI-auditor screening it retained about 30 compliant runs per dataset × model × persona cell, and compliant analyses of an identical dataset, hypothesis, and estimand still reached divergent estimates and conclusions.
  ...
  - **Scope:** The compliant distribution is conditional on the study's three tasks, four chosen LLMs, five prompts, common scaffold, prespecified estimands, and Claude Sonnet 4.5 auditor. Within a cell, dataset, model, persona, scaffold, and sampling policy are held fixed while agents retain autonomy over analytical choices.
  - **Confidence:** High for the experimental design, retained counts, and observed compliant dispersion; the auditor is part of the operational validity criterion rather than independent ground truth.
  - **Limitation:** Within-cell dispersion shows run-to-run variation under fixed cell settings, but the paper does not isolate decoder sampling from tool/runtime nondeterminism or from the different analytical decisions agents make. It therefore cannot attribute that dispersion solely to sampling.

- **Claim (paraphrase):** The study's persona intervention deliberately changes analysts' stated prior plausibility or asks them to seek supporting specifications, while pilot runs included fully hallucinated results and recall of published findings; a transcript-reading AI auditor was introduced to screen methodological validity, and 67% of the final runs passed that screen.
  ...
  - **Scope:** The persona manipulation fixes the estimand and reporting requirements but intentionally changes prior framing or search behavior. The pilot examples motivate the auditor; the pass rate describes the later 4,946-run experiment.
  - **Confidence:** High for the prompt conditions, pilot failure modes, and reported pass rate. The source does not map every excluded final run to a specific failure mode.
  - **Limitation:** Because the persona prompts change priors or the analyst's commission, their differences are not meaning-preserving surface variations. The pilot hallucination and recall cases cannot be counted as final-run auditor rejections, and the AI auditor's judgments remain criterion-relative.
