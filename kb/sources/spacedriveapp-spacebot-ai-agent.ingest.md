...
- [symbolic-scheduling-over-bounded-llm-calls-is-the-right-model-for-agent-orchestration](../notes/symbolic-scheduling-over-bounded-llm-calls-is-the-right-model-for-agent-orchestration.md) -- **exemplifies**: Spacebot's cortex is the cleanest production implementation of the clean scheduling model among reviewed systems. The cortex is a Rust-level symbolic scheduler; channels and branches are bounded LLM calls; workers are deterministic tool executions.
