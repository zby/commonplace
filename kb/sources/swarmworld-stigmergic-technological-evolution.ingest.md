...
snapshot_sha256: 8fccaeb521aa5226dd078d9c897fd3111ce2b76166ed49b8b88123b293040071
ingested: "2026-09-25"
...
Against the [theory-builder](../notes/definitions/theory-builder.md) conditions, the model-authored research state (goal, hypothesis, next checkpoint), notebook evidence, teaching records, and controller programs are localized content (condition 1), and the research state feeds each plan (condition 2). Supplement S3 shows one agent forking a controller with its threshold changed from 0.35 to 0.50, then stating a measured comparison with the parent program and its limits. That is a test of a stated change, shown at the strength of a selected vignette (condition 3). Records, programs, and artifacts persist across the run and are taken up by other agents on later, different constructions, as when one agent's measured material batch grounds another's distinct membrane (condition 4). The source therefore supports a theory builder at the whole-system level, with condition 3 shown only in selected traces. Learning is a separate claim: the experiments vary interaction bundles, not criticism, so no capacity gain is attributed to it. The compared artifacts also have different contexts.

The comparison also exemplifies [learning inside a fixed decomposition](../notes/learning-inside-a-fixed-decomposition-inherits-its-mistakes.md). Agents can change recipes, geometric parameters, hypotheses, local plans, and controller instructions. They cannot revise the experiment's observation interface, material property rules, service objectives, or permitted controller operations. Controllers are straight-line programs limited to 64 instructions over 16 registers, with supplied sensors and actuators. Communication ablations test the removed capability bundles within this space; they do not show that this is the best representation for cumulative learning. The source broadens the concrete substrates to examine in Commonplace's account, but does not require changing the theory-builder conditions.
