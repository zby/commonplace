...
snapshot_sha256: c590ea4de38763f4400abf4faee26f52b9dd0db59e52e80d87ff88642afab053
ingested: "2026-09-25"
...
This supports the trace-extraction note's distinction between readable artifacts and distributed-parametric learning, while showing that the two can be successive stages of one process. The artifact need not remain available during deployment for its training contribution to persist. Against the [theory-builder definition](../notes/definitions/theory-builder.md), the skill library meets condition 1, because the skills are executable programs, and condition 2 during training, where rollouts retrieve and execute them. The library also carries across later training tasks, so retention is present in training (condition 4). Condition 3 decides the negative verdict. The described operations are admission checks of limited formal properties, success-statistic ranking, pruning, and gradient updates; none aims at what a skill says, and the definition classes score selection as trial and error and gradient updates as outside criticism. A step that criticizes why a procedure fails is not ruled out in an unreported implementation detail, but it is not demonstrated here. The deployed policy keeps only weights, which fail condition 1. Improved success and fewer decisions are a separate learning claim, attributed to the compound training method.
