---
description: "Why conjectural learning narrows Popper's process to formulated, addressable theories criticized at a named part, why it adds a knowing subject, what a fixed interpreter changes, and the two conjectures it bets on"
...
of that process: the part where theories are written down in language, can be
revised a piece at a time, and are criticized for what they say, with the
result carried into later work. It sets aside learning that needs no written
...
in two ways: a system that criticizes what a theory says learns more from
each failure than one that selects by outcome, and carrying the results
forward costs less than redoing the work. Neither bet is established.
...
that reads the theories cannot itself change, and states the two bets with
...
weights onto Popper's lower level is the KB's reading.
...
The four labels are the rows of the definition's table.
...
- **Structure: addressable theories.** Criticism that names a probable cause
  ...
  tentative and can still be criticized as a whole, but it offers no part for
  the criticism to name.
...
- **Localization: criticism aimed at a part.** Popper allows a failed test to
  ...
  The KB keeps the addressed case on practical grounds: a failure that names
  no part leaves the successor unconstrained, so little is learned from it.
...
Each conjecture is tested against some of the definition's separate
comparisons, the arrangements that fail one of its two conditions. In an
experiment such an arrangement is a baseline, and a baseline may match or
