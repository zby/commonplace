...
The protocol exists only as a design; no step below has been carried out.
A run would proceed in six steps.

1. **Freeze the declarations.** Before the first scored task, the run fixes,
   among other declarations,
   the consuming project and the tasks it may pose, the seed (Commonplace's
   methods and theories at the start), the exact model versions for the
   builder and for the consuming agents, budget ceilings that are equal
   across every compared condition, the horizon and stopping rule, the
   outcome contract for each task and who may change it, and the
   reliability target and comparison margin. Fixing the stopping rule in
   advance prevents stopping when a desired score first appears.
2. **Deliver a release.** Commonplace delivers a versioned release: the
   knowledge base together with its validators, skills, and indexes.
3. **Run a task.** A task arrives from the project. Before it runs, an
   expectation is written down: which part of the knowledge base should be
   consulted, and what an answer drawn from the wrong source would look
   like. A consuming agent performs the task, and the project's judge
   accepts or rejects the output.
4. **Record the episode.** The record keeps the task and its contract, the
   release version, the consumer's configuration, budget use, the output,
   the judge's decision and reason, the feedback returned, and which
   artifacts the consumer read before a consequential decision. Reading the
   expected artifact shows that it was consulted, not that it caused the
   outcome.
5. **Revise.** A rejection is evidence about the combined task, consumer,
   and knowledge base, not an automatic refutation of one note. Diagnosis
   looks for the part that failed, as in the exporter case: a delivered
   account whose assumption no longer holds is revised in that part, and a
   failure that exposes a limit in Commonplace's own methods leads to
   revising those methods. The revision goes out in the next release. A
   change installed from outside the builder's own revision process is
   recorded as an intervention, and its effect is not credited to the
   builder. Today people would perform much of this step.
6. **Assess.** Tasks whose outcomes guide development are kept apart from
   reserved assessment tasks. At the end, the candidate release is frozen
   and assessed on the reserved tasks. Four comparisons then carry the
   hypotheses: the same task with a retained change removed; control runs
   in which the retained content is removed, replaced by a similar
   distractor, or left stale, or in which another mechanism could supply the
   answer; a baseline that searches the raw records without the learned
   methodology; and a human-staffed builder under the same tasks and
   budget.

Steps 2 to 5 repeat until the declared horizon. The [evidence
supplement](./testing-the-theory-refinement-program.md) gives the full
list of declarations, the rule for reusing reserved tasks, and what each
