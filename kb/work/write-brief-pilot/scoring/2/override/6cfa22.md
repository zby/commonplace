...
Commonplace uses a rough working threshold to sort the two cases. As a rule
of thumb, treat a structure as costly to reverse when replacing it would mean
migrating more than about twenty notes or changing a validator; below that
line, treat it as cheap and reversible. This threshold is a Commonplace
convention for deciding when to apply the test. Pindyck's analysis does not
derive it, and no evidence available here calibrates it. A choice near the
line, or one whose dependents are mostly not notes, still needs judgment
about its actual migration cost.
...
fragmentation, coordination, migration,
