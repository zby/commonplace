...
As a Commonplace rule of thumb, a structure counts as costly to reverse when
replacing it would mean migrating more than about twenty notes or changing a
validator. The threshold is a working convention for deciding when this test
applies. Pindyck does not derive it, and no evidence here calibrates the
number. It does not measure or rank the delay costs listed above, and a
commitment that would cross it still needs one of the three warrants.
...
Apart from that rule of thumb, the claim is qualitative. The available
