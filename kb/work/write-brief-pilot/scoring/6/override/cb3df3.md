...
Analyse one external agentic system at one frozen evidence boundary. Run a mandatory runtime baseline, then scope the memory/context lens and the epistemic lens and run each at a depth proportionate to what the evidence supports — or skip it, with a recorded justification, when scoping finds no trigger evidence for it — reconcile shared records by stable IDs, and return one bounded system synthesis. The consumer is an analysing agent or maintainer; the channel is explicit invocation or trigger-matched skill loading; the force is a prescriptive analysis and result-writing policy.
...
- Conclusion statuses (use exactly these):
  ...
  - `inapplicable` — the stated trigger conditions are false inside that boundary; this is a finding about the system under review; a lens skip is recorded in the step-5 scoping record, not as this status;
...
This step decides, for each lens, how deep it runs or whether it is skipped, and it is where the trigger evidence is named before any lens worker sees it. Every run scopes both lenses.

For each lens (memory/context; epistemic), emit one scoping record: `{lens, trigger evidence IDs, inspected boundary, the routes and objects that evidence points the lens at, warranted depth, rationale}`. Warranted depth is `full`, `brief`, or `skipped`.
...
- **Skip only on no trigger evidence.** If the search of the frozen boundary finds no trigger evidence for a lens, you may skip that lens entirely instead of running a brief pass. A skipped lens gets no worker and no invocation. Its scoping record must state three things: that the lens is skipped; the evidence searched — the sources, components, routes, and documentation examined for trigger evidence, by canonical ID; and the conclusions the skip prevents. Thin or degenerate trigger evidence is still trigger evidence and warrants at least a brief pass. Evidence that was unavailable or not inspected is not an absence of trigger evidence; it cannot justify a skip.
...
- **`uncertain` is not a scoping value and not an exit.** Evidence you cannot resolve becomes an explicit evidence limitation *inside* the lens output, paired with the conclusion it prevents. Never let it end or skip a lens: an exit that means "we could not tell" reads to every later reader as "there is nothing there."
- An absent lens section or file must never carry meaning implicitly. Every run emits both scoping records and both lens outputs; the output of a skipped lens is its skip statement — the evidence searched and the conclusions the skip prevents — or a resolvable pointer to the scoping record that carries it.

**Memory/context evidence.** Look for a path by which material accumulated or changed through use can affect a later invocation or action, in code, documentation, or observation. Static shipped material, ordinary current-run state, and retained material with no later delivery path are not such paths — where these are all you find, there is no memory/context trigger evidence, and the skip rule above applies. A merely claimed path is trigger evidence; the lens output preserves whether the path is `claimed`, `afforded`, or `observed`.

**Epistemic evidence.** Look for a material route that handles truth-apt content, and for any consequential knowledge-production or warrant claim the system makes — including where the eventual finding is failure or absence. Successful knowledge production is never a prerequisite for running the lens: a claimed but failed or unsupported knowledge route is trigger evidence.

**Direct-adaptation exception.** Evaluated direct behavior or policy adaptation with no truth-apt object and no knowledge or warrant claim is not an epistemic object. The exception scopes what the epistemic lens treats as its objects; such a route is not epistemic trigger evidence. It stays in the runtime account, and the scoping record names it for the orchestrator. If the epistemic lens is skipped, the skip record lists these routes and states that their content/update classification is among the conclusions the skip prevents. Otherwise hand it to the invoked epistemic procedure tagged **classify-only**: that procedure classifies every content-changing edge it meets and carries a class for exactly these routes, so withholding one would leave a silent hole in its ledger. Classify-only means the route is recorded in its content/update classification and is *not* analysed for warrant, transformation, or acceptance. If the lens concludes the route is in fact truth-apt, that comes back as a correction under step 3's correction branch — never as a silent expansion of the lens's own scope.

### 6. Run the embedded memory/context lens

Analyse accumulated-from-use mechanisms; this is not a memory-review publication workflow. If the step-5 scoping record skipped this lens, do not run this step. Otherwise work at the depth the step-5 scoping record warrants: the items below are the full pass, and a brief pass covers the same ground proportionately rather than skipping items silently.
...
1. Invoke the procedure in `kb/instructions/analyse-external-system-epistemic-architecture.md` to run the accepted route-analysis method inside this run's boundary. Invoke it in every run whose step-5 scoping record did not skip the epistemic lens; if it was skipped, do not run this step. Do not copy or restate its object-inventory, route-ledger, transformation, lifecycle, claim-comparison, or authority method.
...
1. Verify: source anchors; every conclusion status is one of the seven values listed in step 3, and no record carries `implemented` as a conclusion status; unique, resolving IDs; one boundary and revision across all records; mandatory runtime coverage; both lens scoping records present; both lens outputs present, each either meeting the brief-output floor or, for a skipped lens, stating the skip, the evidence searched, and the conclusions the skip prevents; no lens skipped while its scoping record cites trigger evidence or rests on uninspected evidence; prevented conclusions stated for every thin, negative, or unresolved finding; shared-route ownership respected; no forbidden evidence upgrades.
...
4. Report: result identity and location, boundary/revision/tier, both lens scoping records and the depth each lens ran at or that it was skipped, limitations, and any blockers.
...
- Each lens ran, or was skipped because scoping found no trigger evidence for it. Both scoping records and both lens outputs exist as explicit records; nothing is implied by an absent section. A brief output still names what was inventoried, what was found, and what its thinness prevents; a skipped lens's record names the evidence searched and the conclusions the skip prevents.
