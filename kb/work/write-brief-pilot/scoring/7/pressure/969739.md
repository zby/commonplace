...
Use these ID prefixes, numbered in order of assignment: `SRC-` sources, `OBJ-` objects and parts, `RTE-` routes, `CLM-` system claims. When the invoking workflow supplies a canonical register, use its IDs and propose new records under local labels instead of allocating new canonical IDs. Never reuse an ID.

Assign every source an ID in output 1. Map the ID to its identity or revision and evidence layer. Later records may cite the ID plus a **local anchor**: a source-local section, symbol, line, event, artifact, or other available locator.
...
Use one row per operative part within the material-route boundary and assign each an `OBJ-` ID. Split parts that differ in content, form, checks, producers or consumers, or authority paths.
...
Use one row per consequential function and assign each an `RTE-` ID. Split rows when the function, target, evaluator domain, timing, result, force, epistemic license, operational consequence, consumer, channel, or horizon differs. Record an evidenced absence without inventing an evaluator.

The ledger has a fixed form so that ledgers from different reviews can be placed side by side. Use exactly these columns, in this order and with these names, as a table or as one record per route with the same field labels:

| # | column | value |
|---|---|---|
| 1 | `route id` | `RTE-<n>` |
| 2 | `function` | one functional kind, listed below |
| 3 | `architectural status` | one status, listed below |
| 4 | `object ids` | `OBJ-` IDs the route reads or changes |
| 5 | `content/update relation` | one relation, listed below |
| 6 | `check target` | object, proposition, or class, plus its domain; or `none` |
| 7 | `evaluator` | `human`, `model`, `program`, `environment`, `proof`, `measurement`, or `hybrid`, then `: <name>; domain <domain>`; or `none` |
| 8 | `activation` | `always`, `conditional: <condition>`, `inactive in inspected configuration`, or `not determinable`; then `; <trigger point in the system's own terms>` |
| 9 | `result` | `possible: <values>` or `observed: <values>` |
| 10 | `epistemic authority` | `<content licensed for reliance> within <scope>`, or `none` |
| 11 | `operational authority` | `permits`, `blocks`, or `changes`, then `: <behavior>`; or `none` |
| 12 | `consumer` | the component, actor, or later step that consumes the result or artifact; or `none` |
| 13 | `channel` | how it reaches the consumer (for example prompt injection, API return, file read, gate exit code); or `none` |
| 14 | `force` | `enforcing`, `permissive`, `ranking`, `advisory`, or `none` (no consequential consumer) |
| 15 | `horizon` | `current operation`, `current session or task`, `persistent until changed`, or `not determinable` |
| 16 | `evidence` | `SRC-<n>: <local anchor>`, one or more |
| 17 | `claim ids` | `CLM-` IDs, or `none` |
| 18 | `mismatch` | `CLM-<n>: <one-line mismatch>`, or `none` |
| 19 | `gap/limit` | free text, or `none` |

Fill every cell. Write `none` for an evidenced absence and `not determinable` when the evidence cannot decide; never leave a cell blank. Where a column lists values, copy one verbatim and put any qualification in `gap/limit`. Order rows by function in the order of the functional-kind list below, then by route ID.
...
This example shows the shape of each output, not a reference result. *Retro* is an invented system; do not reuse its values. Records are abbreviated, and a real analysis fills every field.

Retro is a coding agent. After a failed test run, a model writes a one-line "lesson" (for example, "the payments tests need `TZ=UTC`") and appends it to a lessons file. Every later session injects all lessons into the system prompt. A user can flag a lesson, which deletes it. The README says Retro "learns verified lessons from its failures."

1. **Boundary.** System: Retro v1.2; scope: lessons subsystem; excluded: the base model and the test runner. `SRC-1` = `lessons.py` at v1.2, implementation. `SRC-2` = README at v1.2, doctrine/design. Missing: run logs, which prevents any observed candidate state. `CLM-1` = "learns verified lessons from its failures" (`SRC-2: Overview`).
2. **Objects.** `OBJ-1` test-failure output: symbolic log imported from the test runner; truth-apt content "test T failed with message M". `OBJ-2` lesson entry: natural-language generalization written by the model; claimed role "verified lesson".
3. **Ledger.** `RTE-1` content transformation (`OBJ-1` to `OBJ-2`); `RTE-2` check/evidence production (user flag); `RTE-3` disposition/acceptance (deletion on flag); `RTE-4` retention (append to the lessons file); `RTE-5` operational admission/selection/consumption (injection). All are `implemented`. One full record:

   `RTE-5 | operational admission/selection/consumption | implemented | OBJ-2 | no content change | none | none | always; session start | possible: all lessons injected | none | changes: agent actions in later sessions | agent model | system-prompt injection | advisory | persistent until changed | SRC-1: build_prompt() | CLM-1 | CLM-1: "verified" but no check precedes injection | no run logs`

   `RTE-1` has content/update relation `truth-apt transformation: ampliative conjecture`, because a lesson generalizes beyond the single failure it came from. `RTE-2` has evaluator `human: user; domain one lesson` and no recorded criterion. `RTE-3` can only reject.
4. **Lifecycle.** `OBJ-2`: observation and conjecture via `RTE-1` are `implemented`; derived consequence, test, acceptance, and lifecycle integration are `no route found within boundary`, because `RTE-2` and `RTE-3` can reject a lesson but never accept one. Every phase has observed candidate state `no instance observed`, because there are no run logs. `OBJ-1`: non-ampliative, `acquisition/import`, source warrant preserved.
5. **Claims.** `CLM-1`: doctrine/design support `SRC-2: Overview`; implemented routes `RTE-1`, `RTE-4`, `RTE-5`; no observed-run or causal support; supported conclusion: lessons are conjectured and retained, and no check precedes their use; mismatch: "verified".
6. **Conclusion.** Retro conjectures lessons from failures, retains them, and injects them as advisory context until a user deletes one. Within the boundary, no route accepts a lesson, so Retro does not produce accepted ampliative claims, and `CLM-1`'s "verified" has no implementing route. Without run logs, its observed behavior is unknown.
...
- Every material function has a route row in the fixed column order, with every cell filled, listed values copied verbatim, and rows ordered by function and then route ID.
