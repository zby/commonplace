...
Assign every source a stable ID in output 1. Map the ID to its identity or revision and evidence layer. Later records may cite the ID plus a **local anchor**: a source-local section, symbol, line, event, artifact, or other available locator, written `SRC-n @ <anchor>`.

Use these ID forms in every output: `SRC-n` for sources, `OBJ-n` for objects, `RTE-n` for routes, and `CLM-n` for system claims, numbered from 1 in order of assignment. Write every ID in full in lists (`RTE-1, RTE-2`), never as a range or abbreviation. If an invoking workflow supplies canonical IDs, cite them unchanged and follow that workflow's rule for proposing new records.
...
Produce these six blocks in order. Use readable Markdown tables or compact records, except for output 3, which has a fixed table form. The worked example after output 6 shows all six blocks for one small system.
...
The ledger has a fixed form so that ledgers from different reviews can be read side by side. Write it as one Markdown table with exactly this header, in this column order, adding, dropping, or merging no column:

`route id | route function | architectural status | object ids | content/update relation | transition or check target | evaluator/condition and domain | activation | timing | result | implemented force | epistemic authority | operational authority | behavioral-authority path | evidence | claim ids | mismatch | gap/limit`

Fill each cell in its stated form:

| column | form |
|---|---|
| route id | one `RTE-n` |
| route function | one functional kind from the list below |
| architectural status | one status from the list below |
| object ids | `OBJ-n` list, or `none` |
| content/update relation | one value from the list below |
| transition or check target | the object, proposition, or class and its domain, or `none` |
| evaluator/condition and domain | the evaluator or condition and the domain it judges, or `none` |
| activation | `always`, `conditional: <condition>`, `inactive in inspected configuration`, or `not determinable` |
| timing | when the route runs, relative to a named event or route (`session start`, `after RTE-1`) |
| result | `possible: <result>` or `observed: <result>`, listing both when both are evidenced, or `none` |
| implemented force | `advisory`, `ranking`, `permissive`, `enforcing`, `none` (no consequential consumer), or `not determinable` |
| epistemic authority | `<content> within <scope>`, or `none` |
| operational authority | `permits: <behavior>`, `blocks: <behavior>`, `changes: <behavior>`, or `none` |
| behavioral-authority path | `<consumer>; <channel>; <horizon>`, or `none`; horizon is `current step`, `current run`, `across runs until <end condition>`, or `not determinable` |
| evidence | `SRC-n @ <anchor>` list |
| claim ids | `CLM-n` list, or `none` |
| mismatch | `none`, `doctrine without implementation`, `implementation differs from doctrine`, `claim exceeds route`, `not determinable`, or `other — describe` |
| gap/limit | short text, or `none` |

Never leave a cell blank. Write `none` when evidence within the recorded boundary shows the element is absent, and `not determinable` when the evidence cannot decide. Put qualifications that fit no column in `gap/limit`. Order rows by route function in the order of the list below, then by route ID.

Use one functional kind per row: `content transformation`, `check/evidence production`, `disposition/acceptance`, `retention`, `lifecycle integration`, `operational admission/selection/consumption`, `behavior/policy adaptation`, `lineage/freshness/recovery`, or `other — describe`. If one implementation performs several functions, use linked rows that share `object ids` and name each other in `timing` or `gap/limit`. Never merge checking with disposition or retention with lifecycle integration.
...
The system below is invented to illustrate the outputs; it is not evidence about any real system. A coding agent has a lessons memory. After a failed test run, the model calls a `write_lesson` tool, which appends one sentence to `lessons.md`. At the next session start, the prompt builder injects the whole file. The README says lessons are verified against the test suite before saving, so the agent learns from its mistakes. The code has no verification step.

**1. Source-and-claim boundary.** system: example agent | revision: commit `abc1234` | scope: lessons memory; excludes model provider and tool sandbox | question: does the lessons memory produce knowledge? | assessed: lesson write, check, retention, injection | unassessed: none within scope | sources: `SRC-1` repository at `abc1234`, implementation; `SRC-2` README at `abc1234`, doctrine/design; `SRC-3` one session transcript and the next session's prompt, observed run | missing evidence: no runs with lessons disabled -> no conclusion about the lessons' effect on behavior | claims: `CLM-1` "lessons are verified before saving, so the agent learns from its mistakes", `SRC-2 @ Memory`.

**2. Epistemic-object inventory.** `OBJ-1` | `lessons.md` entry: one sentence per failed run | natural-language | model output conditioned on the failure log | producer: model via `write_lesson`; consumer: prompt builder | general claim about the codebase, such as "tests need `DB_URL` set" | guide later sessions | `SRC-1 @ tools/lessons.py`, `SRC-3 @ turn 14` | none.

**3. Authority-route ledger.**

| route id | route function | architectural status | object ids | content/update relation | transition or check target | evaluator/condition and domain | activation | timing | result | implemented force | epistemic authority | operational authority | behavioral-authority path | evidence | claim ids | mismatch | gap/limit |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| RTE-1 | content transformation | implemented | OBJ-1 | truth-apt transformation: ampliative conjecture | failure log -> lesson sentence | none | conditional: test run failed | after a failed test run | observed: one lesson written | permissive | none | permits: retention by RTE-3 | none | SRC-1 @ tools/lessons.py, SRC-3 @ turn 14 | CLM-1 | none | none |
| RTE-2 | check/evidence production | doctrine only | OBJ-1 | no content change | lesson against the test suite | none | not determinable | before save, per SRC-2 | none | none | none | none | none | SRC-2 @ Memory, SRC-1 @ tools/lessons.py | CLM-1 | doctrine without implementation | search covered all `write_lesson` callers |
| RTE-3 | retention | implemented | OBJ-1 | no content change | `lessons.md` append | none | always | after RTE-1 | observed: lesson appended | permissive | none | permits: later injection by RTE-4 | none | SRC-1 @ tools/lessons.py | none | none | no deletion or revision route found |
| RTE-4 | operational admission/selection/consumption | implemented | OBJ-1 | no content change | `lessons.md` -> system prompt | none | always | session start | observed: file injected | advisory | none | changes: model context in every later session | model; system prompt; across runs until the entry is deleted by hand | SRC-1 @ prompt.py, SRC-3 @ next-session prompt | CLM-1 | none | effect on behavior unmeasured |

**4. Per-object lifecycle disposition.** `OBJ-1` | RTE-1, RTE-2, RTE-3, RTE-4 | ampliative conjecture | observation/anomaly: RTE-1, implemented, phase evidenced, `SRC-3 @ turn 12` | conjecture: RTE-1, implemented, phase evidenced, `SRC-3 @ turn 14` | derived consequence: none, no route found within boundary, not reached, `SRC-1` | test/evidence: RTE-2, doctrine only, not reached, `SRC-1`, `SRC-3` | acceptance: none, evaluator none, criterion none, intended use "guide later sessions", no route found within boundary, not reached, scope none, `SRC-1` | lifecycle integration: none, no route found within boundary, not reached (the lesson was retained and injected without acceptance, RTE-3 and RTE-4), `SRC-3` | missing: a test and an acceptance route.

**5. System-claim versus route comparison.** `CLM-1` | lessons are verified, and the agent learns | `SRC-2 @ Memory`, doctrine/design | declares RTE-2 | RTE-1, RTE-3, RTE-4 | one lesson written and injected, unchecked | none | the system generates, retains, and reuses unchecked conjectures | the check is doctrine only; "learns" is unsupported without a check, acceptance, or a lessons-off comparison.

**6. Bounded conclusion.** The lessons memory generates ampliative conjectures (RTE-1), retains them (RTE-3), and injects them into every later session with advisory force (RTE-4). No implemented route tests or accepts them, so no lesson is produced knowledge within this boundary; the documented check (RTE-2) is doctrine only. Whether injected lessons change behavior is unknown because no comparison run exists.
...
- The ledger uses the fixed header and column order, the stated cell forms, full `SRC-n`/`OBJ-n`/`RTE-n`/`CLM-n` IDs, and no blank cell, with rows ordered by route function and then route ID.
