...
Use these ID forms so that results from different reviews can be compared: `SRC-n` for sources, `CLM-n` for system claims, `OBJ-n` for objects or parts, and `RTE-n` for routes, numbered from 1 in order of first appearance. Always write complete IDs: `OBJ-1, OBJ-2`, never `OBJ-1/2` or an ID range. If the caller supplies canonical IDs, use them unchanged and do not renumber.
...
The format below is fixed so that ledgers from different reviews line up column by column. Write one Markdown table with exactly these column headers, in this order, and sort rows by route ID:

| column | fill with |
|---|---|
| `route` | the route ID |
| `function` | one functional kind (listed below) |
| `status` | one architectural status (listed below) |
| `objects` | object IDs, comma-separated |
| `content relation` | one content/update relation value (listed below) |
| `target` | the check or transition target and its domain |
| `evaluator kind` | `human`, `model`, `program`, `environment`, `proof`, `measurement`, `hybrid: <kinds>`, or `none` |
| `evaluator` | the named evaluator or condition and its domain |
| `activation` | the condition under which the route runs, or `always` |
| `timing` | `at intake`, `before retention`, `at retrieval or use`, `after use`, `scheduled`, `on demand`, or `other: <describe>` |
| `result` | the result values, prefixed `possible:` or `observed:` |
| `force` | the implemented force on the consumer: `none`, `advisory`, `ranking`, `permissive`, or `enforcing` |
| `epistemic authority` | the content and scope licensed for reliance |
| `operational authority` | `permits: <behavior>`, `blocks: <behavior>`, or `changes: <behavior>` |
| `consumer and channel` | what consumes the result or artifact, and through which channel |
| `horizon` | `this call`, `this task or session`, `persistent until changed`, `persistent with expiry: <rule>`, or `not determinable` |
| `evidence` | source ID and local anchor |
| `claims` | claim IDs |
| `mismatch` | the mismatch between claim and route |
| `gap/limit` | what the row cannot establish |

Fill every cell. Write `none` for an evidenced absence, `not inspected` where the declared boundary excluded the evidence, and `not determinable` where the evidence cannot decide. Never leave a cell blank or write "N/A". Use the listed values verbatim; put qualifications in `gap/limit` rather than in a variant value.
...
This example shows the shape of each output. The system is invented for illustration; nothing here describes a real product. A real analysis needs complete rows and anchors.

**System X** is a chat-memory service. After each conversation, a model reads the transcript under a prompt that asks it to "infer the user's preferences and traits" and writes one sentence per inferred fact. A program drops any new fact whose embedding is too similar to a stored fact and stores the rest. At each new request, the most similar stored facts are inserted into the response model's prompt. The README says System X "learns facts about the user over time."

**1. Boundary.** System X at commit `abc123`; memory subsystem only, response model excluded. Question: does System X produce knowledge about the user? `SRC-1` = repository at `abc123`, implementation. `SRC-2` = README at `abc123`, doctrine/design. No run traces: this prevents any observed candidate state. `CLM-1` = "learns facts about the user over time" (`SRC-2`, opening section).

**2. Inventory.** `OBJ-1` = stored transcript turn; natural-language; from the user; truth-apt as a user report. `OBJ-2` = inferred fact; one natural-language sentence; produced by the extraction model from `OBJ-1`; truth-apt as a claim about the user.

**3. Ledger** (columns abbreviated here to those that differ between rows; a real ledger carries all columns):

| route | function | status | objects | content relation | target | evaluator kind | timing | force | epistemic authority | operational authority | horizon |
|---|---|---|---|---|---|---|---|---|---|---|---|
| RTE-1 | content transformation | implemented | OBJ-1, OBJ-2 | truth-apt transformation: ampliative conjecture | none | none | after use | none | none | none | persistent until changed |
| RTE-2 | check/evidence production | implemented | OBJ-2 | no content change | similarity of OBJ-2 to stored facts | program | before retention | enforcing | none | blocks: storing a near-duplicate | this call |
| RTE-3 | retention | implemented | OBJ-2 | no content change | none | none | before retention | none | none | permits: later retrieval | persistent until changed |
| RTE-4 | operational admission/selection/consumption | implemented | OBJ-2 | no content change | similarity of OBJ-2 to the request | program | at retrieval or use | advisory | none | changes: response-model prompt | this call |

`RTE-2` checks novelty, not truth, so it has no epistemic authority.

**4. Lifecycle.** `OBJ-2` is ampliative. Observation and conjecture: `RTE-1`, `implemented`, `no instance observed`. Derived consequence, test, acceptance, and lifecycle integration: `no route found within boundary`, `no instance observed`. `OBJ-1` is non-ampliative: acquisition; discovery lifecycle not applicable; source warrant preserved as a user report because the transcript is stored verbatim.

**5. Claim comparison.** `CLM-1`: doctrine `SRC-2`; implemented `RTE-1`, `RTE-3`, `RTE-4`; no observed-run or causal support. Supported conclusion: System X retains model-inferred candidate statements and uses them in prompts. Mismatch: "learns facts" implies accepted content, and no acceptance route exists.

**6. Conclusion.** System X acquires user reports and conjectures user traits from them. It checks candidates only for novelty, accepts none, and retains and injects them as unaccepted candidates with advisory force. Within the boundary it does not produce knowledge. Run traces would be needed to report how candidates behave in practice.
...
- Every material function has a route row in the fixed output 3 table: all columns present in order, every cell filled, listed values used verbatim, and IDs complete.
