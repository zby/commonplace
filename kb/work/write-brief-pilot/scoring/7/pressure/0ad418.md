...
**Ledger conventions.** Fill the ledger so that ledgers from different reviews line up row by row and field by field:

- **IDs.** Number sources `S1`, `S2`, …; objects `O1`, …; routes `R1`, …; claims `C1`, …. Give linked rows split from one implementation the same number with letter suffixes (`R3a`, `R3b`). If a calling workflow supplies canonical IDs, use those instead.
- **Row order.** Sort rows by object ID, then by the order of the functional-kind list above. Put rows with no object last.
- **Fields.** Keep every field in schema order and fill each one. Write `none` when the evidence shows nothing there, and `not determinable` when the evidence cannot decide; put the reason in `gap/limit`.
- **Exact strings.** Copy `route function`, `architectural status`, and `content/update relation` values exactly from the lists above; write the open case as `other — <description>`.
- **Field formats.** Use these forms:
  - `activation and timing`: `<condition, or always>; <timing>`;
  - `possible or observed result`: `possible: <result>` or `observed: <result>`;
  - `implemented force`: `advisory`, `ranking`, `permissive`, `enforcing`, or `none` when no consequential consumer takes the result;
  - `epistemic authority and scope`: `<content licensed for reliance> over <scope>`, or `none`;
  - `operational authority`: `permits`, `blocks`, or `changes`, followed by the behavior, or `none`;
  - `behavioral-authority path`: `<consumer>; <channel>; <force>; <horizon>`, or `none`;
  - evidence: `<source ID>@<local anchor>`, comma-separated;
  - mismatch marker: `<claim ID>: <mismatch>`, or `none`.
- **Evidenced absence.** For a `no route found within boundary` row, write evaluator `none` and force `none`, and name the searched boundary in `gap/limit`.
...
This example is illustrative. "LessonLoop" is an invented coding agent, not a reviewed product; the example shows the shape of each output, not findings about any real system. LessonLoop runs tests after each task. After a failure, an LLM reflection step writes a general "lesson" to `lessons.md`. At the start of later tasks, retrieval injects the five most similar lessons into the prompt. Its README says the agent "learns verified lessons from experience."

**1. Boundary.**

`LessonLoop | v0.3 | memory subsystem: outcome recording, reflection, lesson store, retrieval; excludes the code-editing model | does LessonLoop produce knowledge from task outcomes? | recording, reflection, retention, retrieval | hosted telemetry | S1 -> repo at v0.3, implementation; S2 -> README at v0.3, doctrine/design; S3 -> runs 17-18 logs, observed run | no causal experiment -> no attribution of task success to lessons | C1 "learns verified lessons from experience" -> S2@Features`

**2. Objects.**

`O1 | test outcome record | structured log entry | test runner exit code and failing test names | runner -> reflection | "task run N failed tests T" | outcome record | S1@runner.py:record_outcome | none`

`O2 | lesson | natural-language line in lessons.md | reflection over O1 and transcript | reflection -> retrieval, coding model | a general claim about the repository, such as "run make deps before make test" | verified lesson (C1) | S1@reflect.py:write_lesson | wording varies by run`

**3. Ledger** (rows in the required order; two rows shown in full):

`R2 | content transformation | implemented | O2 | truth-apt transformation: ampliative conjecture | O2: general lesson about the repository, from O1 | none | O1 shows a failure; after task end | observed: lesson written | none | none | none | none | S1@reflect.py:write_lesson, S3@run-17 | C1 | none | the lesson generalizes beyond one failing run`

`R3 | check/evidence production | no route found within boundary | O2 | no content change | O2: truth of the lesson over the repository | none | none; none | none | none | none | none | none | S1@reflect.py, S1@memory/ | C1 | C1: "verified" but no check route | searched boundary: S1 reflect.py, memory/, hooks/`

The other rows are R1 (O1, `content transformation`, `truth-apt transformation: acquisition/import`, warrant preserved from the test runner), R4 (O2, `retention`, append to `lessons.md`, force `none`), and R5 (O2, `operational admission/selection/consumption`, embedding similarity over `lessons.md`, force `advisory`, operational authority `changes prompt content of later tasks`, behavioral-authority path `coding model; system-prompt block; advisory; every later task until the lesson is deleted`).

**4. Lifecycle.** O1 gets the non-ampliative record: `O1 | R1 | acquisition/import | discovery lifecycle: not applicable | R1, warrant preserved from the test runner | none`. O2 gets the ampliative record: observation and conjecture are `implemented` and `phase evidenced` (S3@run-17); derived consequence, test, and acceptance are `no route found within boundary` and `not reached`; lifecycle integration is `not reached`, because run 18 shows O2 retained and used without acceptance.

**5. Claims.** `C1 | learns verified lessons | S2@Features, doctrine/design | stated | R2, R4, R5; no check or acceptance route | S3: lesson written in run 17, injected in run 18 | none | LessonLoop generates, retains, and reuses candidate lessons | "verified" is unsupported`

**6. Conclusion.** LessonLoop acquires test outcomes with preserved warrant. It generates ampliative candidate lessons, retains them, and gives them advisory force over later prompts. No route checks or accepts a lesson, so it produces no accepted claims within the boundary, and C1's "verified" is unsupported. Whether lessons improve task success is unassessed, because no causal experiment exists.
...
- The ledger follows the ledger conventions: ID scheme or caller-supplied canonical IDs, row order, no blank fields, exact strings, and field formats.
