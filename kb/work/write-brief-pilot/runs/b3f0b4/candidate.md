---
description: "Supplement: the automated software house conjecture as an alternative test of theory refinement with fixed models; the 2026-09-02 claim, Naur's program theory, four witness conditions, and a side-by-side comparison with the knowledge-base arrangement"
...
machinery rather than its models. This is a second test, after a knowledge
base, of the lead article's paradigm: learning by theory refinement with
fixed models. The theory a house refines is what Peter
Naur, who argued that programming is theory building, called *program
theory*: the understanding of a program's purpose, organization, and how to
handle new requests. Naur held that it is bound to people. Because software fails visibly, the house gives the
...
working standard is comparative: given the same requests and resources, the
...
in advance. A successful run supports the conjecture only under its reported
workload, horizon, and budget; the open-ended claim covers requests no
...
The conjectured house combines fixed LLMs that interpret and propose,
natural-language project state that retains purposes, commitments, and
decisions, and symbolic software that supplies exact behaviour: the
product, tools, tests, validation, and rollback. No component holds the
program theory alone; a note that is never loaded has no effect. The
conjecture does not fix whether the house
keeps an explicit project theory or reconstructs its understanding from
...
is the nearest formal construction. Its prover admits a rewrite of its
own code only after proving that switching pays, so it "must ignore those self-improvements whose effectiveness it cannot prove"
([Schmidhuber](../sources/goedel-machines-schmidhuber.ingest.md), §2.4,
verbatim). Like the house, it requires every successor state to arise
through the current machinery and permitted inputs. Unlike the house, it
admits a change by proof; the house admits a fallible change on empirical
grounds, observes its consequences, and recovers from a wrong one.

Two reports make parts of it concrete. [OpenAI's agent-first
...
reports benchmark gains from search over agent code around frozen models.
Neither shows program theory sustained across later product changes.
[Nearest existing
...
for consuming projects. Both arrangements test the same paradigm through
an externally assessed product, and they differ on one point, Naur's
program theory. A house's theory is *about* the software, which then works
or does not. A knowledge base's theory *is* the product: consuming agents
read it and act on it, so no second artifact tests the theory
independently of how it was read. The other rows follow from the first.

| | Software house | Knowledge-base arrangement |
|---|---|---|
| Retained theory | A theory *of* the product | A theory that *is* the product |
| Falsifier | A failing test, invalid release, or user who cannot complete a task, whatever the model understood the theory to say | A judge rejecting an agent's work: evidence about task, consumer, and knowledge base combined |
| Evidence per episode | Sharper: the theory or its use failed | Less sharp: separating theory from reading needs further interventions |
| Claim form | Existence: one witness meeting four conditions | Sufficiency across declared areas, with evidence about transfer |
| A failed run refutes | Only that construction | Sufficiency for that area |
| Starting point | A seed that is already a working house | An existing human-inclusive builder that moves roles to computation one class at a time |
| Place in the program | Kept beside, for its stronger evidence | Pursued first |

Neither claim form substitutes for the other. The software house is not
the program's paradigm case or a special case of the knowledge-base
arrangement; it is a different product with its own model cutoff. Whether a broadly capable
knowledge-producing builder must eventually become a software house,
because new domains require it to revise its own production software, is a
...
house](./nearest-existing-constructions-to-a-witness-house.md) reviews
...
states the boundary rule in general form, and the [evidence
supplement](./testing-the-theory-refinement-program.md) gives the
