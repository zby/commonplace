...
supplement gives the full protocol; the walkthrough below follows one run
through it.

### How a run would go

The protocol is a first design, and none of the choices it requires has
been made yet. A run would proceed in five steps.

1. **Freeze the declarations.** Before the first scored task, the run
   fixes its terms: the consuming project and the tasks it may bring; the
   seed, pinned at the start; the exact model versions for Commonplace and
   for the consuming agents; how tasks are supplied and how omitted or
   failed ones are recorded; the acceptance contract for each task and who
   may change it; budget ceilings for computation, time, evidence, and
   internal human work, equal across the compared conditions; the horizon
   and stopping rule, which may not stop when a desired score first
   appears; the reliability target and comparison margin; what feedback
   returns and with what delay; and whether Commonplace may run its own
   diagnostic probes, which are allowed in bounded number and charged to
   the budget.
2. **Deliver and use a release.** Commonplace delivers a versioned release:
   the knowledge base with its validators, skills, and indexes. An agent in
   the consuming project performs a task with it. Before the task runs, an
   expectation is stated: which part of the knowledge base the agent should
   consult, and what an answer drawn from the wrong source would look like.
   The project's judge then accepts or rejects the output and gives a
   reason.
3. **Record the episode.** Each task, from delivery to judgment, is one
   episode. Its record keeps the task and its contract, the delivered
   versions, the consumer's configuration, the budget used, the output, the
   judge's decision and reason, the feedback returned, and which artifacts
   the agent actually read before each consequential decision. Reading the
   expected artifact shows that it was consulted, not that it caused the
   result.
4. **Revise between episodes.** Feedback on incoming work drives
   development. Commonplace diagnoses a rejection and revises the knowledge
   base, or, when the failure exposes a limit in its own methods, revises
   those methods. A rejection is evidence about the combined task, consumer,
   and knowledge base, not an automatic refutation of any one note, so
   choosing what to revise is part of the work being assessed.
5. **Assess on reserved tasks.** Before the final comparison, the candidate
   release is frozen and assessed on reserved tasks whose outcomes did not
   guide its construction. An assessment result that is then used to choose
   or repair a candidate becomes development feedback for its successors,
   not a further assessment.

The hypotheses are decided by comparisons run under the same
declarations. The same later task with one retained change removed tests
whether that change added capability. Control runs, with the retained
content removed, replaced by a plausible distractor, reverted to a stale
version, or with some other mechanism able to supply the answer, bound other
explanations of a gain. Direct search over the raw
records without the learned methodology tests the comparison hypothesis,
and a human-staffed builder under the same demands and resources, with
each human intervention recorded, supplies the reliability comparison.
