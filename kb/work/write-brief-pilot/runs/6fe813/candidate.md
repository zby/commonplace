...
Analyse one external agentic system at one frozen evidence boundary. Run a mandatory runtime baseline, then scope the memory/context lens and the epistemic lens: run each at a depth proportionate to its trigger evidence, or skip it with an explicit skip record when scoping finds no trigger evidence. Reconcile shared records by stable IDs, and return one bounded system synthesis. The consumer is an analysing agent or maintainer; the channel is explicit invocation or trigger-matched skill loading; the force is a prescriptive analysis and result-writing policy.
...
- Conclusion statuses (use exactly these):
  ...
  - `inapplicable` — the stated trigger conditions are false inside that boundary; this is a finding about the system under review; it does not by itself decide whether a lens runs, which step 5 decides from trigger evidence;
...
This step decides, for each lens, whether it runs and how deep. It is where the trigger evidence is named before any lens worker sees it.

For each lens (memory/context; epistemic), emit one scoping record: `{lens, trigger evidence IDs, inspected boundary, the routes and objects that evidence points the lens at, warranted depth: full | brief | skip, rationale}`.

- Warranted depth follows the evidence. Rich trigger evidence warrants a full pass; thin or degenerate evidence warrants a brief one. A brief pass is a result, not an omission.
- **Skip only on no trigger evidence.** When the search below finds no trigger evidence for a lens inside the frozen boundary, you may skip that lens entirely instead of running a brief pass. Any trigger evidence, however thin, degenerate, or merely claimed, rules the skip out: scope the lens brief instead. Unresolvable evidence also rules it out (see `uncertain` below). A skip record sets warranted depth to `skip`, lists no trigger evidence IDs, and must state three things:
  - the skip itself, stated as a decision;
  - the evidence searched: the inspected boundary and what was looked for in it — the sources, components, and route kinds checked;
  - the conclusions the skip prevents, such as any claim about how accumulated material returns, or about how truth-apt content is checked or accepted.
  A skip record states that no trigger evidence was found within the searched boundary. It never states or implies that the system has no memory or no epistemic route.
...
- An absent lens section or file must never carry meaning implicitly. Every run emits both scoping records and both lens outputs. The output of a skipped lens is a short explicit statement that the lens was skipped, citing its scoping record and repeating the conclusions the skip prevents.

**Memory/context evidence.** Look for a path by which material accumulated or changed through use can affect a later invocation or action, in code, documentation, or observation. Static shipped material, ordinary current-run state, and retained material with no later delivery path are not such paths. Where these are all you find, there is no trigger evidence: you may skip the lens, and the skip record names what retained material was found and why none of it qualifies. A merely claimed path is trigger evidence; the lens output preserves whether the path is `claimed`, `afforded`, or `observed`.

**Epistemic evidence.** Look for a material route that handles truth-apt content, and for any consequential knowledge-production or warrant claim the system makes — including where the eventual finding is failure or absence. Successful knowledge production is never a prerequisite for running the lens: a truth-apt route or a knowledge or warrant claim is trigger evidence even when it fails.

**Direct-adaptation exception.** Evaluated direct behavior or policy adaptation with no truth-apt object and no knowledge or warrant claim is not an epistemic object. The exception scopes what the epistemic lens treats as its objects; such a route is not trigger evidence. It stays in the runtime account, and the scoping record names it for the orchestrator. If the epistemic lens is skipped, the skip record names these routes and states that they were not classified by the epistemic procedure. If the lens runs, hand each such route to the invoked epistemic procedure tagged **classify-only**: that procedure classifies every content-changing edge it meets and carries a class for exactly these routes, so withholding one would leave a silent hole in its ledger. Classify-only means the route is recorded in its content/update classification and is *not* analysed for warrant, transformation, or acceptance. If the lens concludes the route is in fact truth-apt, that comes back as a correction under step 3's correction branch — never as a silent expansion of the lens's own scope.

### 6. Run the embedded memory/context lens

Analyse accumulated-from-use mechanisms; this is not a memory-review publication workflow. Work at the depth the step-5 scoping record warrants: the items below are the full pass, and a brief pass covers the same ground proportionately rather than skipping items silently. If the scoping record skips this lens, do not run this step; the lens output is the skip statement step 5 requires.
...
1. Invoke the procedure in `kb/instructions/analyse-external-system-epistemic-architecture.md` to run the accepted route-analysis method inside this run's boundary. Every run invokes it unless the step-5 epistemic scoping record skips the lens; then do not invoke it, and the lens output is the skip statement step 5 requires. Do not copy or restate its object-inventory, route-ledger, transformation, lifecycle, claim-comparison, or authority method.
...
7. both lens outputs, a skipped lens's output being its skip statement;
...
- Publishable limitations include: doc-only evidence, inaccessible components, no observed run, no causal experiment, trigger evidence too thin to resolve, a lens skipped for lack of trigger evidence, and conflicting evidence — each naming its scope and prevented conclusion. Publication blockers include: missing logical records, ID collisions, unsupported material claims, failed applicable validation, and no authorized target contract.

### 10. Verify and report

1. Verify: source anchors; every conclusion status is one of the seven values listed in step 3, and no record carries `implemented` as a conclusion status; unique, resolving IDs; one boundary and revision across all records; mandatory runtime coverage; both lens scoping records present; both lens outputs present, each run lens's output meeting the brief-output floor; each skip record stating the skip, the evidence searched, and the conclusions the skip prevents, with no trigger evidence registered for that lens; prevented conclusions stated for every thin, negative, or unresolved finding; shared-route ownership respected; no forbidden evidence upgrades.
...
4. Report: result identity and location, boundary/revision/tier, both lens scoping records and the depth each lens ran at or its skip, limitations, and any blockers.
...
- Each lens either ran or was skipped for lack of trigger evidence. Both scoping records and both lens outputs exist as explicit records; nothing is implied by an absent section. A brief output still names what was inventoried, what was found, and what its thinness prevents; a skip record names the evidence searched and the conclusions the skip prevents.
