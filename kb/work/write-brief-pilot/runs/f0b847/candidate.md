...
- *The concession*: methodology, heuristics, and conventions may migrate into weights, and structure that stops earning its marginal value should be [relaxed or removed](./codification-and-relaxing-navigate-the-bitter-lesson-boundary.md). The concession does not extend to the core vocabulary. That vocabulary is now settled enough to commit to: adopting a term resolves a choice among live options, so it is a [commitment](./commitment-not-derivation-creates-new-ground-truth.md) rather than guidance waiting for a stronger model to absorb it.
