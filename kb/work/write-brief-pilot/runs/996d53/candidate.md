...
Analyse one external agentic system at one frozen evidence boundary. Run a mandatory runtime baseline, then scope the memory/context lens and the epistemic lens: run each lens whose scoping finds trigger evidence, at a depth proportionate to that evidence, and skip a lens with no trigger evidence only under a recorded skip. Reconcile shared records by stable IDs, and return one bounded system synthesis. The consumer is an analysing agent or maintainer; the channel is explicit invocation or trigger-matched skill loading; the force is a prescriptive analysis and result-writing policy.
...
- Conclusion statuses (use exactly these):
  ...
  - `inapplicable` — the stated trigger conditions are false inside that boundary; this is a finding about the system under review, not a lens disposition — a lens skip rests on step 5's skip record, not on this status;
...
This step decides, for each lens, whether it runs and how deep, and it is where the trigger evidence is named before any lens worker sees it.

For each lens (memory/context; epistemic), emit one scoping record: `{lens, disposition (run or skip), trigger evidence IDs, inspected boundary and the searches made within it, the routes and objects that evidence points the lens at, warranted depth, rationale}`.

- **Run or skip.** If the search within the inspected boundary finds any trigger evidence — however thin, degenerate, merely claimed, or unresolved — the disposition is `run`. If it finds none, you may record `skip` and not run the lens. You may also run a lens that has no trigger evidence; its output then records the search as an absence.
- **The floor for a skip.** A skip record states that the lens was skipped, the evidence searched (the sources, scope, and search terms or inspection paths it covered, citing `SRC-*` IDs), and the conclusions the skip prevents. A complete skip reads like "memory/context lens skipped: searched `SRC-2` state and session modules and `SRC-3` docs for cross-invocation persistence and found none; prevents any claim about memory read-back, learning from use, or memory activation." Register the missing trigger as an `ABS-*` record when it bounds a conclusion.
- Warranted depth for a running lens follows the evidence. Rich trigger evidence warrants a full pass; thin or degenerate evidence warrants a brief one. A brief pass is a result, not an omission.
...
- **`uncertain` is not a scoping value and not a skip.** Evidence you cannot resolve is trigger evidence: run the lens and record the evidence limitation *inside* its output, paired with the conclusion it prevents. A skip means "the recorded search found nothing", never "we could not tell."
- An absent lens section or file must never carry meaning implicitly. Every run emits both scoping records; each lens has either a lens output or, when skipped, its skip record in the scoping record.

**Memory/context evidence.** Look for a path by which material accumulated or changed through use can affect a later invocation or action, in code, documentation, or observation. Static shipped material, ordinary current-run state, and retained material with no later delivery path are not such paths — where these are all you find, there is no trigger evidence, and a skip record names what was found and why it does not qualify. A merely claimed path is trigger evidence; the lens output preserves whether the path is `claimed`, `afforded`, or `observed`.

**Epistemic evidence.** Look for a material route that handles truth-apt content, and for any consequential knowledge-production or warrant claim the system makes — including where the eventual finding is failure or absence. A claimed or candidate truth-apt route is trigger evidence. Successful knowledge production is never a prerequisite for running the lens, and its absence is never grounds for a skip.

**Direct-adaptation exception.** Evaluated direct behavior or policy adaptation with no truth-apt object and no knowledge or warrant claim is not an epistemic object, and such a route alone is not epistemic trigger evidence. Such a route stays in the runtime account, and the scoping record names it for the orchestrator. If the epistemic lens is skipped, the skip record lists these routes as left unclassified. If it runs, hand them to the invoked epistemic procedure tagged **classify-only**: that procedure classifies every content-changing edge it meets and carries a class for exactly these routes, so withholding one would leave a silent hole in its ledger. Classify-only means the route is recorded in its content/update classification and is *not* analysed for warrant, transformation, or acceptance. If the lens concludes the route is in fact truth-apt, that comes back as a correction under step 3's correction branch — never as a silent expansion of the lens's own scope.

### 6. Run the embedded memory/context lens

Skip this step when step 5 recorded a memory/context skip. Otherwise analyse accumulated-from-use mechanisms; this is not a memory-review publication workflow. Work at the depth the step-5 scoping record warrants: the items below are the full pass, and a brief pass covers the same ground proportionately rather than skipping items silently.
...
1. Invoke the procedure in `kb/instructions/analyse-external-system-epistemic-architecture.md` to run the accepted route-analysis method inside this run's boundary, unless step 5 recorded an epistemic skip. Do not copy or restate its object-inventory, route-ledger, transformation, lifecycle, claim-comparison, or authority method.
...
5. runtime account;
6. both lens scoping records, including any skip record;
7. one output per running lens, and for a skipped lens a pointer to its skip record;
...
1. Verify: source anchors; every conclusion status is one of the seven values listed in step 3, and no record carries `implemented` as a conclusion status; unique, resolving IDs; one boundary and revision across all records; mandatory runtime coverage; both lens scoping records present; each lens has either an output meeting the brief-output floor or a skip record meeting the skip floor; no lens with recorded trigger evidence was skipped; prevented conclusions stated for every thin, negative, or unresolved finding; shared-route ownership respected; no forbidden evidence upgrades.
...
- Both scoping records exist. Each lens either ran, with an explicit output, or was skipped for lack of trigger evidence, with a skip record naming the evidence searched and the conclusions the skip prevents; nothing is implied by an absent section, and a brief output still names what was inventoried, what was found, and what its thinness prevents.
