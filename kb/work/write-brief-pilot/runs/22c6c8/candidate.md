...
Produce these six blocks in order. Use readable Markdown tables or compact records; a compact record keeps the block's field names in the listed order.

Use canonical IDs when the caller supplies them. Otherwise number each kind from 1: `SRC-n` for sources, `CLM-n` for system claims, `OBJ-n` for objects, and `RTE-n` for routes. Write complete IDs separated by commas, such as `RTE-1, RTE-2`, never a range or abbreviation.
...
- `truth-apt transformation: <class>`, where `<class>` is `acquisition/import`, `non-ampliative reshaping`, `entailed derivation`, `ampliative conjecture`, or `indeterminate`;
- `non-truth-apt policy/content update: <description>`; or
- `no content change`.

Fill the ledger by these cell rules so that ledgers from different reviews line up field by field:

- Keep every field, under its listed name and in its listed order, in every row, including rows that record an evidenced absence. Do not add, drop, merge, or rename fields.
- Fill every cell. Write `none` when the evidence shows that nothing applies, and `not determinable` when the evidence cannot decide. Do not leave a cell blank or write `n/a`, `?`, or `unknown`.
- Put exactly one listed string in `route function`, `architectural status`, and `content/update relation`. Keep the listed prefix exact where it takes a description. If two strings seem to apply, split the row.
- In `implemented force` and the force part of the behavioral-authority path, use `advisory`, `ranking`, `permissive`, `enforcing`, or `none`. A result with no consequential consumer takes `none`.
- Write `activation and timing` as `<activation condition, or always>; <when the route runs>`. Write `inactive in inspected configuration` as the condition when that applies.
- Write `epistemic authority and scope` as `none` or as `<content licensed for reliance> over <scope>`.
- Begin `operational authority` with `permits`, `blocks`, `changes`, or `none`.
- Write the behavioral-authority path as `<consumer>; <channel>; <force>; <horizon>`, or `none`.
- Write evidence as `<source ID> <local anchor>`, separated by commas. Write a mismatch as `<claim ID>: <mismatch>`.
- Order rows by route function in the order of the functional-kind list above, then by object ID in inventory order, then by activation order.
...
This example shows how the outputs fit together. The system is hypothetical, and its values are not defaults for a real review. It gives two ledger records in full and abbreviates the other outputs; a real review fills every field and every row the boundary requires.

*LessonBank* is a coding agent. After a failed test run, a model writes a "lesson" that generalizes the failure. The agent stores the lesson and later injects the most similar lessons into its prompt. Its README says it "learns from its mistakes."

**1. Boundary.** system: LessonBank | reviewed revision/version: commit `a1b2c3d` | declared scope and excluded components: whole agent; excludes the hosted model | analysis question: does LessonBank produce knowledge from failures? | assessed route families: testing, lesson writing, storage, retrieval | unassessed route families: model fine-tuning | source register: `SRC-1` -> repository at `a1b2c3d`, implementation; `SRC-2` -> README at `a1b2c3d`, doctrine/design; `SRC-3` -> one two-session run log, observed run | missing evidence -> conclusion prevented: no comparison of runs with and without lessons -> no causal claim that lessons improve later runs | system knowledge-production/warrant claims: `CLM-1` "learns from its mistakes", `SRC-2` Features.

**2. Inventory.** `OBJ-1`: test result for one run; symbolic; truth-apt content "revision R passes suite S". `OBJ-2`: lesson text; natural-language; truth-apt content "in this repository, pinning library L avoids import failures"; producer: lesson writer; consumer: next prompt.

**3. Ledger.** Two full records, in function-list order:

- `RTE-1` | content transformation | implemented | `OBJ-2` | truth-apt transformation: ampliative conjecture | the lesson proposition over future tasks in this repository | none | on test failure; end of run | new lesson text | none | none | none | none | `SRC-1` lessons.py:write_lesson, `SRC-3` event 8 | `CLM-1` | none | a single failure is the only input; the storage effect is recorded in `RTE-4`
- `RTE-3` | disposition/acceptance | no route found within boundary | `OBJ-2` | no content change | the lesson proposition | none | none | none | none | none | none | none | `SRC-1` search of lessons.py and runner.py | `CLM-1` | `CLM-1`: "learns" but no acceptance route | informal acceptance by a human operator not excluded

The other rows are `RTE-2` (check/evidence production: the test runner checks `OBJ-1`), `RTE-4` (retention of `OBJ-2`), and `RTE-5` (operational admission/selection/consumption: similarity retrieval injects `OBJ-2`; force `advisory`; path `next agent run; system prompt; advisory; all later runs`).

**4. Lifecycle for `OBJ-2`.** observation/anomaly: `RTE-2`, implemented, phase evidenced, `SRC-3` event 7 | conjecture: `RTE-1`, implemented, phase evidenced, `SRC-3` event 8 | derived consequence: no route found within boundary, not reached, `SRC-3` event 9 shows storage directly after writing | test/evidence: no route found within boundary, not reached, `SRC-3` event 9 | acceptance: `RTE-3`, evaluator none, criterion none, intended use none, no route found within boundary, not reached, accepted scope none | lifecycle integration: no route found within boundary, not reached; retrieval in `RTE-5` is pre-acceptance use | missing: any test of the lesson. `OBJ-1` takes the non-ampliative schema as acquisition/import from the test runner.

**5. Claim comparison.** `CLM-1` | learns from failures | `SRC-2` Features, doctrine/design | yes | `RTE-1, RTE-2, RTE-4, RTE-5` | `SRC-3` shows a lesson retrieved in session 2 | none | generates, retains, and reinjects failure-derived conjectures | no acceptance route; no causal evidence of improvement.

**6. Conclusion.** LessonBank acquires test results with preserved warrant for one revision and suite. It generates ampliative lessons from single failures, retains them, and injects them into later prompts with advisory force. No route tests or accepts a lesson within the boundary, so the lessons are unaccepted candidates, not produced knowledge. `CLM-1` is unsupported as a knowledge-production claim; whether injected lessons change later outcomes is unknown without a comparison.
...
- Every ledger row keeps every field in the listed order and follows the cell rules: no blank cells, one listed string per closed cell, and rows in function-list order.
