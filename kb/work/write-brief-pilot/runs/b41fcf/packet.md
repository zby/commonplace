...
A note claiming that the operations a runtime exposes for turning retained state into active context bound which context policies are reachable, so controller gains inside a fixed interface never validate it; not a system survey or ranking.
