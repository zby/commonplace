...
The steps below follow the protocol proposed in the [evidence
supplement](./testing-the-theory-refinement-program.md), which also gives
its controls and record fields in full. None of the steps has been carried
out, and the protocol may change before the first scored run.

1. **Fix the terms before scoring starts.** The run declares and freezes
   the consuming project and the tasks it may send, the seed Commonplace
   starts from, the exact model versions, the acceptance contract for each
   task and who may change it, budgets that are equal across every compared
   condition, the stopping rule, and the reliability target. Freezing these
   first prevents fitting the run to a result, for example by stopping when
   a good score first appears. None of these declarations has been filled
   in yet.
2. **Deliver a release.** Commonplace gives the project a versioned
   knowledge base together with its validators, skills, and indexes.
3. **Run episodes.** In an episode, one of the project's agents performs
   one task using the release, and the project's judges accept or reject
   the output. The record keeps the task, the release version, the output,
   the judge's decision and reason, and which documents the agent actually
   read. Before each task, a short statement says which document should be
   consulted, so that a correct answer drawn from the wrong source can be
   detected.
4. **Revise from feedback.** Commonplace diagnoses rejected work and
   revises the knowledge base, and its own methods when a failure points
   at a limit in them. It then issues a new release. A
   rejection is evidence about the combination of task, agent, and
   knowledge base; it does not by itself refute any one document.
5. **Assess on reserved tasks.** The final release is frozen and scored on
   tasks whose outcomes guided no revision, so the score does not measure
   the tasks the release was tuned on.
6. **Compare.** The same tasks are run with a retained change removed, by a
   baseline that searches the raw records without the learned methodology,
   and by a human-staffed builder with the same demands and resources.

The steps map onto the three hypotheses. Whether the reserved-task score
reaches the target with no person in an inside role bears on sufficiency.
The comparisons in step 6 bear on the comparison hypothesis. The records of
step 4 episodes in which Commonplace revised its own methods, set against a
matched builder without a theory of its own machinery, bear on reflection.
