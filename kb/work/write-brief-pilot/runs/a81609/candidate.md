...
Produce these six blocks in order. Use readable Markdown tables or compact records; a compact record keeps the schema's field names and order. The worked example after output 6 shows all six blocks for one small system.
...
Ledgers from different reviews must line up column for column, so the ledger format is fixed:

- Use exactly these 17 columns, with these names, in this order. Do not rename, merge, drop, reorder, or add columns. Put review-specific detail in `gap/limit`.
- Fill every cell. Where a cell has no content, write `none` for an evidenced absence, `not determinable` when the evidence cannot decide, or `not applicable` when the column does not apply to the route's function. Never leave a cell blank.
- Use only the closed values listed below for `route function`, `architectural status`, `content/update relation`, and `implemented force`. Put any qualification in `gap/limit`, not in the value.
- Write IDs as `SRC-n`, `OBJ-n`, `RTE-n`, and `CLM-n`. If a calling workflow supplies canonical IDs or asks for a sparse overlay on its IDs, use its IDs and overlay rule, and keep these column names and closed values in every row you return.
- Cite evidence as `SRC-n: <local anchor>`; separate several citations with `;`.
- Write the behavioral-authority path as `consumer; channel; force; horizon`, and the mismatch marker as `none` or `CLM-n: <mismatch in a few words>`.
- Order rows by route function in the order listed below, then by first object ID.

IDs are local to one review. To compare reviews side by side, match rows on route function, content/update relation, and target object role, then compare architectural status, implemented force, and the two authority columns.
...
Record **implemented force** as one of `none`, `advisory`, `ranking`, `permissive`, or `enforcing`. Use `none` when no consequential consumer reads the result.
...
NoteKeeper is an invented memory subsystem used only to show the outputs; its sources, file names, and findings describe no real system. It stores chat messages, has a model infer "user facts" from them each night, has a second model prompt score each fact, stores every fact, and injects the most similar facts into later prompts. Its README says it "learns verified facts about the user". The example abbreviates where noted; a real analysis fills every field.

**1. Boundary.** system: NoteKeeper | revision: `abc123` | scope: memory subsystem; chat model excluded | question: does it produce knowledge about the user? | assessed: ingest, fact extraction, scoring, storage, retrieval | unassessed: none | sources: `SRC-1` -> repository at `abc123`, implementation; `SRC-2` -> README at `abc123`, doctrine/design | missing: run logs -> no observed candidate state can be established | claims: `CLM-1` "learns verified facts about the user", `SRC-2: Features`.

**2. Objects** (abbreviated to four columns).

| object/part id | system name and description | representational form | candidate truth-apt content |
|---|---|---|---|
| `OBJ-1` | message log: verbatim user messages | natural-language | what the user wrote |
| `OBJ-2` | user fact: one inferred sentence per record, with a score field | natural-language | propositions about the user not stated in any single message |

**3. Ledger.** One full row, as a compact record:

`RTE-3` | check/evidence production | implemented | `OBJ-2` | no content change | each new `OBJ-2` fact: plausibility given its source messages | model scorer prompt; domain: consistency with the message batch it came from | always, immediately after extraction in the nightly job | score 0-1 written to the fact record; no run observed | none | none: no route reads the score | none: storage and retrieval ignore the score | none | `SRC-1: scorer.py score_fact`; `SRC-1: retrieve.py top_k` | `CLM-1` | `CLM-1: "verified" but score unconsumed` | prompt judges plausibility, not truth

The remaining rows, abbreviated to the comparison columns:

| route id | route function | architectural status | object/candidate id | content/update relation | implemented force |
|---|---|---|---|---|---|
| `RTE-1` | content transformation | implemented | `OBJ-1` | truth-apt transformation: acquisition/import | not applicable |
| `RTE-2` | content transformation | implemented | `OBJ-1` -> `OBJ-2` | truth-apt transformation: ampliative conjecture | not applicable |
| `RTE-4` | disposition/acceptance | no route found within boundary | `OBJ-2` | no content change | none |
| `RTE-5` | retention | implemented | `OBJ-2` | no content change | permissive |
| `RTE-6` | operational admission/selection/consumption | implemented | `OBJ-2` | no content change | ranking |

`RTE-6`'s behavioral-authority path is `chat model; system prompt; advisory; every later session`.

**4. Lifecycle.** `OBJ-2` (ampliative): observation `RTE-1`, implemented, no instance observed | conjecture `RTE-2`, implemented, no instance observed | derived consequence: no route found within boundary, no instance observed | test `RTE-3`, implemented, no instance observed | acceptance `RTE-4`: evaluator none, criterion none, intended use none, no route found within boundary, no instance observed | lifecycle integration: no route found within boundary, no instance observed; `RTE-5` and `RTE-6` are pre-acceptance retention and use | missing: run logs.

`OBJ-1` (non-ampliative): `RTE-1` | acquisition/import | discovery lifecycle: not applicable | stored verbatim; warrant preserved as a record of what the user wrote, not of its truth | no run logs.

**5. Claim comparison.** `CLM-1` | learns verified facts | `SRC-2: Features`, doctrine/design | declared | `RTE-2`, `RTE-3` | none | none | NoteKeeper generates and scores candidate facts; no route verifies or accepts them | mismatch: "verified" has no consuming route.

**6. Conclusion.** NoteKeeper acquires messages with source warrant preserved as records of statements. It conjectures user facts (`RTE-2`) and scores them (`RTE-3`), but the score has no implemented force and no route accepts a fact. It retains every conjecture and injects it into later prompts as advisory context. Within this boundary it does not produce accepted claims, and `CLM-1`'s "verified" is unsupported by implementation.
...
- The ledger uses the 17 fixed columns in order, closed values in the four closed columns, `SRC`/`OBJ`/`RTE`/`CLM` IDs (or caller-supplied canonical IDs), the fixed row order, and no blank cells.
