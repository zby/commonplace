...
- *The concession*: methodology, heuristics, and conventions are not promised permanence — stable guidance of these kinds may migrate into weights, and structure that stops earning its marginal value should be [relaxed or removed](./codification-and-relaxing-navigate-the-bitter-lesson-boundary.md). The concession does not extend to the KB's core vocabulary, which is now settled enough to commit to. Committing to it means downstream text may build on its terms without hedging against their migration; the terms stay open to criticism like any retained theory.
...
| Substitute versus complement (candidate criterion; no carrier note; carried so far only by the commitment and reproduction row above) | Sorts artifact classes by whether they stand in for a capability the model lacks or supply what no weights hold — current state, commitments, authority, project-specific facts — and so explains the vision case instead of denying it | That any complement persists in its current form; that substitutes — methodology, heuristics, conventions — survive, which the concession already gives up; anything before the historical record has been checked against the criterion |
