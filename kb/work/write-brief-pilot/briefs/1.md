...
[restored] Existing theory to evaluate and cite rather than duplicate:
...
[restored] Any additional durable claim discovered during disposition requires a separate user-authorized run.
...
Operator review 2026-09-26 restored two items, marked [restored] above: the cite-rather-than-duplicate list (an exclusion) and the separate-run rule for additional claims (an exclusion on this note's scope).
...
- Entire "Source and evidence paths" section (user-commission prompt file, collection/type contract paths, advisory connect report, core comparison evidence list of 12 sources/reviews, optional boundary cases, advisory landscape files "do not modify in this run") — evidence/input lists for the run.
