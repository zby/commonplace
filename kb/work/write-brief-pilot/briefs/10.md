Target: `kb/notes/designing-agent-memory-systems.md` — source: `kb/work/agent-memory-design/framing.md` at `c4cd0bd7`. The source is a workshop framing that does not name the target; it commissions the synthesis note through its Goal (design an ideal memory system for agentic systems). Kept everything that states the goal, premise, starting position, requirements, use cases, boundary, and open questions the design must answer; removed the workshop title and the reading list. Markdown link targets into the KB are dropped (link text kept) because relative paths do not resolve from this directory; the in-page anchor link is kept.
...
- Title "# Agent Memory Design Workshop" — workshop framing.
- Link targets on the nine "What we know" premises and the workshop-layer gap (link text kept) — source pointers; relative paths do not resolve here.
- Entire "Relevant KB notes" section (core theory, 5 notes; memory architecture, 4; learning theory, 3; workshop layer, 2) — evidence/source reading list.
