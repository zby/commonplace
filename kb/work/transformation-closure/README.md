...
- [mvp-plan.md](./mvp-plan.md) — phased build plan with self-contained phases: (1) critique into the review system through the batch mechanism as its own bundle, with the parser resolving the expected result kind from the job's bundle — the hard part; (2) rationale on ack; (3) closing-review step 10 in the full-pass instruction plus the observation log with a variance control arm; (4) run on real passes at 100% audit sampling and locate constraints
