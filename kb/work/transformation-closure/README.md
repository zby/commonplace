...
- [mvp-plan.md](./mvp-plan.md) — MVP plan with a restored minimal boundary, shaped by two distinctions located at design time (current state vs. history — acceptance prunes inline per ADR 036, so carry judgments live in the observation record; verdict vs. completion — pairs carry a persisted `result_kind`): (1) core build — result-kind schema + FK-safe v4→v5 migration + critique as the first report-kind virtual lens + pass-scoped report retention; (2) step-10 closing cycle over all five methods with a hand-written per-pass observation record and manually run controls; (3) real runs at 100% counterfactual sampling — no real ack occurs; (4) instrumentation (carry events, ack integration, automated validation) deferred behind an observation gate, design recorded so it isn't re-derived; (5) human-attestation design probe, independent of MVP completion
