...
- [unified-diff-and-ack.md](./unified-diff-and-ack.md) — the MVP's design decision: one diff-and-ack freshness surface for all **note assays** (the workshop's genus for gates, critique, friction, compression, connect), split by the question asked rather than the certainty delivered — detection assays (closed question over a named pattern: fallible verdict, skip license because re-asking is redundant) and attention assays (open question: routed attention, verdictless by necessity, evidence-currency license only); the staleness layer is already verdict-agnostic, transformations save nothing (content catches everything), the ack becomes the carry record and needs a rationale field, and footprints soften to stated intent the cumulative diff verifies
