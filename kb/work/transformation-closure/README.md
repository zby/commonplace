...
2. **Human review anchoring.** Can human attestation be one more factored `(note, contract)` pair in the existing review store (the ADR 038/041 pattern, human actor/partition), and what exactly does such an acceptance certify? Cheapest representation decides; see [factored dependency pairs](../../reference/proposals/factored-dependency-pairs-for-review-freshness.md). The MVP ships this case's simplest slice (phase 4 of [mvp-plan.md](./mvp-plan.md)): one **force-free** `reviewed` attestation under a human partition — anchored and queryable, licensing nothing. The kinds-and-forces space (skimmed / approved / claims-verified, each licensing different things) is large, so the MVP refuses to design it; every moment the single kind gets stretched in use is a located requirement.
...
- [mvp-plan.md](./mvp-plan.md) — phased build plan with self-contained phases: (1) critique into the review system through the batch mechanism as its own bundle, with the parser resolving the expected result kind from the job's bundle — the hard part; (2) rationale on ack; (3) closing-review step 10 in the full-pass instruction plus the observation log with a variance control arm; (4) minimal force-free human attestation under a human partition; (5) run on real passes at 100% audit sampling and locate constraints
