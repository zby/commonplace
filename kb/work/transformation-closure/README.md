...
2. **Human review anchoring.** Can human attestation be one more factored `(note, contract)` pair in the existing review store (the ADR 038/041 pattern, human actor/partition), and what exactly does such an acceptance certify? Cheapest representation decides; see [factored dependency pairs](../../reference/proposals/factored-dependency-pairs-for-review-freshness.md). The MVP ships this case's simplest slice (phase 5 of [mvp-plan.md](./mvp-plan.md), deliberately after the closure experiment): one **force-free** attestation under a human partition — "the user recorded that they reviewed these bytes," not "accepts" (already endorsement-shaped) — anchored and queryable, licensing nothing. It is a separate architectural probe, not a reuse of the acceptance store (which requires completed review-pair evidence). The kinds-and-forces space (skimmed / approved / claims-verified, each licensing different things) is large, so the MVP refuses to design it; every moment the single kind gets stretched in use is a located requirement.
...
- [mvp-plan.md](./mvp-plan.md) — phased build plan with self-contained phases, shaped by two distinctions located at design time (current state vs. history — carry judgments are append-only events because acceptance prunes inline per ADR 036; verdict vs. completion — pairs carry a persisted `result_kind`): (1) persisted result kinds + critique execution through the batch mechanism as a virtual lens; (2) durable carry-event recording with a fixed JSONL contract and required rationale; (3) closing-review step 10 over all five methods — anchored assays via would-ack-or-rerun, unanchored ones re-run directly; (4) real runs at 100% counterfactual-ack sampling; (5) the human-attestation probe, force-free and representation-open
