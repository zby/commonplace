...
4. **Trajectory-aware closure audit — spun out as a proposal.** [Trajectory-aware evaluation of transforming agent workflows](../../reference/proposals/trajectory-aware-evaluation-of-transforming-agent-workflows.md) specifies the blinded trajectory-versus-final-output comparison, trace-completeness boundary, measurements, and adoption gate. It remains protocol meta-evaluation, not another **note assay**, and no longer keeps this workshop open.
...
- **Workshop remains open only for the independent human-attestation probe or an honest close:** trajectory-aware evaluation now lives in its own proposal. If attestation is not chosen here, the remaining broad heuristics should stay documented as untested and the implemented findings should be distilled before closing.
