...
- [mvp-plan.md](./mvp-plan.md) — concrete build list: unbounded assays run through the existing batch mechanism (critique first — the pipeline pins snapshots by construction, where after-the-fact recording would be a self-reported anchor), a class-bearing verdict-free acceptance kind in the review DB, rationale on ack, a closing-review step 10 in the full-pass instruction (one cycle, ack-or-rerun per assay, 100% audit sampling initially), and a plain observation log with a variance control arm
