...
- [unified-diff-and-ack.md](./unified-diff-and-ack.md) — the MVP's design decision: one diff-and-ack freshness surface for all checks, verdict-bearing or not; the staleness layer is already verdict-agnostic, transformations save nothing (content catches everything), the ack becomes the carry record and needs a rationale field, and footprints soften to stated intent the cumulative diff verifies
- [mvp-plan.md](./mvp-plan.md) — concrete build list: verdict-free acceptance kind in the review DB, rationale on ack, a closing-review step 10 in the full-pass instruction (one cycle, ack-or-rerun per check, 100% audit sampling initially), and a plain observation log with a variance control arm
