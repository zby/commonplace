...
- **Class-homogeneous jobs, via separate bundles — already given by shipped packing.** Unbounded assays form their own bundle(s) — `critique` as a one-assay bundle, friction likewise — and a job never mixes result-kind classes, so each job carries exactly one output contract and the parser change is a job-level expected-result-kind, not per-pair logic. Mixing would also blend closed-question and open-question instructions in one sub-agent prompt and, under all-or-nothing finalization, let a malformed critique block void the same note's gate acceptances. No new guard is needed: `--grouping note` already keys jobs by `(note, bundle)` (`_note_groups`, `src/commonplace/cli/review/create_review_jobs.py`), and `--grouping gate` packs per-gate — making critique its own bundle is sufficient.
