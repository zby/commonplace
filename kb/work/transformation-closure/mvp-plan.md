...
- Schema scope is narrow: the new `result_kind` column on `review_pairs`, the view predicate (`rp.decision IS NOT NULL` → completion, e.g. `rp.reviewed_at IS NOT NULL`, with `result_kind` exposed), and a `REVIEW_SCHEMA_VERSION` bump 4 → 5. The `decision` CHECK needs no change (NULL already passes; report pairs keep it NULL forever) and `acceptance` gains no column (class is derivable through the accepted-pair join). Migration is the shipped policy — `init_db` version-gates and says "recreate the review store" — and recreating is costless here: phase 4 starts at 100% rerun sampling, so the first real pass re-earns acceptance state by design.
- Code touchpoints beyond the schema: `upsert_acceptance` rejects null-decision pairs (`src/commonplace/review/review_db.py:763`) and must instead require *completion* (a decision for verdict-kind pairs, completed report for report-kind); the warn queue is untouched (warn-selector filters on `decision = 'warn'`, which never matches a report pair).
