...
If a verdict is wanted (for example, via Bayesian aggregation), it should be its own artifact that reads the neutral map as input and is never merged into it. The map is the durable, defensible object; the verdict is a replaceable opinion computed over it.
