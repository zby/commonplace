# Workshop: epistack-framework-additions_for_fable

> **Fable scope:** This variant is scoped to the LHC and eggs cases only. The original workshop remains unchanged.
Brainstorm of framework additions to Commonplace that would make **epistemic casework** — tasks like the FLF Epistemic Case Study Competition's black-hole / egg cases — easier, plus a recommended personal-epistemology stance for the person running such casework.

This is the *design-thinking* companion to [`epistack-competition`](../epistack-competition_for_fable/README.md), which is only the framework-side pointer to the sibling `epistack-casebooks` repo and the `backlog-to-commonplace.md` protocol. This workshop holds the menu of candidate additions; the sibling repo is where any of them get built and proven.
...
- The former base-note `status` gap is resolved by [ADR 044](../../reference/adr/044-user-verification-replaces-global-note-status.md): Commonplace removed the global lifecycle field and retained only optional explicit user verification. Epistack claim-local state remains a separate type-design question in [claim-type](./claim-type.md).
- **Source classification is settled (2026-07-12, [ADR 045](../../reference/adr/045-source-genre-is-a-single-open-field-on-the-snapshot.md)):** genre is one open field on the snapshot, and a casework KB needing its own vocabulary or richer capture fields **declares a collection-local snapshot type** — not framework enum PRs, not edits to shipped specs. This is the implementation route for [richer-source-type](./richer-source-type.md) (updated with specifics) and the pattern any epistack implementation should follow for source-layer needs. Author identity fields feed the separate [author-authority-ranking](./author-authority-ranking.md) thread and the [authority-ranking workshop](../authority-ranking/README.md).
