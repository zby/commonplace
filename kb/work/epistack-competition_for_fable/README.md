# Workshop: epistack-competition_for_fable

> **Fable scope:** This variant is scoped to the LHC and eggs cases only — the COVID case's content cannot be processed by this operator, so its casework and its case-scoped backlog entries are handled through the original workshop. Backlog entries cased `lhc`, `eggs`, or `cross-case` are in scope here. The original workshop remains unchanged.
...
Structure there: three isolated top-level case namespaces (`kb/lhc/`, `kb/eggs/`, `kb/covid/`), each with a `sources/` collection (captured material) and a `notes/` collection (the casebook, plain `note` type), plus a global `kb/notes/` for transferable theory. Work under this variant touches only the LHC and eggs namespaces (see the scope note above). The sibling editable-installs this repo's `llm-commonplace` package, so framework changes here are live there without reinstall.
