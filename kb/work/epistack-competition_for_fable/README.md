...
Structure there: two isolated top-level case namespaces (`kb/lhc/` and `kb/eggs/`), each with a `sources/` collection (captured material) and a `notes/` collection (the casebook, plain `note` type), plus a global `kb/notes/` for transferable theory. The sibling editable-installs this repo's `llm-commonplace` package, so framework changes here are live there without reinstall.
