...
1. **The two implementations already differ locally.** The same protocol surface supports LHC and eggs while their discourse structures remain visibly case-specific. This demonstrates applicability across the supplied cases, not universal optimality.
...
**Priority if the days run out.** Protect, in order: the submission document; the standalone protocol and conformance guide; clean demonstrations on both cases; the quote-verification walkthrough; then the correlated-evidence matrix. Cut the matrix before weakening the two-case protocol demonstration or losing writing/review time. Do not begin the independent-builder experiment or generic bulk-operations layer.
