...
**Role: a supporting Assessment-layer demonstration for the [Structure-layer protocol and reference implementation](./README.md).** The provenance matrix and computed correlated-evidence clusters address one requirement the brief names explicitly, but they are not the entry's spine. Build the smallest usable version only if it does not displace the protocol specification, two-case conformance demonstration, or submission writing.
...
**Epistemic casework has no such surface.** Across the two retained cases, the corpus spans peer-reviewed papers, safety reports, court filings, government guidance, meta-analyses, and press coverage — different genres, access methods, evidence classes, and capture fidelities. There is no `git clone` for a contested question, and this heterogeneity is what drove the source-genre gap to recur three times before [ADR 045](../../reference/adr/045-source-genre-is-a-single-open-field-on-the-snapshot.md) opened the field.

So the naive retarget — rows are *claims*, axes *adjudicate* them — fails twice: **non-uniform axes** (you cannot ask a court filing and a nutrition meta-analysis the same question and get comparable answers) and **manufactured precision** (a controlled vocabulary over *"does this evidence settle the dispute"* flattens exactly what must not be flattened).
...
Those are answerable uniformly across a court filing, a safety report, and a meta-analysis. **Provenance is uniform even when content is not.** So the matrix's rows are **sources**, and its axes are **provenance and independence** — not claims and verdicts.

That lands on the brief's reachable Assessment bullets (*"flag correlated evidence being treated as independent"*, *"surface what's missing"*), and it collapses two build items into one: **the independence instrument and the matrix retarget are the same thing.** The LHC safety case's shared cosmic-ray dependency and the eggs case's recorded funding relationship stop being prose caveats and become **computed clusters**.
...
3. **Run it over the two cases' sources**; produce the provenance matrix and the correlated-evidence clusters.
...
5. *(Stretch, only if 1–4 land early)* **Multi-method comparison** — two rival assessment specs over the same subquestions, with divergence localized to where each analyst's decomposition differs. Entry shape #4; steps 1–4 stand alone without this.
