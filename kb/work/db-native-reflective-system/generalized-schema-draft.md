...
- **Gap 4 (browsing/agent access)** stays outside this schema entirely — no table addresses "a human can look at this with zero setup" or "an agent can `grep` it." That's confirmed, not just repeated: nothing drafted above changes the interface cost the schema-deferral note names. A DB-native design pays it regardless of how clean the relational model underneath is.
