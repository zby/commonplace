...
- [ADR 018 draft](../link-label-audit/adr-018-draft.md) — proposes `mechanism` and `contrasts`. Complementary: extensions fit inside the theoretical register's default template.
- [`linking-conventions.md`](../link-label-audit/linking-conventions.md) — operational distillation. Under this proposal, the skill still carries a teaching version, but defers to the authoring collection's `COLLECTION.md` and the shared cross-register toolkit for canonical definitions.
...
- [`link-label-audit`](../link-label-audit/framing.md) — separate workshop investigating corpus drift. Findings there inform what labels the register-level defaults (and individual collection grammars) should include; this workshop is orthogonal (about where vocabulary lives, not which labels).
...
6. Decide: fold into library, iterate, or abandon.

## What closes this workshop

- Four proposed `COLLECTION.md` revisions reviewed; each either merged into library or rejected with reasons.
- A decision about where `link-vocabulary.md` lives in the library (candidates: `kb/link-vocabulary.md`, absorbed into `kb/notes/links-index.md`, or split between `kb/reference/` and `kb/instructions/`).
