...
Adopt the artifact-side bound on grounding — a note is sized so its grounding check fits one pass, with the unquoted source as the unit — and retire the review-side reading caps it replaces. The mechanism question of the review-attention-price workshop (closed 2026-08-27) is decided in this option's favour; what remains is the set of operational decisions the proposal *Atomic-step grounding bounds unquoted sources per note* (archived) left free, plus the criterion and validator changes that make the decision binding.
...
- ~~archive the adopted proposal~~ and ~~close review-attention-price~~ — done 2026-08-27;
...
- [Atomic-step grounding bounds unquoted sources per note](../../reference/proposals/archive/atomic-step-grounding-bounds-unquoted-sources-per-note.md) — tests: this workshop is its adoption (archived)
...
- review-attention-price workshop (deleted 2026-08-27, see git history) — supersedes: the mechanism fork is resolved; its Track A and Mechanism B designs are recorded as alternatives in ADR 082
