...
Mode: paper adaptation (mechanical, except that the two protocol sections are replaced by a pointer to Appendix C)
Frozen source tag: pending
Source paths: kb/articles/nearest-existing-constructions-to-a-reachability-witness.md
Live successors: the same path on the main branch
Status: staging — not published
Links: relative to kb/articles/, the path at which the supplement is released
```
...
The conditions a witness must meet, for both the broad conjecture and the
stronger explicit-theory hypothesis, are stated once, in Appendix C of the
paper. This supplement does not restate them. Its six questions above are the
narrower, directly checkable properties the table scores; Appendix C says how
those properties are demonstrated in a run, what a witness declares in advance,
and how retries, rescues, and human interventions are counted.
