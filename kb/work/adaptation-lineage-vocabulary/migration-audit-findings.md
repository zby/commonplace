...
The long-form anchors cited above were already shortened or moved by the subsequent migration, so the live corpus did not contain the literal `redistillation` passages at those line numbers. The remaining live uses in `preserve-evidence-without-loading-history.md` and `serve-multiple-consumers.md` describe one trace reshaped into a use-shaped artifact, not several instances generalized into a rule — `adapted-from`'s case, not `abstracted-from`'s, and not on offer under the old DER/AMP binary Finding 2 was written against. They now use **rework**, matching the verb this same pass already settled on in `lineage-mechanisms/` for the identical operation ("re-run or rework when source revision... changes"), rather than either the old binary's "re-abstraction" or a new unregistered word. The reference gap plan's own **re-abstraction** wording (`commonplace-agent-memory-gap-plan.md`) is therefore left as a separate, lower-confidence site — worth a second look under the same four-way vocabulary, not silently matched. The live probe checklist now says **Abstract updates**, matching the existing `abstract` wording in the parallel output-spec note — that pairing genuinely is instance→rule generalization and was correctly reconciled.
