...
A finer-grained workflow articulation (from the [ChatGPT second opinion](./chatgpt-second-opinion.md)) is to keep five layers separate — raw source → extracted claim → evidence relation → inference → assessment — since most bad epistemology comes from collapsing them. The [belief-ledger practice](./belief-ledger.md) is a concrete habit for the verdict layer.
