...
result: it says factories can work now if the model-plus-artifacts bearer
actually performs the theory function — which is the program's untested bet.
...
A factory on the third carrier is therefore not just a factory that can
work; it is the first on which the factory's theory can be revised from its
