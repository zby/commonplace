...
2. Run the compression bundle per `run-compression-bundle-on-note.md`, passing `kb/reports/full-pass/<note-name>/compression-bundle-review.md` as its `{output-path}` argument. Workshop-local, no DB.
