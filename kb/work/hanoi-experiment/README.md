...
The [specification-level separation note](../../notes/specification-level-separation-recovers-scoping-before-it-recovers.md) predicts that OpenProse-like DSLs recover **scoping** benefits but not **error-correction** benefits. Towers of Hanoi is a pure bookkeeping task — the semantic content is trivial (move a disk) but the recursive structure demands exact tracking of:
