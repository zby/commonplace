...
- **Init only, per project** (2026-09-24). No per-machine setup command. `commonplace-init` copies the skills into the project and writes the harness permissions there. This relaxes "no per-project copy" in the one-tree constraint for skill copies only.
