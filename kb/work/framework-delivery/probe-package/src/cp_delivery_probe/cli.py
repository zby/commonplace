...
def _description(skill_md: Path) -> str:
    ...
    sys.exit(f"{skill_md}: no description")


def _render_routing() -> str:
    """The routing file: library root, entry points, and a skill index.

    The skill index emulates the skills mechanism for harnesses without one, and
    serves agents outside it; stubs are generated from the same skills.
    """
    root = _library_dir()
    skills = "".join(
    ...
    return (
        ...
        "## Skills\n\n"
        "When a task matches a skill's description, read that skill's SKILL.md at the path given "
        "and follow it, resolving its links relative to that file. Where the harness also lists "
        "these skills itself, both routes lead to the same file.\n\n"
        f"{skills}"
    )
