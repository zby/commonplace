...
- `skills/delivery-probe-read/`: a skill that reads two library files through relative links.
- `skills/delivery-probe-library/`: a router skill. Its description says to use it when a delivery-probe procedure is named, and its body indexes the library with relative links.
...
| `cp-delivery-probe-install-skills <dir> [--mode link\|copy] [--remove]` | links or copies the skills into a harness skill directory, or removes them |
...
## Known results from the dry run (2026-09-24, Linux, uv)

- Package data installs under `<uv tool dir>/cp-delivery-probe/lib/python3.X/site-packages/cp_delivery_probe/plugin/`.
- A reinstall at the same Python version keeps that path, and linked skills see the new content immediately.
- Changing the tool's Python version changes the path, and symlinked skills dangle until they are relinked.
