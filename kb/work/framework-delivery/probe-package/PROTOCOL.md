...
10. **Sub-agent.** Ask: "Run the delivery-probe-delegate check." Pass: the agent starts a sub-agent that reads `delegated-step.md` and returns `DELEGATED-STEP-V1`. In a harness that cannot start sub-agents, the agent designs an emulation that gets the read done in a separate context, and reports how it works and what it costs per use; if it finds none, it stops cleanly. In every case the agent must not read the file itself. Record which happened, whether the reading context was fresh, and, for an emulation, who has to act and how often.
...
- **Revision 6** (2026-09-24): adds the `delivery-probe-delegate` skill and case 10, which test whether the harness can hand a step to a sub-agent, and, when it cannot, what emulation an agent designs. Cases 0–9 are unchanged, so a harness that ran revision 5 needs only case 10.
