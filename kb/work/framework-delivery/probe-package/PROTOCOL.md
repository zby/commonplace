...
10. **Sub-agent.** Ask: "Run the delivery-probe-delegate check." Pass: the agent starts a sub-agent that reads `delegated-step.md` and returns `DELEGATED-STEP-V1`. In a harness that cannot start sub-agents, the pass is a clean stop reporting "no sub-agent available", without the agent reading the file itself. Record which happened, and whether the sub-agent had a fresh context.
...
- **Revision 6** (2026-09-24): adds the `delivery-probe-delegate` skill and case 10, which test whether the harness can hand a step to a sub-agent, and whether a skill stops cleanly when it cannot. Cases 0–9 are unchanged, so a harness that ran revision 5 needs only case 10.
