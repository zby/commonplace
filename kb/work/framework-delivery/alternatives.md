...
- **The skill index alone, without stubs.** The generated `library.md` index works in every harness, including those without skills, so it could replace the stubs. It loses what the harness applies only to native skills: forked context (13 of 16 skills), model choice (9), tool limits and invocation by name (all 16), and descriptions that are always in view. Both are kept, generated from one skill list (operator decision, 2026-09-24).
