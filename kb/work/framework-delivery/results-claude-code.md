...
**Result.** With user-level permission rules in place, every case that was run passed; case 6 was dropped with the plugin layer. Without them, the base layer fails in `default` mode: the lookup commands and the library read are both denied. After a switch to an editable install, the read rule no longer covered the library root. In one run the denied agent then read the stale `share/` snapshot through the skill's relative link.
...
| 6 Harness plugin | Dropped | The marketplace was added. The install, run from the agent session, refused with `command_source_refused` and printed the command hash, so it needed the operator's terminal. Before that step, the operator dropped the plugin layer from the design (see `design.md`, rejected alternatives), and the case was not completed. | — |
...
Removed the two skill links, the `cp-delivery-probe-market` marketplace (no plugin was installed), and the `cp-delivery-probe` uv tool. Restored `~/.claude/settings.json` from the pre-run backup, and its SHA-256 matched. `installed_plugins.json` matched its backup. `known_marketplaces.json` differed only in an existing marketplace's `lastUpdated` timestamp, which Claude Code refreshed on its own. The scratch copies and logs were deleted. No hooks were used, no credentials were touched, and the real `llm-commonplace` installation was not changed.
