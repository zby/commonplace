...
## The causal hypotheses are working frames, held tentative
...
A second generator is better established in the KB: reasoning quality degrades as the volume, complexity, and interference in a context grow, often well inside the provider window. See [soft degradation often binds before the hard cap when evidence fits](../../notes/soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md), [context efficiency is the central design concern in agent systems](../../notes/context-efficiency-is-the-central-design-concern-in-agent-systems.md), and [a goal-holding interpreter fails soft](../../notes/a-goal-holding-interpreter-fails-soft-workarounds-tax-a-bounded-budget.md). Several catalogue behaviors are predicted by both generators: premature closure, current-context dominance, and salience of obvious defects each follow from a short evaluation horizon and from a loaded context.

The two generators may not be independent. The operator's conjecture (2026-09-21) is that part of context degradation is itself an effect of human-oriented optimization: a human rater attends shallowly to a long exchange, so faithful use of the whole context is weakly selected. The workshop does not need to decide this, and no entry may depend on the answer. An entry's candidate explanations name whichever generators predict the behavior; a countermeasure is chosen and tested against the behavior. The distinction would start to matter only if two countermeasures diverged by generator, for example if shrinking the context fixed a behavior that separating proposer from critic did not.
