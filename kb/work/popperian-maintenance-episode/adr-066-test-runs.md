...
**Run 1 — statistical reframe down, easiest case.**
`kb/notes/task-fitted-structure-costs-cross-task-reuse.md`
The body already states the statistical qualifier ("often invisible and rarely revisited, not that it is always wrong"); only the title mode is missing. Expected: keep-reframe to a statistical title that states its refuter (what revisit/retire prevalence would falsify it). Failure tell: a retitle to bare "often costs" with no refuter — the vacuity guard should have blocked it.
