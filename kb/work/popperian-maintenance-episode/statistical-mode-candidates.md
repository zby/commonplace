...
2. `structure-activates-higher-quality-training-distributions.md` — **A**. Body is entirely prevalence-shaped ("on average", "tends to", 5–12pp benchmark gains) and already carries a survived null (Sonnet code-QA 84.8% vs 85.3%). Refuter: representative sample of tasks/models with no mean lift. *Outcome (run 18, pass `20260820T143104Z-cda404`): the prevalence shape routed a statistical candidate, but bounded study cells plus one null did not supply representative prevalence evidence or a guarded population-level refuter. The note instead landed on a causal-identification limit and was renamed `structured-prompt-gains-do-not-establish-distribution-selection.md`.*
