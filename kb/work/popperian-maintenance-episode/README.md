...
- **Sourcing debt (open)** — Kiczales's metaobject-protocol work and Erlang's `code_change` are named but unsnapshotted in both the instantiation note and the idealization criterion note; one shared snapshot pass (`cp-skill-snapshot-web` → `kb/sources/`) serves both
