...
1. **Separate the critique from the repair as a disposition.** When the defeats hit the argument's bridge rather than its scope — the pass has no vocabulary for this yet — stop after step 7 with a pending disposition (`return-to-author` alongside `delete`/`merge`/`rehome`), handing back the objections and the packet. The pass already stops for dispositions it cannot execute; this would make "cannot repair without the theory" one of them. Cheapest change; matches the operator's assessment directly.
