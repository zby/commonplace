...
**Decided 2026-08-27.** The maintainer chose atomic steps. The remaining operational decisions moved to [atomic-step-adoption](../atomic-step-adoption/README.md); this workshop closes when that one's ADR lands, with Track A (cost plumbing) retained only as an optional side measurement and Mechanism B as the recorded rejected alternative.

**2026-08-27, earlier.** The maintainer proposed a third mechanism that reverses this workshop's direction: cap the artifact, not the review — a note is sized so its grounding fits one pass ("atomic steps"), and a note with more sources quotes them verbatim so the check becomes mechanical (ADR 046). Under that mechanism Track B and most of Mechanism B are unnecessary. The atomic-step-article workshop (closed 2026-08-27) extracted it into two notes — [a note is an atomic step relative to the check that reads it](../../notes/a-note-is-an-atomic-step-relative-to-the-check-that-reads-it.md) and [a linked note discharges its own grounding, so a citing note owes representation, not re-grounding](../../notes/a-linked-note-discharges-its-own-grounding-so-a-citing-note-owes.md) — and the proposal [atomic-step grounding bounds unquoted sources per note](../../reference/proposals/atomic-step-grounding-bounds-unquoted-sources-per-note.md), which is this workshop's third mechanism candidate; the mechanism decision here waits on it. What it changes for this workshop: under atomic steps the unit of a grounding step is the *unquoted* source — a source quoted verbatim in the note is discharged by the ADR 046 validator, and note links are charged to the representation criteria rather than to grounding — so Track B (degradation) and Mechanism B's code-assembled pack are unnecessary if the artifact-side rule is adopted, and Track A (cost) stays useful only as a side measurement. The number, fail-or-warn, and ADR 079's fate remain this workshop's decisions; the proposal lists them as free choices. The corpus counts are in the proposal's current-state anchor; the reinterpretation of the four-miss evidence is in the first note.
