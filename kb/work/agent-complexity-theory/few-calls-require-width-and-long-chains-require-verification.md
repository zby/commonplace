...
- [Agent context is constrained by soft degradation, not hard token limits](../../notes/soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md) explains why "few calls require width" is substantive. Prompt width means larger effective burden on the bounded call, not just more raw tokens.
...
- [agent context is constrained by soft degradation not hard token limits](../../notes/soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md) — clarifies: the "width" side of the frontier is effective burden on bounded calls, not merely token count
