...
Working out formal consequences of the [bounded-context orchestration model](../../notes/bounded-context-orchestration-model.md) and its [conditional conversion lemma](../../notes/any-symbolic-program-with-llm-calls-is-a-select-call-program.md). The model is a deliberately simple normal form for workflows with symbolic non-call execution between barriers, explicit state, and independent call batches; the lemma transfers results only to programs that meet those preconditions.
