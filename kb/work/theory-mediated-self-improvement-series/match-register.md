...
| Craik 1943 ([ingest](../../sources/craik-hypothesis-on-the-nature-of-thought-1943.ingest.md)) | Why an organism benefits from an internal model | P3 | Downstream: Craik's generator is the cost, delay, or danger of full-scale external trials; P3's is the gap between retained, interpreted theory and what actually governs operation. They share only the later problem of using a model's predicted consequences to select action | Try alternatives on the model before the world | **Conjectured** for model-mediated action selection, with no carrier note yet: it applies where retained theory preserves the action-relevant relations, interpretation derives candidate consequences, and those consequences influence selection. No match is claimed for learning, revising, validating, or codifying the model | Learning and validation of the model, which the source leaves open |
...
| FunSearch ([ingest](../../sources/mathematical-discoveries-program-search-large-language-models.ingest.md)); AlphaDev ([ingest](../../sources/faster-sorting-algorithms-discovered-using-deep-rl.ingest.md)) | Search-selected programs retained and reused in bounded settings | P5 | No depth: instance | Existence evidence that the selected-localized cell is populated | **Instance**: existence evidence that the selected-localized cell is populated; no mechanism borrowed | Economy of search, evaluator–deployment match, generalization |
...
[cue and packet-policy decision](../../reports/retained/planning-delegation-theory/instruction-machinery-refinement.md#cue-and-packet-policy-decision)
