...
| O8 | Warranted transfer out of the human cut is adversely selective: under a fixed boundary, objective, horizon, and workload, the residual decisions tend to be harder to warrant per decision. Later envelope expansion therefore does not by itself establish convergence to closure; the human-work list is a residue, not a capacity claim. | Candidate mechanism; library note `kb/notes/warranted-transfer-leaves-people-the-hardest-to-warrant-decisions.md` | Classify each residual decision by the reason it resisted transfer; keep distinct from the elastic-backlog relocation mechanism; do not infer prevalence or non-convergence from the conditional. |
| O9 | Scoped structural closure and remote-programmer capability are separate coordinates. Evaluator adequacy governs whether results are warranted; the benchmark's fixed client separately exports demand choice and acceptance. | Candidate framing; author direction | Name the exported decisions; test structural closure from path coverage, capability from outcomes, and warrant from the evaluator; do not make one coordinate decide another. |
| O10 | The program's theory is an operational instrument in the KB: classifying a path's residual human decisions by reason and routing each class to a mechanism is a theory-building wiki function, and its use on Commonplace's own transfers supplies a mediation trace. | Candidate; author direction; earned by recorded use | One recorded application on a Commonplace path with classification, chosen transfer, and outcome; cite it from the bootstrap article. Tool-usefulness and traceability only; not closure. |
| O12 | Holding a program theory means sustaining coherent search under delayed feedback: at the least-warrantable modification decision, Naur's coherent-modification test and warranting the hardest residual are the same process-level problem. | Promoted to library note | Test over a horizon of novel demands and recovery; distinguish theory-guided search from luck or generic search; do not require one-shot correctness or a complete theory. |
| O11 | Read as a build plan, the classification orders what to build next for an LLM wiki, an LLM coding agent, or an LLM agent generally, because the theory is stated over decisions rather than tasks. | Candidate program claim; author direction | Separate the plan's ordering from any power claim; a power gain is a conjecture tested by matched comparison, not derived from the plan's shape. |
...
| C2 | Durable behavior change selected by the system's evidence is system-level learning even when weights do not change. | Resolved against an existing library note; see the transfer record | Use the note's criterion (evidence-responsive selection plus operative retention). Do not extend it to concept formation, which C3 bounds. |
...
Quote counts come from the consumed workshop; verify against the ingest before
relying on a count.
...
|---|---|---|---|---|---|
| O8; T5; R6 | Promoted to library note | Distinct residue classes require distinct functions in a self-improving architecture | `kb/notes/residue-classes-need-different-mechanisms-so-architecture-is-mixed.md` | Argued from library notes only; no source grounding consumed | Reciprocal footer links and the hub article reconciled; carrier boundaries remain provisional |
...
| S2; S3; S4 | Folded into an existing note | A theory-mediated loop closes only by causal co-indexing, not by co-occurrence inside one boundary | `kb/notes/theory-mediated-self-improvement-needs-interpretation-and-retention.md` § The functions share one path, not one substrate | Compressed from the satellite note; argued from library notes only | Inbound links repointed or dropped |
...
| O5; T3 | Folded into an existing note | Programming-tool progress is a partial order on accepted outcomes per total human effort, not a scalar | `kb/notes/increasing-computational-autonomy-relocates-human-effort.md` § What to measure instead | Compressed from the satellite note; argued from library notes only | Inbound links repointed or dropped |
| C4; C1 | Folded into an existing note | Explicit artifacts buy addressability, not credit assignment, coherence, retrieval, or admission | `kb/notes/reflection-buys-addressability.md` § What addressability does not buy | Compressed from the satellite note; argued from library notes only | Inbound links repointed or dropped |
| O3; T6 | Retired as trivial | Removing a human judgment can degrade the judgments that remain — kept as one sentence plus its defeater in the parent note | `kb/notes/warranted-transfer-leaves-people-the-hardest-to-warrant-decisions.md` (Consequences for closure) | Parent note | Inbound footer links removed |
| O12 | Promoted to library note | Holding a program theory means sustaining coherent search under delayed feedback | `kb/notes/program-theory-sustains-search-under-delayed-feedback.md` | Synthesized from Naur's bearer tests, the least-warrantable-residue analysis, the proposal-selection loop, and the operator's resolution that human theory-holding is fallible search with backtracking rather than one-shot deduction | Shared model, target problems, article roles, and hub article reconciled |
| C2 | Resolved against an existing library note | Durable behavior change counts as system learning when an evidence-responsive improvement process selects and retains it so that it affects later executions; the unit is the deployed behavior-determining organization, not the model alone | `kb/notes/the-deployed-system-not-the-model-is-the-unit-of-learning.md` | The note supplies both distinctions this row demanded: it fixes the learning unit at the deployed system and narrows the writable surface to consequential revisable choices, and it separates evidence-selected retention from durable-but-unevaluated change through the proposal-selection loop. Sutton and Javed's stronger concept-formation target is a different claim, already dispositioned at C3 as defeated as stated for the computational part | The program-statement article cites the note in its opening and takes its title from the term; C3 continues to bound the concept-formation claim |
