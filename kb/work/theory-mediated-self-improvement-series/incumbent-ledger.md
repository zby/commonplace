...
| O12 | At the coherent-modification decision, holding the theory is equivalent to warrant under conditions: bearer tests passed with test 3 as track record under exposure; refuting exposures not authored by the candidate; outcomes read back and recorded; a declared exposure-frequency rule. The human holder is the standard, and the human's independent check is likewise delayed and occasional. | Candidate; operator direction (2026-08-28) | Show the four conditions for a named composite and path; reread captured-evaluator cases as non-holder self-assessment or holder-without-exposure; test over a horizon of later demands, not per-task acceptance. |
