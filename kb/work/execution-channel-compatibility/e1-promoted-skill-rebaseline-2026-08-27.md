...
| `cp-skill-validate` | One Bash `if`/`for` program using glob expansion, `test`, `basename`, `dirname`, command substitution, and fail-fast exits | Remove the shell program after V1. The skill becomes one shell-neutral `commonplace-validate <target>` invocation and passes `all`, `types`, `landings`, `redirects`, a collection, or a path unchanged. | V1 closed on 2026-09-25 without an `all` target (ADR 086 removed the nested library collections); the package-owned `all` target moved to [E1](../system-contract-consistency/plans/e1-windows-execution.md). |
