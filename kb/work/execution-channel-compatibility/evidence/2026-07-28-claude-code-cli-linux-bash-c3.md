...
- Procedure ID: `execution-channel-round1-v5-2026-07-28`
- Prior related report: [2026-07-28-claude-code-cli-linux-bash-c2.md](./2026-07-28-claude-code-cli-linux-bash-c2.md) (same environment, procedure `v4`). Contrasting environments: [2026-07-28-codex-api-posix-linux-a1.md](./2026-07-28-codex-api-posix-linux-a1.md), [2026-07-28-codex-api-posix-linux-a4.md](./2026-07-28-codex-api-posix-linux-a4.md)

Fresh full run of the `v5` procedure in the same environment, not an edit of the `v4` report. All commands were re-executed; nothing was carried over. Local prefixes are normalized per step 10: `<WORKSPACE>` is the observed workspace root, `<HOME>` the user profile directory.

## Identity

| Field | Value | Basis |
|---|---|---|
| Date/time | `2026-07-28T10:30:04Z`; local timezone `CEST` (`+0200`) | observed |
...
|---|---|---|---|
| PID | `268427` | `268430` | context only; differing PIDs are consistent with fresh processes |
...
| Probe shell function | kind `function` | `absent` | did not persist |
...
- Result: exit `0`; 35 first-heading matches across the instruction tree, including `kb/instructions/ingest-directory.md:6:# Ingest a directory` and `kb/instructions/verify-review-quote-grounding.md:6:# Verify review quote grounding`. Recursion, glob filtering, anchored patterns, and the `-m` limit behaved as the instruction expects. Result ordering differed between runs, consistent with parallel directory traversal; the match count was stable.

### direnv state

Collected with the `v5` shell-native filter, so only the three classifications entered the transcript.
...
10. **The `v5` disclosure controls cost nothing in this environment.** — observed. Every capability conclusion above survives normalization; the filtered direnv check, kind-only classification, and `<WORKSPACE>`/`<HOME>` substitution removed local identifiers without weakening a single result. Path *suffixes* carry the evidence this survey needs; the prefixes never did.
11. **Repeating the probe across `v4` and `v5` reproduced every capability result.** — observed. Only PIDs, timestamps, and rg result ordering differed.
...
| `cmd.exe` / direct-execution branches of steps 3, 5, 7, 8 | Interface is a POSIX shell |
...
- Finding 10 bears on any option whose verification step must run in an untrusted or shared context: capability evidence for this survey is prefix-independent, so a probe can be made disclosure-safe without losing discriminating power.
...
- Full environment, PATH, and raw configuration output: absent — `PATH` was tested by membership only; `direnv status` was classified through the `v5` filter so raw status never entered the transcript; `.envrc`, shell-profile contents, alias targets, function definitions, and direnv marker values were not read or recorded; Git status was restricted to one named fixture.
- Optional automated scan: not run — no scanner established in this environment; the procedure forbids installing one
- Withheld or residual sensitive-looking material: none. The `find` census row names the wrapper's kind and origin without its definition, which is the intended `v5` level of detail.
