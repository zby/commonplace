...
> **Formulation constraint — bind the choices you name.** Title and opening argument must be statable in general terms, even when derived from a specific system. Where a claim names a choice some system made, quantify over it: universally ("for any system that chooses X, Y follows") or existentially as a witness ("at least one system does X, so X is feasible"). A free occurrence — the sentence reads as general, but its truth conditions depend on a selection the reader does not share — fails, because published to a reader without that selection the claim is false or empty. Two repairs: bind the variable and keep the claim, or move the proposition to `kb/reference/` because it only ever reported what was selected. Binding is usually better; it adds reach rather than relocating content.
