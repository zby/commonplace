...
Complete. [ADR 069](../../../reference/adr/069-collection-contract-bundles-become-one-time-prototypes.md) records the decision and implementation. The scaffolded user-collection prototypes state their purpose directly; an opt-in dialectical/evidential prototype carries an experimental banner; both root routing tables omit `Role`; and the library-wide profile vocabulary has been retired. This was the **first** step of the vocabulary cleanup and deliberately excluded relocating `text-contract`; [its own task](./text-contract-and-profiles.md) later folded the remainder into the collection definition.
...
4. Edit the then-current theory-side text-contract definition in place: extract all profile-related content from its frontmatter and body, moving or recasting any material still needed for collection prototypes into the replacement reference artifact(s). Do not move, rename, or delete it during this task; preserve its text-contract definition for the separate placement decision.
...
- The then-current theory-side definition stayed at its path during this task and contained only the binding local text-contract definition. ADR 071 later folded that remainder into the collection definition and retired the duplicate file.
