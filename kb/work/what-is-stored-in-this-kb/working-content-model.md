...
The initial candidate set was `tag-readme-trace-observed-causal-connection.md`, `harness-sub-agent-model-selection-regression.md`, and `commonplace-as-a-reflective-system.md`. Reassessment on 2026-08-23 moved the causal trace and reflective classification to `kb/notes/evidence/`: binding Commonplace as a witness leaves substantive feasibility and coverage claims, and both artifacts state their limits. The harness regression remains in `kb/reference/` because its intended contribution is the dated external-harness incident and its effect on Commonplace review provenance.
