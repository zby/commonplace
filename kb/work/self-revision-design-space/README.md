...
Explain where architectural redesign occurs in the systems compared by the [reflective self-improvement article](../../articles/reflective-self-improvement.md). The five experimental pathways mostly add one improvement level whose organization their research teams redesign from outside the reported loop. Commonplace has made parts of that human–agent builder loop explicit, retained, and operative, with an affordance for turning some current organizing machinery into a later revision target. The theoretical Gödel machine supplies the other comparison point: extraordinarily broad self-referential redesign, but only when its benefit is provable from the incumbent formalization. The [plain account](./plain-account.md) is the current statement of this explanation.
...
1. Does the outer-builder-loop claim survive hostile reading — does any experimental paper make researcher-led architectural redesign part of the reported system's retained, later-consumed improvement pathway, beyond the Darwin Gödel Machine's viability-gated ranker stage?
...
- [Plain account](./plain-account.md) — **primary**: the article-facing explanation, notation-free by constraint.
- [Problem map](./problem-map.md) — live problems, narrowed formulations, evidence base, hard cases, promotion candidates.
- [Revision profiles and moving boundaries](./boundary-map.md) — backstage formal synthesis; parked unless a consumer needs a record the plain account cannot carry.
- [Discriminating tests](./discriminating-tests.md) — the three case tests that forced the narrowings; backstage.
