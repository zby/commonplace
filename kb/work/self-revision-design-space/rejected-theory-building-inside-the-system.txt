---
description: "With a judgment-capable interpreter, functions of theory building can move from human to computational actors inside a declared boundary; records a partial, dated allocation — criticism and proposal warranted computational, the rest untraced or human"
...
The table records the allocation in Commonplace as its traced episodes show it. The rule for a row is who supplied the consequential decision content, not which component emitted the output: a row is *computational* when the traced instances show a model call supplying the decision without a human, *human* when a person supplied it, *joint* when the machinery proposed and a person decided, and *not traced* when the record does not separate the two. Two rows are warranted by traced episodes; the rest are the current best reading and are marked accordingly.

| Function of theory building | Actor in traced episodes | Evidence |
|---|---|---|
| Apply the theory to a case | not traced | routed notes are consumed in deciding calls, but presence is not use, and no trace shows the note fixing the decision |
...
| Derive consequences — procedures, validators | computational drafting, human admission; deciding call not traced | skills derived from methodology; the [topic-index convention codified as a validator](../notes/evidence/tag-readme-trace-observed-causal-connection.md) |
| Propose a revision to the theory | computational | the repair episode: each cycle produced a candidate repair |
| Evaluate a candidate revision | joint | model-call gates apply criteria a maintainer fixed; the [reflective-system evidence](../notes/evidence/commonplace-as-a-reflective-system.md) reports joint search, split evaluation, human adoption |
...
Language models act on both sides of that mapping. They make informal theory operative, and they make formalization cheaper to produce, though acceptance stays [bounded by what can be verified](../notes/the-boundary-of-automation-is-the-boundary-of-verification.md). What they do not supply is a check that the formal assumptions match the world. In the table above, that check is a row of its own, on the human side: derivation inside the model has moved to the machinery because it can be checked; the theory–model correspondence is where the check, and the reallocation, currently run out.
