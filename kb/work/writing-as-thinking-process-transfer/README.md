...
**Byproduct — fixed 2026-08-10:** the P4 finding (a real gap in `critique-note.md` — no sanctioned "note is sound / no strong critique exists" outcome, forcing fabricated opposition) was fixed by adding a first-class **no surviving attack** outcome. A verification re-run of the premise gate confirmed P4 RESOLVED but surfaced the inverted risk — a weak critic declaring no-surviving-attack on an unsound note — so the fix was hardened with a **low confidence — needs domain check** flag that makes that residual visible for the other five lenses to catch rather than silent. Gate calibration stays a recurring problem; visibility, not elimination, is the win.
