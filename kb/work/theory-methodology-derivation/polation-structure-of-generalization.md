...
2. **The bet doctrine, formalized.** Ord stresses that all three polations are the same Bayesian method — prior over functions, update on the data — differing only in where the queried point lies. Classifying an artifact (derivation-dominant vs ampliative-dominant) is therefore a bet about where its claims sit relative to the source, and the verification burden scales with off-manifold distance: hyperpolation is the least well-posed (Ord), which is the discovery note's "a generative model can be compellingly wrong (phlogiston)" restated.
