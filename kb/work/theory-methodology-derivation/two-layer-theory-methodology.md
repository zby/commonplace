...
The KB has the two directions for the *code* case — [codification and relaxing](../../notes/codification-and-relaxing-navigate-the-bitter-lesson-boundary.md) — but there the general-purpose fallback layer is the LLM itself, always present for free. Here the general layer is a *retained artifact* that is too costly to apply per-case yet must be kept, understood, and maintained. Derivation names the operator that produces the methodology, but nothing names the runtime architecture: fast path, fallback semantics, promotion at the boundary.
...
In the [polation thread](./polation-structure-of-generalization.md)'s hull language: the coverage bet is a bet about where the *future query distribution* lies relative to the derived methodology's hull. (A different, assistant-proposed bet — classifying a mixed artifact by its dominant regime — lives in the [vocabulary thread](./derivation-selection-vocabulary.md); that one is about where an artifact's *claims* lie relative to its source. Same geometry, different random variable.)

**The bet is itself a discovery-shaped claim** (operator observation): the coverage bet is a *theory about the example distribution* — "the cases we will actually face concentrate inside this region." That is an ampliative, falsifiable, general claim, i.e. discovery-side content. So every derived artifact bundles a discovery-shaped rider even when its *content* is pure derivation: the body of the methodology is entailed by the theory, but the decision of *which region to derive* encodes an empirical hypothesis about future use. Consequences: (1) "selection is simple" needs a qualifier — selection is mechanically simple but theory-laden, since the selection criterion *is* the distributional hypothesis; (2) part of the audit's authored-judgment gap dissolves — some of what "ad-hoc distillation" packages as authored judgment is exactly this rider; (3) the fallback-rate metric is not just the bet's adjudicator, it is the *test* of this embedded theory — the coverage hypothesis is a discovery-in-progress, and the two-layer system carries its own experiment. This also closes a loop with the [inductive-bias thread](./methodology-as-inductive-bias.md): the acquired prior and the distributional theory are the same object seen from ML and epistemology respectively.
...
- Terminology: this note already uses "generator theory / derived methodology" throughout, and "generator–derivative" in claim 1 above — the [vocabulary thread](./derivation-selection-vocabulary.md) still needs to settle the noun for the produced artifact once selection/derivation are formally promoted, without reintroducing "distillate".
