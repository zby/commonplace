...
| `a-derived-copy-of-recomputable-truth-must-be-checked-or-absent.md` | ln 20 "**every** distilled artifact is a derived copy of its sources" — the "copy" identity is false for abstracted artifacts, but (operator correction) the staleness *machinery* in the same sentence is regime-universal: lineage tracking and staleness detection apply to both | Split the sentence's freight, don't just narrow it: lineage + staleness detection stay universal; the copy identity and re-derive-and-compare repair are derived-only; abstracted artifacts get re-examination as the repair (evidence revision re-opens support rather than invalidating), and lineage staleness is only a *partial* detector for them — new contrary instances arrive outside the link graph, so continued testing is the rest. "Judging fidelity" becomes fidelity-or-support; the Level B point and the core checked-or-absent rule survive for both regimes | yes |
