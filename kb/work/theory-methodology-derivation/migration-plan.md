...
- The **discovery lifecycle** owns all ampliative traffic, graded by the polation axis (now in [discovery-lifecycle.md](../../notes/definitions/discovery-lifecycle.md)'s grading section; thread closed): routine induction (extrapolation) at the shallow end, generative insight (hyperpolation) at the deep end. The compound is the technical term (decided 2026-07-17, per the naming rule in [prose-has-no-scope](../vocabulary-governance/prose-has-no-scope.md)); bare "discovery" stays ordinary English.
- Mixed artifacts carry an explicit dominance **bet** (the dominant-regime call, now specified in [link-vocabulary.md](../../reference/link-vocabulary.md); vocabulary thread closed) instead of a hybrid category.
...
- **ADR recorded** (2026-07-18): [ADR 053](../../reference/adr/053-retire-distillation-without-a-successor-term.md) records the whole decision — retire without successor, labels carry the boundary, discovery lifecycle owns ampliative traffic — with the remaining Wave 2/3 churn named in its Consequences. ADR 011's gloss and the gate examples still update with the Wave 3 batch.
- **Wave 1 planned and started** — full inventory with per-file treatments, gate flags, and lineage-edge classifications in [wave-1-worksheet.md](./wave-1-worksheet.md). Already executed: `distillation-is-transformation-not-selection` retired (`757d6daa`, operator decision — its polemic target dissolved with the term); the COLLECTION.md `derived-from` glosses fixed and `abstracted-from` authorized (`9e252b41` — both files instructed writers to mis-label). Remaining: five gated rewrites (tradeoff-note retitle, the false "every" in `a-derived-copy…`, source-tracking convention update, evolving-understanding retitle, memory-management contrast), `write-instruction.md`'s two provenance stories, ~24 `derived-from` edges mostly flipping to `abstracted-from`, and 9 `Distilled into:` footers to classify. **Wave 2's obvious library subset** (the second-wave table in [obvious-distillation-cases.md](./obvious-distillation-cases.md)) runs in parallel — safe batch work for a mechanical executor; three held-back notes reclassified into it (see worksheet). Then the rest of Wave 2 and the Wave 3 META surfaces.
...
The receiving-vocabulary draft was fully consumed by Wave 0 — its derivation/selection material became source text for the structure note's theory section and the link-vocabulary spec, its discovery amendment became `definitions/discovery-lifecycle.md`, and its hard-case table passed against the promoted surfaces — and the thread was closed and deleted 2026-07-17.
