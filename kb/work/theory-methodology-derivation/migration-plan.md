...
- **Next**: Wave 0 item 3 (the discovery-lifecycle amendment — the last receiving surface); then the remaining Wave 1 entangled cases (the definition's instance list, the `distillation-is-transformation-not-selection` split, `write-instruction.md`'s two provenance stories, two-stage pipeline descriptions, footer classification) and the re-scoped Wave 2.
