...
- [ ] `kb/reference/proposals/structured-output-codec-for-review-protocol.md` — the doc's own "Current state (as of 2026-07-01)" section says the trigger is not met (schema output ships in one harness's workflow scripts; subprocess CLIs `claude -p`/`codex exec` have no equivalent). But its Adoption criteria only require *a single medium the project actually uses* to expose schema output — and `kb/log.md` line 22 records a 2026-06-12 workflow-orchestrated review run that did exactly that. Reconcile: either update "Current state" to note the workflow medium already qualifies, or explain why that one run doesn't count. `agent({schema})` itself is a Claude-Code-Workflow-specific surface, not a cross-harness guarantee — the underlying forced-tool-call-plus-schema-validation technique is available on other providers' APIs too, but no other medium this project uses (subprocess CLI, live-agent file artifact) currently exposes it.
