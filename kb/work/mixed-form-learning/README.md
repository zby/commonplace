...
| Summary/theory operation boundary *(added in discussion)* | **promoted** (2026-07-28) | New note: [a summary claims less than its episodes; a distilled theory claims more](../../notes/a-summary-claims-less-than-its-episodes-a-theory-claims-more.md) — the Meta-Harness raw-traces result is real but bounds naive summarization in the attribution channel, not theory-formation, which no arm attempted; MDL objection handled via SuperARC. The central note cites this for its Meta-Harness handling and for the precise fourth-quadrant claim: existing loops conjecture only transiently — Meta-Harness's proposer formulates hypotheses from raw traces, but they feed code candidates and are retained nowhere as scoped, tested theory artifacts. |
