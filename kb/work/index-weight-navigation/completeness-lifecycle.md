...
**(c) The** `covered` **mark — designed, adoption deferred to the first real case.** _(Revised after review comment c1: an earlier draft dismissed this; the routing framing changes the verdict.)_ A grown tag can make a different enforceable claim than `complete`: not "every member is linked here" but "every member carries at least one of the child tags this README routes to" — `covered: true`, checked as membership(T) ⊆ ∪ membership(child_i), the same query machinery as the membership check. Its properties differ from `complete` in exactly the ways growth punishes:
...
exit (b) split with overlap           [only if substructure is editorially real;
    ...
    │ exit (c) covered: true          [designed, unadopted: enumerated children
...
A large tag's default end state is _selective head + rg membership + build-time tail_ — the same surface set as a tag that never declared complete. The `complete` mark is a small-tag affordance that degrades gracefully, not a status to defend; `covered` is the designed (unadopted) growth-proof alternative for tags with real substructure.
...
5. The `covered` mark (c) is designed but not adopted: never write the unenforced prose version of the claim ("the children cover this tag"); adopt the enforced mark the first time a real tag needs typed routing at scale, with `learning-theory` as the natural candidate. Watch for the catch-all-child smell.
