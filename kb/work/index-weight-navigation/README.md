...
7. ~~Migrate the tag indexes + hub~~ — **done (2026-06-10), 15 of 16 per approved list:** renamed via `commonplace-relocate-note` (backlinks + mkdocs redirects) and re-typed to `tag-readme`. **Deferred: `learning-theory-index.md`** (18.8 KB — would hard-fail the weight gate) stays type `index` until a separate editorial pass trims it or splits the tag (also the `covered_by` pilot candidate); the dual-type hook keeps it fully functional meanwhile. Validation green; one expected soft warn (`computational-model-README.md`, 10.5 KB).
