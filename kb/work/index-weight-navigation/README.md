...
- **Weight gates re-scope (revises decision #3):** the 8/16 KB gates apply to *selective* tag-READMEs, where size is editorial discipline. A `complete: true` README is membership-sized: exempt from the hard fail, weight reported only, and heavy weight signals *split the tag*, not shrink the file. `learning-theory-index` is no longer forced to split — it either declares complete (after adding the missing tagged notes with context phrases) or stays selective and trims.
...
3. ~~Soft/hard weight thresholds for curated indexes~~ — **resolved (2026-06-10): bytes, global, soft 8 KB warn / hard 16 KB fail** — *re-scoped same day by the type split above: the gates apply to selective `tag-readme` artifacts; `complete: true` READMEs are exempt from the hard fail (weight reported only; heavy = split the tag).*
   ...
   - **Sequencing (ADR 024 blast-radius rule — audit before flipping):** land the soft warn first; the hard fail goes in only after `learning-theory-index.md` either declares `complete: true` (exempting it) or is trimmed/split below the gate, otherwise validation is red on day one.
...
6. Define the `tag-readme` type: type spec + schema (binding field for the tag key, optional `complete` mark), and validation — membership check when `complete: true`; weight gates (8 KB warn / 16 KB fail, bytes + entry count reported) when selective.
...
8. Re-key the mkdocs hook on the new type (tag badge links, tail injection, hub tail); update `navigation.md`, `cp-skill-connect` (skip the by-tag rg when the destination tag-README declares `complete: true`), `kb/types/index.md` (shrinks to build-time virtual pages), and `maintain-curated-indexes.md`.
...
- Curated heads stay committed at every scope under one name: directory `README.md` (free form) and `<tag>-README.md` (`tag-readme` type). Selective READMEs sit under the weight gates; `complete: true` READMEs pass the enforced membership check instead.
