...
8. Update the instruction surfaces (impact analysis 2026-06-10):
   - **`cp-skill-write` — the one real contract change.** Marks extend the write operation: before finalizing tags, check each tag's README frontmatter for marks; `complete` → add the entry + context phrase to that README in the same change (explicit carve-out from the single-artifact shape); `covered_by` → the note must carry one of the listed child tags. The validate step must also validate the READMEs of every marked tag the note carries (the cross-artifact gap).
   ...
   - **`AGENTS.md`** — minimal: rename Key Indexes entries after migration; one added sentence that marked tag-READMEs carry enforced contracts, pointing at the type spec. No procedure detail.
