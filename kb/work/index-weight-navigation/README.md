...
3. ~~Soft/hard weight thresholds for curated indexes~~ — **resolved (2026-06-10): bytes, global, soft 8 KB warn / hard 16 KB fail, scoped to `type: kb/types/index.md` artifacts.**
   ...
   - **Sequencing (ADR 024 blast-radius rule — audit before flipping):** land the soft warn first; the hard fail goes in only after `learning-theory-index.md` is split, otherwise validation is red on day one.
...
6. Add curated-index weight reporting + validation thresholds — thresholds decided (decision #3): bytes + entry count reported per index-typed artifact; warn at 8 KB now, fail at 16 KB once `learning-theory-index.md` is split below it.
