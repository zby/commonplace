...
6. Define the `tag-readme` type: type spec + schema (binding field for the tag key, optional `complete` mark), and validation — weight gates on every instance (8 KB warn / 16 KB fail, bytes + entry count reported; the type contract is *small*), plus the membership check when `complete: true`.
