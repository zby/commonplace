...
6. ~~Define the `tag-readme` type~~ — **done (2026-06-10):** type spec + schema (binding field for the tag key; optional `complete` and `covered_by: [children]` marks), and validation — weight gates on every instance (8 KB warn / 16 KB fail, bytes + entry count reported; the type contract is *small*), membership check when `complete: true`, coverage check + fan-out warn (~7 children) when `covered_by` (full rules in [completeness-lifecycle.md](./completeness-lifecycle.md)). The type spec is the **authoritative home** for the whole contract — including the `complete`/`covered_by` instructions — and is **maintenance-path only**: a tag-README is understood standalone (self-describing field names, ordinary curated body), so readers never load the spec; only maintenance work (declaring/dropping marks, fixing validator warnings, executing exits) reads it, routed there by the validator message. Skills and AGENTS.md point at it rather than duplicating. Note the new mechanism class: the membership/coverage checks are **cross-artifact** — a new note can invalidate a marked README while validating clean itself.
7. ~~Migrate the tag indexes + hub~~ — **done (2026-06-10), all 16:** renamed via `commonplace-relocate-note` (backlinks + mkdocs redirects) and re-typed to `tag-readme`. `learning-theory` was initially deferred (18.8 KB), then split the same day as the **`covered_by` pilot**:
   - **Seven children** (`covered_by: [deploy-time-learning, constraining, distillation, discovery, artifact-analysis, agent-memory, llm-interpretation-errors]`) — five newly minted (established vocabulary names), two pre-existing (`agent-memory`; `llm-interpretation-errors` was already the README's migrated Oracle & Verification child). Exactly at the ~7 fan-out boundary.
   - **83 members tagged** with child tags (every note keeps `learning-theory` — split discipline held; no partial migration). Parent README shrank 18.8 KB → 4.3 KB and routes by kind.
   - **`complete` pilot too:** `distillation` (6), `discovery` (8), and `artifact-analysis` (16) declare `complete: true`; their build-time tails are auto-suppressed. `constraining` and `deploy-time-learning` are selective.
   - **The validator earned its keep immediately:** the coverage check caught a member the manual sweep missed (`continual-learning-open-problem…`) because its tags were block-style YAML — exposing a real blind spot in the by-tag rg recipe (inline-form-only), now documented as a caveat in `navigation.md`. Possible follow-up: validate/normalize tags to inline form.
   - Validation green; one expected soft warn remains (`computational-model-README.md`, 10.5 KB — next maintenance candidate).
8. ~~Update the instruction surfaces~~ — **done (2026-06-10) per the impact analysis below:** AGENTS.md (mark meanings + renames), `cp-skill-connect` (skip-rg on `complete`), `navigation.md` (marks as affordances), `kb/notes/COLLECTION.md` (types table), `kb/types/index.md` (shrunk to build-time pages), `maintain-curated-indexes.md` (tag-README terminology, fix-route framing, type-spec pointer), `control-plane-goals.md` + two notes (renamed references). The hook was re-keyed in step 6 (dual-type). FIX-SYSTEM untouched by design — its strategy taxonomy grows when a warning class is first fixed. Original analysis:
