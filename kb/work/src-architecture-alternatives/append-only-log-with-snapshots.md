...
- **Where the log lives — DECIDED: outside Git.** Review state, including the new log and its blobs, stays out of Git under `kb/reports/state/`, continuous with ADR-010's "reviews leave Git" decision. This is coherent with the snapshot rule: the store depends on Git in *neither* direction — it doesn't read prior content from Git, and it isn't committed to Git. The log + blobs are a self-contained, local, rebuildable review store.
