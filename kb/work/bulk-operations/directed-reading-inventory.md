# Task-framed reading contract inventory

This historical scan treats task-framed reading as reading selected material
through an explicit lens to produce a lens-shaped result. The standalone
`directed reading` technical label was retired on 2026-08-23 because ordinary
language plus [context engineering](../../notes/definitions/context-engineering.md)
carries the distinction. The inventory remains an input to the broader
...
- `kb/agent-memory-systems/types/agent-memory-system-review.md` is type-driven task-framed reading: read a repository and existing KB frame through a code-grounded related-system review lens, then write the review note required by the type.
...
The note-review system uses task-framed reading even though it does not use handwritten instruction notes. `src/commonplace/review/protocol/prompt.py` renders run-specific prompts that include:
