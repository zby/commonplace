...
1. The scheme survives contact with the existing catalogue: every current technique (validators, gates, conformance pairs, critique, friction gate, operator review) places cleanly, and every governing claim either cites existing notes or is marked as this workshop's conjecture.
2. A decision per exposed gap: build (proposal in `kb/reference/proposals/`), defer (named in the systematisation), or reject.
3. The durable claims promoted — likely one structure note (detection as a two-layer execution system with promotion-by-recurrence) and the technique grid either folded into it or kept as reference material.
