...
The `areas:` frontmatter field is the first defense. But `areas:` requires the agent to know what indexes are available. Without that knowledge, the field stays empty or incomplete, and everything downstream (Topics footer, index listing) inherits the gap.
