...
Named-system classifications belong to the
[boundary-case assessment](./boundary-case-assessment.md), which separates
the deployment boundary, observed evaluation, available evidence, and
continuing responsibility. A computational procedure need not satisfy the
base theory-builder definition, and a proof obligation does not establish
reliability beyond what it covers.

The research target is an autonomous, reflective theory builder whose
reliability is assessed under the
[downstream protocol](./commonplace-evidence-protocol.md). Its
[fixed-model constraint](./README.md#goal) restricts the research target,
not the meaning of autonomy.
