...
**Posed by:** the operator, 2026-08-26. Restructure [`kb/articles/reflective-self-improvement.md`](../../articles/reflective-self-improvement.md) — not discard it — into the third article pillar beside [the bitter-lesson article](../../articles/the-bitter-lesson-does-not-require-everything-to-live-in-weights.md) and [the human-inclusive revision article](../../articles/what-makes-human-inclusive-self-revision-non-trivial.md): theory-mediated methodology, with Naur's *Programming as Theory Building* as the hook.
...
3. **Why the binding held, and what changed.** The basis note's argument: inexpressibility targets formulated criteria; the bridge to humans is that a program executes formulated criteria; accurate in 1985; trained recognizers separate execution from formulated criteria on Naur's own examples (faces, tunes, wine). Merge the adjoiner's Part 1 here: reflective systems existed for decades with the causal wire and no evaluator; the missing resource was judgment over a declared objective, which is the same application judgment Naur located in the programmer. Proof / benchmark / LLM judgment as the regimes. *Sources:* basis note; [adjoiner Part 1](../reflective-article-direction-adjoiner.md). *Notes:* goedel-machines; reflection-buys-addressability.
...
- The adjoiner's candidate note ("LLMs supply the proposer and the judge; direction under a fuzzy objective is the remaining human function") still has no home; §8 would consume it into the article without a note.
