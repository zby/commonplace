...
| Artifact representation | Plain Markdown with no frontmatter, permitted by the workshop contract. | Frontmatter declares `type: note` and adds a retrieval description. | The pass changed type even though its packet first judged the implicit `text` type to fit. |
