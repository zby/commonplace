...
- **The extensions are exactly the LLM-shaped delta, and are flagged.** Retrieval as the causal wire (discovery over retained artifacts replacing the interpreter), [addressability](../../notes/reflection-buys-addressability.md) (retained changes treated as commitments that can be read, criticized, rescoped — not just accessed), and the prose route of [reach-assessment](../../notes/definitions/reach-assessment.md) (semantic judgment of whether a commitment in natural language genuinely generalizes) are each marked "Commonplace's own" in a Provenance section. Together these three are precisely the coverage the new capability needs and established reflection lacks.
...
- Three **base terms** doing the criterial work: [behavior-determining organization](../../notes/definitions/behavior-determining-organization.md), [operative change](../../notes/definitions/operative-change.md), [evidence bearing on an improvement objective](../../notes/definitions/evidence-bearing-on-an-improvement-objective.md), all resting on [behavioral authority](../../notes/definitions/behavioral-authority.md).
...
BFO forces a choice the cluster has not made: is *self-improving* predicated of a system in virtue of a **disposition** (it has an improvement pathway, currently dormant or not), or in virtue of **participation in a process** (evidence-responsive change is actually occurring over some interval)? The definition's present tense — "makes operative changes" — reads as participation; the [boundary-case table](../../notes/the-self-improving-system-definition-classifies-its-boundary-cases.md) classifies standing mechanisms, which reads as dispositional; nothing states which reading is intended or over what interval membership is assessed. This is not pedantry: it decides whether Commonplace is a self-improving system *between* improvement episodes, and therefore how the phase-1 audit should phrase every attribution. The cheap resolution is the one the cluster already uses for operativity — relativize to a declared horizon: *a system is self-improving over a stated assessment horizon iff evidence-responsive operative self-change occurs within it; the dispositional reading ("has a standing improvement pathway") is available but must be marked as such.* One paragraph in the definition settles it. → **Ledger candidate.**
...
| 7 | Profile-not-ladder lacks a continuity citation (Parasuraman et al. per-function grading) and a coverage comparison (self-aware computing levels have no slot for semantic self-interpretation) | gap | Optional provenance line for continuity; a short coverage comparison would state the cluster's reason for existing in one worked example |
...
- [Philosophy-borrowing workshop](../philosophy-borrowing/README.md) — see-also: owns the Carnap-explication borrowing this evaluation applies
