...
- **Canonical frame** — the declared boundary exists only inside [Commonplace as a reflective system](../../reference/commonplace-as-a-reflective-system.md); repeated applications will re-derive and silently diverge without a reusable anchor. The [external-theory evaluation](./external-theory-evaluation.md) (finding 3) supplies the theoretical anchor: the predicate is frame-indexed — the bearer is a bounded system, not a substrate — and the definition's Provenance section should say so once.
- **Membership tense** — the definition does not state whether membership is dispositional (a standing improvement pathway) or occurrent (evidence-responsive change actually occurring), nor over what assessment interval; decides whether Commonplace qualifies between improvement episodes. Suggested resolution mirrors operativity: relativize to a declared assessment horizon. See [external-theory evaluation](./external-theory-evaluation.md), finding 2.
- **"Closure" collision with the autopoietic sense** — the [closure note](../../notes/methodological-and-computational-closure-track-different-changes.md) never disambiguates its two readings from Varela's operational closure; the exclusion lives only in the reflective-system note. One contrast sentence at the closure note resolves it per the write-time collision rule. See [external-theory evaluation](./external-theory-evaluation.md), finding 5.
