...
**The instruction sprawl this predicted had already happened.** One archiving rule was stated in four places — `kb/reference/COLLECTION.md`, `proposals/README.md`, `proposals/archive/README.md`, and the `design-proposal` type spec — each mixing what must be true with how to do it. This is the recurrence that [where subtree-scoped write-time contracts live](../../reference/proposals/archive/where-subtree-scoped-write-time-contracts-live.md) names as the only evidence that would warrant a general mechanism: *"the pull toward area READMEs re-emerging after instances have been absorbed case-by-case."* `proposals/archive/README.md` appeared one day after the collection-local type absorbed the proposals area — absorbed, and re-emerged immediately, one level deeper. That proposal's current-state anchor predates the archive directory and does not mention it.
