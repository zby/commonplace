...
**Blocking the map now:** an eligibility rule for note retirement, and a destination for source snapshots. The note *operation* exists, so the map can describe the transition; what it cannot yet say is what triggers it. Beyond that, the claims worth promoting to `kb/notes/` each need a second worked case: the archive-versus-delete test (strongest, and source snapshots are the case that would test it), tiers-as-coordinates, and the workshop deletion bet — the last now derivable from the archive-versus-delete test rather than standing alone (see "Still open" in [lifecycle-model.md](./lifecycle-model.md)).
