...
4. **Retirement protocol.** What signals a note is stale? What is the action — revise in place, mark deprecated, delete? How do inbound links get handled?
   ...
   **Still open for the other kinds.** [disposal-and-retirement-survey.md](./disposal-and-retirement-survey.md) holds the per-kind audit data and now tracks what remains: **note retirement** — the original target of this question — has no disposal operation at all, and neither do source snapshots. The framing the proposals case established carries forward: the operative retention cost is the search surface, not storage, so the axis is "in the frontier vs. out of it, and still citable vs. not," and disposal is the operation that removes an artifact from the frontier. Whether that transfers to notes is genuinely open — a note has no adopting successor to carry its extraction, and `kb/notes/` is itself the frontier.
...
**Blocking the map now:** a disposal operation for notes and one for source snapshots — the two kinds with no end-of-life path at all. Until at least the note case is worked, the map would name a transition it cannot describe. Beyond that, the claims worth promoting to `kb/notes/` each need a second worked case: recomputable-cache versus committed-compression, tiers-as-coordinates, and the workshop deletion bet (see "Still open" in [lifecycle-model.md](./lifecycle-model.md)).
