...
4. **Retirement protocol.** What signals a note is stale? What is the action — revise in place, mark deprecated, delete? How do inbound links get handled?
   ...
   **Answered for proposals, shipped 2026-07-25** — [ADR 056](../../reference/adr/056-adopted-and-retired-proposals-archive-out-of-the-frontier.md). Start from [lifecycle-model.md](./lifecycle-model.md), the consolidated discussion summary; [disposal-and-retirement-survey.md](./disposal-and-retirement-survey.md) holds the per-kind audit data behind it. Note retirement — the original target of this question — remains open.

   **Survey landed (2026-07-25):** [disposal-and-retirement-survey.md](./disposal-and-retirement-survey.md) widens this from notes to every artifact kind. Five disposal operations are in live use (delete-with-git-pointer, archive-in-place with status flip, archive-in-place with ad-hoc banner, DB-target retire, move-content-out-keep-residual), only some licensed by a contract where they occur. The live defect is `kb/reference/proposals/`: the contract names the adoption transition but no disposal operation, so adopted/retired proposals sit in a directory whose README promises "finished but unadopted" — one was deleted, two were banner-annotated ad hoc. The survey's working direction (sharpened with maintainer input, 2026-07-25): the operative retention cost is the search surface, not storage, so the goal is keeping the current frontier quickly locatable — workshops keep deleting; proposals archive by directory separation; and the archive is a link sink on the workshop model — archiving obligates full extraction, incomplete extraction is repaired by re-extraction rather than by linking in, and the only admissible frontier→archive links are from archive-operating instructions. That settles the ADR question: the decision record is the ADR alone, with the archived proposal as re-extraction substrate, not a citable appendix.
...
Added 2026-07-25: a defined disposal clause for `kb/reference/proposals/` — an amendment to its README (and, since it amends ADR 028's contract, probably recorded as an ADR) that defines the archived state — together with the companion expansion of the ADR type contract (a required considered-alternatives section, schema-checked), so the ADR can carry the decision data the demoted proposal pointer used to reach. Current lean: an archive subdirectory governed as a link sink — adopted/retired proposals move out of the frontier after full extraction; existing frontier links into them (ADR 045's supersedes edge, the collections-never-own citation of assertion-force) are re-extracted or inlined rather than rewritten; the no-inbound-links rule is validator-checkable. The remaining free choices are listed at the end of the disposal survey.
