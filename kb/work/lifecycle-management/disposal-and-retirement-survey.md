...
## Working direction (hypothesis, not decided)

Match the disposal operation to what cites the artifact afterwards:

- **Nothing durable cites it** → delete (workshops; scratch). Keep the git-history pointer convention but treat it as courtesy, not archive.
- **Durable artifacts cite it as history** → archive in place with a *licensed* state marker, on the ADR model: amend `proposals/README.md` to define an adopted/retired state (banner + description prefix, or a frontmatter field), update the directory's opening claim, and make the ADR's `supersedes` edge the licensing event. The two existing banners become the worked examples instead of drift.
- **State lives in the store** → `commonplace-freshness-retire` and kin; already right.

Open free choices for the proposal amendment: banner-only vs. frontmatter status field (a field is checkable — marks doctrine says checkable-or-omitted); whether retired-as-YAGNI and adopted-into-ADR are one state or two; whether an `archive/` subtree buys anything over in-place marking (it breaks inbound paths, so probably not).
