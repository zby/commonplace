...
The claim-disposition stage added to `cp-skill-write-multistage` (preserved in `evidence/writing-machinery-integration.md`) is not that first transition. It is a process correction prompted by the episode's artifact-shape failure — synthesis where an atomic claim was warranted — not an installation of the retained epistemology. The note's ideas, warrant per claim-scope pair and the two thresholds, have not yet been folded into writing or review machinery, so step 2 of the pathway remains open.
...
## The main question: who invented the writing machinery

The workshop's governing question is now attribution over the machinery revision itself: how much of the new writing machinery was invented by agents. The criterion is fuzzy — "invented" is not yet operationalized the way note-attribution was, and a stricter test may replace this reading.

A preliminary read from the preserved evidence: the human's commissioning prompt specified the functional design — a stage that inventories the claims being made, distinguishes what folds into existing notes from what warrants new notes from the central contribution, and involves the user for folds and for multiple new artifacts. Agents contributed the diagnosis of why the synthesis shape occurred (four process causes), the concrete six-category disposition scheme, the defaults (one citable proposition per claim-bearing artifact; synthesis only when the composition is itself the contribution), the integration of the disposition into the downstream stages, and the dry-run tests. Two lineage details complicate a clean split: the fold-versus-separate vocabulary entered through the human's restructure-episode prompt, while the disposition worked out there — central claim kept, one fragment folded into a sibling, candidates deliberately not promoted — was agent analysis that the machinery design then mirrors. On this reading the machinery episode is a third attribution profile: human-specified functional design, agent-supplied mechanism and diagnosis — between the generation episode's agent-dominant elaboration and the consolidation episode's human-dominant judgment.
