...
- Of the 85 hardcoded `kb/{notes,reference,instructions}/` references in shipped instructions, only a subset needs translation: references to specific shipped artifacts (e.g. `kb/reference/adr/010-...md`, `kb/instructions/run-review-bundle-on-note.md`). Generic references ("a collection such as `kb/notes/`", "read `kb/<collection>/COLLECTION.md`") resolve correctly to the user's own collection and need no change. An audit is needed to split the 85 into these categories.
