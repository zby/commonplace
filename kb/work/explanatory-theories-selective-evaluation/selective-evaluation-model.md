...
A compact working model represents this chain with six provisional objects:

- `S` is the prediction boundary. It includes the relevant assumptions about fixed model parameters, runtime and outer capabilities, environment, optimizer and evaluator configuration, decoding, authority, and routing—not only mutable harness state.
...
- `T` is a revisable, scoped, assumption-bearing theory of the system's behavior-generating organization. It may be natural-language, causal, programmatic, symbolic, distributed-parametric, or mixed. Its content, assumptions, and applicability conditions must remain criticizable; calling it explanatory does not establish explanatory-reach.
...
- `G*` is a commitment rule that states how observed and omitted obligations bear on acceptance.

The derived object `I_T(S, Delta)` is the impact projection supported by `T` for this candidate and boundary. It may contain mechanically established non-influence, deterministic consequences, probabilistic beliefs, or explicitly unresolved effects. It is not the theory itself: one retained theory can yield different projections for different candidates, and a bad derivation can fail even when some part of the theory is sound.

Together, `T` and the candidate produce `I_T`; `M` identifies ways to observe the projected effects; `pi` chooses what evidence to acquire; and `G*` determines what that evidence warrants:

`T + (S, Delta) -> I_T(S, Delta) -> obligations -> M -> pi -> evidence -> G*`
...
The impact question concerns the consequences of a proposed change, but the retained theory need not be a formal causal model. Several routes can produce all or part of `I_T`, and mixed theories may compose them:
...
The conservative baseline is therefore candidate-relative applicability followed by full evaluation of the applicable set. Enforced structure may prove some obligations inapplicable; where it cannot, the baseline keeps them applicable. Probabilistic selection over the remaining obligations is a candidate extension, not a substitute for an incomplete dependency account.
...
A design must determine candidate-relative applicability and declare its non-negotiable constraints before cost enters the selection policy. HCL's validity checks span syntax, output-schema, tool-use, task, and environment checks. Cost alone cannot make any of them inapplicable.

The selection objective also remains unsettled. Candidates include expected harmful missed loss, observer-relative value of information, and a constrained ceiling on residual risk. Where `I_T` supplies probabilities, a numerical policy would require them to be calibrated, along with commensurable loss and cost, a stated horizon, dependencies among evaluations, and explicit risk tolerance. Deductive claims require soundness checks rather than probability calibration. When a procedure must be synthesized, its cost includes construction and validation as well as execution. Evidence can also be valuable because it changes a decision, tests or revises `T`, or calibrates `I_T`, as [Information value is observer-relative](../../notes/information-value-is-observer-relative.md) argues.
