...
**Core note.** Candidate claim-title: *"Review-time verification sees state, not history."* The claim: a checker arriving after production sees the artifact's state — its text plus the witnesses it carries — never its history, and any state is consistent with many histories, so history-dependent properties are undecidable from the artifact in principle. The note then answers "so what does a KB do about it" with the machinery this file develops: the two bridges (records and re-derivability), the review-decidable definition with its bounded evidence radius, the carried-witness family with its checkable/credence regimes, decay, and the non-retrofittability rule. Anchors: Reichenbach's discovery/justification and Shapin's virtual witnessing on the philosophy side; PROV, in-toto, and credence goods on the systems and economics side. Naming hazards to settle first: "state" is overloaded in software (mitigated by "artifact state"); avoid "intrinsic/extrinsic" (philosophy baggage) and bare "observability" (collides with the verifiability gradient's rewardability property).

**Corollary note.** Candidate claim-title: *"Criteria edits invalidate verdicts; process edits invalidate artifacts."* A pure-claim note: the two halves of an artifact's contract propagate change differently — criteria edits stale judgments (the artifact stands; re-judge it), process edits stale artifacts (the verdicts stand; consider regenerating). Re-review after a process edit is not just wasteful but *misleading*: it re-passes the unchanged criteria while the real deficiency stays invisible. The cache-key framing carries the argument, with its two predicted failure modes (too coarse → spurious cohort staleness; too fine → falsely-fresh verdicts, the pre-ADR-038 bug). Anchors: Build Systems à la Carte for the formal model; the replication crisis and pre-registration as the institutional-scale witness. Composes with `link-graph-plus-timestamps-enables-make-like-staleness-detection`. The graduated ladder deliberately stays out — it is design guidance, not theory, and belongs in the proposal.
