...
1. Core note — candidate claim-title: "Review-time verification sees state, not history" (with reification, re-derivability, decay, and non-retrofittability inside it; phil-sci anchors: Reichenbach's discovery/justification, virtual witnessing). Naming hazards: "state" overloaded in software; avoid "intrinsic/extrinsic" (philosophy baggage) and bare "observability" (collides with the verifiability gradient's rewardability).
2. Corollary note — candidate claim-title: "Criteria edits invalidate verdicts; process edits invalidate artifacts". Composes with `link-graph-plus-timestamps-enables-make-like-staleness-detection`; the replication crisis / pre-registration is the institutional-scale witness.
