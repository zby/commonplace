...
| Reviews | Review jobs own generated prompt, bundle, manifest, and per-pair result artifacts; canonical decisions and current acceptance live in SQLite. Freshness compares current files with DB-owned note/gate snapshots. Type specs and `COLLECTION.md` contracts reuse the gate side as factored dependencies. | Lineage is explicit for each `(note_path, gate_path, model_partition)` acceptance: accepted note/gate snapshots point back to completed review evidence. | Result files remain readable evidence, but selectors bind on guarded current acceptance; prompt scaffolding is deliberately outside the freshness hash. |
...
| Review | select stale note/gate pair(s), create jobs, run agent review, finalize all-or-nothing | job-owned markdown artifacts plus DB decisions, snapshots, and current acceptance | current acceptance, warn/fix queue, note edits |
...
The review subsystem shows why this may require relational edge-state, not only file conventions. Review prose remains Markdown, but review lineage lives in SQLite because selectors need indexed current-state queries over a changing note/gate relation. Successful finalization upserts one current acceptance per `(note_path, gate_path, model_partition)` and prunes superseded evidence; history is not the canonical abstraction. If automatic derivation expands beyond reviews, a similar edge-state surface may eventually be needed for compiled views, cues, source-derived refresh meshes, or merge-back acceptance — but only once a concrete second mesh develops comparable churn and queries.
