...
| Reviews and anchored critique | Review jobs own generated prompt, bundle, manifest, and per-pair result artifacts; persisted `result_kind` separates closed-ended verdicts from open-ended reports. Schema v6 persists `criterion_path` and criterion snapshots, groups cross-note jobs by `criterion`, and reports `criterion-changed`. Current acceptance lives in SQLite and freshness compares current files with DB-owned note/criterion snapshots. | Lineage is explicit for each `(note_path, criterion_path, model_partition)` acceptance: accepted snapshots point back to completed evidence. Verdict pairs complete with a decision; critique completes with `REPORT`, `reviewed_at`, and a null decision. | Result files remain readable evidence, but selectors bind on guarded `current_criterion_acceptances`. Critique embeds its captured criterion snapshot in the job prompt; type/collection conformance still references the live dependency document and tolerates a bounded TOCTOU window. The v5→v6 vocabulary migration recreates the operational store rather than transforming it. |
...
| Full-pass closure evidence | Each pass retains `initial/`, `closing/`, and `controls/` reports under a pass ID. A committed workshop JSONL records counterfactual carry judgments, comparisons, model provenance, and `{path, sha256}` report references. | The JSONL preserves non-regenerable process history that current-state acceptance and inline pruning cannot retain; report bytes remain available long enough to audit the references. | This is hand-written experimental evidence, not a durable cross-class event service. At 100% sampling every anchored assay reruns, so no real carry event changes acceptance. |
...
- ignored under `kb/reports/`: connect reports, critique/full-pass copies, friction reports, fix reports, bundle-review artifacts, review DB, and autoreason outputs.
...
The ambiguity matters because ignored reports can still contain high-value maintenance observations. The connect-maintenance workshop exists because generated reports had useful signals that needed a promotion pass. Transformation closure adds a second retention mode: ignored report instances may be temporarily preserved as evidence for a committed observation record, then deleted after the workshop extracts durable conclusions.
...
| Full-pass closure | retain initial reports, transform the note, rerun all methods on final bytes, and duplicate controls | pass-scoped ignored reports plus committed observation JSONL | located constraints, an instrumentation decision, and durable workshop conclusions |
...
The review subsystem shows why this may require relational edge-state, not only file conventions. Assay prose remains Markdown, but review lineage lives in SQLite because selectors need indexed current-state queries over a changing note/criterion relation. Successful finalization upserts one current acceptance per `(note_path, criterion_path, model_partition)` and prunes superseded evidence; history is not the canonical abstraction. If automatic derivation expands beyond reviews, a similar edge-state surface may eventually be needed for compiled views, cues, source-derived refresh meshes, or merge-back acceptance — but only once a concrete second mesh develops comparable churn and queries.
...
### Advisory report staleness can be fuzzy until it is anchored

Strictness follows behavioral authority and the chosen protocol. Connect, friction, and compression outputs remain advisory and unanchored, so stale detection can remain a procedural rerun trigger. Critique is still open-ended and non-endorsing, but now uses exact note and criterion snapshots because the full-pass experiment needs current evidence. A fresh critique means only that this report matches these bytes. Review acceptance deliberately excludes most mechanical prompt scaffolding from the freshness hash; critique is the exception that embeds the criterion snapshot into the rendered prompt. A scaffolding change that alters judgments requires an explicit corpus-wide re-review or acknowledgement decision rather than silently widening every review key; the boundary is documented in [review architecture](../../reference/review-architecture.md#freshness-mechanism).

### Churning many-to-many edge state needs relational structure

`kb/notes/many-to-many-edge-state-is-where-files-yield-to-a-database.md` extracts the storage rule from the review case. ADR 010 moved review state to SQLite after review artifacts stopped behaving like authored files and started behaving like operational lineage: keyed by note, gate, and model; updated as current acceptance; queried by selectors for freshness. The extracted theory names the structural condition: churning state on an ownerless many-to-many edge. It also softens the implementation claim: a directory of edge files can represent the relation for a while, but that is already a filesystem-backed relational store, and a real database wins when keyed lookup, churn, and consistency demands exceed that surface.
