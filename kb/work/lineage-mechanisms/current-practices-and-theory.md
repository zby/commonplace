...
| Reviews | Review markdown files remain generated report outputs; canonical freshness/acceptance state lives in SQLite acceptance events. Freshness is DB-owned note/gate snapshot hashes, not Git SHAs ([ADR 032](../../reference/adr/032-review-freshness-uses-db-snapshots-not-git.md)). | Lineage is explicit for review pairs: note snapshot, gate snapshot, model partition, acceptance event. | The pinned contract is only the note and the leaf gate; the type-spec a note is judged against is an unmodeled freshness input, so a type-definition change invalidates no acceptance (tension 13). |
...
### 13. A derived artifact's freshness contract can be narrower than its true contract

Review freshness pins the note and the leaf gate file, but a note is judged against its resolved type-spec too — the schema and the authoring guidance that define what a correct note of that type looks like. That type-spec is an unmodeled dependency edge: revising a type definition changes the standard every existing note of the type is held to, yet invalidates no acceptance. The schema half is already covered mechanically by `commonplace-validate` (stateless and always-current, so a tightened schema re-flags on the next run); the prose-guidance half is judgmental and currently tracked by nothing. `review-architecture.md` already names the fix — widen the leaf gate-file hash to an effective review-contract hash — and a proposal now holds the concrete design ([review freshness should track the full review contract](../../reference/proposals/review-freshness-tracks-the-full-contract.md)). Generalized past review: any freshness mechanism that hashes only an artifact and its immediate producer can miss the upstream contract or definition files the artifact was derived against.

Question: for each derived class, what is the full set of contract inputs whose change should invalidate — and how is it bounded so that a high-reach root-contract edit does not invalidate the whole corpus at once?
