...
The transformation-closure experiment supplied the first concrete weight-2 witness. Its carry judgments and comparisons are non-regenerable process history, so each pass commits a hand-written observation JSONL under `kb/work/transformation-closure/observations/`, while the compared full-pass reports remain gitignored but are retained by path and hash through workshop closure. This is not yet a general ledger: no command owns the writes, no real ack occurs at 100% counterfactual sampling, and the evidence disappears or is promoted when that workshop closes. It nevertheless sharpens the middle tier: current state belongs in acceptance; process history that must outlive pruning belongs on an append-only or committed event surface.
