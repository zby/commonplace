...
The 2026-08 product-direction session (Commonplace as an llm-wiki for software documentation and scientific article outlines) added two external-referent grounding cases: documentation depending on code regions, and claims/outlines depending on captured literature. Both land on matrix decisions 1–3 rather than opening a new track. The docs-on-code case initially looked like the strongest candidate for the "second churning mesh" that the deferred escalations wait on. [A seven-artifact disposition sweep](../../notes/evidence/seven-documentation-cases-left-routing-and-synthesis.md) then disconfirmed that candidate in Commonplace: direct source access removed exact-fact prose, leaving routing and synthesis without a repeated doc×code staleness selector. The carrier case remains, but watched-edge storage now waits for a consuming project that actually retains positive-value exact documentation and repeatedly selects it for refresh. See [external-referent-grounding-cases.md](./external-referent-grounding-cases.md), which records the result and drafts a mutability-based carrier rule for immutable/external sources.
...
- work the external-referent grounding cases (docs-on-code, outlines-on-literature) through matrix decisions 1–3 and extract the carrier/direction contract; keep the external-referent review-pair proposal deferred until a real consumer demonstrates a watched-edge requirement — see [external-referent-grounding-cases.md](./external-referent-grounding-cases.md);
...
- [external-referent-grounding-cases.md](./external-referent-grounding-cases.md) - forcing witnesses from the llm-wiki product direction: docs-on-code and outlines-on-literature mapped onto matrix decisions 1–3, with a draft mutability-based carrier rule for immutable/external sources.
- [eigenius-contact-points.md](./eigenius-contact-points.md) - external whole-system witness: Eigenius mapped onto verification-locus theory with code-grounded qualifications on factivity, gate coverage, invalidation, event provenance, and host-agent enforcement.
- [scienceflow-contact-points.md](./scienceflow-contact-points.md) - contrasting external whole-system witness: recoverable work state, non-rewindable evidence and resource history, source-preserving context folding, and consumer-relative lineage closure.
