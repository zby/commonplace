...
- The object: [self-improving system](../../notes/definitions/self-improving-system.md) — operative changes to the system's own behavior-determining organization, causally responsive to evidence bearing on an improvement objective. The central distinction is **reflective versus non-reflective self-improvement**, pathway-relative. Under Commonplace's strictly computational reflective-system boundary, a pathway that is reflective at all is thereby also autonomous; Commonplace's own pathways split — reflective-and-autonomous where a validator or agent consults an explicit self-representation, non-reflective-and-human-inclusive where a maintainer notices or judges. What reflection distinctively buys is [addressability](../../notes/reflection-buys-addressability.md); compounding is available without it.
- The loop: [a proposal-selection improvement loop requires search, evaluation, and operative retention](../../notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md) — the named subtype Commonplace instantiates; the gate/oracle machinery is scoped there, not to the base definition.
- The bound: [warranted autonomy is bounded by oracle domain](../../notes/warranted-autonomy-is-bounded-by-oracle-domain.md) and [the boundary of automation is the boundary of verification](../../notes/the-boundary-of-automation-is-the-boundary-of-verification.md).
...
- Classification: [Commonplace as a reflective system](../../reference/commonplace-as-a-reflective-system.md) — a **pathway-mixed self-improving system**, reflective-and-autonomous and non-reflective-and-human-inclusive stated pathway by pathway.
...
> **Internal note for whoever drafts §1–2 and §4 (2026-07-15, spine revised) — not submission text.** Commonplace's reflective-system boundary is strictly computational: a maintainer's judgment never satisfies causal connection, however established the role. So reflection is pathway-specific and hard-won, not cheap machinery, and a pathway that qualifies as reflective at all is thereby also autonomous. Write the system's pathways as **split** — some reflective and autonomous, some non-reflective and human-inclusive. Do not write that reflection is cheap, and do not write that Commonplace as a whole is one reflective system.

## The pitch

**A knowledge system that improves runs a proposal-selection improvement loop — search, evaluation, operative retention — on itself. In an agent-operated system, the thing it runs *through* is its methodology: the retained prose an agent reads before it acts. So methodology, not capability, is what compounds. We present a methodology for agent-operated epistemic investigation, a runnable reference implementation, and three stress-test casebooks — and we are candid that the binding constraint is the evaluator, which is the part we are actively building.**
...
**Where we sit — say it plainly, and say it early.** Commonplace is a self-improving system whose loop closes, runs on the system's own artifacts, and aims at an objective it could have failed — but not as one uniform thing. Its pathways **split**: where a validator or an agent consults an explicit self-representation, the pathway is reflective and autonomous; where a maintainer notices what's worth fixing or judges whether a fix is the right shape, it is non-reflective and human-inclusive, sitting exactly where no adequate oracle reaches ([classification](../../reference/commonplace-as-a-reflective-system.md), pathway-by-pathway).

**Never write the loop as a maturity ladder, and never sell autonomy as progress.** Bare autonomy is free — hand a gate to a model with a rubric and nobody runs it tomorrow. The evaluator is fallible but it still rejects things, so the loop still closes and the system stays self-improving *while quietly getting worse*. What is bounded is [**warranted** autonomy](../../notes/warranted-autonomy-is-bounded-by-oracle-domain.md): the loop can run unattended only over candidates its oracles can assess with the required confidence.
...
2. **Search reach and evaluation strength are independent limits.** *Evaluation cannot select a candidate that search never reaches.* We report both, and we report that our search splits between a human noticing what's worth fixing and an agent formulating the specific candidate.
...
2. **The loop** — the frame that positions the methodology and bounds what it may claim. Commonplace is a self-improving system whose pathways split — reflective and autonomous where a validator or agent consults an explicit self-representation, human-inclusive where a maintainer notices or judges. This is discharged against **one** observed maintenance trace (ADR 026) — enough to establish the causal connection, not enough to characterize its reliability.
...
Sync with [self-improving system](../../notes/definitions/self-improving-system.md), [the improvement loop](../../notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md), [warranted autonomy](../../notes/warranted-autonomy-is-bounded-by-oracle-domain.md), [the false-positive asymmetry](../../notes/false-positive-generation-is-filtered-before-retention.md), [actionable methodology](../../notes/definitions/actionable-methodology.md), [the classification](../../reference/commonplace-as-a-reflective-system.md), and [DRM](../../reference/design-rationale-management.md).
...
- **"Reflective system" is a jargon risk, not a false claim, in judge-facing prose.** Under the current definition the term is a real, pathway-specific, hard-won claim, not a cheap one — but it still invites a sci-fi reading from a lay judge. Prefer the plain mechanism when in doubt: *the system's own rules are what its validators and agents read, so changing a rule changes what it enforces and what it searches.*
- **Say the split, not a blanket qualifier.** Don't say "self-improving" without noting the loop is pathway-mixed: some pathways are reflective and autonomous, some are human-inclusive. A judge should hear neither a claim of blanket autonomy nor a claim that nothing here runs unattended — both are false.
