...
The composition sketch already exists as a menu candidate — [ranking by authority: identity is a field, track record is a dossier, rank is derived](../epistack-framework-additions/author-authority-ranking.md) — and the guard rails are settled doctrine: no stored authority scalars ([confidence must be attributed, never a mark](../epistack-framework-additions/attributed-confidence-not-marks.md)), ranking stays a downstream, replaceable adjudication output ([adjudication as a separate layer](../epistack-framework-additions/adjudication-as-separate-layer.md)). What is *not* settled, and what this workshop owns, is the structure of the thing being computed:
...
- **Relational effects.** Independence discounting ([independence clustering](../epistack-framework-additions/independence-clustering.md)) makes the authority of a *set* of sources non-additive in its members. A per-source rank may be the wrong type entirely — the meaningful object may be the weighted evidence set for one claim.
...
Framework-side design only. Author dossiers and any concrete ranking experiment happen casebook-first in `epistack-casebooks` per the [backlog-to-commonplace protocol](../epistack-competition/README.md); this workshop consumes what that casework proves and designs the transferable mechanism. Doctrine constraints are inputs, not open questions: no stored scalars, adjudication downstream, frontmatter semantics stay type-owned ([collections never own frontmatter semantics](../../reference/collections-never-own-frontmatter-semantics.md)).
