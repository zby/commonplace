...
- what should be stored in external symbolic state — explicitly stored artifacts like structured data, compressed summaries, and traces, as opposed to in-context state
- what should actually be loaded into the next bounded call — a single LLM invocation with a deliberately assembled context

These are not the same decision. In the [bounded-context orchestration model](./bounded-context-orchestration-model.md), the scheduler's accumulated external state `K` can grow freely; bounded context is expensive. Storing a trace is not the mistake. The mistake is letting a session runtime decide that stored history should automatically become the next call's context instead of letting `select(K)` — the function that chooses which stored artifacts to load into the next call's prompt — decide what the next call should see.
...
With raw SDK calls, there is no built-in transcript problem. Application code assembles a prompt, calls the model, stores the result in `K`, and decides what to do next. The trouble begins when a higher-level interface changes the primitive:
...
For any call that needs focused context, that is usually the wrong trade:
...
- at scale, the accumulated trace physically cannot fit in the context window — transcript inheritance becomes not just inefficient but infeasible

This is a version of the return-value problem from the [scoping note](./llm-context-is-composed-without-scoping.md) in architectural form. In clean bounded orchestration, each call sees only the prompt assembled by the selection function, and the boundary is real. The discipline breaks when implementations package many bounded steps as one continuing session, letting history inheritance replace selective loading.

## The right split: storage vs next-context loading

The scheduler — the orchestration layer that dispatches bounded calls — can afford to keep many artifact kinds in external symbolic state: judgments, extracted claims, scrubbed conclusions, compressed episodes, structured data, symbolic code, failure signals, and raw traces for audit. What matters is that the next bounded call usually wants a much stricter projection.

"History" conflates what we can usefully distinguish as at least three trace types with different loading profiles:
...
- **Reasoning traces** — chain-of-thought, planning, deliberation. Reveals the agent's intermediate steps, not what it concluded. Worth storing for debugging and alignment research — almost never worth loading into the next call.

This decomposition is not exhaustive. Structured intermediate artifacts — relevance labels, extracted claim lists, partial syntheses, sub-goal decompositions — are judgment outputs that do not fit neatly into any of these three categories. They are often exactly the right material to load into the next call, which reinforces the point: the loading decision should be deliberate, not inherited from a session log.
...
The key separation is not "store vs discard" but **"persist in symbolic state vs load into bounded context."** In the intended model, loading happens through the selection function, not by inheriting a session log. The next bounded call should see a representation chosen for its task — including when the next decision is whether to retry, unwind, or escalate.
...
Across these systems, the shared move is [distillation](./definitions/distillation.md) at the execution boundary:
...
- [Spacebot](./related-systems/spacebot.md), an LLM-based research agent, returns only a scrubbed conclusion from its branches
...
This is not just summarization — it is interface design. The execution boundary is the natural place to compress, but the deeper question is what the selection function should load from stored state into the next bounded context. Compression at the boundary produces the artifact; the loading decision determines whether and how much of that artifact enters the next prompt.
...
Interactive sessions are designed for continuity and visibility. Orchestration wants selective loading.
