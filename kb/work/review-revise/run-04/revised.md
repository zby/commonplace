...
At the extreme, accumulated history exceeds the token limit entirely, forcing truncation and losing information unpredictably — a hard failure rather than gradual degradation.
...
External symbolic state accommodates many artifact kinds — judgments, extracted claims, scrubbed conclusions, compressed episodes, structured data, symbolic code, failure signals, and raw traces for audit. What matters is that the next call usually wants a much stricter projection.

"History" conflates several trace types — conversation transcripts, tool/action traces, reasoning traces, planning state — that differ in how much signal they carry for the next call. All may belong in external state, but they are rarely the right default material for the next prompt. Conversation mixes signal with noise; reasoning traces reveal how the agent thought, not what it concluded; tool traces are more structured but still situational; planning state is useful for coordination yet loading the full plan tree into a call that only needs its own sub-goal is the same overloading problem.

The key separation is not "store vs discard" but **"persist in symbolic state vs load into context."** In the [bounded-context orchestration model](./bounded-context-orchestration-model.md), loading happens through a deliberate selection step, not by inheriting a session log. The next call should see a representation chosen for its task — including when the next decision is whether to retry, unwind, or escalate. A call may return a structured failure artifact while the raw trace is stored separately; the existence of a trace does not imply that the trace should be loaded into the recovery prompt.
