...
- what should be stored in external symbolic state (data persisted outside the model)
- what should actually be loaded into the next bounded call (a single LLM invocation with a finite context window assembled for one task)

These are not the same decision. In the [bounded-context orchestration model](./bounded-context-orchestration-model.md), storage in the scheduler's accumulated symbolic state (`K`) is cheap; bounded context is expensive. The mistake is not that a trace was stored. The mistake is letting a session runtime decide that stored history should automatically become the next call's context instead of letting `select(K)` — the function that projects a subset of `K` into the next call's prompt — choose what the next call should see.
...
[The chat-history model trades context efficiency for implementation simplicity](./the-chat-history-model-trades-context-efficiency-for-implementation-simplicity.md). When the caller does not yet know what matters, raw history is the easiest way to preserve maximum information: no premature compression, no lost false starts, no need to design a return schema upfront, immediate usefulness for UI and debugging.
...
Under bounded context, that is usually the wrong trade:
...
This is the frame-boundary interface problem from the [scoping note](./llm-context-is-composed-without-scoping.md) in architectural form: when the boundary has no declared interface, the transcript becomes the implicit return value. In clean bounded orchestration, each call sees only the prompt assembled by `select(K)`, and the boundary is real. The discipline breaks when implementations package many bounded steps as one continuing session, letting history inheritance replace selective loading.

## The right split: storage vs next-context loading

The orchestrator can afford to keep many artifact kinds in external symbolic state — judgments, extracted claims, scrubbed conclusions, compressed episodes, structured data, symbolic code, failure signals, and raw traces for audit. What matters is that the next bounded call usually wants a much stricter projection.

A useful decomposition: "history" conflates at least three trace types with different loading profiles:
...
- **Reasoning traces** — chain-of-thought, deliberation, intermediate reasoning steps. Reveals the path to a conclusion, not the conclusion itself. Worth storing for debugging and alignment research — rarely worth loading into the next call. One important exception: planning artifacts (sub-goal decompositions, task plans) are reasoning-shaped but often *are* the deliverable the next call needs to execute.

The argument against loading traces as next-context is sharpest for pure deliberation traces, strong for conversation transcripts, and most nuanced for tool traces and planning artifacts. All three may belong in external state. But they are rarely the right default material for the next prompt.
...
- When the caller does judgment-heavy selection before dispatch, the callee need not inherit the caller's search trace
- [Spacebot](./related-systems/spacebot.md) (a branching agent architecture) branches return only a scrubbed conclusion
...
The execution boundary is the natural place to compress, but the deeper question is what `select` should load from stored state into the next bounded context. Compression at the boundary produces the artifact; `select` decides whether and how much to load.
...
- use **trace-preserving storage** (store the raw trace in full) early, when you do not yet know the right interface or what later learning may need
...
The default mistake is to let a chat interface or framework-owned tool loop decide what the next bounded call should inherit. Interactive sessions are designed for continuity and visibility. Orchestration requires selective loading.
