...
**What makes memory agent-usable.** Agent memory needs [discoverable, composable, and trusted knowledge under bounded context](./notes/agent-memory-needs-discoverable-composable-trusted-knowledge-under.md). Discoverable knowledge can be selected for a task without loading everything. Composable knowledge exposes relationships through [explicit link semantics](./notes/links-README.md), so readers can follow lines of support, contrast, or consequence. Trustworthy use depends on visible scope, lineage, validation, and evaluation of effects rather than on storage alone.
