...
This shifts the boundary in Naur's theory-building account rather than rejecting its central claim: [Naur binds program theory to humans only through the premise that machines follow formulable rules](./naur-binds-theory-to-humans-via-premise-that-machines-follow-rules.md), and a trained interpreter falls outside that partition. Naur's program boundary excludes the theory held by human developers; a software-plus-developers boundary already contains it. An LLM-based system can bring part of that theory inside its technical boundary: retained artifacts preserve addressable claims and the model interprets them. The retention is partial because retrieval and context must still activate the claims, and behavior must show that interpretation supports coherent modification.
...
- [Naur binds program theory to humans through the premise that machines only follow rules](./naur-binds-theory-to-humans-via-premise-that-machines-follow-rules.md) — grounds: why Naur's human-binding conclusion does not reach a trained interpreter, and the acquisition, retention, and reliability conditions his cases impose on the composite this note requires
