...
- Whether the three conditions can be satisfied without a human somewhere in the loop. The two closest systems do not settle it: the [Gödel machine](./goedel-machines-are-a-proof-governed-case-of-self-modification.md) is proof-gated and unimplemented, and [Commonplace](../reference/commonplace-as-a-reflective-system.md) runs but is human-inclusive, with only narrower functions computationally closed — [methodological and computational closure track different changes](./methodological-and-computational-closure-track-different-changes.md).
