---
description: "Three registers — theory, description, prescription — with distinct quality goals; formulation constraint and maintenance asymmetry make the split real, distillation connects them"
type: note
...
- Is "prescription" one register or two? Prescriptions (do X) and proscriptions (don't do X) might have different properties.
- Where do sources fit? Raw material for theory — a sub-register of description, or their own category?
- Is the three-register framing general (any reader with bounded attention) or LLM-specific? The derivation context is LLM, but many claims transfer. Scope should be mapped per-claim, not blanket-decided.
...
- [An agentic KB maximizes contextual competence](./an-agentic-kb-maximizes-contextual-competence-through-discoverable-composable-trusted-knowledge.md) — grounds: the three properties (discoverable, composable, trustworthy) apply differently per register; reach is the quality criterion for the theoretical register, economy and precision serve the other two
