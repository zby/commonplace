...
- autonomous steps between human interventions (intervention density's inverse);
...
- The load-bearing premise is the elastic backlog. Where the workload is genuinely fixed — a bounded migration, a system in maintenance-only mode — closing the loop *should* reduce human hours, and observing that there would confirm the mechanism rather than refute this note.
