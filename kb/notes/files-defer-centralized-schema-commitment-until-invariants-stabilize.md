...
Those distributed conventions are still not free to revise. Changing a frontmatter field across thousands of notes or repairing links after a path convention changes is still a data migration.

What they buy is the order in which commitments bind. Files can [add constraints only after repeated use exposes a stable pattern](./progressive-constraining-commits-only-after-patterns-stabilize.md): raw Markdown can acquire frontmatter, typed links, validators, and derived indexes one observed need at a time. Each addition should make an actual access pattern cheaper or a known invalid state impossible.

This is [constraining](./definitions/constraining.md) applied incrementally: postpone enforcement whose requirements are unknown, but do not postpone information that later enforcement will need. Stable identities and provenance needed later should be captured from the first record; write invariants already known should be enforced from the outset. A database can also defer normalization, for example through a document table with flexible metadata. The file choice is therefore warranted by the available authoring and inspection tools, not by a claim that databases are inherently rigid.

## The boundary is derived views versus authoritative state

Once a system needs capabilities beyond direct file access, the correctness question is whether the supporting structure is rebuildable or authoritative. Cost can still argue for a database on either side of that line. Read-side capabilities can remain derived when the canonical documents contain everything needed to reconstruct them. Semantic search, retrieval ranking, quality scores, and a traversable link graph can fit this pattern. A derived copy is safe only when it is [checked against its recomputable source or omitted](./a-derived-copy-of-recomputable-truth-must-be-checked-or-absent.md); otherwise the convenience layer quietly becomes a second, stale source of truth.
...
[Graphiti](https://github.com/getzep/graphiti), a graph-based agent memory system, is the temporal-validity case: its store maintains bitemporal relationship validity — when a fact held in the world and when the system recorded it — and updates that validity as facts change, so the graph cannot be discarded and rebuilt from source documents. Commonplace is the ownerless-edge case at smaller scale: documents remain Markdown, while churning review freshness on `(note, criterion)` edges lives in SQLite because [no single document owns that mutable relation](./many-to-many-edge-state-is-where-files-yield-to-a-database.md).
