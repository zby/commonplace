...
- **Currentness**, for mutable records — the authority must say what is operative *now*. Parametric state can meet this requirement only if its governed update cycle keeps pace with deployment. This is the gap identified by [deploy-time learning is the missing middle](./deploy-time-learning-is-the-missing-middle.md).
