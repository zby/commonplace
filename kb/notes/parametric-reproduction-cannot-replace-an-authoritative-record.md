...
- [ROME: Locating and Editing Factual Associations in GPT](../sources/rome-locating-and-editing-factual-associations-in-gpt.ingest.md) — evidenced-by: demonstrates a rank-one factual edit with specificity and counterfactual-generalization tests; applicability scope, rationale, lineage, and invalidation are target-side comparison properties
- [Exo](../agentic-systems/reviews/exo.md) — evidenced-by: a self-modifying agent harness that protects canonical state, lifecycle, and the record of prior attempts beneath a rewritable executor
...
- [Claude Workstream Kit and Fable agent scaffolding](../sources/claude-workstream-kit-fable-agent-scaffolding.ingest.md) — abstracted-from: an author's report that a Fable-oriented rebuild removed checklists and compliance machinery while retaining cited-evidence closure and human-authority constraints
