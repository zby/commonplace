...
The lesson does not preclude learning in natural-language and symbolic [representational forms](./definitions/representational-form.md). The criterion is indifferent to form and to production origin, [since the lesson selects production methods, not representational forms](./the-bitter-lesson-selects-production-methods-not-representational.md). A system that searches theory space, derives consequences, and tests them against cases that could refute them is running scalable search. A system that states such theories, acts on them, criticizes what they say, and retains the results is also a [theory builder](./definitions/theory-builder.md); whether it learns is a separate claim. Retained theories are not hand-supplied priors when search produced them, whatever form retains them. This answers the production-method objection without presuming the learning result that the [sample-efficiency conjecture](./retained-theories-may-improve-sample-efficiency.md) must test.
...
- [Retained theories may improve sample efficiency under structured shifts](./retained-theories-may-improve-sample-efficiency.md) — extends: the pathway this note clears of the objection, and the acceptance test it makes conditional
