...
- **Stronger base model, thin or absent artifact layer.** Removes the explanation that a newer model would have supplied the same content unprompted — the bitter-lesson-shaped alternative, sharpened by the observation that [scaling absorbs scaffolding at fixed difficulty rather than at the frontier](./scaling-absorbs-scaffolding-at-fixed-difficulty-not-at-the-frontier.md). This mandatory arm is also the most easily poisoned: where the artifact layer under test is public — this KB is — its content can enter a newer model's training corpus, biasing the arm in the layer's favor or against it depending on what was absorbed. A clean reading needs the confound handled in the design, not discovered after: tasks dated past the model's training cutoff, checks for reproduction of layer-specific content, or a private-corpus arm.
...
- [Ablation baselines for the declared objective](../reference/proposals/ablation-baselines-for-the-declared-objective.md) — see-also: the objective-level ablation design space (raw-history, episodic, and no-review arms) that complements these leverage baselines, and the origin of the contamination caveat on the stronger-model arm
