...
- **[Progressive refinement](./definitions/constraining.md)** — each level (text → note → structured-claim) adds structure that makes the content more extractable. A structured claim with Evidence/Reasoning/Caveats is more accessible to an agent than the same argument in unstructured text.
...
**[Recognition](./recognition-not-linking-is-the-hard-problem-in-knowledge-systems.md) cost is observer-relative** but not easily optimized — the data from which a connection could be inferred is present before anyone sees it, but extracting the pattern requires computation that grows with the depth of what must be recognized. [Naming](./minimum-viable-vocabulary-is-the-naming-set-that-most-reduces.md) is the partial solution: once a structure has a name, recognizing instances becomes cheap.
...
- [a linked note's durable payload is what its consumption path cannot reliably supply](./linked-note-durable-payload-is-what-consumption-path-cannot-supply.md) — extends: develops the resolution question for the framework-activation case, with the recognition rule and the behavioral boundary test
...
- [recognition, not linking, is the hard problem in knowledge systems](./recognition-not-linking-is-the-hard-problem-in-knowledge-systems.md) — evidenced-by: recognition cost varies with the observer's vocabulary and tools, and with the depth of what must be recognized
...
- [Structured-prompt gains do not establish training-distribution selection](./structured-prompt-gains-do-not-establish-distribution-selection.md) — contrasts: arrangement-sensitive extraction makes distribution selection plausible but does not identify it as the cause of a prompt gain
