...
- **Complementarity** — notes that are independent arguments for the same conclusion (like the [three output-quality arguments](./type-system.md)). Worth linking explicitly.
...
**Size limits follow from context limits.** The [split threshold of ~40 notes](../WRITING.md) isn't arbitrary — it's the approximate point where an area stops fitting in working context alongside the instructions and reasoning space the agent needs. An area that can't be read together can't serve either operation.
