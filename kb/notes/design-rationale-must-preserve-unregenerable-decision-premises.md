...
In his essay *Programming as Theory Building*, Peter Naur locates design theory in the programmer. The programmer can map program parts to world affairs, justify those parts, and judge whether a requested change is similar on the world side. A reflective knowledge base — one whose retained text is read when the system redesigns itself — splits these functions between an interpreter and retained text: in [the current LLM-plus-artifact realization](./a-complete-theory-path-does-not-establish-improved-capacity.md#the-current-llm-plus-artifact-realization), a model interprets the theory and an artifact preserves it across bounded calls. The split does not by itself say how the three functions divide between the two; the recovery test below decides that case by case. Naur's situated programmer already holds the theory; the essay does not establish how much a fresh interpreter can reconstruct from artifacts. The retention problem here is therefore an extension of Naur's account — [his human-binding conclusion rests on equating machine execution with formulated criteria](./naur-equates-machine-execution-with-formulated-criteria.md), and his cases tested one supplied artifact package rather than retained rationale as a route — which decision premises must the retained text supply when the interpreter lacks that situated history?
...
- [Naur binds program theory to humans by equating machine execution with formulated criteria](./naur-equates-machine-execution-with-formulated-criteria.md) — grounds: why Naur's argument leaves the interpreter/premise split open, and why his compiler case names the premises this note's test retains
