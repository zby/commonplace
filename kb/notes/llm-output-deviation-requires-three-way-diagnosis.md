...
[Ma et al.'s prompt stability study](../sources/prompt-stability-code-llms-emotion-personality-variations.ingest.md) generates prompt variants intended to preserve semantics and interfaces while changing emotion and personality framing. With a uniform temperature of 0.2, it draws 16 independent samples per prompt. Its reported stability result aggregates those samples into per-prompt elasticity and then averages across prompts. That comparison does not separately identify within-prompt sampling dispersion, so sampling variation and framing sensitivity remain mixed in the result. Conditional on the variants preserving public meaning as intended, `V` is fixed and the study does not test underspecification. Across 14 models, the reported Pass@1–AUC-E rank correlation is Spearman ρ = −0.433 (p = 0.122; 95% CI [−0.875, 0.249]). This nonsignificant correlation does not by itself establish that performance and stability are decoupled optimization objectives.

[Bertran, Fogliato, and Wu's agentic data-science multiverse experiment](../sources/many-ai-analysts-one-dataset-agentic-data-science-multiverse.ingest.md) illustrates all three questions in one workflow, with qualifications. It retains about 30 auditor-compliant runs per dataset × model × persona cell. Their within-cell dispersion is run-to-run variation under fixed cell settings and is consistent with execution indeterminism, but the paper does not isolate decoder sampling from tool or runtime nondeterminism or from the different analytical choices agents make. Materially different auditor-accepted analyses support a plural `V`, conditional on that auditor. Of 4,946 final runs, 67% passed compliance screening, so the excluded runs operationally illustrate mass outside the auditor-defined `V`; separately, pilot runs hallucinated results or recalled published findings and motivated the auditor, but they are not identified as members of the final rejected set. Persona comparisons are not clean tests of meaning-preserving framing sensitivity because the prompts change the stated prior or commission. Within a cell the dataset, model, persona, scaffold, and sampling policy are fixed; across the study the selected three datasets, four models, five personas, tools, and auditor bound what the workflow can expose.
...
- [LLM debugging separates specification gaps, instruction violations, and run-to-run variation](./llm-debugging-starts-with-retry-versus-rewrite-triage.md) — extends: applies the three diagnostic questions to choosing a debugging move and interpreting retries
...
- [Ma et al. — Prompt Stability in Code LLMs](../sources/prompt-stability-code-llms-emotion-personality-variations.ingest.md) — evidenced-by: measures sensitivity across intended meaning-preserving prompt variants under fixed decoding, while repeated samples estimate rather than isolate sampling spread
