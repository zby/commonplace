...
[Ma et al.'s prompt stability study](../sources/prompt-stability-code-llms-emotion-personality-variations.ingest.md) generates prompt variants intended to preserve semantics and interfaces while changing emotion and personality framing. With a uniform temperature of 0.2, it draws 16 independent samples per prompt and uses them to estimate stability across variants. It does not separately estimate within-prompt sampling dispersion, so it cannot by itself separate sampling variation from framing sensitivity. Conditional on the variants preserving public meaning as intended, `V` is fixed and the study does not test underspecification. Across 14 models, the reported Pass@1–AUC-E rank correlation is Spearman ρ = −0.433 (p = 0.122; 95% CI [−0.875, 0.249]). This nonsignificant correlation does not by itself establish that performance and stability are decoupled optimization objectives.
