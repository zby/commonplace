...
## The deterministic special case of a general rule

Every distilled artifact is a derived copy of its sources, and the KB already runs the general machinery for that: sources carry forward lineage so [a source change names the downstream artifacts to review](./distilled-artifacts-need-source-tracking-at-the-source.md); [make-like timestamp comparison](./link-graph-plus-timestamps-enables-make-like-staleness-detection.md) detects when review is due; and the fix is [re-distillation, not patching](./evolving-understanding-needs-re-distillation-not-composition.md). But in the general case verification itself is judgment: deciding whether a revised source invalidates a distilled instruction is a Level B check in the [text testing pyramid](./text-testing-framework.md) — an LLM reading both and judging fidelity, expensive to run and probabilistic in its findings. Judgment checks are too costly to run always and too noisy to block on, so the general rule tops out at *managed staleness*: lineage, triggers, advisory review. The hand-maintained derived artifact is not forbidden there — it is unavoidable.

What flips the regime here is the check price. When the derivation is mechanical, verification drops to Level A: deterministic, near-zero cost, binary outcome, runnable on every validation pass — detection and verification collapse into the same free step, where the general case needs a cheap detector and an expensive judge. At that price the economics that excused the unchecked copy disappear: trusting it saves nothing, so enforce-or-omit stops being aspirational and becomes the rule, and findings stop being advisory and block. Same structure throughout — derived copy, moving source, staleness risk — at a different check price; the price is what changes the balance.
...
- [Distilled artifacts need source tracking at the source](./distilled-artifacts-need-source-tracking-at-the-source.md) — extends: the general staleness rule for distilled knowledge that this note specializes; there, verification is judgment so the rule tops out at lineage plus review
