...
- **`complete`/`covered_by` marks on tag-READMEs** — enforced and shipped. A validator re-derives membership from the scoped `rg` sweep and fails on mismatch (the mark contract is in the [`tag-readme` type spec](../types/tag-readme.md)). The sweep pattern is itself a hand-authored heuristic rather than a strict mechanical extraction — precondition 4 is what keeps that acceptable instead of a quiet regress.
...
- [Artifacts produced from sources need lineage recorded at the source](./distilled-artifacts-need-source-tracking.md) — extends: the general managed-staleness rule this note specializes; there verification is judgment, lineage placement carries the interrupt, and the rule tops out at review
