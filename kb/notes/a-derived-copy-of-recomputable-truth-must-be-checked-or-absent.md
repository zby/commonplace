...
- **`complete`/`covered_by` marks on tag-READMEs** — enforced and shipped. A validator re-derives membership from the scoped `rg` sweep and fails on mismatch (the mark contract is in the [`tag-readme` type spec](../types/tag-readme.md)).
...
- [LLM recompute cost inverts the store-vs-recompute default](./llm-recompute-cost-inverts-the-store-vs-recompute-default.md) — complements: the value half — recompute being dear to a model is what makes a checked derived copy worth keeping in the first place
