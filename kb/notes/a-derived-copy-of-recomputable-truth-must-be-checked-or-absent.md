...
Every distilled artifact is a derived copy of its sources, and the KB already runs the general machinery for that: sources carry forward lineage so [a source change names the downstream artifacts to review](./distilled-artifacts-need-source-tracking.md); [make-like timestamp comparison](./link-graph-plus-timestamps-enables-make-like-staleness-detection.md) detects when review is due; and the fix is [re-distillation, not patching](./evolving-understanding-needs-re-distillation-not-composition.md). But in the general case verification itself is judgment: deciding whether a revised source invalidates a distilled instruction is a Level B check in the [text testing pyramid](./text-testing-framework.md) — an LLM reading both and judging fidelity, expensive to run and probabilistic in its findings. Judgment checks are too costly to run always and too noisy to block on, so the general rule tops out at *managed staleness*: lineage, triggers, advisory review. The hand-maintained derived artifact is not forbidden there — it is unavoidable.
...
- [Distilled artifacts need source tracking at the source](./distilled-artifacts-need-source-tracking.md) — extends: the general staleness rule for distilled knowledge that this note specializes; there, verification is judgment so the rule tops out at lineage plus review
