...
- **The `complete` mark on tag-READMEs** (the curated per-tag heads) — enforced and shipped. A validator re-derives membership across the participating collections and fails a mark whose head does not reach every member in one hop (the mark contract is in the [`tag-readme` type spec](../types/tag-readme.md)). The sweep pattern is hand-authored, but it executes as a strict mechanical extraction — and precondition 4's amortization argument is what keeps the pattern's authorship acceptable instead of a quiet regress.
...
- [Indexes lower recall when they suppress retrieval that would find more](./indexes-lower-recall-when-they-suppress-retrieval-that-would-find-more.md) — grounds: false completeness can suppress the recomputation that an absent copy would trigger
...
- [Source changes should surface downstream review targets, while reverse lineage can remain searchable](./artifacts-produced-from-sources-need-lineage-recorded-at-the-source.md) — extends: the general managed-staleness rule this note specializes; there verification is judgment, a forward lineage view carries the change-time interrupt, and the rule tops out at review
...
- [tag-readme type spec](../types/tag-readme.md) — evidenced-by: the `complete` mark is the shipped, validator-enforced instance of the rule; the mark contract lives here
...
- [ADR 027 — Package scaffold assets without source-tree symlinks](../reference/adr/027-package-scaffold-assets-without-source-tree-symlinks.md) — evidenced-by: source-of-truth for the byte-identical `AGENTS.md.template` duplication cited as the fourth instance
...
- [Local materialization should outperform distant natural-language declarations](./local-materialization-should-outperform-distant-declarations.md) — extends: conjectures that a mechanically derived local view may outperform declaration-only presentation while retaining this note's checked-or-absent boundary
