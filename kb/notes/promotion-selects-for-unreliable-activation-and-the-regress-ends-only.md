...
Rooting is necessary for an intended unprompted delivery path, but it is not sufficient for value. A stale index can fire perfectly and route to nothing. [Stale indexes reduce discovery when they suppress fallback search](./stale-indexes-reduce-discovery-when-they-suppress-fallback-search.md); here, satisfying the navigation cue suppresses a search that would have found the needed item.
