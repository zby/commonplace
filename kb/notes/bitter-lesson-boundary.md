...
Both are narrow. Both are human-engineered. The relevant difference isn't scope — it's whether the specification fully captures the problem.

**Chess** shows the boundary running through a single system. The rules of chess — legal moves, win conditions, board state — are fully specified; move generation is arithmetic. But chess *strategy* is a proxy-spec problem: grandmaster heuristics (control the center, develop pieces early, king safety) were theories about what good play looks like, not definitions of it. The bitter lesson ate the strategy — Deep Blue, IBM's chess-playing system, used massive search to beat programs that relied on selective, knowledge-heavy play, and AlphaZero, DeepMind's self-play system, later replaced hand-crafted evaluation with learned functions entirely. The rules survived. NP-hard optimization (vehicle routing, job-shop scheduling) follows a similar pattern: the objective and constraints are fully specified — you can verify any candidate solution — but finding good solutions is where learned methods have increasingly outperformed hand-crafted heuristics in recent benchmarks. Real systems are hybrids like this: arithmetic components composed by learned ones, with the boundary running through the architecture.

## In practice, the boundary is a working heuristic

The boundary between fully specified problems and proxy-spec problems is real, but identifying which side you're on is harder. Fully specified problems are the easy case: we know exactly what we want, and we've known since the start. But most practical solutions don't arrive with that clarity. Many foundational technologies — the transistor, the Fourier transform, public-key cryptography — were not obviously practical at inception. Practicality emerged from composition, not from any single step. You can't use "is this obviously practical?" as a reliable test.

The vision researchers faced exactly this. Each individual feature — edge detection, corner detection, scale-invariant keypoints — was genuinely useful in isolation. The failure was in composition: the pieces didn't add up to "seeing." But that failure was only visible in retrospect, after learned representations demonstrated a better path. **Composition failure is a strong tell** — when individually sound components don't compose into the larger capability, the specs are probably theories, not definitions.
...
For how to operate in the hybrid regime where you can't perfectly identify which side you're on, see [codification and relaxing navigate the bitter lesson boundary](./codification-and-relaxing-navigate-the-bitter-lesson-boundary.md).
...
- [codification and relaxing navigate the bitter lesson boundary](./codification-and-relaxing-navigate-the-bitter-lesson-boundary.md) — operationalizes: codify when the spec IS the problem; relax when scale reveals your spec was a theory
...
- [Fintool: Lessons from Financial Services](../sources/lessons-from-building-ai-agents-for-financial-services-2015174818497437834.ingest.md) — exemplifies the fully specified regime: fiscal period normalization (10,000+ company calendars) is a clean calculator-regime case
- [Fintool: Lessons from Financial Services](../sources/lessons-from-building-ai-agents-for-financial-services-2015174818497437834.ingest.md) — exemplifies relaxing: "the model will eat your scaffolding" is the relaxing prediction articulated as design principle
...
- [codify-versus-LLM decision heuristics](./codify-versus-llm-decision-heuristics.md) — synthesizes: walkthrough of codification heuristics derived from this boundary, oracle strength, underspecification, and pattern stability
