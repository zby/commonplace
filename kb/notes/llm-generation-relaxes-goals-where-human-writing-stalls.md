...
That search is **semi-decidable**: success is conclusive, failure is not. Finding the witness proves the goal reachable; failing to find it does not prove the goal unreachable — only that you did not reach it within the budget you spent. Maybe no point exists; maybe you lacked the trade-off, the abstraction, the insight. So composition does not sort ideas into meaningful and meaningless. It enforces a **burden of proof on an existential claim**: assert that a goal is reachable only once you hold the witness. (Fernando Borretti's "*ex falso* anything can be imagined" is the failure of those who skip the burden — the vague dream holds a thousand contradictory things at once precisely because nothing has yet forced it to exhibit one.)
...
LLM generation has no such stop. The model samples from a policy trained to make text look right. The useful way to model what comes out is as an **argmax over plausibility** — the most plausible-looking artifact that policy yields. (This is a reader's idealization of a trained tendency, not an operation the decoder runs, and the *conjunction of constraints* lives in the reading, not the model.) When generation lands on a full witness, that artifact *is* the witness; every conjunct is met. When it does not, it returns a **relaxation**: a plausible witness for a weaker problem with one conjunct dropped, emitted with no halt and no "unsatisfiable." The relaxation is just what plausibility-shaped generation yields whenever no full witness is in reach.

Because there is no discontinuity at the boundary, the dropped conjunct is demoted from hard to soft in silence. The output is not a beacon but **camouflage** that looks maximally satisfied, its surface pointing *away* from the one place it failed. It is a **counterfeit witness** — a real witness for the weaker problem, read as one for the stated problem, because nothing in the output marks whether the most-plausible artifact was total. The hidden delta is the dropped constraint.

## Why the relaxation lands on the crux

Two things make this worse than mere concealment.

**The relaxation is typicality-biased.** Where the novel constraint is also the least probable to render fluently — usually the case, since novelty is what makes it rare — it is the one most likely to be shed, leaving the typical, well-trodden constraints in place. And the novel constraint is usually the *point* of the idea. So the relaxation tends to sacrifice the load-bearing crux and preserve the boilerplate, gaining form without the idea gaining structure — the mechanism behind [reverse-compression](./reverse-compression-is-when-llm-output-expands-without-adding.md). This is [confidence tracks typicality, not soundness](./llm-generation-confidence-tracks-typicality-not-soundness.md) operating on constraints: what survives is what is typical, and the conjunct that made the idea worth having is the first thing typicality discards.
