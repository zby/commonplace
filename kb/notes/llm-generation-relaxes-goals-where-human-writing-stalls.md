...
The loss also shrinks wherever an external oracle re-imposes the burden downstream. Code often has hard checks for constraints encoded in types, compilation, or tests, so a relaxation that drops one of those constraints can fail after generation even when generation was frictionless. Constraints outside those surfaces — including untested semantic, security, or product requirements — can still disappear silently. Training against available code oracles may also make generation more consistent, a distinct effect those same checks enable. Discovery-heavy prose often lacks a cheap, comprehensive oracle for its central argumentative constraints. When composition was the only operation exposing such a gap, removing the stall leaves no equivalent check unless a separate validated review operation is added. The loss therefore bites hardest in the oracle-poor domain this KB works in — [the boundary of automation is the boundary of verification](./the-boundary-of-automation-is-the-boundary-of-verification.md).
