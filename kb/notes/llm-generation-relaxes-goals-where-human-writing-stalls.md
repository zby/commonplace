...
LLM generation has no such stop, because it is always running one operation: return the **argmax** — the most plausible artifact available. When a full witness exists, that argmax *is* the witness; every conjunct is met. When none exists, the argmax is simply a **relaxation**: a plausible witness for a weaker problem with one conjunct dropped, returned with no change in behavior — no halt, no "unsatisfiable." The relaxed witness is just what plausibility-maximization yields whenever no full witness is in reach.

Because there is no discontinuity at the boundary, the dropped conjunct is demoted from hard to soft in silence: the output is not a beacon but **camouflage**, optimized to look maximally satisfied so its surface points *away* from the one place it failed. It is a **counterfeit witness** — a real witness for the weaker problem, presented as one for the stated problem, because nothing in returning the argmax marks whether the argmax was total. The hidden delta is the dropped constraint.
...
- [The machine searches for the camouflage a human writer only slips into](./machine-searches-for-camouflage-human-writer-slips-into.md) — extends: the human-vs-machine asymmetry (the machine *searches* for the camouflage) and the training corollary that follows from it
