...
LLM generation has no such stop. The model samples from a policy trained to make text look right. The useful way to model what comes out is as an **argmax over plausibility** — the most plausible-looking artifact that policy yields. (The argmax is an analyst's idealization of a trained tendency, not an operation the decoder runs; the *conjunction of constraints*, likewise, is imposed by that analysis, not represented in the model.) When generation lands on a full witness, that artifact *is* the witness; every conjunct is met. When it does not, it returns a **relaxation**: a plausible witness for a weaker problem with one conjunct dropped, emitted with no halt and no "unsatisfiable." The relaxation is just what plausibility-shaped generation yields whenever no full witness is in reach.
