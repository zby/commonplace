...
The fourth resolves a question the parent note left open — whether it is a real alternative to a meta-criterion or a disguised instance of one. It is neither. It is an outside level, so it satisfies the claim, but it is not a ranking over objectives: it applies a standard to each objective's relation to the evidence, which is [explanatory-reach applied to criteria](./first-principles-reasoning-selects-for-explanatory-reach-over.md) rather than a preference among them.
