...
- [Effective context is task-relative and complexity-relative not a fixed model constant](./effective-context-is-task-relative-and-complexity-relative-not-a.md) - Synthesizes Paulsen MECW, ConvexBench, GSM-DC, and the web-agent long-context benchmark — usable context varies with task type, compositional complexity, and irrelevant context load, so nominal window size is a misleading abstraction
