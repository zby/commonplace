...
That is the route formal reach-assessment can use: a proposed commitment is accepted not because it predicts the training cases, but because the mechanism it asserts supports the intended intervention, counterfactual, or shift claims. [Invariant prediction](../sources/causal-inference-using-invariant-prediction.ingest.md) tests candidate predictor sets for an invariant conditional relation across observed environments, then intersects the sets it does not reject. Under the paper's assumptions and valid setwise tests, this yields conservative confidence control; it may also return an empty or only partially identifying set. [Causal-learn](../sources/causal-learn-causal-discovery-in-python.ingest.md), a Python library for causal discovery, collects algorithms that infer causal structure from observational data under method-specific assumptions. [DoWhy](../sources/dowhy-expressing-and-validating-causal-assumptions.ingest.md), a causal-inference library, makes the same boundary operational by requiring declared assumptions before identification, estimation, and partial validation or refutation tests.
...
- [The Risks of Invariant Risk Minimization](../sources/the-risks-of-invariant-risk-minimization.ingest.md) — evidenced-by: the environment-count threshold, and a predictor that discharges the invariance obligation while recovering the wrong commitment
