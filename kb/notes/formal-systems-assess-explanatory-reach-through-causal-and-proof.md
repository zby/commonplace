...
The invariance case has been worked out formally, and it shows the failure is not a matter of carelessness. [Rosenfeld, Ravikumar, and Risteski](../sources/the-risks-of-invariant-risk-minimization.ingest.md) analyse the invariant risk minimization objective inside a Gaussian structural equation model and construct a predictor that is near-optimal under the penalized objective and near-identical to the invariant predictor on training data, yet behaves like plain empirical risk minimization once the test environment drifts far enough. The obligation is discharged, training behaviour is indistinguishable from the intended solution, and the commitment recovered is still the wrong one. The mechanism generalizes past IRM: the penalty that construction pays scales with the mass of the region where it misbehaves, and that region is rare in training. An acceptance criterion evaluated on the distribution that produced the candidate can price bad off-distribution behaviour at nearly nothing.

The same analysis states when the route does pay. In the linear regime it establishes a threshold in the number of training environments `E` against the dimension `d_e` of the environment-varying features: above it, any feasible linear featurizer must discard those features and the invariant predictor is recovered; at or below it, a predictor using only environmental features is feasible and attains lower risk, so the objective does not prefer the invariant solution. The causal route is therefore worth taking where genuine environments are plentiful relative to the spurious-feature dimension and the model class is restricted enough for feasibility to bite — and where those conditions fail, the route returns an answer that carries no more warrant than fitting would have.

Two boundaries keep this from proving more than it does. The result bounds the IRM *objective* and close relatives, not invariance-based causal inference generally, so it does not touch [invariant prediction](../sources/causal-inference-using-invariant-prediction.ingest.md) as used above: Peters and colleagues analyse a hypothesis-testing procedure with confidence bounds, Rosenfeld and colleagues an optimization objective over a rich hypothesis class, and the latter's failure is what a rich class buys. The non-linear construction is also an existence proof about what the objective permits, not evidence that gradient training finds such a solution — enough to show the obligation underdetermines the commitment, not enough to say practitioners land there.
...
- [Theory-mediated learning may improve sample efficiency under structured shifts](./theory-mediated-learning-may-improve-sample-efficiency-under-shifts.md) — extends: puts the formal routes developed here alongside the prose route, as two ways of discharging the same assessment
