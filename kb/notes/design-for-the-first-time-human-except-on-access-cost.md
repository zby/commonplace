...
- [LLM contexts interpret instructions and content through the same token medium](./llm-context-interprets-instructions-and-content-through-one-medium.md) — grounds: the closing text-as-instruction exception rests on there being no program/data boundary in the window, so any loaded text can act as instruction
