---
description: "Tests whether redesign of a named organizational aspect is a one-off builder intervention or part of a retained, operative, and revisable improvement path"
type: ./types/structured-claim.md
traits: [title-as-claim, has-comparison, has-external-sources]
tags: [foundations, self-improving-systems]
---

# The builder loop becomes internal through an operative redesign path

Every improvement loop depends on a builder loop that chooses its roles, interfaces, objectives, evaluators, update rules, and editable surfaces. Including the builders inside a declared system boundary shows that the larger system *can* change this organization. It does not show that organizational redesign has become part of the system's improvement machinery. For a named class of redesign, the builder loop becomes internal only where evidence can reach a represented redesign path, an authorized decision can install the result, later operation depends on it, and the path remains available for later challenge.
...
Consider a harness optimizer allowed to rewrite a system prompt and three tool descriptions. A fixed test suite decides which candidate is promoted. The optimizer can repair a bad prompt because the prompt lies inside its update space. If the test suite rewards the wrong behavior, the optimizer cannot repair the evaluator that defines success. Researchers may add adversarial cases, change the promotion rule, or expose another component, but that intervention belongs to the external builder loop unless it enters machinery the assessed system retains and later uses.

The distinction appears across five reported systems, but a hostile reading rules out sorting whole systems into "internal" and "external" builder-loop bins. [Self-Harness](../sources/self-harness-harnesses-that-improve-themselves.ingest.md) can propose subagent, skill, and middleware structure; its reported subagent and skill branches were rejected, while a narrower tool-error middleware change was retained. Its objective, failure-signature representation, edit surface, and two-split gate remain supplied.

[Continual Harness](../sources/continual-harness-online-adaptation-foundation-agents.ingest.md) is a stronger counterexample. Its Refiner creates, edits, and deletes sub-agent definitions. The same paper's mixed human–agent GPP record reports a structural rewrite in which per-decision logic was absorbed into a master agent that dispatched to named sub-checks, followed by continued updates and later use of inherited sub-agents. This is demonstrated builder-loop reach over sub-agent organization. It does not establish reach over the four-part harness partition, Refiner rule, observation and action interfaces, or reward design.
...
Internalization also establishes less than benefit. A validator can reliably apply a bad criterion; a retained ADR can install an expensive role; a proof can be valid under a poor formalization. Following an authorized path, making a good change, and being safe enough to try the change are different claims. The path supplies addressability and control. Outcome evidence supplies benefit, and evidence from later episodes is needed to show leverage or compounding.
...
- An ordinary research organization can satisfy the criterion. If its issue, design, review, CI, merge, and deployment path governs later operation and remains revisable through that path, it is another internal builder loop. Commonplace is one implementation, not a technologically unique category.
- Internalization does not imply computational autonomy. Humans may perform diagnosis, judgment, or acceptance inside the declared path; actor allocation is a separate property.
...
- [An omitted improvement-loop function and a frozen one need different repairs](./an-omitted-loop-function-and-a-frozen-one-need-different-repairs.md) — extends: classifies the fixed placements exposed once the builder boundary is located
...
- [Reflective leverage is tested in the next episode](./reflective-leverage-is-tested-in-the-next-episode.md) — extends: supplies the later-episode test for whether internalized machinery improves subsequent revision
