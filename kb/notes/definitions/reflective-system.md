...
- [A proposal-selection improvement loop requires search, evaluation, and operative retention](../a-proposal-selection-loop-requires-search-evaluation-and-retention.md) — extends: reflection supplies one causal path into a proposal-selection loop, but not the search, evaluation, or operative retention that subtype needs
...
- [A complete theory path does not establish improved capacity](../a-complete-theory-path-does-not-establish-improved-capacity.md#the-functions-share-one-path-not-one-substrate) — extends: what a claim of observed recurrence needs beyond a two-way connected path: causal connection between theory use, criticism, its result, and later use
