...
Each mutation would be speculative — staged separately, surfaced for human review only when it scores high enough. This is the automated version of what [constraining as learning](./definitions/constraining.md) describes as the manual constrain/relax cycle — the same system-level adaptation, but with the agent proposing mutations instead of a human driving each one.
...
**Oracle difficulty varies by learning type.** [Pi Self-Learning](../agent-memory-systems/reviews/pi-self-learning.md) sidesteps the oracle problem entirely by picking the easiest available oracle: mistakes have a natural verifier (the fix). Extract what went wrong, score by recurrence, inject the top patterns — no quality judgment needed beyond "did this keep happening?" The KB's harder learning problems — "should these notes be linked?", "is this synthesis correct?", "does this index need a new entry?" — lack that natural verifier. The [quality signals brainstorm](./quality-signals-for-kb-evaluation.md) proposes manufacturing a soft oracle from many weak structural signals, but that's still speculative. Pi-self-learning's success at the easy end of the oracle spectrum clarifies where the hard end actually is: not in the learning mechanism (scoring, ranking, injection are all straightforward) but in manufacturing evaluation for judgment-heavy mutations.

**A bounded positive case for automated curation.** [Knowledge-Centric Self-Improvement](../sources/knowledge-centric-self-improvement-2607.19592.ingest.md) runs a fully automated version of several cauldron mutations — extraction into evidence-grounded claims, cross-claim criticism, synthesis and retirement through distillation — as a three-stage protocol: a task-level forum converts attempts into local claims, a cross-task forum tests which claims recur beyond their originating task, and distillation retains typed, scoped guidance (transferable insights, confirmed constraints, rejected hypotheses, pitfalls, checks, next steps) that later agents consume. Its frozen distilled artifact improves held-out task performance across tasks and LLM families. This sits at the easy end of the oracle spectrum the previous paragraph marks out, pushed further than Pi Self-Learning: claims are grounded in benchmark pass/fail evidence, claim survival is adjudicated by LLM forum debate, and the artifact's end value is measured mechanically as held-out solve rate. What it does not touch is this note's hard end — curation for open-ended question-answering, where no solve-rate oracle exists and value is defined by evolving questions. The protocol shows the loop's mechanism is buildable where the oracle is given; the open problem remains manufacturing evaluation where it is not.
...
The [fixed-artifact distinction](../notes/fixed-artifacts-split-into-exact-specs-and-proxy-theories.md) separates exact-spec artifacts from proxy-theory artifacts. The KB's infrastructure — file formats, frontmatter schema, sync scripts — is exact-spec-like. The knowledge organisation — which links exist, how notes are grouped, what gets extracted — is proxy-theory-like. A learning loop would be the mechanism for continuously improving the proxy-theory layer. We're not ready to build it, but the distinction tells us where it would operate.
...
- [Pi Self-Learning](../agent-memory-systems/reviews/pi-self-learning.md) — exemplifies: succeeds at automated learning by picking the easiest oracle (mistake recurrence); clarifies that our harder learning problems stall not on mechanism but on oracle construction
...
- [Knowledge-Centric Self-Improvement ingest](../sources/knowledge-centric-self-improvement-2607.19592.ingest.md) — evidence: a benchmarked, fully automated curation loop (task forum → cross-task forum → distillation) with held-out transfer gains; the strongest bounded positive case, leaving open-ended KB judgment untouched
