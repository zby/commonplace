...
- [Decorrelated reviewers still share the field's prior, so declare which claims you will defend](./reviewers-share-the-fields-prior-so-declare-which-claims-to-defend.md) — extends: the component of a claim on which decorrelating the proposer leaves the checks correlated — the field's prior — and what to do about it
