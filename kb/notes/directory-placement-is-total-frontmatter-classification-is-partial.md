...
- [Artifact classification separates content kind, lineage, and authority](./artifact-classification-separates-content-kind-lineage-and-authority.md) — extends: collection placement selects a local writing contract without deciding the artifact's region- or path-level classifications
- [a universal knowledge framework demotes content taxonomies to defaults](./a-universal-knowledge-framework-demotes-content-taxonomies-to-defaults.md) — grounds: what is framework-level is what is claimed from the consumer or substrate rather than abstracted from seen content, which licenses treating file-substrate properties as framework-level claims
- [069-Collection contract bundles become one-time prototypes](../reference/adr/069-collection-contract-bundles-become-one-time-prototypes.md) — abstracted-from: the current Commonplace application of placement selecting one complete local contract, not independent evidence for this substrate claim
