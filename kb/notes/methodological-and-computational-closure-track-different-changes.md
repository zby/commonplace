...
- [Each residue class needs a different mechanism, so a self-improving architecture must be mixed](./residue-classes-need-different-mechanisms-so-architecture-is-mixed.md) — extends: maps the representation–settlement–warranted-execution conversion onto the architectural parts that supply each stage
