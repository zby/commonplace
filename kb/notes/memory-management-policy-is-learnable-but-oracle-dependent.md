...
AgeMem's STM operations (Retrieve, Summary, Filter) are context-shaping — extracting and compressing from a larger body of information into the working context, shaped by the current task. The LTM curation operations (Update, Delete) refine and retire stored facts.
...
Commonplace stores both in the same medium — files in a repo. A fact ("AgeMem uses GRPO") and a policy ("always search before writing") are both markdown artifacts. Policies are themselves knowledge: searchable, linkable, refinable, composable with other notes. You can [constrain](./definitions/constraining.md) a policy (move it from a convention to a script), extract from it (work a skill out of methodology notes), or [discover](./discovery-is-seeing-the-particular-as-an-instance-of-the-general.md) that two policies are instances of the same principle. Policies participate in the same [accumulation](./learning-is-not-only-about-generality.md) loop as everything else.
...
1. **Memory management decomposes into retention judgment and context-shaping.** LTM operations (what to remember) are relevance judgments about the store. STM operations (what to load into working context) are use-shaping for the current task. The two require different skills, which is why AgeMem's curriculum trains them separately.
...
- [Theory and methodology form a two-layer execution system](./theory-and-methodology-form-a-two-layer-execution-system.md) — applies: AgeMem's STM operations produce use-shaped working context from a retained store — the fast-path-over-retained-source arrangement at session scale
