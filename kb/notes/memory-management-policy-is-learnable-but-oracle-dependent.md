...
To close the learning loop, AgeMem accepts a split between knowledge substrate and system-definition artifact: facts go into a memory store (somewhat inspectable key-value pairs), but the policy for managing them goes into model weights (opaque, non-diffable, only changeable through retraining).
...
- [Axes of substrate analysis](./axes-of-substrate-analysis.md) — sharpens: AgeMem's policy is a clean opaque-substrate case, and the "split" AgeMem makes is across the role axis — facts (knowledge substrate) in memory, policy (system-definition artifact) in weights
