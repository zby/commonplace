...
The same machine also shows why the subtype boundary is a reading, not a component list. Nothing in the Homeostat separately represents a candidate, and nothing in it *is* an evaluator — the viability criterion is what the physical dynamics respond to. Described one way it is a minimal proposal-selection loop; described the other it is direct viability-driven adaptation, the non-loop way a [self-improving system](./definitions/self-improving-system.md) can be responsive to evidence. Both descriptions are faithful, because the improvement criterion a self-improving system needs is semantic, and the explicit evaluator is required only by the architecture this note describes. Systems above the floor come apart cleanly: a Gödel machine has an evaluator you can point to; a gradient learner demonstrably has none, since no update is ever rejected.
