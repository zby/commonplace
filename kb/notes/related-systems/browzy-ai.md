...
- [Files beat a database for agent-operated knowledge bases](../files-not-database.md) — complicates: browzy is a good example of readable files as source artifacts with SQLite as a justified derived index
- [Agents navigate by deciding what to read next](../agents-navigate-by-deciding-what-to-read-next.md) — exemplifies: browzy's ranking and section extraction are a runtime implementation of the "what should I read next?" problem
- [A functioning knowledge base needs a workshop layer, not just a library](../a-functioning-kb-needs-a-workshop-layer-not-just-a-library.md) — sharpens: browzy's digests and crystallized drafts behave more like workshop artifacts than mature library notes
- [Deploy-time learning is the missing middle](../deploy-time-learning-is-the-missing-middle.md) — frames: browzy's session digests are the clearest deploy-time artifact-learning case here, while crystallized drafts are a weaker pre-promotion variant
