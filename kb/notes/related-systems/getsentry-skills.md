...
- [deploy-time learning: the missing middle](../deploy-time-learning-is-the-missing-middle.md) — the iteration path (labeled examples → holdout evaluation → behavior deltas → re-authoring) is deploy-time learning with enough structure to be reproducible
