...
- [files-not-database](../files-not-database.md) — contrast lens: Playground gets many of the inspectability benefits we want from files, but it reaches them through a typed append-only graph rather than a human-readable repository
- [agents-navigate-by-deciding-what-to-read-next](../agents-navigate-by-deciding-what-to-read-next.md) — contrast lens: Playground preassembles memory and moment into one prompt, while commonplace makes the reader decide what to load next
- [oracle-strength-spectrum](../oracle-strength-spectrum.md) — analogy: Playground's trace substrate is strong, but its memory promotion still looks more like weak-oracle curation than hard-verified learning
