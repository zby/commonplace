...
- [files beat a database for agent-operated knowledge bases](../files-not-database.md) — convergence: Archie keeps deployment logic, planning packets, and agent guidance as inspectable repo files instead of hidden runtime state
- [a functioning knowledge base needs a workshop layer, not just a library](../a-functioning-kb-needs-a-workshop-layer-not-just-a-library.md) — exemplifies: `docs/work-items/` and `docs/agents/` are a repo-local workshop layer for staged execution
- [AGENTS.md should be organized as a control plane](../agents-md-should-be-organized-as-a-control-plane.md) — contrasts: Archie uses `AGENTS.md` as a broad implementation handbook, not a minimal routing control plane
- [inspectable substrate, not supervision, defeats the blackbox problem](../inspectable-substrate-not-supervision-defeats-the-blackbox-problem.md) — exemplifies: deployment scripts, plans, and agent-facing docs all stay in a substrate that another agent can inspect and revise
