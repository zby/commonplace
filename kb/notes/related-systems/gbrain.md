...
- [files beat a database for agent-operated knowledge bases](../files-not-database.md) — contrasts: GBrain is the boundary case where a large personal corpus plausibly earns a Postgres/pgvector derived index, while still trying to keep markdown inspectable
- [pointer design tradeoffs in progressive disclosure](../pointer-design-tradeoffs-in-progressive-disclosure.md) — extends: GBrain's chunks, search scores, stale flags, and page summaries are query-time and page-local pointers rather than authored relationship phrases
...
- [Claw learning loops must improve action capacity not just retrieval](../claw-learning-loops-must-improve-action-capacity-not-just-retrieval.md) — exemplifies: GBrain's skillpack targets meeting prep, entity enrichment, communication, and briefing, not only question answering
- [Distillation](../definitions/distillation.md) — sharpens: compiled truth is directed compression of timelines and raw traces into current assessments
