...
This is [progressive compilation applied to methodology](./underspecification-and-indeterminism-complicate-programming-for-prompts-in-distinct-ways.md) — new best practices should start as underspecified natural-language guidance and constrain toward precise, deterministic enforcement as they prove out:
...
- [Harness Engineering (Lopopolo, 2026)](../sources/harness-engineering-leveraging-codex-agent-first-world.ingest.md) — exemplifies: three runtime pillars (instructions → structural tests → automated cleanup agents) map to the constraining gradient; "every mistake is a harness bug" is the maturation trajectory in practitioner language
