...
A claim's content is what it rules out. Popper's account ([*Logic of Scientific Discovery*, §36](../sources/popper-logic-of-scientific-discovery.ingest.md)) gives two ways to raise it: assert the claim over a wider class of cases, or assert something sharper about each case. Both raise what the claim forbids. Popper stops there: when one statement is more universal and another more precise, he treats the two as not comparable. The trade this note describes, where universality rises as precision falls and content stays roughly flat, is this note's extension of his account, not his claim. "All orbits are closed curves" is more universal than "planetary orbits are closed curves", and less precise than "all orbits are ellipses".
...
Static analysis has a formal version of this trade. In abstract interpretation, a widening operator makes an iterative analysis terminate by jumping to coarser values; the result stays sound but loses precision, so a guarantee is bought with precision, as here. [Amato et al.](../sources/efficiently-intertwining-widening-narrowing.ingest.md) report two findings that bear on this repair. Precision lost early and propagated through dependent equations generally cannot be recovered by a later refining pass. And applying widening only at loop heads, then removing a head from that set once its value stabilized, raised precision at many program points. Both support stating the general form once, where the argument needs it, and correcting it before other claims build on it. The parallel is structural: the paper's widening carries a soundness guarantee that abstract vocabulary lacks, and its evidence covers one analyzer and one domain. The paper's *narrowing* also runs opposite to the [companion note's](./narrowing-bought-to-survive-review-is-paid-for-in-content.md): it recovers precision and cannot go below a fresh recomputation, while subject narrowing drains content.
...
- [Efficiently intertwining widening and narrowing](../sources/efficiently-intertwining-widening-narrowing.ingest.md) — evidenced-by: formal widening spends precision to buy termination; propagated loss is generally unrecoverable, and localized, removable widening points restored precision
