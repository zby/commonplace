...
Real LLMs produce outputs outside the space of valid interpretations. The spec rules them out, but the LLM fails to comply. This is not [underspecification](./agentic-systems-interpret-underspecified-instructions.md) (where multiple outputs are valid). And it is not [indeterminism](./execution-indeterminism-is-a-property-of-the-sampling-process.md), even when the wrong output shows up on some runs and not others — sampling decides only which draw from the output distribution you see, not whether invalid outputs were in the distribution to be drawn. **Interpreter failure** is a property of the distribution itself: it puts mass outside the valid set. The mark is bias, not noise — it survives resampling and averaging, where noise cancels. For a theoretical deterministic LLM it is simply the wrong output; it is the gap between what a conforming interpreter of the spec's public meaning would do and what a real LLM does.
