...
- [subtasks that need different tools force loop exposure in agent frameworks](./subtasks-that-need-different-tools-force-loop-exposure-in-agent.md) — related consequence: some fresh calls are needed because child tasks need different tool surfaces
...
- [cross-task transition policy remains scheduling behind a tool interface](./cross-task-transition-policy-remains-scheduling-behind-tools.md) — parallel classification: codified cross-task transition authority is scheduling wherever it is placed, while structural overflow separately forces decomposition
