...
- [Programming-tool progress is a partial order on accepted outcomes per human effort, not a scalar](./programming-tool-progress-is-a-partial-order-not-a-scalar.md) — extends: applies the fixed-frame, no-percentage discipline at the human–tool boundary rather than the pathway's allocation profile
