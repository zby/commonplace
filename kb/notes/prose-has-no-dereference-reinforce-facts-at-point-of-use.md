...
- [LLM context is composed without scoping](./llm-context-is-composed-without-scoping.md) — contrasts: the over-reach dual of this note. No scoping lets content leak where it shouldn't apply (spooky action at a distance); no dereference lets a declared fact fail to reach where it should — the same diffuse attention field (not binding plus scope) read from opposite ends
