---
description: Human writing genres evolved to prevent reasoning failures, and the same structures help LLMs because LLMs share human-like failure modes (content effects on reasoning) — transfer evaluated per convention, not wholesale analogy
...
- [The machine searches for the camouflage a human writer usually slips into](./machine-searches-for-camouflage-human-writer-slips-into.md) — contrasts: shared failure modes ground convention transfer here, but that note marks the boundary — a shared failure class can still differ in default process and observability
