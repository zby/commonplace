...
- [Each residue class needs a different mechanism, so a self-improving architecture must be mixed](./residue-classes-need-different-mechanisms-so-architecture-is-mixed.md) — contrasts: motivates the scheduler/model split from warrant classes instead of from context scarcity
