...
On the outcome side the same refusal of hours takes the form of a partial order. Fix the task class and the acceptance threshold. A tool change is forward when it produces no worse accepted outcomes with no more *total* human programming effort and strictly improves one of the two, or grows the accepted task set under a fixed effort budget. Total effort counts configuration, review, recovery, and repair; leave a stage out and displaced work looks like saved work. The order is partial: a change that trades outcome quality or warrant against effort is incomparable until an objective is declared, and no percentage or hours-saved scalar follows from it. Bounded mechanisms count under this order, and forward steps do not compose into convergence, [since warranted transfer leaves people the hardest-to-warrant decisions](./warranted-transfer-leaves-people-the-hardest-to-warrant-decisions.md).
...
- [Figma agent security review thread](https://x.com/frgx/status/2081739292859421118) — evidenced-by: a practitioner case where codified security policy and repeated evaluation let a fixed amount of human judgment govern much broader review, audit, and remediation work
