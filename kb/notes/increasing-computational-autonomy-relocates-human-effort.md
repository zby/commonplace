...
- [Removing a human judgment can degrade the judgments that remain](./removing-a-human-judgment-can-degrade-the-judgments-that-remain.md) — contrasts: the residue's judgment quality can fall with the workload held fixed, which the elastic-backlog mechanism does not predict
- [Programming-tool progress is a partial order on accepted outcomes per human effort, not a scalar](./programming-tool-progress-is-a-partial-order-not-a-scalar.md) — contrasts: the outcome-per-effort side of the same refusal of hours, holding task class fixed instead of the function list
