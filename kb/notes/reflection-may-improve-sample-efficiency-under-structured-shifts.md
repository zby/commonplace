...
Route an improvement pathway through the system's own causally connected self-representation — the move that makes it reflective — and what the system retains is no longer just machinery it runs on: [since reflection buys addressability](./reflection-buys-addressability.md), it can read a retained lesson, say what that lesson claims and where it applies, and carry it to a problem other than the one that produced it. A lesson carried that way is structure the new problem does not have to supply. So when the task shifts, adaptation may take fewer new target observations — a **sample-efficiency** advantage, and one a built system could be measured for and fail to show.
