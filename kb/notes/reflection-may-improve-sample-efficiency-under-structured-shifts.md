...
A reflective improvement pathway is one where changes route through the system's own causally connected self-representation. Because [reflection buys addressability](./reflection-buys-addressability.md), one advantage expected of such a pathway is reuse and transfer: a retained lesson keeps paying off on later tasks. This note narrows that advantage to **sample efficiency** — fewer new target observations needed to adapt after a shift — and treats it as a hypothesis to be tested against built systems.
