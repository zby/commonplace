...
A reflective improvement pathway is one where changes route through the system's own causally connected self-representation. What that routing adds is addressability — [since reflection buys addressability](./reflection-buys-addressability.md), the system can read a retained lesson, say what it claims and where it applies, and carry it to problems other than the one that produced it. That carrying step — reuse and transfer — is the expected advantage this note takes up. The note narrows it to **sample efficiency** — fewer new target observations needed to adapt after a shift — and treats it as a hypothesis to be tested against built systems.
