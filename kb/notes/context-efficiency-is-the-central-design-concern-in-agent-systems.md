...
- **Feasibility — the per-window face.** Within a single inference call the model's competence degrades as the window fills, by [soft degradation rather than a hard token limit](./soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md). This is a *capability* ceiling: past it the task becomes impossible or degrades past usefulness, at any price. It is the lowest-degree-of-freedom face — the attention budget is unitary within a call, cannot be tiered at the attention level, and cannot be enlarged without architectural change. This face also carries to the decomposed case: [under sub-agent decomposition, feasibility is the heaviest fork's net load](./feasibility-is-the-heaviest-forks-net-load.md).
...
If context is the fundamental scarce resource, one applicable representation is [bounded-context orchestration](./bounded-context-orchestration-model.md): when its conditions hold, explicit symbolic state can be kept outside bounded LLM calls. Context scarcity motivates that arrangement but does not entail it or establish that it is best. Whatever the architecture, context efficiency should be evaluated at design time, not retrofitted — where sub-agent boundaries go, what loads when, and what gets frontloaded determine it structurally.
...
- [soft degradation often binds before the hard cap when required evidence fits](./soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md) — grounds: establishes the often-binding soft-bound premise behind the feasibility face
...
- [bounded-context orchestration model](./bounded-context-orchestration-model.md) — extends: a conditional representation for separating explicit symbolic state from bounded calls under context-scarcity pressure
- [Minimum Viable Ontology / Domain Maps](https://x.com/melodyskim/status/2029332670115614799) — exemplifies: MVO is compression under context-efficiency pressure — compress domain knowledge into the smallest vocabulary that fits the context window
...
- [Harness Engineering as Cybernetics (@odysseus0z, 2026)](https://x.com/odysseus0z/status/2030416758138634583) — evidenced-by: frames context-efficient agent runtime design as feedback-loop calibration from control theory
...
- [Components of A Coding Agent (Raschka, 2026)](https://magazine.sebastianraschka.com/p/components-of-a-coding-agent) — evidenced-by: independent practitioner convergence — "apparent model quality is really context quality"
