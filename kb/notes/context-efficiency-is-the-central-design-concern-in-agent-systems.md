...
- Liu et al. (2026). [ConvexBench: Can LLMs recognize convex functions?](https://arxiv.org/html/2602.01075v2) — empirical evidence that compositional depth, not token count, drives reasoning degradation.
- Paulsen (2025). [Context Is What You Need — The Maximum Effective Context Window](https://arxiv.org/pdf/2509.21361) — convergent evidence: MECW << MCW across 11 models, but tasks confound volume with LLM-hard exact enumeration, making this a volume × task-difficulty finding rather than pure volume degradation.
- Lopopolo (2026). [Harness engineering: leveraging Codex in an agent-first world](https://openai.com/index/harness-engineering/) — independent practitioner convergence on context-as-scarce-resource from 1M LOC agent-generated codebase.
- Koylan (2026). [Koylanai Personal Brain OS](https://x.com/koylanai/status/2025286163641118915) — 40% token reduction from module isolation demonstrates volume-dimension context efficiency.
- Raschka (2026). [Components of A Coding Agent](https://magazine.sebastianraschka.com/p/components-of-a-coding-agent) — independent practitioner convergence: "a lot of apparent 'model quality' is really context quality," with a six-component decomposition of what context quality means operationally.
...
- [Harness Engineering as Cybernetics (@odysseus0z, 2026)](https://x.com/odysseus0z/status/2030416758138634583) — grounds: frames context-efficient agent runtime design as feedback-loop calibration from control theory — the shift from direct code production to sensor-actuator design is the same shift this note identifies as moving from capability to context structure
- [The Anatomy of an Agent Harness (Vtrivedy10, 2026)](https://x.com/Vtrivedy10/status/2031408954517971368) — exemplifies: derives runtime components (filesystem, sandbox, context management, skills) by working backwards from model limitations — instantiates the architectural responses this note describes abstractly with concrete primitives (compaction, tool-call offloading, progressive tool loading)
- [Components of A Coding Agent (Raschka, 2026)](https://magazine.sebastianraschka.com/p/components-of-a-coding-agent) — grounds: independent practitioner convergence — "apparent model quality is really context quality" — with a six-component decomposition of what context quality means operationally
- [Epiplexity (Bates et al., 2026)](https://arxiv.org/html/2601.03220v1) — grounds: formalizes the complexity dimension of context cost — epiplexity quantifies structural accessibility under computational bounds, giving theoretical backing to "how hard the tokens are to use"
- [AgeMem (Yu et al., 2025)](https://arxiv.org/html/2601.01885v1) — exemplifies: RL-trained STM operations (Retrieve/Summary/Filter) achieve 3-5% token reduction while maintaining performance — empirical evidence that learned context management can outperform heuristic approaches
- [Skill Graphs 2.0 (Sakhuja, 2026)](https://x.com/shivsakhuja/status/2047124337191444844) — extends: the depth-degradation observation is a complexity-dimension context-efficiency failure (deep skill chains break even at low token count); the "brain RAM" leverage argument extends context scarcity from agent window to operator working memory as a second finite-attention resource
