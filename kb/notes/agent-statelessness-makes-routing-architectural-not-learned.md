...
- [skills derive from methodology](./skills-derive-from-methodology.md) — instance: the skill-specific case of routing-as-architecture; skill derivation is the compilation pattern applied to methodology→skill
- [claw learning loops must improve action capacity not just retrieval](./claw-learning-loops-must-improve-action-capacity-not-just-retrieval.md) — extends: the action-oriented knowledge types (preferences, procedures, precedents) also need routing-as-architecture treatment
