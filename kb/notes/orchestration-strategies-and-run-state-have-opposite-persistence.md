...
A shipped instance partially confirms the prediction. Claude Code's [dynamic workflows](../agentic-systems/reviews/claude-code-dynamic-workflows.md) have the model author an ephemeral orchestrator script per task and let a completed run be saved as a reusable `/<name>` command — the promotion path, in the wild. But the shipped form is **coarse and manual**: it promotes the *whole* script by hand, with no test gate and no fragment-level split, so the *fit* and *trust* gates above collapse into a single human decision. The promotion *pole* therefore exists while the machinery this note specifies — the two gates and `select`-fragment granularity — remains unbuilt. The direction is corroborated; the mechanism is not yet.
...
- [Claude Code dynamic workflows](../agentic-systems/reviews/claude-code-dynamic-workflows.md) — evidenced-by: shipped instance of the predicted promotion path — model-authored ephemeral orchestrator, saveable as a `/command` — but coarse and manual, with no test gate or fragment-level split; the analysis grounds the docs and adds the in-harness API contract
