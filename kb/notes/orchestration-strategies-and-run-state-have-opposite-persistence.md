...
- [Claude Code dynamic workflows](../agentic-systems/claude-code-dynamic-workflows.md) — evidenced-by: shipped instance of the predicted promotion path — model-authored ephemeral orchestrator, saveable as a `/command` — but coarse and manual, with no test gate or fragment-level split; the analysis grounds the docs and adds the in-harness API contract
