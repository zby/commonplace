...
Retained explanations create a recursive version of the same load. Several commenters in the [AI;DR discussion](../sources/hacker-news-ai-dr-ai-didnt-read.ingest.md) report that retained AI-generated code comments become later model input, can mislead or anchor the model on its own prior ideas, and may compound context poisoning. That gives a possible path by which a misleading explanation increases exposure and constrains later work. It does not establish that later outputs became defects. The reports are self-selected, unverified anecdotes from one discussion, so they establish neither prevalence nor effect size.

## Implications for this KB

The KB already distinguishes parts of a possible response: a [maintenance operations catalogue](./maintenance-operations-catalogue-should-stage-stable-procedures.md) lists candidate cleanup operations, [external triggering](./periodic-kb-hygiene-should-be-externally-triggered-not-embedded-in.md) argues that periodic hygiene should run outside routing, and a [staleness-detection proposal](./link-graph-plus-timestamps-enables-make-like-staleness-detection.md) would compare links and timestamps to flag possible changes. Sizing such a response requires a signal for harmful inflow, not a count of all new notes or links.
