...
**Form coverage** asks which forms' aspects enter the self-representation at all. **Operation depth** asks what processes inside the boundary can do to a covered component. Four operations, ordered by how far into the component they reach:
...
**TODO:** Reconsider whether “operation depth” and an ordered list are the right ontology. The concrete cases show selection without observation and modification without configuration, which may form a capability profile or partial order rather than one depth dimension. Keep the current four-operation scheme until that comparison is worked through.
...
- [A self-improving system needs a profile, not a ladder](./a-self-improving-system-needs-a-profile-not-a-ladder.md) — extends: locates form breadth and operation depth within the wider pathway profile
