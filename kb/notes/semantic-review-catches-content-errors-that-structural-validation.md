...
The current implementation target is the experimental review system documented in [`kb/reference/README-REVIEW-SYSTEM.md`](../reference/README-REVIEW-SYSTEM.md). It routes explicit review work through review bundles and sweeps, stores acceptance state in SQLite, and treats rendered markdown as inspection output rather than canonical state. That storage choice is a scoped exception to the repo's file-first design, explained in [010-review state should move to sqlite once reviews leave git and accumulate operational metadata](../reference/adr/010-review-state-should-move-to-sqlite-once-reviews-leave-git-and.md).
