...
The asymmetry with constraint-derived design is quantity vs weight — a working heuristic, not a law. Observations are plentiful — every session generates friction signals, every review surfaces patterns. But each individual observation is weak: it could be a local quirk, a one-time context, an artefact of current scale. Inherited constraints are scarce — finite context, no import mechanism, text-in text-out are the ones this KB leans on most — but each is strong while the commitment that forces it holds, because the argument runs from the framework's own commitments rather than from any single case. A constraint-derived argument can reshape the whole design; a single observation usually can't. Observations accumulate into confidence through repetition; a constraint-derived argument carries its confidence from the commitment it rests on, and loses it if that commitment changes.
...
- [underspecification and indeterminism complicate the transfer](./underspecification-and-indeterminism-complicate-programming-for.md) — develops: the practices transfer but two LLM properties make them harder in distinct ways; testing is doubled for two different reasons
