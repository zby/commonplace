...
A [self-improving system](./definitions/self-improving-system.md) makes operative changes to its own behavior-determining organization in response to evidence about an improvement objective. The definition is deliberately broad: it asks only for that functional shape, nothing about substrate or architecture, so a weight-level learner qualifies — and so does a dev team, since people revising the code and conventions that steer their own system, in response to what its failures teach, meet every clause once the declared boundary includes them. Membership this cheap carries little design information; what differentiates designs is whether an improvement pathway is **reflective** — routed through a representation of the system's own behavior that the system can read and edit, such as a lessons file — or non-reflective, landing in a substrate nothing inside can read, as a weight update does. Most systems in [agent-memory-systems](../agent-memory-systems/README.md) mine their traces for lessons and load them into later runs. That is a bid at the reflective kind, and it succeeds only when the loaded lessons function as a representation of the system's own behavior, not just accumulated knowledge.
...
1. Retained lessons become **addressable** — later rounds can read, criticize, and selectively revise them: [reflection buys addressability](./reflection-buys-addressability.md).
...
3. The standing discount — a lesson that never surfaces contributes nothing: [retrieval failure is reflection failure](./retrieval-failure-is-reflection-failure.md).
