...
- [semantic sub-goals that exceed one context window become scheduling problems](./semantic-sub-goals-that-exceed-one-context-window-become-scheduling.md) — complements: some sub-goals require fresh calls not only because tools change but because the sub-goal itself exceeds one context window
