...
Tracecraft shows why coordination infrastructure and memory infrastructure should not be collapsed just because they share a storage substrate. Its S3 layout stores `memory` keys beside task claims, messages, barriers, and artifacts. These are different jobs. Coordination state answers "who is doing what, in what order, with which handoff?" Memory state answers "what is known, remembered, or reusable?" Task claiming needs an isolation guarantee; barriers need visibility; messages need delivery expectations; shared memory needs consistency and conflict handling. A single file/object substrate can host all of them, but the required guarantees differ by primitive.
