...
- [Soft degradation can bind before the hard cap when required evidence fits](./soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md) — grounds: frontloading can avoid degradation from volume, complexity, and interference in affected workloads rather than only avoiding hard token overflow
...
- [Frontloading is partial evaluation, not divide-and-conquer](./frontloading-is-partial-evaluation-not-divide-and-conquer.md) — mechanism: the theoretical framing for why pre-compute-and-insert is precise, not metaphorical, in a shared token medium
...
- [Machine Studying](../sources/machine-studying.ingest.md) — evidenced-by: in preliminary Qwen3.5-9B Studying-DSPy runs, a repository cheatsheet improved low-budget performance and the unmodified agent caught up under forced 20-iteration search; reading this as frontloading is this note's bounded transfer, and the source reports no corresponding OpenClaw effect
