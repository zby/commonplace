...
Three instances of the same demotion, all shipped, each keeping machinery while opening the set: types became path references, so the type set is open and collection-local (ADR 018); link vocabulary became collection-owned selections from a shared catalogue, replacing one universal set (ADR 019); the three [registers](./definitions/text-contract.md) became default text-contract profiles ([ADR 042](../reference/adr/042-register-becomes-a-default-profile-under-open-ended-text-contracts.md)). Status/lifecycle semantics are a predicted fourth instance: the note type's status field currently fuses structural lifecycle with first-person endorsement, which is one answerability relation hardwired one level too high.
...
- [042-Register becomes a default profile under open-ended text contracts](../reference/adr/042-register-becomes-a-default-profile-under-open-ended-text-contracts.md) — evidence: shipped third instance (registers become default profiles)
