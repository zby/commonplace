...
Not every closed set is a content taxonomy. Sets fixed by the consumer's architecture or the framework's machinery (e.g., the syntactic shape of frontmatter or the existence of a lifecycle) may stay closed. A semantic category does not become machinery merely because tools consume it: machinery may fix the interface shape while content values remain defaults. The stronger membership test is that [a framework rule is a first principle only when no rival preserves its boundary invariants](./framework-rule-is-a-first-principle-without-boundary-preserving-rival.md). A rule stays universal when changing it requires changing a consumer, substrate, domain, or machinery commitment, rather than merely choosing another workable classification under the same commitments. Finally, the claim applies to frameworks that *aim* at universality. A single-purpose KB can benefit from hardcoding its profile; the burden begins when that profile is exported. This last concession is contested: [task-fitted structure costs cross-task reuse](./task-fitted-structure-costs-cross-task-reuse.md) argues that a single-purpose KB's own question set drifts, so the burden arrives through time even when the profile is never exported.
...
- [Task-fitted structure costs cross-task reuse](./task-fitted-structure-costs-cross-task-reuse.md) — contradicts: contests the single-purpose-KB caveat — question-set drift brings the burden forward to the instance, before any export
