...
A useful warning case is [Ingest: Large Language Model Agents Are Not Always Faithful Self-Evolvers](../sources/large-language-model-agents-are-not-always-faithful-self-evolvers.ingest.md). That paper intervenes on raw versus condensed experience in self-evolving agents and finds that compressed summaries are often less behaviorally active than the raw traces they were distilled from. The warning is not "don't distill," but that semantic plausibility is not enough — a distillate can look adequate while failing to exert causal control at use time.
