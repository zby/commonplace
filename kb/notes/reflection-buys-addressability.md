...
- [Localized retention pays where change is sparse in a matching decomposition](./localized-retention-pays-where-change-is-sparse-in-a-matching.md) — mechanism: the change-topology condition under which the expected advantages actually pay, and the dense-change case where they do not
