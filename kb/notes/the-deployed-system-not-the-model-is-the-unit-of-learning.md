...
- [Machine Studying](../sources/machine-studying.ingest.md) — evidenced-by: independently formulates the unit as the pair (model, harness), with studying allowed to change weights, prompts, tools, indexes, or notes — a convergent external statement of this note's system boundary
- [Ingest: Position: AI Agents in Scientific Teams Should Be Studied as Human-Agent Systems](../sources/position-ai-agents-in-scientific-teams-as-human-agent-systems.ingest.md) — evidenced-by: independently applies a human-inclusive evaluation boundary to scientific agents; the abstract-page capture corroborates the boundary but does not advance its measurement
