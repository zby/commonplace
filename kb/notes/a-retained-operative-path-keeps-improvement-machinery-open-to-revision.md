...
4. The determined successor is installed in instructions, configuration, checks, code, or another live behavioral path.
...
This does not require an infinite tower of controllers or a permanently external component. Some incumbent condition governs each transition: a current update law, acceptance rule, scope boundary, or authorized decision determines the successor or whether a proposed successor becomes incumbent. A later transition may replace that condition under the conditions then incumbent. Authority can rotate without any component being permanently frozen.
