...
- [constraining](./constraining.md) — suggestive parallel: the constraining spectrum (constraint strength) and the discovery hierarchy (abstraction depth) are both gradients where each step trades generality for power, though they operate on different axes
