...
- [natural-language content lacks reliable dereference, so facts need reinforcement at point of use](./prose-has-no-dereference-reinforce-facts-at-point-of-use.md) — contrasts: the under-reach dual of this note's over-reach — distinct facets of underspecification (no boundaries vs no resolution), so neither grounds the other. But the flatness compounds the decay: with no boundaries the declared fact must win attention against the entire global stream, deepening its failure to reach a distant point of use
