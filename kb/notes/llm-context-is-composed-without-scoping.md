...
- [prose has no reliable dereference, so a declared fact must be reinforced where it applies](./prose-has-no-dereference-reinforce-facts-at-point-of-use.md) — contrasts: the under-reach dual of this note's over-reach — the same absence of name machinery that lets early context leak forward (spooky action) also lets a declared fact fail to reach its distant point of use
