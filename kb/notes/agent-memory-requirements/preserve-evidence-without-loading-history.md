...
[Session history should not be the default next context](../session-history-should-not-be-the-default-next-context.md) because persistence and loading are separate decisions. Store-everything is only a capture posture. Raw traces should usually remain outside the acting agent's ordinary context, loading only for provenance checks, dispute resolution, debugging, reconstruction, or evaluation.
...
- Session checkpoints, commit records, retry handles, and archive directories that let follow-up work resume, audit, or reconstruct artifacts from a bounded run.
