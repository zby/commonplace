...
| T1 agent-agnostic tool | Reusable external component whose value does not depend on one fixed agent | In our terms this is often an advice-bearing knowledge artifact or capability artifact that can serve many consumers, but still needs import, authority, and activation rules. |
...
| Skill library | Distilled procedural memory, often system-definition authority | The article emphasizes acquisition mechanism; our notes also ask about invocation, routing, execution policy, provenance, and lifecycle. |
