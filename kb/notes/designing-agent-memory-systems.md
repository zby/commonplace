...
An agent memory system should be designed from the effects memory must have, not from the store that holds it or the mechanism that produced each memory. The success criterion is contextual competence: remembered material earns its place when it helps an agent answer, act, create artifacts, or update behavior under bounded context.

This note is therefore a requirements map, not a proposed layer architecture. A realistic system has to preserve useful evidence, turn experience into future capacity, expose contracts for memory artifacts, assemble the right context for bounded calls, steer future behavior when past lessons apply, and revise or retire memory when it stops earning its cost.

The contextual-competence test from [Agent memory needs discoverable, composable, trusted knowledge under bounded context](./agent-memory-needs-discoverable-composable-trusted-knowledge-under-bounded-context.md) makes the requirement concrete. Whatever created a memory -- direct authoring, import, trace extraction, synthesis, or codification -- the result must be discoverable enough to find, composable enough to use with other knowledge, and trustworthy enough to rely on without redoing the original work.

## Memory Requirements Come From Consumers

The requirements should be derived from the consumers memory has to serve, not from the storage mechanism. Acting agents need compact, timely context before they answer, edit, decide, or call tools. Context schedulers need triggers, priorities, metadata, budgets, and enough source shape to decide what not to load. Learning loops need traces, failures, corrections, outcomes, and candidate observations. Reviewers need provenance, confidence, source evidence, stale-state signals, and contract checks. Maintainers need readable rationale, ownership, update paths, and lifecycle state. Governance needs retention, access, redaction, authority, and deletion controls. Work surfaces need current unresolved state, dependencies, task queues, alternatives, and expiry.

Each consumer imposes a different failure mode. Acting agents fail by repeating known mistakes or missing relevant knowledge. Context schedulers fail by spending context on the wrong material. Learning loops fail by extracting weak candidates or overlooking strong signals. Reviewers fail when a memory has no auditable basis. Maintainers fail when memory cannot be updated without reconstructing its purpose. Governance fails when stored material escapes its retention, access, or authority limits. Work surfaces fail when unresolved state either disappears into traces or contaminates durable library memory.

This consumer map explains why the needs below cannot collapse into one retrieval interface. Evidence retention serves reviewers, learning loops, and future redistillation. Artifact contracts serve writers, reviewers, maintainers, and acting agents. Activation serves agents that would not know to search. Compiled views serve runtime surfaces that need behavior-facing context. Lifecycle serves every consumer because stale memory consumes attention, search rank, maintenance time, and behavioral authority.

## Memory Needs A Boundary Test

An artifact is part of the memory system when it preserves evidence, knowledge, preference, procedure, decision, or learned constraint for future use, and when it is governed by memory operations such as discovery, activation, provenance, promotion, authority, lifecycle, or evaluation.

This keeps "memory is more than retrieval" from becoming "everything that can affect behavior is memory." System-definition artifacts can carry memory, but not every system-definition artifact is memory. A test added because of a repeated failure is memory-mediated behavior when the memory system manages it as a learned constraint, with provenance, authority, lifecycle, and evaluation obligations. A random pre-existing test is just part of the ordinary execution substrate unless it is governed that way. An `AGENTS.md` rule is memory when it carries accumulated project knowledge or routing policy; it is ordinary control-plane configuration when it only declares static execution constraints.

For the implementation mapping, [Agent memory coverage](../reference/agent-memory-coverage.md) tracks how Commonplace currently satisfies these requirements and where gaps remain.

## Memory Is A Context Engineering Problem

Bounded context makes memory more than preservation or retrieval. Agents need external information because it is impossible to put all useful knowledge into model weights, but they also need context-construction machinery because all available external information cannot fit in one context. The agentic loop must repeatedly decide what to preserve, what to inspect, what to load, what to execute, what to update, and what to ignore.

That makes agent memory a [context engineering](./definitions/context-engineering.md) (right-knowledge-into-bounded-context) problem. As [Agent memory is a crosscutting concern, not a separable niche](./agent-memory-is-a-crosscutting-concern-not-a-separable-niche.md) argues, memory cuts across the runtime rather than sitting in one component: storage belongs on the execution substrate, retrieval and activation belong in the context engine, and learning belongs in the loop that turns experience into readable or executable artifacts.

## Artifact Axes Orient The Requirements Map

[Axes of artifact analysis](./axes-of-artifact-analysis.md) provides the vocabulary needed for orienting on the requirements map. Backend says where an artifact lives. Artifact class says how the learned result is represented: opaque, prose, or symbolic. Role says how the consumer reads it: knowledge-role artifacts answer questions, while system-definition-role artifacts steer behavior. The same remembered material can serve either role: a decision rationale is knowledge when the agent asks "why did we choose this?", and system-definition when it prevents the agent from proposing the rejected alternative again.

Class and role are separate axes. Prose and symbolic artifacts can both serve knowledge or system-definition roles. A symbolic artifact has formal semantics consumed by a deterministic interpreter; schemas, tests, scripts, tools, validators, and typed data are symbolic in that sense. When a learned procedure becomes deterministic enough, it should move toward [codification](./definitions/codification.md) (committing procedure to a symbolic medium), because [bookkeeping work is more reliable on a symbolic substrate than when re-run through the LLM each time](./scheduler-llm-separation-exploits-an-error-correction-asymmetry.md). Codification is a class move, not automatically a behavior-role move: a schema is knowledge when read as documentation and system-definition when an interpreter validates with it.

## Memory Is More Than Retrieval

With the axes in view, retrieval is necessary but insufficient. A memory system must decide not only what past information to expose, but also which remembered constraints, routines, and maintenance mechanisms should shape future work. Durable behavior-changing artifacts belong inside the memory problem because they are one way past experience changes future capacity.

We are convinced that the role split is the right design commitment, even if it is not yet a settled theory of memory. Knowledge-role artifacts and system-definition-role artifacts need different contracts. Ordinary knowledge needs fidelity, provenance, scope, and relevance. Instructions, skills, tests, checks, tools, guardrails, and work-surface changes also need authority, activation conditions, priority, rollback, maintenance, and behavioral evaluation. Search can answer direct questions, but it does not by itself decide which routines should be compiled, when they should fire unasked, or when they should be retired. [Knowledge storage does not imply contextual activation](./knowledge-storage-does-not-imply-contextual-activation.md): a stored lesson has not helped unless it appears in the right bounded context, with enough priority and framing to change what happens next.
...
The error-correction asymmetry in [Scheduler-LLM separation exploits an error-correction asymmetry](./scheduler-llm-separation-exploits-an-error-correction-asymmetry.md) strengthens the symbolic-artifact claim: behavior-role learning at scale needs to happen through symbolic artifacts. Their contracts, activation, source alignment, lifecycle, authority, and evaluation therefore become part of the memory-system requirements.
...
The needs do not imply one integrated architecture or one universal sequence. [Commonplace agent memory gap plan](../reference/commonplace-agent-memory-gap-plan.md) translates these requirements into a plan for closing Commonplace's current gaps: authority for automatic memory operations, candidate queues, lifecycle scheduling, compiled-view source alignment, session-trace capture, trace-derived extraction, situation cues, behavioral evaluation, ranking, import/reingest maturity, and retrospective episodes where they answer real queries.

The requirements are stable at a different level from any implementation sequence. Commonplace should grow from validated usefulness, not from an attractive taxonomy, but its roadmap should address the gaps in the shipped system rather than replay the generic requirements list.
...
- [Agent memory needs discoverable, composable, trusted knowledge under bounded context](./agent-memory-needs-discoverable-composable-trusted-knowledge-under-bounded-context.md) — grounds: useful memory should improve future action by being findable, usable with other knowledge, and reliable enough to act on
