...
- the check that warrants acting on the result unattended becomes an oracle whose domain covers the case, [since warranted autonomy is bounded by oracle domain](./warranted-autonomy-is-bounded-by-oracle-domain.md). The completeness judgment moved from a maintainer's eyeball sweep into a validator that recomputes membership mechanically.
