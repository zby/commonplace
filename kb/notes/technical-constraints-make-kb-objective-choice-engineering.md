...
- **The constraints are inherited, not universal.** They arrive in the constraint packet of this framework's boundary commitments — an LLM consumer, a file substrate, a knowledge domain, the built machinery — so "engineering, not philosophy" is relative to those commitments, since [what counts as an inherited constraint is relative to a declared framework boundary](./a-framework-rule-with-a-boundary-preserving-rival-is-not-inherited.md). A framework built on a different consumer inherits a different packet and a different pruned space. The value question is dissolved *for this framework*, not in general.
