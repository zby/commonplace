...
**2. The codification option.** Any part of an objective whose inputs are already known can be moved out of prose and into a symbolic artifact with formal semantics — [codification](./definitions/codification.md). This converts a contested convention into a runnable test: the argument "is this the right rule?" becomes "does the validator pass?" Codification is a real lever, not a formality — whether to discharge a sub-objective in code or leave it to the LLM is itself a tradeoff with a decision procedure (see [codify-versus-LLM heuristics](./codify-versus-llm-decision-heuristics.md)). Its mere availability means an objective's contested parts can often be settled by construction rather than by debate.
...
- [codification](./definitions/codification.md) — defined-in: the move-logic-into-code option that settles contested objective parts by construction
