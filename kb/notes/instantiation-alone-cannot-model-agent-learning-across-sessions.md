...
Class-based systems often mark or price this path for changing definitions. A [metaobject protocol](../sources/metaobject-protocols-why-we-want-them-and-what-else-they-can-do.ingest.md) exposes a separate interface for changing language organization. *[Monkey-patching](../sources/monkey-patch.ingest.md)* gives runtime modification a warning name. The V8 JavaScript runtime can [deoptimize code whose object-layout assumptions stop holding](../sources/fast-properties-in-v8.ingest.md). The Erlang/OTP language platform treats live definition change as [versioned release work with migration and rollback](../sources/erlang-release-handling.ingest.md).

These examples attest that the source domain distinguishes ordinary execution from definition change. Under [the domain-pricing criterion](./domain-pricing-routes-an-exception-to-idealization-assessment.md), they route the fixed-class model to an adequacy assessment rather than settling the question outright. For this note, the relevant issue is fence integrity, not how often reflective mutation occurs. Marked crossings still give us vocabulary for the second relation. If class change became ordinary and unmarked in the systems being compared, the idealization would fail for this purpose. Its broader adequacy outside that use remains open.
...
The Machine Studying practitioner report provides one bounded example. Its study pass changes an agent by writing a reusable repository map before later inference. The reported gain is concentrated at inference budgets where an unprepared session cannot afford equivalent exploration. This supports the distinction between starting a fresh session from an unchanged definition and preparing a changed definition for later sessions. It does not establish a general performance law.
...
- [Fast properties in V8](../sources/fast-properties-in-v8.ingest.md) — evidenced-by: attests runtime costs when optimized layout assumptions stop holding
- [Erlang/OTP release handling](../sources/erlang-release-handling.ingest.md) — evidenced-by: attests live definition change as versioned deployment with migration and rollback
- [Erlang/OTP compilation and code loading](../sources/erlang-compilation-and-code-loading.ingest.md) — evidenced-by: supplies the current/old-version mechanism behind a governed transition
