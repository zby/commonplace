...
Those reflective paths do not make the closed-world reading naive. The fixed class is class-based object orientation's first-order model — the idealization that buys invariants, substitution reasoning, and compiled dispatch — and the paradigm prices its own exceptions. Class mutation goes through a marked reflection interface rather than ordinary class syntax — the [metaobject-protocol design](../sources/metaobject-protocols-why-we-want-them-and-what-else-they-can-do.ingest.md) makes the separation explicit while base-level syntax and defaults stay intact; the practice carries a warning name in [monkey-patching](../sources/monkey-patch.ingest.md), with documented incompatibility and conflict pitfalls attached; runtimes charge for it by deoptimizing code specialized on the old shape, as [V8's layout-guarded specialization](../sources/fast-properties-in-v8.ingest.md) documents; and where class change must happen in production, it is ritualized into versioned changesets and migration callbacks, as in [Erlang/OTP release handling](../sources/erlang-release-handling.ingest.md). On the criterion in [domain pricing routes an exception to idealization assessment but does not decide it](./domain-pricing-routes-an-exception-to-idealization-assessment.md), that pricing routes the reflective counterexamples to idealization assessment rather than letting them defeat the model outright: in the model, instances do not modify their class, and real class-based systems mark every path that does. The assessment's second half is still owed — how much behaviour reflective mutation carries in the systems this note reasons about, and what the model's declared use tolerates, are not yet established — so the idealization is declared here with its adequacy record open for a future pass to attack. One commitment on that record can be entered now: the declared use here tolerates marked exceptions regardless of their frequency, because this note's argument consumes the fence rather than the absence of crossings — the fenced meta-level is what supplies the second relation's vocabulary below. What the declared use cannot tolerate is the fence dissolving. Reflective mutation practiced as ordinary, unmarked class change would refute the idealization for this note's purpose without any new counterexample.
...
- [Metaobject Protocols: Why We Want Them and What Else They Can Do](../sources/metaobject-protocols-why-we-want-them-and-what-else-they-can-do.ingest.md) — evidenced-by: attests the marked-interface pricing signature — reflective language change routed through an explicit meta-level protocol
- [Monkey patch](../sources/monkey-patch.ingest.md) — evidenced-by: attests the warning-name pricing signature — the community's own vocabulary marks runtime class modification as a workaround with documented pitfalls
- [Fast properties in V8](../sources/fast-properties-in-v8.ingest.md) — evidenced-by: attests the runtime-charge pricing signature — specialization on stable layouts, measurable costs on shape churn
- [Erlang/OTP release handling](../sources/erlang-release-handling.ingest.md) — evidenced-by: attests the governance-ritual pricing signature — live definition change as versioned deployment with migration and rollback
- [Erlang/OTP compilation and code loading](../sources/erlang-compilation-and-code-loading.ingest.md) — evidenced-by: supplies the mechanism behind the governed transition — current/old module versions in bounded coexistence
