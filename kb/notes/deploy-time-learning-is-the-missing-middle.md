...
The table picks out today's common configurations; substrate and timing are orthogonal in principle. A fourth cell — weight substrate with deployment timing — exists but stays rare because training infrastructure is heavy. [OpenClaw-RL](../sources/openclaw-rl-train-any-agent-simply-by-talking.ingest.md), which runs live RL from user interactions, is a current example.
...
Agile was already doing deploy-time learning, with an asymmetry. Code and specs co-evolved — running code informed specs, revised specs drove new code — but only code executed. Moving a concern back to prose meant taking it out of production. LLMs close the asymmetry: prompts execute, so loosening a codified behavior back to prose keeps the system running.
