...
[OpenClaw-RL](../sources/openclaw-rl-train-any-agent-simply-by-talking.ingest.md) runs live RL from user interactions, occupying a fourth cell in the substrate × timing grid (weight substrate, deployment timing). The table picks out today's common configurations; the axes themselves are orthogonal in principle, and training-infrastructure weight is what keeps the fourth cell rare.
...
Deploy-time learning generalizes agile's core innovation. Waterfall's assumption was sequential handoff: finish the specs, then write the code. Agile's real insight was that code and specs can coexist and co-evolve — running code informs and revises specs; revised specs drive new code. The two media feed each other continuously.

Deploy-time learning follows the same pattern with a different pair of media: deterministic code and LLM prompts. You deploy with some behavior in prompts, observe what works, and progressively codify the understood parts — while the prompts evolve in response to what the code now handles. At any given moment, some behavior is deterministic code, some is still in prompts, and the boundary moves as understanding accumulates. The learning loop looks the same as an agile sprint: start underspecified, deploy, observe, harden the understood parts, repeat.

Where deploy-time learning diverges from agile is in where the boundary stops moving. Agile treats natural-language specs as temporary — stories waiting to become code. The implied end state is fully codified software. Deploy-time learning recognises that some parts *should stay in prose* because they require judgment, interpretation, or context-sensitivity that deterministic code can't capture. A data-report system might move statistics into Python while leaving narrative interpretation with the LLM, and there is no expectation to codify the narrative. The hybrid is the end state, not a waypoint.

This reframes what agile calls "changing requirements" too. Some of that change [conflates genuine change with late-surfacing disambiguation failures](./changing-requirements-conflate-genuine-change-with-disambiguation-failure.md): downstream specs silently commit to one interpretation of an underspecified upstream spec, and the error only surfaces when deployed. Short iterations limit how far wrong interpretations can propagate, not just how fast teams respond to genuine change — and the same logic applies to prompts silently committing to an interpretation that only deployment reveals.

## Failure modes

- **Premature codification.** Committing to one interpretation before enough runs reveal which is right locks in brittle assumptions. The remedy is to constrain only when patterns have emerged across runs, and to relax back toward prose when new requirements expose the wrong commitment.
