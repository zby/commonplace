...
- **Does oracle strength predict bitter-lessoning?** If so, the spectrum is prescriptive — invest in codification where oracles are hard, invest in learned approaches where oracles are soft. Deutsch's [explanatory-reach](./first-principles-reasoning-selects-for-explanatory-reach-over.md) concept suggests a mechanism: hard oracles survive scaling because they ARE the problem specification — they have explanatory-reach beyond any particular model's capabilities. Soft oracles encode adaptive fit (theories about what correct looks like) which scale reveals as approximations, just as it did for vision features. This would make oracle strength a proxy for how much explanatory-reach the verification has. External support: [Tam et al.](https://x.com/amytam01/status/2031072399731675269) observe that agentic coding tools automate engineering (hard oracle — tests, specs, benchmarks) while research problem selection (no oracle — "you can't know in advance whether a solution exists") resists automation entirely. Quant firms pay $600k for "research taste" precisely because it's a no-oracle domain. This is the oracle-strength prediction stated in market-economics language.
...
- [Harness Engineering (Lopopolo, 2026)](https://openai.com/index/harness-engineering/) — grounds: the implicit thesis that oracle quality (verification infrastructure), not generation quality, is the leverage point — structural tests and linter rules as the bottleneck
