...
The same repository supplies the second case: adopting `covered_by` on the `learning-theory` head, the new symbolic check caught a tag member the documented `rg` recipe had missed — [the trace retrieval failure is reflection failure reads for what the wire needs to hold](./retrieval-failure-is-reflection-failure.md). The fix was not to the code. Executing the symbolic artifact produced evidence that the prose was wrong, and the search recipe in `kb/reference/navigation.md` was corrected, not the check. Had the recipe not been a revisable artifact inside the system, the check would have been left reporting a discrepancy nobody could act on.
...
- [Retrieval failure is reflection failure](./retrieval-failure-is-reflection-failure.md) — evidence: the full telling of the `covered_by` trace this note reads for the symbolic-to-prose direction
