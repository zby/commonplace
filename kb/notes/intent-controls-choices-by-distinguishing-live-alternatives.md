...
- [An author should fix what the executor can't determine, not what it will](./fix-what-the-executor-cant-determine-not-what-it-will.md) — extends: applies the discrimination test to whether a choice can safely remain with execution or must be fixed upstream
- [A capable agent needs methodology selection, not just relevant knowledge](./capable-agents-need-methodology-selection.md) — extends: gives mutually incompatible methodologies as a concrete set of live alternatives that intent and constraints can make governing
- [A specific intent may out-yield local rationales, but contingent facts stay separate](./specific-intent-may-out-yield-local-rationales-facts-stay-separate.md) — extends: develops the specificity boundary by conjecturing that one purpose can constrain many local judgments while facts it does not entail remain separate
- [A failure explanation becomes search control only when it changes a later branch decision](./failure-explanation-changes-later-branch-decisions.md) — contrasts: applies the same null-effect test to a retained explanation and later search branches rather than intent and local means
