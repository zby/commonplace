...
**The linking claim has to be stated, and stated faithfully.** An unrecorded justification cannot be found false; the target simply persists as a settled commitment whose reason nobody holds. This is a design-rationale problem before it is an evaluation problem, and the rationale surfaces where the reason would live [do not enforce continuity from a shipped commitment back to what justified it](../reference/design-rationale-management.md). Recording one is not sufficient either, [since a legible but unfaithful rationale makes the repair confidently wrong rather than merely uninformed](./revision-guided-by-rationale-needs-faithfulness-not-just-legibility.md) — a plausible reason attached to a target that is not the reason it was adopted leaves the composition-failure signal unreadable in the same way as no reason at all.
...
A proximate target is a design choice justified by a claim, not [a constraint inherited from a consumer or a substrate](./a-framework-rule-with-a-boundary-preserving-rival-is-not-inherited.md) — but it presents like one. Structural properties are stable, statable, and cheap to check for achievement, and those are the same properties that make something look like a settled goal rather than a contestable bet.
...
- [A framework rule with a boundary-preserving rival is not an inherited constraint](./a-framework-rule-with-a-boundary-preserving-rival-is-not-inherited.md) — contrasts: the inherited constraint a proximate target is mistaken for
...
- [Revision guided by rationale needs faithfulness, not just legibility](./revision-guided-by-rationale-needs-faithfulness-not-just-legibility.md) — extends: what stating a linking claim has to achieve, and why legibility alone leaves the repair confidently wrong
...
- [Design rationale management in Commonplace](../reference/design-rationale-management.md) — evidenced-by: distributed rationale surfaces that retain justifications without enforcing continuity from a commitment back to them
- [The tag-readme trace read as a self-improving loop](../reference/tag-readme-trace-as-self-improving-loop.md) — evidenced-by: the split observed in a shipped case — the validator passing means the marks are consistent, not that the type split made the KB better
