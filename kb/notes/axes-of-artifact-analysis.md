...
| Intern-S2-Mobius checkpoint | Model-artifact store; checkpoint lineage not assessed here | Distributed-parametric shared FFN or sparsely activated MoE-like blocks plus self-attention reasoners | Hidden-state queries select shared feed-forward computation; decoder execution gives the selected parameters behavioral force |

The [Mobius ingest](../sources/intern-s2-mobius-arxiv-v1.ingest.md) supplies a bounded role-separation case. The paper proposes a globally shared feed-forward “Memory” queried by multiple self-attention “Reasoners” through hidden states, and at larger scales partitions the shared FFN into sparsely activated MoE-like blocks. These labels mark different architectural roles and authority paths, but both operative parts remain distributed-parametric. The split therefore does not supply localized-artifact governance: it does not create readable claims with claim-level provenance, selective invalidation, or selective rollback.
