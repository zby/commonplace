...
- [Removing a human judgment can degrade the judgments that remain](./removing-a-human-judgment-can-degrade-the-judgments-that-remain.md) — extends: develops the tool-usefulness sentence into two mechanisms, calibration loss and context loss
- [Programming-tool progress is a partial order on accepted outcomes per human effort, not a scalar](./programming-tool-progress-is-a-partial-order-not-a-scalar.md) — extends: states the leverage order this note assumes, with warrant losses recorded rather than netted against effort
