...
The relevant contrast is not human structure versus learning in the abstract. It is structure exposed to revision versus structure treated as an immutable coordinate of learning. [The Bitter Lesson note](./bitter-lesson-selects-against-unearned-reach-not-against-structure.md) offers a narrower case-level Commonplace conjecture: an inspectable requirement-to-objective proxy used beyond its assessed scope may lose to computation-scalable search or learning on the same objective and regime when proxy mismatch, rather than a rival explanation, causes the failure. Treating a fixed decomposition as such a proxy is this note's analogy; that mismatch has not been established. Applied as a conjectural warning, moving search into a controller or harness does not answer the criticism if consequential task categories, distinctions, operations, or mappings remain fixed one layer above it.
...
[ACM, an agentic context-management approach](../sources/acm-agentic-context-management-for-long-horizon-tasks.ingest.md), learns when to use a supplied two-operation scheme: summarize and archive all messages up to the previous summary boundary, then query archived messages by identifier. Its policy can improve invocation and abstention within that scheme, but it cannot replace chronological prefix compression with a different state representation. In the Qwen3.5-9B training ablation, ACM data raised Pass@1 from 0.635 to 0.727 on BrowseComp-Plus, 0.405 to 0.425 on DeepSearchQA, and 0.508 to 0.530 on SWE-Bench Verified. Those gains show that the compound treatment worked in that setup. The retained setup reports comparisons with system-level baselines, not a matched test of alternative action bases or dynamic state representations. The results therefore do not by themselves show that the supplied operations defined the right learning space.

[Meta-Harness's fixed summary-without-traces ablation](./an-experiment-identifies-only-the-contrast-it-actually-runs.md) examines an outer-loop system that searches task-specific LLM harness code. The experiment fixes a different decomposition of evidence: scores only, scores plus a fixed summary without raw-trace access, or access to the raw traces. The feedback representation and summarizer remain outside harness search. Here the fixed choice produces an observed failure: the supplied summary treatment underperforms raw traces. This result shows that the tested summary-without-traces representation lost useful distinctions for this proposer and task. It does not establish rawness as the general cause or show that every derived representation has the same defect, because episode-backed, query-conditioned, and jointly searched representations were unavailable.
...
- [Ingest: Co-Harness: Co-Evolving Harnesses and Model Weights for LLM Agents](../sources/co-harness-co-evolving-harness-and-model-weights.ingest.md) — evidenced-by: in a two-round experiment, alternates validated changes to prompts, tool schemas, skills, middleware, and memory with model fine-tuning; treating that as a broader effective update space is this note's transfer
