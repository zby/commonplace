...
The [constraining gradient](./methodology-enforcement-is-constraining.md) describes how practices move from instructions through skills and hooks to scripts, trading flexibility for reliability. But the gradient treats enforcement and instruction as separate concerns: instructions inform, hooks and scripts constrain. The most effective constraining artifacts do both simultaneously — they block the wrong output *and* teach the agent the correct one in the same interaction.
...
The [methodology enforcement gradient](./methodology-enforcement-is-constraining.md) has five layers: instruction, skill, hook (warn), hook (block), script. Each layer is characterised by its trigger reliability and response determinism. But there is an orthogonal axis: **how much context the enforcement artifact delivers to the agent when it fires**. A hook that says `FAIL` constrains maximally but informs minimally. A hook that says `FAIL: description must be under 200 chars, yours is 247 — trim the last sentence` constrains equally but informs maximally.
...
The insight is that **moving right on this axis is cheap constraining** — it requires no change in trigger mechanism or enforcement strength, only better error messages. Yet the reliability gain is substantial because it eliminates the agent's need to search for or infer the correct response.
