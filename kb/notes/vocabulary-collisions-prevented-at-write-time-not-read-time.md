...
Session-level isolation does not substitute: if a task needs both artifacts, sandboxing them into separate sub-agent frames defeats the composition the task requires. Body-composability needs either a collision-free vocabulary or visible, checkable scope for the technical use — and collection-local vocabulary is reliably safe only when a colliding sense cannot co-load, which a cross-linked KB makes hard to guarantee.
