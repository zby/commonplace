...
This deficit is not inherent to prose as a medium. Books are prose, and books do not suffer this collision problem in practice. What protects a book's vocabulary is not formal scope but a boundary object: opening a book is a manual act of leaving one attentional frame and entering another, and that frame stays active for the whole reading session. A chemistry textbook's *reduction* (gain of electrons) never collides with a rhetoric textbook's *reduction* (simplifying a claim) because no reader is inside both books at once. A knowledge base removes that boundary object. Retrieval and co-loading pull notes across former book-equivalent boundaries with no signal that a crossing happened — composition is concatenation, not the reader closing one book and opening another. The risk this note addresses is therefore not that prose lacks formal scope; ordinary reading has survived without it for as long as books have existed. The risk is that KB composition strips away the informal single-source boundary that let prose get away with lacking formal scope, and puts nothing in its place.

`Distillation`—the retired term for targeted transformation of recorded material into a use-shaped artifact for a particular downstream consumer—illustrates the collision pattern when one bare word names two operations: entailment-preserving reshaping (methodology → skill) and evidence-to-rule generalization (traces → preference rule). The first produces an artifact that can be checked against its retained source. The second produces a rule that must earn authority through testing. Because the word alone does not mark this maintenance boundary, co-loaded notes can blur which regime applies.
...
- **Reader vigilance** — A reader can notice the clash, the way a reader might spot that one note's "table" means a database table while another's means an HTML element. This remains a useful backup, but a silent and fallible inference is too weak to be the only maintenance control.
- **Symbolic disambiguation** — Writers can declare per-note senses ("in this note, X means…") or use qualified terms, and readers can resolve those declarations before composing. This can work, but it adds resolution work to reads and declaration work to writes. A missing declaration leaves the original ambiguity.
- **Write-time prevention or visible scope** — Writers can reserve each unqualified load-bearing term for one sense across co-loadable artifacts. When reuse is necessary, they can place the technical use in a schema slot (`derived-from` as a link-label token) or a linked clause frame (`actionable`, below). These controls reduce dependence on read-time inference.
...
The two strongest rows compose. A rare hyphenated compound occupying a validator-defined schema slot inherits both properties at once: the exact string is cheap to search corpus-wide, the way any rare compound token is, and the slot's position is what a gate can actually check, rather than depending on writer discipline to keep the compound out of unscoped prose. `derived-from`, `abstracted-from`, and their siblings used as link-label relation names are this composition — the spaced ordinary-English phrase (`derived from`) stays free for prose, while the hyphenated form is reserved for the schema-checked link-label slot. Neither ingredient alone would do the job: a bare rare compound (row 4) still relies on writers not repurposing it in unscoped prose elsewhere; a schema slot alone (row 5) does not by itself guard against a common word being placed there and dragging in its ordinary associations. Making the identifier a distinctive multi-word compound closes that gap, at the cost of two visually similar strings — spaced and hyphenated — that a reader or writer must keep apart.
...
That is why this note admits clausal binding only with a required definition link. The link does not prevent mis-predication. It makes the intended technical use visible and spot-checkable. Under this convention, an unlinked "actionable" has its ordinary reading; only the linked methodology-predicate frame invokes the technical relation. Grammar supplies limited scope, while the required link supplies the check.

## Boundary and caveats

Minimality still matters here too: a rare compound is cheaper to keep *apart* from other terms, not cheaper to *learn*, and each coinage is still a global name every reader carries. The other limits on these devices — an existing association helping only when it agrees with the KB sense, and clausal binding's required link remaining a writing convention rather than a validator-enforced gate — are noted where each device is introduced above.
