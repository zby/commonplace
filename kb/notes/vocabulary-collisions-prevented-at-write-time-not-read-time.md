...
Three candidate remedies offer different levels of assurance. Reader vigilance — noticing the clash at read time — is a useful backup but too silent and fallible to be the only control. Symbolic disambiguation — per-note sense declarations ("in this note, X means…") that readers resolve before composing — can work, but adds work to every read and write, and a missing declaration silently restores the original ambiguity. Write-time prevention or visible scope — reserving each unqualified load-bearing term for one sense across co-loadable artifacts, and placing necessary reuse in a schema slot or linked clause frame — removes the dependence on read-time inference.

Retrieval-time disambiguation — treating the collision as word-sense disambiguation for the composer to solve by assembling context in which each occurrence resolves correctly — sounds like a fourth remedy but decomposes into the first two. Where the retriever resolves senses from declared markers, it is symbolic disambiguation executed mechanically: stronger than leaving declarations to readers, but the declarations must exist and travel with every excerpt, and writing them is write-time scoping by another name. Where it resolves senses semantically, it is reader vigilance moved into the pipeline — the same silent inference, and weakest exactly here, because classic word-sense disambiguation leans on sense inventories a model's priors already know, while a KB's load-bearing senses are local coinages defined in single notes: an excerpt that drops the qualifier silently restores the ambiguity, and the prior pulls toward the common reading. The assurance a retrieval solution adds is precisely its symbolic, write-time ingredient; the semantic remainder inherits the failure profile of the reader it replaced.

Session-level isolation does not substitute — and neither does a retriever that keeps colliding notes apart, which is the same move made earlier in the pipeline: if a task needs both artifacts, separating them defeats the composition the task requires. Body-composability needs either a collision-free vocabulary or visible, checkable scope for the technical use — and collection-local vocabulary is reliably safe only when a colliding sense cannot co-load, which a cross-linked KB makes hard to guarantee.
