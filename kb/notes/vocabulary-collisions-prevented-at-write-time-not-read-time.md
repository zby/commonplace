...
Technical terms are hard to use in a knowledge base because natural-language content offers no reliable, mechanically enforced namespace for an unqualified technical sense. In the risk model proposed here, a technical sense is **load-bearing** when misresolving it changes what an agent does, which maintenance regime applies, what authority a claim carries, or whether a fallback is licensed. Symbolic systems use scope to keep identifiers apart: two `map`s in different namespaces need not collide, because formal position participates in resolving meaning. Natural language has no equivalent: readings are delimited only by grammar and context — conventional, fallible cues. A lexical technical sense without a visible clause frame or schema position therefore binds *globally and weakly*:
...
This is the corpus-scale form of a pathology already named at session scale: [LLM context is composed without scoping](./llm-context-is-composed-without-scoping.md) names it collision. At corpus scale the colliding senses live in durable notes written months apart, and the collision can recur whenever the notes are co-loaded.

## The failure is at composition, and it is silent

Context assembly is **concatenation, not import**. A symbolic system resolves name collisions at link time — qualified names, renaming, or a duplicate-symbol error. Concatenated natural-language content has no rename step and no linker error. An agent can read two notes that use one term in different technical senses, blend them, and produce a conclusion that appears to inherit authority from both notes while following from neither. A reader sometimes notices the clash, but that noticing is itself an inference — vulnerable to distance, phrasing, and load — and it emits no error signal when it fails to fire. This is the shape of [natural-language content has no reliable dereference](./prose-has-no-dereference-reinforce-facts-at-point-of-use.md): a formal operation replaced by an inference that may work but cannot be relied on as a control.

The deficit is not inherent to natural language as a medium: books are prose, and they do not suffer this collision problem in practice. What protects a book's vocabulary is not formal scope but a boundary object. Opening a book is a manual act of entering an attentional frame that stays active for the whole reading session; a chemistry textbook's *reduction* never collides with a rhetoric textbook's *reduction* because no reader is inside both books at once. A knowledge base removes that boundary object: retrieval pulls notes across boundaries that used to separate books, with no signal that a crossing happened. The risk, then, is not that natural language lacks formal scope — ordinary reading has survived without it for as long as books have existed. It is that KB composition strips away the informal single-source boundary that let natural language get away with lacking it, and puts nothing in its place.
...
Session-level isolation does not substitute for this, and neither does a retriever that simply keeps colliding notes apart — that only relocates the same move earlier in the pipeline: if a task needs both artifacts, separating them defeats the composition the task requires. The both-notes-needed case rules out *separation*, not co-loading: annotating both notes with sense-distinguishing markers — provenance metadata, sense qualifiers — does make the co-load safe, but that is not a rival remedy. It is visible scope executed as metadata: the same write-time scoping by another name. Body-composability needs either a collision-free vocabulary or visible, checkable scope for the technical use, and collection-local vocabulary is reliably safe only when a colliding sense cannot co-load — which a cross-linked KB makes hard to guarantee.

## Devices rank by binding strength

Schema-defined positions offer the strongest scope available: a link-label token, a `tags:` value, a type-spec field, or a frontmatter key can be unambiguous because it occurs in a validator-defined slot. **Position can disambiguate even when spelling alone cannot.** Moving a binding from natural-language content into such a slot is [codification](./definitions/codification.md) applied to vocabulary itself. The natural-language devices rank below it by binding strength:

| Device | Binding | Failure mode |
|---|---|---|
| Definition note + vocabulary entry | Corpus-wide natural-language convention, not mechanically scoped | Drift or conflicting use |
...
### Prototype shorthands are not aliases

A familiar prototype may introduce a broader technical category in outward-facing language, but only as a visibly approximate gloss, not as a second canonical name. “Prompts, code, and weights,” for example, can orient a reader toward natural-language, symbolic, and distributed-parametric artifacts. The mapping is not a synonym table: a natural-language artifact need not be a prompt, a symbolic artifact need not be code, a prompt can contain both natural-language and symbolic operative parts, and weights are only one kind of distributed-parametric state.

The prototypes select common instances or uses, whereas the categories name representational forms. Since [artifact analysis separates representational form from consumption path, lineage, and behavioral authority](./axes-of-artifact-analysis.md), treating a prototype as its category would collapse axes the vocabulary exists to keep separate. Production method does not determine form: a program-generated note can remain natural-language, while a hand-authored schema can be symbolic where a validator assigns its elements defined consequences.

The approximation must therefore carry visible scope — “in practical terms,” “roughly,” “prototypically,” or an explicit example frame — and the canonical term must return before a load-bearing classification, maintenance rule, schema value, or inference depends on the distinction. “Prompts are the most familiar example of operative natural-language artifacts” preserves the boundary. Letting a prototype alternate unqualified with its broader category creates the same globally weak binding this note warns against.
...
The remediation audit should be **classification-led and baseline-anchored**. Record each technical occurrence against a stable path-and-heading locator before rewriting it; choose among the replacement category, a narrower prototype, a precise local noun, and deliberate preservation; then reconcile the result with a broad residual search. This ordering matters because a blind replacement cannot distinguish the captured technical sense from ordinary editorial use, named machinery, or historical wording, while a zero-hit goal would destroy those legitimate uses. Independent verification of the occurrence ledger supplies the missing check that a self-reported corpus rewrite cannot provide. The same procedure applies when the successor is another category term, not only when a term is retired without replacement.

The `distillation` retirement ([ADR 053](../reference/adr/053-retire-distillation-without-a-successor-term.md)) is a worked instance of the full loop: a 464-occurrence audit detected two load-bearing senses with opposite maintenance regimes sharing one word, and remediation retired the term, moved the boundary into lineage link labels, and routed the ampliative traffic to a coined compound. The definition note that was supposed to hold the binding had existed, impotently, throughout the drift.
...
- [Natural-language content lacks reliable dereference, so facts need reinforcement at point of use](./prose-has-no-dereference-reinforce-facts-at-point-of-use.md) — contrasts: sibling "natural-language content lacks a formal-language operation" claim (no resolution vs. no reliable namespace)
