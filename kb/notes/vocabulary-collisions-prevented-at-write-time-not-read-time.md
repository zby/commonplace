...
Four candidate remedies offer different levels of assurance. Reader vigilance — noticing the clash at read time — is a useful backup but too silent and fallible to be the only control. Retrieval-time disambiguation — treating the collision as word-sense disambiguation for the composer to solve by assembling enough context that each occurrence resolves correctly — is the same control moved into the pipeline: resolution still happens by read-time inference, with the same silence when it fails. And it is weakest exactly where a KB needs it most. Classic disambiguation leans on sense inventories the model has seen many times; a KB's load-bearing senses are local coinages defined in single notes, so the retriever must ship the defining context with every use site, an excerpt that drops the qualifier silently restores the ambiguity, and the model's prior pulls toward the common reading. Symbolic disambiguation — per-note sense declarations ("in this note, X means…") that readers resolve before composing — can work, but adds work to every read and write, and a missing declaration silently restores the original ambiguity. Write-time prevention or visible scope — reserving each unqualified load-bearing term for one sense across co-loadable artifacts, and placing necessary reuse in a schema slot or linked clause frame — removes the dependence on read-time inference.
