...
- [agent orchestration occupies a multi-dimensional design space](./agent-orchestration-occupies-a-multi-dimensional-design-space.md) — synthesis: scheduler placement, persistence horizon, coordination primitives, and return artifacts vary independently, so architectures do not sit on a single ladder
- [execution traces are observational byproducts, not default handoff artifacts](./execution-traces-are-observational-byproducts-not-default-handoff-artifacts.md) — protocol-level claim: traces are useful for UI and debugging, but orchestration usually wants a compressed boundary artifact on a separate channel
