...
- [tool loop](./tool-loop-index.md) — the implementation-level claim: tool calls are not enough unless frameworks keep state progression and recursion exposed to application code
- [subtasks that need different tools force loop exposure in agent frameworks](./subtasks-that-need-different-tools-force-loop-exposure-in-agent-frameworks.md) — sharpens: the decisive boundary is when decomposition creates child tasks that need fresh tool surfaces rather than one fixed loop
- [semantic sub-goals that exceed one context window become scheduling problems](./semantic-sub-goals-that-exceed-one-context-window-become-scheduling-problems.md) — extends: some semantic goals are too large for one call and therefore require symbolic partitioning, staged aggregation, and possible recursion
- [stateful tools recover control by becoming hidden schedulers](./stateful-tools-recover-control-by-becoming-hidden-schedulers.md) — concession: hidden loops can recover substantial control, but only by relocating the scheduler into an exceptional tool/runtime
- [codified scheduling patterns can turn tools into hidden schedulers](./codified-scheduling-patterns-can-turn-tools-into-hidden-schedulers.md) — codification consequence: once next-step policy stabilizes into code, hiding it in tools collapses orchestration into covert runtime logic
...
- [rlm-has-the-model-write-ephemeral-orchestrators-over-sub-agents](./rlm-has-the-model-write-ephemeral-orchestrators-over-sub-agents.md) — RLM instantiates the symbolic-scheduler model by having the LLM write the scheduler as code; achieves clean separation but discards the scheduler after each run
