...
- [Any symbolic program with bounded calls is a select/call program](./any-symbolic-program-with-bounded-calls-is-a-select-call-program.md) — Any program whose symbolic execution between bounded LLM calls can be reified as explicit state can be mechanically converted into the select/call loop with the same call sequence
- [Context engineering](./definitions/context-engineering.md) — Definition — context engineering is the discipline of designing systems around bounded-context constraints; its operational core is routing, loading, scoping, maintenance, and observability for each bounded call
