...
- [A complete theory path does not establish improved capacity](./a-complete-theory-path-does-not-establish-improved-capacity.md) — grounds: gives the interpreter/retained-text split that makes a seed's yield the retention question
...
- [Revision guided by rationale needs faithfulness, not just legibility](./revision-guided-by-rationale-needs-faithfulness-not-just-legibility.md) — contrasts: examines repair that uses rationale as a diagnostic guide; a seed can reconstruct that guide without retaining every local explanation
