...
That hook is not a private invention of this argument. The [agent-harness survey](https://www.preprints.org/manuscript/202604.0428) elevates **lifecycle hooks** — pre/post-invocation interception points — to a named, first-class component of its six-part harness definition, alongside the execution loop itself. This is independent convergence on the same mechanism: the place application code reclaims control is a hook around the call, not a rewrite of the loop.

The layering is also shippable in production, not just a design ideal. [iii](https://x.com/mfpiccolo/status/2060069083878408689) ships the turn loop as one swappable layer among ~13–15 others, each addressed through a uniform `trigger()` call over name-based function ids, so the loop can be replaced without disturbing its neighbours. That makes thin-vs-thick a **slider, not a fork**: a position on the "framework owns progression ⟷ application owns progression" axis becomes a reconfiguration rather than a one-time architectural commitment — exactly what "keep the loop optional" asks for.

The requirement has since stopped being hypothetical: a flagship harness ships a partial instance. Claude Code's [dynamic workflows](../agentic-systems/reviews/claude-code-dynamic-workflows.md) expose a returning, per-call-parameterized `agent()` beneath the conversational loop, composed by ordinary JavaScript control flow — exactly the layering this note asks for. But the exposure is qualified on three axes: **audience** — the orchestrator script is model-authored per run, not an application-programmer surface; **substrate** — a sandboxed guest language with no filesystem or shell, so the scheduler cannot read its own working set; and **granularity** — capability surfaces come from an agent-type registry rather than per-call tool sets, and the script surface has neither a tool-execution hook nor a caller-supplied stop predicate. The withheld parts are precisely the capability-surface decisions this note centres, so the argument survives the counterexample in sharpened form: the question is no longer *whether* the loop gets exposed but on what terms — to whom, on what substrate, and at what granularity.
...
- [computational-model](../tags/computational-model-README.md) — prior framing: argues from expressivity loss; this draft restarts from why tool loops are a useful convenience layer in the first place
...
- [Agent Harness for Large Language Model Agents: A Survey](https://www.preprints.org/manuscript/202604.0428) — evidenced-by: independently makes lifecycle hooks a first-class harness component, converging on the middle layer this note proposes
- [How to build your own agent harness (iii)](https://x.com/mfpiccolo/status/2060069083878408689) — evidenced-by: production stack ships the loop as one swappable layer behind a uniform calling convention — thin-vs-thick as a slider, not a fork
...
- [Claude Code dynamic workflows](../agentic-systems/reviews/claude-code-dynamic-workflows.md) — evidenced-by: shipped partial instance of the requested layering — a returning `agent()` beneath the frozen loop — qualified on audience, substrate, and granularity
