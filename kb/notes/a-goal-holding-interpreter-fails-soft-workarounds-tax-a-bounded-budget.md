...
- [Apparent success is an unreliable health signal in framework-owned tool loops](./apparent-success-is-an-unreliable-health-signal-in-framework-owned.md) — extends: supplies the mechanism and the economics behind that note's weak health signal — the workaround consumes the failure event that detection would have needed
- [soft degradation often binds before the hard cap when required evidence fits](./soft-degradation-often-binds-before-the-hard-cap-when-evidence-fits.md) — grounds: the silent saturation mechanism the workaround budget rides on
