...
**The agent reader needs the claim.** An LLM reader can use an explicit completeness signal as a stopping rule. [Stale indexes reduce discovery when they suppress fallback search](./stale-indexes-reduce-discovery-when-they-suppress-fallback-search.md): if an agent treats a curated head as exhaustive and skips the by-tag sweep, a missing note lowers recall along that route even though other routes may still find it. So completeness stops being a nicety and becomes information the reader must have: `complete: true` is precisely the signal that tells the agent it may skip the by-tag sweep, and the absence of the mark tells it it may not. The reader now demands the guarantee the human reader could do without.
...
- [Stale indexes reduce discovery when they suppress fallback search](./stale-indexes-reduce-discovery-when-they-suppress-fallback-search.md) — grounds: the fallback-suppression mechanism that makes the agent reader need the completeness claim a human reader could do without
