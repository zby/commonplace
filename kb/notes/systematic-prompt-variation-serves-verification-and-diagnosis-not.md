...
The [reach note](./first-principles-reasoning-selects-for-explanatory-reach-over.md) uses "can you vary it?" in a different sense. Deutsch's test asks: Can you change a premise in the explanation? Can you predict what changes in the conclusion? Does that reveal causal structure that transfers beyond the original case?
...
- [first-principles reasoning selects for explanatory reach over adaptive fit](./first-principles-reasoning-selects-for-explanatory-reach-over.md) — contrasts: Deutsch's "vary" test changes the explanation to test mechanism and reach, not the prompt to test model behavior
