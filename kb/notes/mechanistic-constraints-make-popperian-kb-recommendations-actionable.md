...
This means criticism must be externalized into the notes themselves. Claim titles and [link semantics](./links-README.md) already do part of this — [title as claim enables traversal as reasoning](./title-as-claim-enables-traversal-as-reasoning.md), and typed links make foundation/extension/contradiction explicit. But the current system lacks two things: a way to state what would *defeat* a claim, and a systematic pass that looks for contradiction before looking for agreement.
...
- [deploy-time learning: the missing middle](./deploy-time-learning-is-the-missing-middle.md) — mechanism: instruction refinements persist across sessions
