...
Starting from familiar solution categories can reverse that order when their seams are treated as fixed. Labels such as prompt, database, memory, workflow, or weights bundle several decisions. These packages preserve useful engineering experience as priors, but comparing instances without unpacking them explores only the alternatives their seams admit. A later explanation may justify the selected package without ever exposing a rival decomposition. Since [a framework rule counts as an inherited first principle only when no rival preserves its boundary invariants](./framework-rule-is-a-first-principle-without-boundary-preserving-rival.md), constraints should bound the map; inherited packages should not silently determine the positions within it.
...
Agreement across rival maps is corroboration, not proof, because the derivations may share a hidden premise. Disagreement focuses investigation on their points of divergence but does not show which map is wrong. Nor does surviving several maps turn a requirement into a first principle; that status comes from inheritance from a boundary commitment. As [only derivation and inheritance warrant a decomposition's scope claim](./only-derivation-and-inheritance-warrant-a-scope-claim-use-earns-it.md), even a valid map holds only while its generating constraints and distinctions remain operative.
