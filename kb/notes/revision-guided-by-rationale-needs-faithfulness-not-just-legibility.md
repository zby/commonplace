...
[Concept bottleneck models](../sources/concept-bottleneck-models-paper-v3.ingest.md) supply a worked analogy. Their supervised concept layer is a legible intermediate the prediction passes through. In the paper's OAI experiments, correction used an oracle supplying true concept values. Replacing the nonlinear concept-to-label map with a linear one reduced correction benefit despite similar pre-intervention task and concept accuracies. In a separate ablation, a joint control model with concept-loss weight λ = 0.01 learned a less aligned concept representation, and correction slightly increased test error. These results distinguish the accuracy of a concept interface from how effectively its downstream predictor uses corrections. The experiment holds the supplied concept vocabulary fixed; transfer to retained natural-language rationales remains a target-side analogy.
...
- [Retained theories may improve sample efficiency under structured shifts](./retained-theories-may-improve-sample-efficiency.md) — grounds: the selective-revision payoff whose rationale-guided realization this note examines
...
- [Concept bottleneck models](../sources/concept-bottleneck-models-paper-v3.ingest.md) — evidenced-by: OAI oracle-correction experiments separate downstream response at similar initial accuracy from misalignment under low concept-loss weight
