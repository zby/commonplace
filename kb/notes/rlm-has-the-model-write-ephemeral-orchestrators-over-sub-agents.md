...
The model-authored orchestrators are structurally the same thing a programmer would write by hand — exactly the symbolic orchestration over sub-agents that the [tool loop](../tags/tool-loop-README.md) argument calls for. The elegance of RLM is how it packs this into the tool-loop model: the REPL tool gives the model a substrate for writing orchestrators without anyone having to write reentrant framework code.
...
- [tool loop](../tags/tool-loop-README.md) — boundary case: RLM implements the full solution (symbolic orchestration over sub-agents) by having the model author it in a REPL tool
