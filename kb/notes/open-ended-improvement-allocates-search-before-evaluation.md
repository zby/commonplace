...
- [Candidacy evidence licenses escalation to assessment, not acceptance](./candidacy-evidence-licenses-escalation-not-acceptance.md) — grounds: explains why evidence too weak for adoption can still warrant spending further assessment effort on a branch
- [Lightweight search control allocates further search without licensing adoption](./lightweight-search-control-does-not-license-adoption.md) — extends: names the limited-authority control role that allocates further work without turning an early branch choice into acceptance
- [A search controller is tested by what it brings to stronger evaluation](./a-search-controller-is-tested-by-what-it-brings-to-stronger-evaluation.md) — extends: evaluates allocation through downstream candidates, evidence, failures, and recovery without assuming exhaustive counterfactual search
- [A failure explanation becomes search control only when it changes a later branch decision](./failure-explanation-changes-later-branch-decisions.md) — mechanism: shows how evidence from one reached branch revises the allocation of later search
...
- [Holding a program theory means sustaining coherent search under delayed feedback](./program-theory-sustains-search-under-delayed-feedback.md) — extends: develops one candidate account of how open-ended program search can remain coherent before delayed evidence arrives
