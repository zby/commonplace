...
- **A human selecting a good output.** [Selecting an LLM output fixes a result, not its interpretation](./selecting-an-llm-output-fixes-a-result-not-its-interpretation.md): the generator produced candidates, and the selection determines which result becomes the maintained artifact without proving that its text has one meaning.
...
A tag-README carries curated editorial prose *and* a validator-enforced `complete` mark. The mark is a mechanical derivation; the curation is a commitment. Both live in one file, and the file's disposal behaviour is per-region: dropping a mark costs a reader one scoped sweep, dropping the curation destroys editorial judgment nothing re-derives. This is why [ADR 026](../reference/adr/026-tag-readme-type-with-completeness-and-coverage-marks.md) could put a validator behind the mark without pretending to validate the prose around it. The [lineage vocabulary](../reference/link-vocabulary.md) handles the same fact at the artifact scale by labelling a mixed artifact by its dominant regime — an explicit, revisable call. When classifying, ask which regime a given *claim inside* an artifact belongs to; the file is often mixed.
...
- [Selecting an LLM output fixes a result, not its interpretation](./selecting-an-llm-output-fixes-a-result-not-its-interpretation.md) — grounds: selecting which generated result becomes the maintained record is a commitment that cannot be recovered from the prompt alone
...
- [Retaining the episode keeps a distilled rule re-derivable](./retaining-episode-evidence-keeps-a-distilled-rule-open-to.md) — contrasts: re-examining a committed generalization against its retained evidence is a fresh arbitration that may disagree with it, unlike a recompute
