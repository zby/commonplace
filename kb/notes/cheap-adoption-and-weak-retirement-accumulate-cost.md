...
Each dependant can make an adopted structure more useful now and more expensive to remove later. That expense does not establish that the structure transfers to new questions. [Current-task fit alone does not warrant costly structural entrenchment](./current-task-fit-alone-does-not-warrant-costly-entrenchment.md) separates those propositions: adoption by more tools or notes raises migration cost without earning wider scope.
...
Once a committed artifact already has a chosen disposition, [Retire an artifact](../instructions/retire-artifact.md) supplies a concrete extraction, inbound-reference, redirect, and freshness-retirement procedure. That procedure starts with a named target. It does not supply the earlier signal, decision owner, or rule that makes accumulated structure enter retirement review.

## Bounded observation from agent-memory systems

The [comparison of 148 code-grounded agent-memory systems](../agent-memory-systems/agentic-memory-systems-comparative-review.md) reports 139 automatic writers. Nineteen perform no curation operation, while only seven perform all seven tracked lifecycle operations. This is bounded evidence that complete lifecycle machinery is rare in that reviewed corpus even when writing is automatic. It does not measure the cost caused by missing operations, establish a universal wiki law, or show that every partial lifecycle is inadequate.
...
- Which signals show that a structural element has outlived its question, and what rule should make that signal trigger review or retirement rather than passive accumulation? [Operational signals that a component is a relaxing candidate](./operational-signals-that-a-component-is-a-relaxing-candidate.md) develops signals for the narrower case of a brittle proxy component, but not for structural obsolescence in general. [Automating KB learning is an open problem](./automating-kb-learning-is-an-open-problem.md) places retirement among the judgment-heavy mutations whose oracle remains unresolved.
...
- [Raw accumulation does not create usable memory](./raw-accumulation-does-not-create-usable-memory.md) — contrasts: that note explains why accumulated content needs ingress and lifecycle pressure; this note explains how the structural machinery itself accumulates when retirement lags adoption
- [Maintenance capacity must match harmful-artifact inflow](./maintenance-capacity-must-match-harmful-artifact-inflow.md) — contrasts: harmful-artifact inflow creates a quality-control queue, while locally sound structures can create joint routing and migration cost through dependency accumulation
- [Operational signals that a component is a relaxing candidate](./operational-signals-that-a-component-is-a-relaxing-candidate.md) — extends: develops one narrower class of signals that can move a brittle proxy component toward replacement
- [Retire, Redact, Supersede, And Relax Memory](./agent-memory-requirements/retire-redact-supersede-relax.md) — grounds: names the lifecycle operations whose operative absence leaves stale material and structure in active use
- [Current-task fit alone does not warrant costly structural entrenchment](./current-task-fit-alone-does-not-warrant-costly-entrenchment.md) — extends: supplies the adoption-versus-entrenchment distinction and the migration-cost boundary this lifecycle mechanism develops
- [Retire an artifact](../instructions/retire-artifact.md) — see-also: executes retirement after a committed artifact and its disposition are already known, without supplying the trigger this note leaves open
