...
- [computational-model](../tags/computational-model-README.md) — consequence: framework design concerns one dimension of the larger space
...
- [Claude Code dynamic workflows](../agentic-systems/reviews/claude-code-dynamic-workflows.md) — evidenced-by: spans the persistence-horizon axis from one authoring point — a model-authored orchestrator script, ephemeral by default, promotable to a cross-session reusable `/command` — and its withheld decisions (audience, substrate, capability-surface granularity) populate further independent axes from one coherent system
