...
- [Claude Code dynamic workflows](../sources/claude-code-dynamic-workflows-docs.md) — evidence: spans the persistence-horizon axis from one authoring point — a model-authored orchestrator script, ephemeral by default, promotable to a cross-session reusable `/command` — showing authorship and persistence vary independently
