...
- **Prose** — human-authored or trace-extracted heuristics in prompts, playbooks, notes, or skills.
...
Representational form does not say how the policy was acquired. A prose policy may be written upfront, generalized from execution traces, or generated by an agent after failure analysis. The LongCoT-mini RLM case shows the trace-extracted prose point: Claude Code inspected failing RLM trajectories and produced prompt tips that shifted a free-form RLM away from brute force and toward checked graph decomposition. That is not yet trained policy and not durable verified code, but it is more structured than ad hoc prompting.
...
- [Ingest: A Mini Exercise on the Mismanaged Geniuses Hypothesis (RLMs on LongCoT)](../sources/mini-exercise-mismanaged-geniuses-longcot-rlm.ingest.md) — evidence: trace-extracted prompt tips occupy a prose decomposition-policy point between hand-authored heuristics and trained policy
