...
- [Ingest: A Mini Exercise on the Mismanaged Geniuses Hypothesis (RLMs on LongCoT)](../sources/mini-exercise-mismanaged-geniuses-longcot-rlm.ingest.md) — evidence: trace-extracted prompt tips occupy a natural-language decomposition-policy point between hand-authored heuristics and trained policy
- [Ingest: The Y-Combinator for LLMs](../sources/the-y-combinator-for-llms-solving-long-context-rot.ingest.md) — evidence: λ-RLM restricts RLM orchestration through a typed symbolic combinator runtime without changing the per-run persistence boundary
