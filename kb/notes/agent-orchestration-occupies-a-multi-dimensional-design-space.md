...
Representational form does not say how the policy was acquired. A natural-language policy may be written upfront, generalized from execution traces, or generated by an agent after failure analysis. The LongCoT-mini RLM case shows the trace-extracted natural-language point: Claude Code inspected failing RLM trajectories and produced prompt tips that shifted a free-form RLM away from brute force and toward checked graph decomposition. That is not yet trained policy and not durable verified code, but it is more structured than ad hoc prompting.

RLM illustrates another mixed point: the model authors symbolic orchestrator code, but the result is ephemeral rather than a durable repo artifact. λ-RLM takes a different route by restricting orchestration to a pre-verified symbolic combinator library, improving verifiability without changing the per-run persistence boundary. The Mismanaged Geniuses hypothesis points at yet another point: codify the decomposition language, then relax the policy that chooses within it into a distributed-parametric learned artifact. Representational form, acquisition path, storage substrate, and persistence can therefore vary independently even when the visible scheduler shape looks similar.
...
- [Ingest: A Mini Exercise on the Mismanaged Geniuses Hypothesis (RLMs on LongCoT)](../sources/mini-exercise-mismanaged-geniuses-longcot-rlm.ingest.md) — evidenced-by: trace-extracted prompt tips occupy a natural-language decomposition-policy point between hand-authored heuristics and trained policy
- [Ingest: The Y-Combinator for LLMs](../sources/the-y-combinator-for-llms-solving-long-context-rot.ingest.md) — evidenced-by: λ-RLM restricts RLM orchestration through a typed symbolic combinator runtime without changing the per-run persistence boundary
