...
- [Claude Code dynamic workflows](../agentic-systems/claude-code-dynamic-workflows.md) — evidence: spans the persistence-horizon axis from one authoring point — a model-authored orchestrator script, ephemeral by default, promotable to a cross-session reusable `/command` — and its withheld decisions (audience, substrate, capability-surface granularity) populate further independent axes from one coherent system
