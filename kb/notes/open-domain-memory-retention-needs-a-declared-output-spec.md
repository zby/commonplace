...
- [Elicitation requires maintained question-generation systems](./elicitation-requires-maintained-question-generation-systems.md) — enables: its seed/enforce/log-misses/abstract/prune loop is the maintenance machinery this note's open question about keeping the output spec current still needs
- [What the matrix shows across 148 agent memory systems](../agent-memory-systems/agentic-memory-systems-comparative-review.md) — evidenced-by: the automatic-capture-and-push camp versus the curated-pull-only camp found across 148 reviewed systems corroborates the input-driven/output-driven split at aggregate scale
