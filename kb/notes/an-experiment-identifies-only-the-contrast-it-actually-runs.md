...
If an absent-instruction condition differs only by the instruction-bearing component, then the component is the treatment grain. A finer claim about semantic uptake needs a finer contrast. If omission also removes a delivery slot, changes prompt length, suppresses retrieval, or changes the opportunity to act, then the intervention remains a bundle. Empty, shuffled, irrelevant, corrupt, filler, and replacement controls can hold different non-target properties fixed, so they answer narrower but distinct questions. Whether any one of them isolates semantic content depends on which other relevant properties it actually preserves, as the self-evolver intervention design demonstrates ([paper](https://arxiv.org/html/2601.22436v3), [local analysis](../sources/llm-agents-are-not-always-faithful-self-evolvers.ingest.md)).
