...
Harness-IF makes this problem visible. Its zero-injection probes withhold a target rule and show that unprompted behavior can already align with the rule. Its aggregate-to-AP-Acc gap supports prior-alignment inflation in the benchmark score, but AP-Acc compares different rule strata. It is not a paired estimate of the effect of injecting each rule on matched opportunities ([captured paper](../sources/harness-if-instruction-following-across-instruction-surfaces.md), [local analysis](../sources/harness-if-instruction-following-across-instruction-surfaces.ingest.md)).

If an absent-instruction condition differs only by the instruction-bearing component, then the component is the treatment grain. A finer claim about semantic uptake needs a finer contrast. If omission also removes a delivery slot, changes prompt length, suppresses retrieval, or changes the opportunity to act, then the intervention remains a bundle. Empty, shuffled, irrelevant, corrupt, filler, and replacement controls can hold different non-target properties fixed, so they answer narrower but distinct questions. Whether any one of them isolates semantic content depends on which other relevant properties it actually preserves, as the self-evolver intervention design demonstrates ([captured paper](../sources/llm-agents-are-not-always-faithful-self-evolvers.md), [local analysis](../sources/llm-agents-are-not-always-faithful-self-evolvers.ingest.md)).
...
Memento-Skills compares its full system with a Read-Write condition that retains skill retrieval, execution, and feedback while jointly disabling failure attribution, skill rewriting, and skill discovery. For Gemini-3.1-Flash, the full system reports 66.0% test accuracy against 52.3% for Read-Write on GAIA, and 38.7% against 17.9% on HLE ([captured paper](../sources/memento-skills-let-agents-design-agents.md), [local analysis](../sources/memento-skills-let-agents-design-agents.ingest.md)).
