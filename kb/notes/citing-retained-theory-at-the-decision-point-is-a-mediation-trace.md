...
- [A theory-mediated loop closes only by causal co-indexing, not by co-occurrence inside one boundary](./a-theory-mediated-loop-closes-only-by-causal-co-indexing.md) — extends: the closure test a citation plus read-back serves; a citation alone is a pointer, the four co-indexed links are the loop
