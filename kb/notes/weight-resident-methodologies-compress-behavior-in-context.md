...
The compression is not equivalent to an import from an explicit library. A methodology name is not a stable pointer to a canonical object. What becomes active is the model's learned reconstruction, which may vary by model, context, or interpretation. This competence can contribute to an automated decision without becoming an explicit project methodology. [Explicit retention provides direct targets for selective revision](./only-explicit-retention-is-durable-writable-and-addressable.md); a compact cue instead controls the conditions under which the model reconstructs the method. Neither path guarantees the intended interpretation.
...
- [Explicit retention provides direct targets for selective revision](./only-explicit-retention-is-durable-writable-and-addressable.md) — contrasts: model reconstruction can govern behavior without supplying the same revision targets as a retained method
