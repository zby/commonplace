...
- **The `mark` concept** — `complete`/`covered_by` on tag-READMEs — is a validated symbolic cache consumed by agents. The membership query is recomputable but costs a scoped sweep on every read; the mark materializes the answer so the agent reads it, and a validator re-derives it so it can't silently rot (the mark contract is in the [`tag-readme` type spec](../types/tag-readme.md)). Value plus safety, both halves present. The name *mark* fits this whole pattern — a validated cache read by an agent to spare a recompute — not just the tag-README fields. **TODO:** promote *mark* to its own definition note (`./definitions/mark.md`) once a second instance ships; with one example, the definition would just restate this bullet.
...
**The default has a volume condition, and it can flip back.** Materializing pays only when reading the materialized form costs less than the recompute path it replaces. [Frontloading states the same condition at instruction scale](./frontloading-spares-execution-context.md) — on raw volume it saves only when the inserted result is smaller than the material it replaces. The sharper form is a comparison of *selectable units* rather than whole artifacts: [a materialized layer helps only when its smallest addressable unit is finer than the source's](./addressability-grain-not-compression-ratio-decides-whether-a.md). This note's instances sit far on the safe side — a boolean mark stands in for a scoped sweep over hundreds of files — but the condition is what stops the default being read as unconditional, and it bites hardest on prose summaries, whose selectable unit is a section where the source's is a symbol.
...
- [Addressability grain, not compression ratio, decides whether a summary layer helps](./addressability-grain-not-compression-ratio-decides-whether-a.md) — contrasts: supplies the condition under which this note's default flips back, by comparing selectable units rather than whole artifacts
