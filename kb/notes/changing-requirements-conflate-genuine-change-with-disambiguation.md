...
- Are there spec-writing practices (from [legal drafting](./legal-drafting-solves-the-same-problem-as-context-engineering.md) or elsewhere) that specifically reduce cascading interpretation errors rather than just reducing ambiguity in a single spec? The emerging answer seems to be lifecycle-dependent: DeAngelis's [Spec Driven Development](https://x.com/juliandeangeIis/status/2033303156340240481) reduces ambiguity when the intent is knowable upfront, while Augment's [bidirectional spec pattern](https://x.com/augmentcode/status/2025993446633492725) catches interpretation choices discovered during execution. [Specification strategy should follow where understanding lives](./specification-strategy-should-follow-where-understanding-lives.md) unifies these as different responses to where the missing knowledge currently sits.
...
- [Augment bidirectional spec](https://x.com/augmentcode/status/2025993446633492725) — exemplifies: agents surface interpretation choices back to the spec in real time, a concrete practice for reducing cascading disambiguation errors
