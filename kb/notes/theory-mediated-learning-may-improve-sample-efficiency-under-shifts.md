...
This is also the central risk. A broad but wrong theory produces broader negative transfer than a local association does, because it is wrong just as widely as it would have been useful. Theory-mediated learning helps only where the system can distinguish explanatory structure from a generalization that merely fits the source cases — [stating where a lesson stops](./abstract-an-experience-only-when-you-can-state-the-boundary.md) is itself a judgment, not a derivation from the evidence.
...
So the contrast is not neural versus symbolic, nor computational expressivity. It is **purely parametric adaptation versus learning that can operate on theories as theories**, and the axis it runs along is [representational form](./definitions/representational-form.md). An LLM using retained prose is a hybrid on that axis: the weights supply the semantic interpreter, the prose supplies persistent identity and selective revisability. Addressability also comes in degrees — a theory addressable only as an indivisible document can be replaced or deleted but not rescoped by one premise, which requires its content, assumptions, and applicability conditions to be separately accessible.

Symbolic systems implement the same pathway wherever the theory space and acceptance criteria are formalized: causal discovery searches for structure whose consequences survive specified interventions, and [DreamCoder](../sources/dreamcoder-wake-sleep-bayesian-program-learning.ingest.md) grows a library of reusable program abstractions that compound across a task family. DreamCoder also shows what formalization buys the acceptance test: its retention gate is statistical — an abstraction is kept when it reduces description length — so the decision to retain is mechanical. Nothing plays that role for a theory still in prose, where stating where the theory stops remains a judgment. Their limitation is not an inability to reason about theories but dependence on a supplied language, variables, primitives, search procedure, and acceptance test — worked out in [formal symbolic systems assess explanatory-reach only through causal and proof obligations](./formal-systems-assess-explanatory-reach-through-causal-and-proof.md). LLMs offer a different trade: a much more open-ended natural-language theory space before formalization, with weaker guarantees.
...
The bitter lesson is often read as a third objection here, and on inspection it is not one. What it prescribes is betting on methods that scale with compute — search and learning — against humans hand-encoding domain structure. A system that autonomously searches theory space, derives consequences, and tests them is search and learning; the theories it retains are not hand-supplied priors. The objection that survives is narrower and lands on the retention layer rather than on theory-mediation: a sufficiently scaled system might run the same search implicitly in activations, with no externalized addressable object anywhere, in which case the artifact is scaffolding that scale eventually absorbs. That is a live worry about representational form, and nothing here rebuts it — [and which side of that boundary a component sits on is not identifiable until scale tests it](./codification-and-relaxing-navigate-the-bitter-lesson-boundary.md), which is why the question is empirical rather than settled by argument.
