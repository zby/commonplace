...
Framework-integrated online trace learning that promotes one live conversation into either bounded profile knowledge or reusable procedural skills, then maintains the learned skill library through a separate lifecycle curator.

**Trigger.** Independent turn counters request memory or skill review after a completed foreground turn; interrupted turns skip review. A daemon-thread agent fork performs the review without adding its prompt or messages to the foreground history. Skill lifecycle maintenance runs on a slower periodic or explicit curator pass.

**Source format.** The fork consumes an in-memory snapshot containing user and assistant messages plus tool calls and results. It normally replays the full snapshot on the main model's warm prompt prefix; a separately routed auxiliary model receives a compact digest plus the most recent messages. The authoritative raw conversation also persists in a profile-scoped SQLite session database with FTS indexes.
...
- [Designing a Memory System for LLM-Based Agents](../notes/designing-agent-memory-systems.md) — rests-on: turns the survey's repeated techniques into a needs map for direct-authored and trace-extracted memory, including capture, activation, promotion, lifecycle, and evaluation
