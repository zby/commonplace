...
- **Operations** — which store-changing operations the system performs on memory **already in the store**, beyond trivial create/update/delete. *Acquisition* — creating an entry from raw material, whether extracting from a document (that is Lineage `imported`) or distilling from a trace (that is `trace-derived`) — is **not** a curation operation, however sophisticated the extraction; record it on those axes. Each token below is a distinct design choice:
