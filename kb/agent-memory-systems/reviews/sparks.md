...
- **Lineage:** `authored` `imported` — Humans author inbox/raw material and agents author wiki pages, logs, tasks, and descriptions; Sparks imports that material into manifest rows, parsed frontmatter, link edges, archived inbox files, collection pages, and status/query/brief reports. It does not automatically derive durable semantic artifacts from agent traces, so `trace-extracted` is not assigned.
