...
- **Representational form:** `natural-language` `symbolic` — Wiki bodies, memory text, raw captures, source summaries, and skills are natural-language, while frontmatter, wikilinks, page categories, review fields, schema markers, operation journals, backlinks JSON, MCP tool schemas, and search indexes are symbolic.
...
**Wiki pages.** Storage substrate: `wiki/**/*.md` plus `_backlinks.json`, `index.md`, `log.md`, and `_link_schema.json`. Representational form: natural-language pages with symbolic frontmatter, wikilinks, types, aliases, tags, source counts, maturity, dates, and graph indexes. Lineage: authored or imported into the wiki from raw sources, then indexed from current file contents. Behavioral authority: knowledge when returned through `search_wiki`, `get_context`, `query_link`, or the viewer; routing/ranking when titles, aliases, tags, and links select later context.

**Memory pages.** Storage substrate: `wiki/memories/*.md`. Representational form: natural-language memory bodies wrapped in symbolic frontmatter for `memory_type`, `scope`, `visibility`, `status`, `review_status`, project, source, review/expiry dates, and tags. Lineage: authored through explicit `remember_memory`/CLI calls or trace-extracted from accepted capture proposals. Behavioral authority: knowledge and personalization context; review status, active/stale/archive state, expiry, project scope, and visibility affect recall and sharing.

**Raw captures and proposals.** Storage substrate: `raw/memory-captures/*.md` and transient proposal payloads returned by CLI/MCP. Representational form: natural-language captured notes plus symbolic frontmatter and proposal records. Lineage: trace-extracted from chat/session notes; accepted proposals become memory pages with the capture path as source. Behavioral authority: learning and review input until accepted, then knowledge.
...
**Distilled form:** `natural-language` `symbolic` — Accepted memories become natural-language memory pages with symbolic frontmatter, review state, scope, visibility, tags, source, and lifecycle metadata.
