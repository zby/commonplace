...
- **Representational form:** `natural-language` `symbolic` — Article text, references, source-cache facts, changelog summaries, and the Claude prompt are natural-language; JSON manifests/contracts/caches, HTML ids, stable paragraph comments, shell functions, CLI flags, freshness checks, and sync markers are symbolic. I found no vector store, embedding index, learned ranker, or model-weight artifact in the repository.
