...
- **Representational form:** `natural-language` `symbolic` `parametric` — Documents, chunks, fact text, observations, mental-model content, directives, prompts, and recalled context are natural-language; bank ids, document ids, tags, metadata, schemas, API/tool definitions, links, timestamps, operation records, source ids, and validation/config records are symbolic; embeddings, HNSW/vector distance, BM25 scores, cross-encoder scores, cached LLM prefixes, and learned reranking signals are parametric.
...
**Documents, chunks, and raw memory units.** Storage substrate: relational rows in `documents`, `chunks`, and `memory_units`. Representational form: original/chunk natural-language content plus symbolic ids, tags, metadata, fact type, timestamps, document ids, and embeddings. Lineage: imported content or trace-extracted transcript content, then LLM-extracted into facts. Behavioral authority: source evidence for recall, consolidation, mental-model refresh, transfer, and audit.
...
Hindsight is stronger where an application needs low-friction runtime memory, automatic extraction, cross-framework integration, user/session scoping, and prompt-time recall. Commonplace is stronger where durable knowledge should be inspectable as authored natural-language content with explicit source citations, review comments, and deterministic validation.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Trace content becomes natural-language facts/observations/mental-model text, symbolic entities/links/tags/source ids/operations, and parametric embeddings/reranker scores/cached-prefix behavior.
...
**The README's learning claim is implementation-backed, but not review-backed.** There is real trace-extracted consolidation into observations and mental models. The missing piece, from a Commonplace perspective, is a human-readable rationale artifact for each learned observation/update.
...
- [Use trace extraction as meta-learning](../../notes/agent-memory-requirements/use-trace-extraction-as-meta-learning.md) - exemplifies: coding-agent transcripts and application events become learned observations for later tasks.
