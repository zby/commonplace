...
**Distilled form:** `natural-language` `symbolic` — Raw eval/session events become symbolic attempts, score histories, checkpoint links, logs, and heartbeat triggers; agents are prompted to distill those traces into natural-language notes, skills, roles, focus notes, and synthesis files. There is no code-grounded parametric distilled form.
...
- Whether CORAL adds automatic semantic distillation from logs or attempts into notes/skills; that would move it from trace recording plus prompts toward a stronger trace-learning system.
...
- [Use trace extraction](../../notes/agent-memory-requirements/use-trace-extraction-as-meta-learning.md) - relates: CORAL records eval/session traces and prompts agents to distill them, while keeping semantic extraction mostly manual.
