...
**Direction.** CORAL is both pull and push from the agent's perspective. The agent pulls attempts, diffs, notes, skills, logs, and checkpoint history with CLI commands or file reads. The manager pushes generated instructions at startup/resume and pushes eval-result plus heartbeat prompts after scored attempts.

**Trigger and relevance signal.** The strongest push path is heartbeat. Triggers are not semantic retrieval over note content; they are engineered control signals: per-agent eval count, global eval count, real-versus-tune budget class, score history, plateau streak, score direction, and optional epsilon thresholds. The selected action name (`reflect`, `consolidate`, `pivot`, `lint_wiki`, or custom actions) is a typed cue that determines which prompt is injected.
