...
- **Representational form:** `natural-language` `symbolic` — Wiki pages, raw/normalized source Markdown, prompts, AGENTS policy, newsletters, papers, logs, and conflict notes are natural-language; frontmatter, JSON extraction outputs, SQLite schemas, monitor state, priority constants, prompt registries, route schemas, git commit hashes, status fields, and command schedules are symbolic. The repo uses LLMs and BM25 ranking, but I did not find a retained embedding or model-weight store in this checkout.
...
**Synthesis outputs.** Newsletter and paper agents read recent or topic-matched wiki pages, ask an LLM to synthesize a report or paper, and save Markdown outputs with frontmatter ([tools/newsletter_agent.py](https://github.com/kenhuangus/llm-wiki/blob/d2bd485921f6c7b6071afbb389f44ab8e4e83c69/tools/newsletter_agent.py), [tools/paper_agent.py](https://github.com/kenhuangus/llm-wiki/blob/d2bd485921f6c7b6071afbb389f44ab8e4e83c69/tools/paper_agent.py)). This is an automatic promotion from compiled wiki memory into a higher-level report or paper, but source-span provenance is coarse.
...
**Distilled form:** `natural-language` `symbolic` — Distilled outputs include experiment Markdown, research hypothesis entries, generated newsletters/papers, validation reports, and SQLite rows summarizing extraction/integration quality.
...
**Authority at consumption.** Search results and article reads are advisory knowledge. Lint, validation, prompt optimization, and daemon schedules have stronger operational authority because their outputs can mark conflicts, change confidence, generate experiments, or trigger further writes. Generated newsletters and papers are natural-language outputs with high apparent confidence metadata, but their source grounding is coarse.
...
**Research papers are memory consumers and memory products.** Paper generation reads compiled wiki pages and emits new Markdown papers. That can be valuable as synthesis, but without stronger provenance it can also launder low-confidence wiki claims into polished natural-language.
