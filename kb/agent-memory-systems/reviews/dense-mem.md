...
- **Representational form:** `natural-language` `symbolic` `parametric` — Fragment content, summaries, and evidence are natural-language; triples, policies, schemas, statuses, scopes, audit rows, import decisions, and graph relationships are symbolic; embeddings and verifier/embedding model outputs are parametric behavior-shaping state.
- **Lineage:** `authored` `imported` `trace-extracted` — Tool schemas, gates, query guards, and service code are authored; documents, historical summaries, manual fragments, and skill-pack JSON are imported; live conversation evidence and host-extracted claims are trace-extracted into fragments, claims, and facts.
...
The deepest difference is where truth becomes machine state. Commonplace preserves source citations and asks agents/reviewers to reason over natural-language. Dense-Mem turns host-extracted assertions into facts only after evidence entailment and predicate policy. That gives Dense-Mem clearer operational state, but it also means memory quality depends on host extraction and verifier calibration.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Raw evidence becomes natural-language fragments, symbolic claims/facts/skill-pack items/community summaries, and persisted fragment embeddings used for later ranking.
...
- Whether community summaries move from deterministic rendering to LLM-generated natural-language content. That would add another derived artifact needing lineage and quality controls.
