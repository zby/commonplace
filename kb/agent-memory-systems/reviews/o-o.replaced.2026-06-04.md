...
> Replaced 2026-06-04. See [o-o](./o-o.md) for the current review.
...
**Read-back:** `pull` — A user, index command, cron job, or other external runner invokes a document, then the update agent reads the embedded contract, source cache, and changelog; the inspected code does not implement relevance-gated pre-action memory push.
