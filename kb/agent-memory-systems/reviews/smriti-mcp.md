...
- **Representational form:** `natural-language` `symbolic` — Memory bodies, recall bundles, snippets, and generated index lines are natural-language; YAML frontmatter, ids, categories, tags, status, scope, source metadata, trace NDJSON, wikilinks, links/backlinks, access counts, salience, health reports, MCP tool schemas, and machine index YAML are symbolic. The inspected code does not implement embeddings, model weights, or another parametric retained form.
...
**Distilled form:** `natural-language` `symbolic` — Consolidated outputs are Markdown natural-language content plus symbolic metadata, trace sources, ids, tags, scope, status, salience, and links. There is no parametric learned state in the inspected code.
