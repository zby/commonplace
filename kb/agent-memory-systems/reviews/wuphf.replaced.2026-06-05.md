...
> Replaced 2026-06-05. See [WUPHF](./wuphf.md) for the current review.
...
- **Representational form:** `prose` `symbolic` — Prose Markdown and prompt/context blocks, plus symbolic paths, wikilinks, categories, authorship, git commits, fact/index records, workflow state, tool schemas, and compiled skill artifacts
- **Lineage:** `authored` `imported` `trace-extracted` — Authored by agents or humans, migrated from legacy backends, promoted from notebooks, and extracted from artifacts, workflow events, learnings, executions, lint, and wiki history
- **Behavioral authority:** `knowledge` `instruction` `enforcement` `routing` `validation` `ranking` `learning` — Wiki and notebook content provide knowledge; playbooks and compiled skills instruct; workflows and permissions enforce/route; lint and extraction validate; indexes rank/retrieve; learning and execution logs feed later synthesis
...
**Wiki fact logs, entities, graph, and indexes.** Storage substrate: JSONL fact/entity logs, `WikiIndex` stores, text index, typed fact store, and generated catalog/cache state. Representational form: symbolic triples/facts/entities plus indexed text and ranking state. Lineage: extracted from committed artifacts and wiki material, merged/reinforced through resolver logic, invalidated by source edits or extraction/reconcile changes. Behavioral authority: ranking and retrieval system-definition artifacts; facts shape lookup answers, lint checks, typed graph walks, and entity briefs, while the source Markdown remains the human-readable evidence layer.
...
**Team learnings, playbooks, compiled skills, and execution logs.** Storage substrate: wiki-backed learning log/index, `team/playbooks/*.md`, compiled playbook skill files, and append-only execution logs. Representational form: prose guidance, symbolic metadata, and compiled skill artifacts. Lineage: authored playbooks, recorded executions, explicit team learning records, and synthesis jobs. Behavioral authority: playbooks and compiled skills can become instructions for future agents; execution records and learnings are knowledge artifacts until a future tool or synthesis path reads them back into a task.
...
**Learning timing:** `online` `staged` — Agents can capture records during work, while promotion, extraction, reconciliation, lint, and playbook synthesis happen after writes or in later cycles.

**Distilled form:** `prose` `symbolic` — Distilled outputs are readable Markdown/wiki/notebook/playbook surfaces plus symbolic fact logs, indexes, workflow state, lint findings, metadata, and compiled skill artifacts.
