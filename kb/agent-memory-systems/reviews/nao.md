...
- **Representational form:** `natural-language` `symbolic` `parametric` — RULES, docs, table descriptions, story markdown, memories, prompts, and skills are natural-language; YAML config, schema/type metadata, folder/share/favorite rows, SQL filters, tool schemas, story tags, query ids, cron schedules, permissions, and migration tables are symbolic; LLM-generated memory extraction and AI summaries use parametric model judgments, though nao does not retain embeddings as a visible memory index.
...
**User memories.** The `memories` table stores normalized natural-language content, category, user id, optional source chat id, timestamps, and `supersededBy`. The operative split is important: `personal_fact` memories are advisory knowledge, while `global_rule` memories become instruction-like because the system prompt labels them as "Global User Rules" and inserts them into the model call. Supersession is retained as a pointer from old row to new row rather than hard overwriting every old fact.
...
**Distilled form:** `natural-language` `symbolic` — The durable output is natural-language memory content plus symbolic category, user id, source chat id, timestamps, and supersession links.
...
The repository has two memory systems sharing one product: a project context builder for analytics work and a personal-memory extractor for user preferences/facts. The former is inspectable and tool-pulled; the latter is compact, trace-extracted, and prompt-pushed.
