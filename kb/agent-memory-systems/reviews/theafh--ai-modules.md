...
**Distilled form:** `natural-language` `symbolic` — The distilled output is natural-language wiki content with symbolic frontmatter, paths, page types, tags, links, source references, contradiction markers, index entries, and log entries.
...
**Survey relation.** `ai-modules` strengthens the survey category where trace-learning is not an autonomous memory daemon: a human-visible session transcript is mined into proposed durable natural-language, then governance gates decide what survives. It also splits raw trace retention from distilled behavior authority; the visible session is evidence, while approved wiki pages or procedures are the retained artifacts that later change behavior.
