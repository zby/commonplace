...
- **Representational form:** `natural-language` `symbolic` — Raw and wiki Markdown carry natural-language; frontmatter, scenes/types, wikilinks, `@include` directives, context registries, lint reports, prompt templates, and CLI command contracts provide symbolic structure.
...
**Distilled form:** `natural-language` `symbolic` — The first-stage output is Markdown transcript/summary natural-language content with symbolic frontmatter, source path, timestamps, turn count, and role headings.
