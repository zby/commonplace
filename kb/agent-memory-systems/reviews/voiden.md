...
- **Representational form:** `natural-language` `symbolic` — Markdown docs, request notes, extension readmes, generated skills, and response/history text are natural-language; YAML block payloads, node types, UIDs, manifests, settings, environment variables, request pipeline hooks, assertions, scripts, stitch configs, history JSON, and Git status are symbolic. I did not find retained embeddings or learned parameters in this revision.
...
**`.void` request documents.** The operative artifact is a hybrid Markdown/YAML document: natural-language carries API documentation and intent, while fenced `void` blocks carry executable node structure. The app can parse, render, modify, execute, and serialize those blocks, so the symbolic fields have direct operational consequences.
