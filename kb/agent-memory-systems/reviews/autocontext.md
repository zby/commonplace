...
- Whether the MLX/CUDA distillation produces practically useful scenario-specific models at scale. If yes, it validates [deploy-time learning via weight modification](../../notes/deploy-time-learning-is-the-missing-middle.md); if no, the artifact-only path stays dominant.
...
- [deploy-time-learning-the-missing-middle](../../notes/deploy-time-learning-is-the-missing-middle.md) — extends: playbook accumulation, typed lessons with lifecycle, harness mutations, and MLX distillation are four distinct deploy-time learning mechanisms within one system (three artifact-layer, one weight-layer)
