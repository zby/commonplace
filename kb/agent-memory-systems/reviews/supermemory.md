...
**Distilled form:** `natural-language` `symbolic` `parametric` — Outputs include natural-language memory facts, profile facts, summaries, and prompt blocks; symbolic relations, version flags, forget fields, metadata, project tags, and processing states; and embeddings/similarity/rerank signals for retrieval.
