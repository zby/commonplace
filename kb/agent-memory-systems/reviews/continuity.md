...
- **Representational form:** `natural-language` `symbolic` — Memory content, narrative briefings, learning observations, tool descriptions, and injected prompt text are natural-language; row schemas, types, scopes, tags, source labels, versions, soft-delete fields, relationship types, tool schemas, sync cursors, and plugin capabilities are symbolic. I did not find embedding/vector indexes or model-weight updates in the reviewed memory path.
...
**Distilled form:** `natural-language` `symbolic` — Raw memory and learning observations become natural-language narrative briefings and symbolic sections/confidence/version/snapshot metadata. There is no parametric distilled artifact in the reviewed code path.
