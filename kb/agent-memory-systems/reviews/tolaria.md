...
- **Representational form:** `natural-language` `symbolic` — Note bodies, AGENTS guidance, bundled docs, prompts, transcripts, and generated agent replies are natural-language; YAML frontmatter, type documents, wikilinks, views, cache entries, settings, MCP schemas, CLI permission modes, Git status, and search result records are symbolic. I did not find retained embeddings, vector indexes, model weights, or adapters in this revision.
