...
- **Representational form:** `natural-language` `symbolic` — Vault bodies, dogfood reports, skills, rules, and tidy reports are natural-language; YAML/TOML frontmatter, tags, task checkboxes, link graphs, schemas, saved views, lint rules, command flags, JSON envelopes, error objects, and snapshot/BM25 indexes are symbolic.
...
**Markdown vault files.** A vault file bundles natural-language body content with symbolic frontmatter, tags, tasks, links, headings, and optional type fields. hyalo reads those parts as knowledge when returning file content, and as routing/validation/ranking handles when filtering, linting, moving, or building indexes.
...
The promotion path is symbolic: natural-language files can gain stronger operational authority when frontmatter, tags, schemas, saved views, lint rules, links, generated rules, or command hints make them selectable, checkable, or actionable. hyalo supplies the machinery; humans or instructed agents decide what content deserves that promotion.
