...
- **Representational form:** `natural-language` `symbolic` `parametric` — Dialogue turns, summaries, profiles, and knowledge strings are natural-language; ids, timestamps, page links, heat values, thresholds, JSON objects, MCP schemas, and prompt templates are symbolic; FAISS/Chroma vectors and sentence embeddings are parametric retrieval state.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — The durable outputs include natural-language summaries/profiles/facts, symbolic ids/timestamps/session metadata/heat fields, and embeddings or Chroma vectors.
