...
- **Representational form:** `natural-language` `symbolic` `parametric` — Skill descriptions, prompts, QA answers, critiques, and event observations are natural-language; JavaScript skills, control primitives, JSON task/cache records, parsed action code, and Mineflayer calls are symbolic; the dense skill and QA-question embeddings are distributed-parametric retrieval state. That last classification describes their encoding, not an update to model weights.
- **Lineage:** `authored` `imported` `trace-extracted` `other-compiled` — Prompts, base control primitives, and framework code are authored; bundled/community skill libraries are imported when supplied through `skill_library_dir`; generated skills, descriptions, event records, completed/failed task lists, QA cache entries, and chest memory are extracted from interaction traces and task execution. The vector entries are compiled from retained skill descriptions and QA questions; replacing a skill replaces its indexed description, and new QA questions receive new vector entries.
...
**Embedding generation and model pinning.** Both indexes use `OpenAIEmbeddings()` and add vectors through `add_texts`: skill descriptions supply one index, and retained QA questions supply the other. These paths generate retrieval representations from text; they do not train the embedding model or update the LLM's weights. Regenerating such an index is compatible with a fixed-model experiment when the embedding model and generation algorithm are pinned. The reviewed constructors do not establish that pinning, and no run with an audited model lineage was inspected ([voyager/agents/skill.py](https://github.com/MineDojo/Voyager/blob/55e45a880755d0c8c66ca7fb5fe7962ac8974f89/voyager/agents/skill.py), [voyager/agents/curriculum.py](https://github.com/MineDojo/Voyager/blob/55e45a880755d0c8c66ca7fb5fe7962ac8974f89/voyager/agents/curriculum.py)).
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Raw observations and critiques become natural-language descriptions and QA answers; successful behaviors become symbolic JavaScript functions and JSON records. Embedding those retained descriptions and questions produces distributed-parametric retrieval state without a model-weight update.
