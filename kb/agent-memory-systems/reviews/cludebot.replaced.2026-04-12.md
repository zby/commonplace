...
> Replaced 2026-04-12. See [cludebot](./cludebot.md) for the current review.
...
Contrasts [inspectable substrate not supervision defeats the blackbox problem](../../notes/inspectable-substrate-not-supervision-defeats-the-blackbox-problem.md) -- cludebot's Supabase storage is opaque without API/SQL mediation; the local JSON mode is the closest it gets to inspectable substrate, and even that requires parsing structured JSON rather than reading prose.

Extends [memory management policy is learnable but oracle-dependent](../../notes/memory-management-policy-is-learnable-but-oracle-dependent.md) -- the strategy refiner learns procedural rules from action-outcome patterns, with tweet engagement as the oracle; the oracle's quality (engagement != correctness) bounds the quality of learned strategies.
