...
- **Representational form:** `natural-language` `symbolic` `parametric` - Document text, chunk text, wiki Markdown, summaries, prompts, episodes, and tool outputs are natural-language; schemas, slugs, IDs, scopes, roles, tool definitions, graph edges, links, statuses, RBAC, and pipeline stages are symbolic; embeddings, vector distances, reranker scores, and some LLM extraction/ranking judgments are parametric.
...
**Distilled form:** `natural-language` `symbolic` `parametric` - Conversation traces become natural-language episode summaries and symbolic graph entities/relationships; document/wiki pipelines create natural-language Markdown, symbolic links/refs/slugs, and parametric embeddings or reranker-ready retrieval state.
