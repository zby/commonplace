...
- **Representational form:** `natural-language` `symbolic` `parametric` - Task trajectories, key steps, failure reasons, and insight rules are natural-language; labels, scores, correlations, serialized state graphs, JSON records, prompt contracts, and graph edges are symbolic; sentence-transformer embeddings and Chroma similarity are parametric retrieval state.
...
**Distilled form:** `natural-language` `symbolic` - Distilled outputs are natural-language key steps, failure reasons, and insight rules, plus symbolic scores and positive/negative task correlations.
...
- [Agent memory needs discoverable, composable, trusted knowledge under bounded context](../../notes/agent-memory-needs-discoverable-composable-trusted-knowledge-under.md) - compares: G-Memory is strong on trace-extracted discovery and prompt delivery, weaker on reviewable trust and durable governance.
