...
- **Representational form:** `natural-language` `symbolic` `parametric` — Vault notes, frontmatter, skill guidance, and identity blocks are natural-language; CLI/MCP/HTTP schemas, config, SQLite rows, FTS indexes, graph edges, docids, tags, migration plans, health reports, and OpenAPI are symbolic; embeddings, sqlite-vec rows, folder centroids, GGUF model files, and reranker/orchestrator outputs are parametric or model-mediated selection state.
