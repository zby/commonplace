...
- [inspectable substrate not supervision defeats the blackbox problem](../../notes/inspectable-substrate-not-supervision-defeats-the-blackbox-problem.md) — contrasts: OpenSage's file memory is inspectable but its Neo4j graph memory is opaque without API mediation; the system trades inspectability for richer structure
