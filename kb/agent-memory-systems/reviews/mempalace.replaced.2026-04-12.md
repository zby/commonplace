...
AAAK is the clearest naming-versus-mechanism tension. The repo calls it "lossless" compression, but `Dialect.compress()` extracts entities, topic words, one key sentence, emotion signals, and flags into a compact summary string. That representation is not invertible. The system only stays lossless because the original drawer remains elsewhere in Chroma. So AAAK currently behaves more like a derived shorthand or [constraining](../../notes/constraining-and-distillation-both-trade-generality-for-reliability.md) move than a lossless compiled memory substrate.
