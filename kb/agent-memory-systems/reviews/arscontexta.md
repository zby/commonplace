...
- **Representational form:** `natural-language` `symbolic` — Natural-language instructions, methodology claims, notes, manuals, and context files are combined with symbolic YAML frontmatter, JSON hook/plugin manifests, Bash hooks, queue/config schemas, qmd MCP config, and skill manifests.
...
**Distilled form:** `natural-language` `symbolic` — Corrections become natural-language methodology notes or observations with symbolic YAML; mature findings can alter natural-language context files, templates, config, or skill instructions.
