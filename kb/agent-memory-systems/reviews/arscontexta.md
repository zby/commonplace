...
**Direction.** Ars Contexta uses both pull and push. Pull includes `/ask`, `/architect`, `/health`, `/next`, qmd/grep search, reading notes/MOCs, and command skill invocation. Push includes hook-driven session orientation, write validation feedback, auto-commit, and condition messages emitted before the agent starts ordinary work.

**Trigger and relevance signal.** The strongest push trigger is structural rather than semantic: `SessionStart` always runs in detected Ars Contexta vaults and emits scoped orientation from tree, session state, goals, identity, recent methodology snippets, and count-based maintenance conditions. `PostToolUse` with matcher `Write` fires validation and auto-commit. This is engineered push activation because the read-back is event-keyed, pre-action or immediately post-action, and gated by vault detection plus file/condition checks; precision and recall of the pushed material are not verified from code.
