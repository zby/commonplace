...
**Distilled form:** `natural-language` `symbolic` `parametric` — Distilled artifacts include natural-language briefs, wiki pages, learning descriptions, lint reports, and skills; symbolic JSONL facts, triplets, YAML, review state, and compiled skill metadata; and embedding-backed notebook clustering when enabled.
...
- [Rule-based context selection needs a pre-existing signal](../../notes/rule-based-context-selection-needs-a-pre-existing-signal.md) - explains: WUPHF's targeted read-back relies on paths, slugs, task ids, scopes, query classes, and typed predicates being available as signals or inferred from query text.
