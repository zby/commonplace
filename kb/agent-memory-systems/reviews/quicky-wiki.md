...
- **Lineage:** `authored` `imported` — user files, dashboard edits, configuration, and saved answers are authored; fetched URLs and local source material are imported; source records, extracted claims, summaries, confidence events, FTS indexes, wiki pages, and exports are derived from those authored/imported inputs. The reviewed code does not derive durable memory from agent session/action traces.
