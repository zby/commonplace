...
> Replaced 2026-06-04. See [Mem0](./mem0.md) for the current review.
...
**Read-back:** `both` — Core SDK/API/MCP search is pull, but the proxy wrapper, OpenClaw hooks, and resume branch of `mem0-plugin` implement engineered pre-action memory push paths that search or select retained memories and inject them into the receiving prompt/context
