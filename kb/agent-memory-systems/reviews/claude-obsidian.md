...
claude-obsidian and Commonplace share the view that durable agent memory should be inspectable files with human-readable text and symbolic structure. The major difference is scope. claude-obsidian optimizes for end-user adoption in Obsidian: setup scripts, visual graph/canvas affordances, skills, hot cache, and optional retrieval infrastructure. Commonplace optimizes for a typed methodology corpus where artifact kind, source grounding, and review status are explicit.
