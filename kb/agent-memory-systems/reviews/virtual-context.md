...
- **Representational form:** `natural-language` `symbolic` `parametric` — Segment summaries, tag summaries, fact `what` strings, context blocks, and tool descriptions are natural-language; schemas, canonical turns, tags, aliases, facts, fact links, compaction operations, payload format adapters, and tool definitions are symbolic; chunk/tag-summary embeddings and embedding tagger signals are parametric.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Traces become natural-language summaries and prompt blocks, symbolic canonical turns/facts/fact links/tags/aliases/tool refs, and parametric chunk/tag-summary embeddings.
