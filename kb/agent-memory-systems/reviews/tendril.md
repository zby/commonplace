...
Read-back: pull plus always-load. The system prompt is pushed into every agent session and requires list-first behavior, but capability content enters action when the agent deliberately calls `listCapabilities()` and then `execute`; I did not find a code-grounded relevance-gated or hook-based memory-content push path that warrants `push-activation`.
