...
- **Representational form:** `natural-language` `symbolic` — README/docs/templates carry natural-language guidance and section structure; the schema examples, decision routes, workflow examples, tables, checklist fields, source/date/confidence fields, and repository policies are symbolic shapes. There is no parametric retained state in the inspected checkout.
