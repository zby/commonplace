...
- **Representational form:** `natural-language` `symbolic` `parametric` — Trajectories contain natural-language model responses and observations plus symbolic roles, actions, rewards, loss flags, node values, and tree structure; after training, the retained correction behavior is parametric model state.
...
**Promotion path.** Agent-R promotes task experience through a staged chain: environment rollout -> MCTS tree -> high/low path pairing -> verifier-guided revision conversation -> external fine-tuning -> checkpoint-loaded evaluation. The promotion crosses from files and symbolic/natural-language traces into parametric authority. It does not promote individual memories into citations, rules, validators, or a governed review state.
...
**Distilled form:** `parametric` — The durable learned behavior after the full workflow is a fine-tuned checkpoint; intermediate distilled examples are natural-language/symbolic JSONL conversations.
