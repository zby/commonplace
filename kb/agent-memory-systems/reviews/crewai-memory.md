...
- [Substrate class, backend, and artifact form are separate axes that get conflated](../../notes/substrate-class-backend-and-artifact-form-are-separate-axes-that-get-conflated.md) — sharpens: CrewAI's database backend still stores symbolic artifacts; the backend choice is separate from substrate class
- [Flat memory predicts specific cross-contamination failures that are empirically testable](../../notes/flat-memory-predicts-specific-cross-contamination-failures-that-are-empirically-testable.md) — predicts: CrewAI's single record model plus scope tree should be tested for operational debris and preference/knowledge mixing
