...
- **Representational form:** `natural-language` `symbolic` `parametric` — Record content and recalled blocks are natural-language; scopes, categories, metadata, importance, source, privacy flags, tool schemas, and consolidation plans are symbolic; embeddings and vector distances supply parametric retrieval signals.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Raw outputs become natural-language memory statements with symbolic scope/category/metadata/importance/source/private fields and embeddings.
...
**Selection, scope, and complexity.** Crew task push uses five matches; standalone kickoff uses twenty; general recall defaults to ten, oversamples vector search candidates, caps distilled query phrases at three, caps candidate scopes at twenty, filters by scope/categories/source/private state, and ranks by semantic similarity, recency, and importance. Complexity is not just count: injected natural-language content can include multiple scoped memories, metadata fields, and LLM-selected evidence gaps.
