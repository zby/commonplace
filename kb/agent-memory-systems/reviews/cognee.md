...
- **Representational form:** `natural-language` `symbolic` `parametric` — Document chunks, summaries, Q&A, session feedback, graph-context text, and skill procedures carry natural-language; datasets, DataPoints, node sets, graph edges, query routes, ACLs, typed memory entries, session metadata, tool schemas, and pipeline state are symbolic; embeddings and vector distances supply parametric retrieval signals.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Traces become natural-language session feedback, Q&A text, graph-context summaries, and skill-run summaries; symbolic graph nodes/edges, weights, node sets, QA metadata, and trace records; and parametric embeddings or vector-ranked summaries.
