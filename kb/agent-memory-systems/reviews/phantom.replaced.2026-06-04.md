...
> Replaced 2026-06-04. See [Phantom](./phantom.md) for the current review.
...
**Read-back:** `both` — Phantom has pull surfaces through MCP, UI, and reflective tools, plus memory push from relevance-gated per-query Qdrant injection and coarse always-loaded evolved/working-memory prompt blocks. The instance-targeted pre-action Qdrant memory context is the strongest activation path, not baseline docs, coarse always-loaded memory, or ordinary memory-search tools.
