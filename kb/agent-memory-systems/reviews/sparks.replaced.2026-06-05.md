...
> Replaced 2026-06-05. See [Sparks](./sparks.md) for the current review.
...
- **Representational form:** `prose` `symbolic` - Prose Markdown carries raw captures, wiki pages, logs, and agent instructions; symbolic TOML, SQLite rows, YAML frontmatter, wikilink edges, command schemas, and Go code carry the deterministic contract.
- **Lineage:** `authored` `imported` - Raw captures are authored or imported by the human, wiki pages and indexes derive from raw captures and manifest state, and command reports derive from scans over the current vault filesystem.
- **Behavioral authority:** `knowledge` `instruction` `routing` `validation` — Raw captures and wiki pages advise agents, while the embedded contract, generated agent files, CLI/MCP operations, manifest, indexes, lint/status/affected reports, and config instruct, route, validate, or audit later work.
