...
> Replaced 2026-06-04. See [HyperAgents](./hyperagents.md) for the current review.
...
- **Lineage:** `authored` `imported` `trace-extracted` — meta-agent edits author patch artifacts, copied worktrees import the current repository, and chat/evaluation traces are extracted into diffs, reports, metadata, and archive state
- **Behavioral authority:** `knowledge` `instruction` `routing` `validation` `ranking` — chat histories and reports serve as evidence, generated README/code instruct future agents, archive metadata routes and ranks parent lineages, and evaluation outputs validate candidate parents
...
**Learning timing:** `staged` — traces are converted to patches, reports, metadata, and selected parent lineages between generations rather than during a single answer.

**Distilled form:** `symbolic` — the durable learned objects are code diffs, JSON/CSV reports, metadata, and archive routing records, with no retained parametric store.
