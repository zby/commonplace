...
- **Representational form:** `natural-language` `symbolic` `parametric` — Memory content is natural-language; ids, timestamps, keywords, links, categories, tags, context strings, JSON metadata, and response schemas are symbolic; sentence-transformer embeddings and Chroma distances are parametric retrieval state.
...
**LLM metadata and evolution prompts.** Storage substrate: authored prompt strings in the package plus generated metadata retained inside notes and Chroma metadata. Representational form: natural-language prompts with symbolic JSON schemas and generated symbolic/natural-language fields. Lineage: generated from the new note and nearest-neighbor memories. Behavioral authority: learning and routing, because the LLM may add links to the new note or rewrite neighbor context/tags.
