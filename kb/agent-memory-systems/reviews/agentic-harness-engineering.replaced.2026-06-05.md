...
> Replaced 2026-06-05. See [Agentic Harness Engineering](./agentic-harness-engineering.md) for the current review.
...
**Write agency:** `automatic` `manual` — the benchmark loop, debugger reports, history files, attribution records, and git commits are generated by the evolution workflow, while the evolution agent still performs the workspace edits as an authoring actor from the pushed evidence

**Curation operations:** `synthesize` `promote` `invalidate` — debugger reports, evolution queries, manifests, histories, and harness edits synthesize new behavior from rollouts; trace-derived reports and manifests are promoted into git-tracked harness changes; later change attribution can mark fixes as ineffective or harmful for rollback guidance
