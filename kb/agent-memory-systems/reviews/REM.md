...
- **Representational form:** `natural-language` `symbolic` `parametric` — Raw episode text and semantic facts are natural-language; schemas, route handlers, prompts, metadata, graph edges, counters, and API wrappers are symbolic; embeddings in Qdrant are distributed-parametric selectors.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Durable outputs include natural-language traces/facts, symbolic metadata/counters/source ids/routes, and embeddings.
