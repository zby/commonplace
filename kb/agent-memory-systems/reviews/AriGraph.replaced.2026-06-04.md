...
> Replaced 2026-06-04. See [AriGraph](./AriGraph.md) for the current review.
...
- **Representational form:** `prose` `symbolic` `parametric` — symbolic triplets and navigation edges, prose observations/prompts/triplet strings, and distributed-parametric embeddings for triplets, entities, and episodic observations
- **Lineage:** `authored` `imported` `trace-extracted` — authored prompts/retrieval code, imported QA documents treated as observations, and in-run observations/actions extracted into graph facts, navigation edges, and episodic memory
- **Behavioral authority:** `knowledge` `instruction` `routing` `ranking` `validation` — retrieved facts and episodes advise prompts; prompts instruct extraction/planning/action; graph edges route navigation; retrieval scores rank; refinement prompts validate/prune conflicting facts

**In-run triplet graph.** The storage substrate is Python process memory: lists and dictionaries on `TripletGraph` / `ContrieverGraph`, with no implemented save/load path for graph state in the main pipeline. The representational form combines symbolic triplets and navigation edges, prose triplet strings, and distributed-parametric embeddings for triplets and entities. Lineage is observation-derived and action-derived inside one attempt: LLM extraction turns observations into triplets; movement actions add navigation edges; a refinement prompt removes predicted outdated facts. Behavioral authority is both knowledge-artifact and system-definition-artifact authority: retrieved facts advise planner/action prompts, and navigation edges are used as routing state for `go to` actions.

**Episodic observation store.** The storage substrate is also in-process memory: `obs_episodic`, `obs_episodic_list`, and `top_episodic_dict_list` on `ContrieverGraph`. The representational form combines prose and distributed-parametric embeddings: each stored observation is the textual game observation paired with extracted triplet strings and an embedding. Lineage is raw-ish observation text plus derived graph facts from the same step; it is retained only for later steps in the same run. Behavioral authority is knowledge-artifact authority when inserted as "most relevant episodic memories" into the planner/action prompts.
