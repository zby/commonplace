...
- **Representational form:** `natural-language` `symbolic` `parametric` — Chat facts, wiki pages, summaries, prompts, and answers are natural-language; message ids, channels, entities, relationships, fact types, schemas, page ids, policies, write intents, and tool contracts are symbolic; embeddings, HNSW vector search, similarity clustering, and name vectors are parametric retrieval state.
...
**Atomic facts.** Storage substrate: Weaviate `MemoryFact` objects. Representational form: natural-language `memory_text` plus symbolic metadata and parametric vectors. Lineage: extracted from normalized messages, enriched by extraction/classification/embedder stages, then persisted with source message identity and provenance fields. Behavioral authority: knowledge artifacts when surfaced to QA/MCP/humans; ranking artifacts when embeddings, BM25, topic tags, quality scores, importance, and MMR select the context served.

**Topic clusters and channel summaries.** Storage substrate: Weaviate objects with `tier="topic"` or `tier="summary"`. Representational form: natural-language summaries and symbolic member ids, tags, counts, graph highlights, dirty flags, and centroid vectors. Lineage: derived from atomic facts and graph context; invalidated by new fact membership through `summary_dirty`. Behavioral authority: knowledge artifacts and context-compression artifacts for QA/wiki readers; routing artifacts when page generation and topic lookup use cluster ids/tags.

**Graph entities and relationships.** Storage substrate: Neo4j, with optional Nebula/null graph abstraction behind the graph protocol. Representational form: symbolic nodes/edges with natural-language relationship context and optional name vectors. Lineage: extracted from message batches, validated/deduplicated across batches, normalized, healed with stub entities, and linked back to fact/message provenance. Behavioral authority: graph traversal, decision-history, and expert-finding tools use this as structured knowledge and routing/ranking input.

**Wiki pages.** Storage substrate: MongoDB `wiki_pages` and legacy wiki cache documents, with Neo4j wiki-page graph nodes for some navigation surfaces. Representational form: natural-language sections plus symbolic page ids, slugs, kind schemas, sections, versions, redirects, dirty flags, and cross-links. Lineage: generated from facts, clusters, entities, wiki prompts, and page-maintainer updates; page versions change on user-visible content differences. Behavioral authority: knowledge artifacts for users and QA agents, and a frontloaded context surface that can displace raw transcript retrieval.
...
**QA/MCP tools and citation registry.** Storage substrate: authored Python/FastMCP code plus runtime service state. Representational form: symbolic tool schemas/docstrings and natural-language prompts; citation outputs carry symbolic source handles and natural-language excerpts. Lineage: authored implementation that consumes trace-extracted memory at runtime. Behavioral authority: routing and knowledge authority for callers; tools choose which retained memory can enter an answer and under which principal/channel access checks.
...
**Use graph memory for relational questions, not as a universal substrate.** Atlas's graph tools are valuable for "who/what is connected" questions while Weaviate/wiki remain the natural-language content/fact surfaces. Commonplace could borrow a derived relationship index only where authored links and frontmatter are insufficient.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Raw chat becomes natural-language facts, summaries, and wiki sections; symbolic entities, relationships, tags, page schemas, write intents, and citations; and parametric embeddings/name vectors used for ranking, clustering, and matching.
...
- [Use Trace Extraction As Meta-Learning](../../notes/agent-memory-requirements/use-trace-extraction-as-meta-learning.md) - exemplifies: Atlas turns communication traces into facts, graph structures, topic summaries, and wiki pages.
