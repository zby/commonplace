...
**QA/MCP tools and citation registry.** Storage substrate: authored Python/FastMCP code plus runtime service state. Representational form: symbolic tool schemas/docstrings and prose prompts; citation outputs carry symbolic source handles and prose excerpts. Lineage: authored implementation that consumes trace-extracted memory at runtime. Behavioral authority: routing and knowledge authority for callers; tools choose which retained memory can enter an answer and under which principal/channel access checks.
