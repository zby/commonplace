...
- **Representational form:** `natural-language` `symbolic` `parametric` — Memory blocks, prompts, messages, summaries, archival passages, and tool source are natural-language; schemas, metadata, tags, tool rules, IDs, groups, run state, and git frontmatter are symbolic; message/passage/tool embeddings are parametric retrieval structures.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Distilled outputs include natural-language block edits, summary messages, archival passages, source-derived memory blocks, symbolic tags/metadata/run records, and embeddings used for later retrieval.
