...
- **Representational form:** `natural-language` `symbolic` `parametric` — Pipeline code, examples, generated docs, and skill text carry natural-language and authored procedural guidance; app configs, stable paths, target-state records, schemas, connector handlers, fingerprints, table/collection/graph definitions, and CLI/MCP-adjacent command surfaces are symbolic; embeddings and vector indexes in user pipelines add parametric retrieval state.
