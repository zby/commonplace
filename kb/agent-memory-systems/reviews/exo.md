...
- **Representational form:** `prose` `symbolic` — Messages, remembered facts, profiles, and skill bodies are prose; event envelopes, artifact metadata, task/todo state, skill indexes, tool schemas, TypeScript modules, and harness code are symbolic. A skill bundle deliberately combines prose instructions with symbolic frontmatter and may include scripts.
