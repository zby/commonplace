...
- **Representational form:** `natural-language` `symbolic` — Topic bodies carry natural-language facts, rules, decisions, flows, and examples, while `<bv-topic>` attributes, element names, paths, relations, runtime-signal records, logs, review statuses, manifests, and command schemas carry symbolic state.
...
**Distilled form:** `natural-language` `symbolic` — Curate transforms task/file/user context into natural-language topics embedded in symbolic `<bv-topic>` structure; access and update traces become symbolic ranking and maturity signals.
