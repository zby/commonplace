...
| Learned substrate | Per-env list of up to three plan strings; prompt-only in programming | Typed notes, links, instructions, ADRs, indexes |
| Addressable unit | None — memory is `env_configs[i]['memory']`, sliced `[-3:]` | Individual markdown files with frontmatter |
...
**Trace-derived learning placement.** Trace source: per-trial logs of failed attempts at a single benchmark task, with the trigger boundary at one full task attempt (for ALFWorld/WebShop/HotPotQA) or one failed unit-test run (for programming). Extraction: a second LLM call reads the failed trajectory plus the last three prior plans and emits a fresh plan or self-reflection string; the oracle is binary benchmark success, and `update_memory` is the gate. Promotion target: prompt-visible text only — `env_configs[i]['memory']` is the entire durable store, JSON-serialized for trial resume but never mined, indexed, or promoted further. Scope: per-task and per-benchmark only; programming runs discard reflections across items, ALFWorld/WebShop keep them per env index. Timing: online within a multi-trial run, sequential, one reflection per failed trial. On the [survey's](../trace-derived-learning-techniques-in-related-systems.md) axis 1, Reflexion fits the **trajectory-run pattern** — repeated bounded attempts at the same task rather than a single live session or an event stream. On axis 2, it is squarely **artifact-learning** at its lowest structure — a list of free-text plans clamped at three, with no CRUD verbs, no counters, no typed fields. Reflexion reinforces rather than splits any existing survey claim; it remains the historical anchor for the "verbal RL without weights" subtype and the floor for artifact structure (even Dynamic Cheatsheet maintains a single long blob, which is more structure than Reflexion's clamped tail of independent strings). No new subtype is warranted.
