...
- **Representational form:** `natural-language` `symbolic` `parametric` — Markdown bodies and observations carry natural-language; frontmatter, route rules, graph indexes, fact tuples, checkpoints, templates, manifests, and command/plugin schemas carry symbolic state; hosted embeddings and optional rerankers add parametric retrieval signals.
- **Lineage:** `authored` `trace-extracted` — Users and agents author notes through CLI/SDK/plugin paths, while session transcripts, hook events, observations, facts, checkpoints, maintenance outputs, graph indexes, and embedding caches are produced from later use traces or generated views.
...
**Distilled form:** `natural-language` `symbolic` `parametric` — Session traces become natural-language observations and notes, symbolic facts/graphs/checkpoints/routes, and embedding/reranking state for retrieval.
...
**Scope and timing.** The trace-extracted layer is vault-scoped and agent-aware when OpenClaw agent vault mappings are configured. Online paths update raw logs, observations, facts, checkpoints, and prompt context; staged paths turn inbox or observations into curated/distilled notes and reflections.
...
- Whether the Python SDK grows the same observer/plugin/trace-extracted surface as the Node implementation; at this commit it is a narrower file/search/checkpoint interface.
...
- [Use trace extraction](../../notes/agent-memory-requirements/use-trace-extraction-as-meta-learning.md) - exemplifies: ClawVault extracts observations, facts, and distilled notes from sessions and hook events.
