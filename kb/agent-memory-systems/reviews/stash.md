...
**Substrate class.** The learned state is symbolic database structure plus prose fields. Facts, relationships, causal links, hypotheses, goals, failures, contradictions, and checkpoints are structured rows; fact and pattern content remains natural language; embeddings are opaque numeric support state.

**Role.** Most learned artifacts are knowledge: retrieved facts and episodes inform future prompts. Some rows are closer to system-definition memory: `/self/preferences`, failure lessons, active goals, and confirmed hypotheses can change how the agent plans or behaves when recalled. The system still relies on the consumer agent to ask for or heed those memories.
