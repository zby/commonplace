...
- **Representational form:** `natural-language` `symbolic` `parametric` — Episodes, facts, contexts, goals, failures, hypotheses, patterns, and contradiction notes are natural-language text; namespaces, schemas, ids, confidence values, status enums, source links, checkpoints, MCP tool schemas, and prompt templates are symbolic; embeddings and vector distances are parametric retrieval state.
