...
**Direction.** Both. The strongest push paths are startup/session sync, native skill-description exposure, OpenClaw sidecar instructions, compact/subagent context injection, and prompt-hook context for explicit skill creation. Pull remains present when an agent chooses a skill, reads a local `SKILL.md`, calls an MCP skill tool, or asks the registry/API for details.

**Trigger and relevance signal.** Claude Code activation is collection-scoped and host-skill mediated: SkillNote chooses the active collection, then the host agent decides from skill descriptions and user/task context. `UserPromptSubmit` adds regex-triggered context for skill creation. OpenClaw sync pushes all registry skills to `sn-*` files, while `/v1/openclaw/context-bundle` filters by optional collection, over-fetches candidates, sorts by recent usage and rating, and leaves semantic relevance picking to the resolver subagent ([openclaw.py](https://github.com/luna-prompts/skillnote/blob/7303ba7ab2098f9675e320fd68296458b4703752/backend/app/api/openclaw.py), [openclaw.py schema](https://github.com/luna-prompts/skillnote/blob/7303ba7ab2098f9675e320fd68296458b4703752/backend/app/schemas/openclaw.py)). MCP activation is tool-list exposure plus client-side tool choice; the server can filter by collections.
