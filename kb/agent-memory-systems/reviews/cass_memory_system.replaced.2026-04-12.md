...
> Replaced 2026-04-12. See [cass_memory_system](./cass_memory_system.md) for the current review.
...
**Where commonplace is stronger.** Knowledge has richer internal structure — notes contain arguments, evidence, caveats, and articulated links rather than flat rule strings with counters. [Link semantics](../../notes/links-index.md) capture *why* notes relate (extends, grounds, contradicts), not just *that* they're similar. The maturation path (text -> note -> structured-claim) produces higher-quality knowledge over time rather than just scoring existing rules up or down. Most importantly: commonplace has a [theory of learning](../../notes/continuous-learning-requires-durability-not-weight-updates.md) that explains which operations count as learning and why — cass-memory has a pipeline that works but no framework for reasoning about when its automation is reliable.

**The deepest divergence** is the granularity and depth of the learned artifact. A cass-memory playbook bullet is a one-liner with metadata: "Always check token expiry before other auth debugging." A commonplace note is a multi-paragraph argument with evidence, caveats, links to grounding sources, and articulated relationships to other notes. The playbook format optimizes for retrieval speed and automated scoring; the note format optimizes for [contextual competence](../../notes/an-agentic-kb-maximizes-contextual-competence-through-discoverable-composable-trusted-knowledge.md) — the ability to reason well about a domain, not just recall tips. These serve different use cases: cass-memory is better for quick procedural guidance ("do this, avoid that"), commonplace is better for understanding ("here's why this works and when it doesn't").
...
- [automating-kb-learning-is-an-open-problem](../../notes/automating-kb-learning-is-an-open-problem.md) — exemplifies: the feedback-scoring pipeline automates extraction and scoring but quality depends on agent compliance with feedback conventions — the curation gap applies here too
...
- [constraining](../../notes/definitions/constraining.md) — exemplifies: playbook bullets constrain agent behavior by specifying rules and anti-patterns; confidence scoring is a mechanism for adjusting constraint strength over time
- [context-efficiency-is-the-central-design-concern-in-agent-systems](../../notes/context-efficiency-is-the-central-design-concern-in-agent-systems.md) — exemplifies: `cm context` pre-assembles relevant rules and history to spend agent context tokens on proven guidance rather than raw session logs
