...
- **Representational form:** `natural-language` `symbolic` - The system is mostly natural-language instructions and logs, with symbolic task IDs, decision IDs, statuses, dates, JSON feature records, hook configuration, shell scripts, file names, archive-page conventions, section numbers, sidecar tables, and git command protocols.
...
The main design split is authority depth. Context Guard's safeguards are operationally forceful because its command natural-language tells Claude to treat them as mandatory, but the repository does not include an independent semantic checker that proves the agent preserved user intent or obeyed recalled decisions. Commonplace has weaker automatic session recovery, but stronger typed validation and review conventions around durable knowledge artifacts.
...
**Do not borrow command-natural-language enforcement as the only gate.** Ready now as a caution. Context Guard shows how far careful instructions can go, but Commonplace should keep deterministic validators and semantic review for high-authority artifacts.
...
**Distilled form:** `natural-language` `symbolic` - Session traces become natural-language summaries, comments, decisions, learned behaviour, resume notes, audits, and sidecar descriptions, plus symbolic task IDs, statuses, feature pass/fail JSON, decision categories, archive references, and section numbers.
...
**The task registry is closer to an operational ledger than a memory note.** Its value is not natural-language richness; it is the hard rule that every task must have a row, status, and source.
...
- [Axes of artifact analysis](../../notes/axes-of-artifact-analysis.md) - applies: safeguard files, command natural-language, hooks, git state, plans, audits, and sidecar indexes carry different authority.
