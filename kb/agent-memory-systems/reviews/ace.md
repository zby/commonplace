...
- **Representational form:** `natural-language` `symbolic` `parametric` — Playbook bullets are natural-language advice with symbolic ids and helpful/harmful counters; optional deduplication uses transient parametric embeddings to decide which natural-language bullets to merge.
...
**Curator operations.** Storage substrate: transient model responses plus operation-diff logs. Representational form: symbolic JSON with natural-language reasoning/content. Lineage: trace-extracted from current playbook, recent reflection, task context, and playbook stats. Behavioral authority: learning and instruction authority over the retained playbook, but at this commit applied operations materially add bullets only; non-additive operations are accepted syntactically but not implemented by `apply_curator_operations`.

**Best-playbook snapshots.** Storage substrate: files under the run directory and the `self.best_playbook` in-memory field. Representational form: natural-language/symbolic playbook text plus validation metrics. Lineage: selected from the evolving playbook based on validation accuracy during offline training. Behavioral authority: ranking/salience authority, because final testing uses the best validation snapshot instead of necessarily using the last snapshot.
...
**Optional semantic dedup as maintenance, not source of truth.** ACE's bullet analyzer uses embeddings only to identify candidate redundancy, then rewrites retained natural-language content. Commonplace could use embedding similarity to propose duplicate-note or duplicate-claim reviews while keeping authored Markdown as the retained authority. Needs a review gate before acting automatically.
...
**Distilled form:** `natural-language` `symbolic` — The durable learned output is a natural-language playbook with symbolic ids, sections, and helpful/harmful counters.
