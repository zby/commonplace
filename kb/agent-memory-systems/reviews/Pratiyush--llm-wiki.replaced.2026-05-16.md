...
> Replaced 2026-05-16. See [Pratiyush/llm-wiki](./Pratiyush--llm-wiki.md) for the current review.
...
**Representational form and behavioral authority.** The retained form is readable artifacts: raw transcript markdown, synthesized wiki pages, navigation files, static HTML, JSON/JSON-LD exports, and MCP-readable text. Most outputs are knowledge artifacts: future agents can query what happened and what pages say. Some seeded files (`AGENTS.md`, `CLAUDE.md`, skills, commands, `MEMORY.md`) are system-definition artifacts, but the inspected automatic path mainly promotes traces into knowledge pages, not into stronger rules or executable tools.
