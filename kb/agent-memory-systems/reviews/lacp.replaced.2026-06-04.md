...
> Replaced 2026-06-04. See [LACP](./lacp.md) for the current review.
...
**The Self-Memory System gives session-start memory gates.** `self_memory_system.py` stores episodes, epochs, narrative, and self-model JSON/JSONL under `~/.lacp/sms`, reads a focus brief as the "working self," applies significance decay, and builds a session context from current focus, preferred approaches, known biases, failure patterns, narrative arc, and recent significant episodes ([hooks/self_memory_system.py](https://github.com/0xNyk/lacp/blob/003eef16a583dbaa3fdd56e5efe393397f463f3f/hooks/self_memory_system.py)). The session-start signal is simple but explicit: current focus is included, self-model/narrative fields are included when present and confident, and episodes are filtered by significance, recency, and a small bound; the goal-overlap helper biases explicit queries rather than selecting session-start episodes.
...
- **Storage substrate:** `files` — Files under `~/.claude/projects/<slug>/memory/`, especially `MEMORY.md` plus topic files described in `CLAUDE.md`
- **Representational form:** `symbolic` — Prose with implicit routing structure; the hard line cap is enforced by hook warning and optional guard scripts rather than a formal schema
- **Lineage:** `authored` `imported` `trace-extracted` — retained state is authored by users/agents and repo config, imported or synced from local files/Obsidian/research/session sources, and extracted from transcripts, hooks, provenance, and workflow traces
- **Behavioral authority:** `knowledge` `instruction` `enforcement` `routing` `validation` `ranking` `learning` — artifacts serve as evidence/context, hook-injected instructions, policy gates, route/profile inputs, quality checks, priority/significance ordering, and trace-derived learning outputs
...
**Read-back:** `both` — Operators and agents can pull via `lacp brain-*`, `lacp obsidian`, `lacp sms`, `lacp reflect`, and explicit search/read commands; Claude lifecycle hooks also push retained focus, handoff, and SMS memory into the receiving model's context. Context modes and workflow briefs are shipped/configured baseline surfaces, and Stop/eval feedback is a live gate, not retained-memory read-back.
...
**Learning timing:** `online` `offline` `staged` — Stop-hook extraction and SMS updates run online, while brain-expand, reflection, curation, and review operate as staged/offline promotion loops.

**Distilled form:** `prose` `symbolic` — distilled outputs are notes, summaries, lessons, handoffs, episodes, self-model fields, JSON reports/contracts, canary records, and generated skills rather than learned weights.
...
**Direction.** LACP is both pull and push for retained memory. Pull surfaces include Obsidian read/search, brain commands, SMS CLI, reflection summaries, canary reports, lessons linting, and explicit `lacp` commands. The memory push surface is the Claude `SessionStart` `systemMessage` injection of retained focus, handoff, and SMS state. Context modes and workflow briefs are automatically loaded but are shipped/configured baseline context rather than read-back; Stop/eval feedback and quality-gate blocking shape behavior but are live gate outputs rather than retained memory returning to context.
