...
term fits. It is not a delta from Commonplace. Current borrowable ideas or
differences are selective [transfer scans](../instructions/scan-agentic-system-transfer/SKILL.md)
...
- Generated by [`scripts/build_systems_matrix.py`](../../scripts/build_systems_matrix.py): scans `reviews/` and extracts each review's comparison data (doc-grounded reviews under `lightweight/` are excluded). Re-run after reviews or triage artifacts change — `python3 scripts/build_systems_matrix.py`. Any parser flag blocks the write; resolve all flags before treating the matrix as current.
...
- Synthesized publicly by [`synthesize-agent-memory-landscape`](../instructions/synthesize-agent-memory-landscape/SKILL.md): pins one zero-flag matrix plus its row-linked reviews and ontology inputs at a reconstructable revision, computes every quantitative claim mechanically, and verifies qualitative examples against the frozen reviews.
...
Only closed, population-complete fields support corpus counts. Open-ended ontology observations such as an instance of frontloading support qualitative examples and combinations; omission is not assessed absence, so they receive no prevalence claim until every review has been assayed for that concept. Legacy `Comparison with Our System`, `Borrowable Ideas`, and `What to Watch` sections are excluded from public synthesis.
