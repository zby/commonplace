...
[`systems.csv`](./systems.csv) is the machine-generated comparison matrix — one row per review (repo-backed and lightweight), one column per comparison axis. It is **built by parsing the reviews, not hand-maintained**:

- Generated by [`scripts/build_systems_matrix.py`](../../scripts/build_systems_matrix.py): scans `reviews/` and `lightweight/` and extracts each review's comparison data. Re-run after reviews change — `python3 scripts/build_systems_matrix.py`.
