...
[`systems.csv`](./systems.csv) is the machine-generated comparison matrix — one row per **code-grounded** review (`source-tier: code-grounded`, in `reviews/`), one column per comparison axis plus sample-origin metadata. It is **rebuilt from source artifacts, not hand-maintained**:
...
- `found_karpathy_llm_wiki_gist` from exact GitHub repository URL matches in [the consolidated Karpathy gist core](./asisas-2026-experiment/karpathy-gist-agent-memory-reproducible-core.md). The core has 51 members: 50 code-grounded rows represented in this matrix and one doc-grounded lightweight review outside `systems.csv`.
