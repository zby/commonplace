...
- [`types/`](./types/) - frozen review type contract and schema used by the reviews.
...
The reviews were generated under the Commonplace agent-memory review workflow: the review type contract in [`types/agent-memory-system-review.md`](./types/agent-memory-system-review.md), source-grounded review writing, deterministic validation, semantic review gates where applied, and collection navigation/linking machinery such as the connect workflow. Those system-definition artifacts live in the Commonplace repository, not in each review.
