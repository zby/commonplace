...
**Promotion path.** Meta-Harness promotes raw episode evidence into evaluation summaries, frontier records, pending-candidate manifests, generated code, and eventually optional final test evaluation. The strongest promotion crosses from trace-derived knowledge artifacts into symbolic system-definition artifacts: a new memory system or agent scaffold that future model calls actually run.
...
**Trace-to-code promotion with explicit benchmarks.** Meta-Harness cleanly separates proposer creativity from evaluator authority. Commonplace should borrow that separation before allowing trace-derived edits to instructions, validators, or routing rules. Needs a concrete benchmark target.
...
- [Use trace-derived extraction as meta-learning](../../../notes/agent-memory-requirements/use-trace-derived-extraction.md) - exemplifies: Meta-Harness converts prior run traces into revised harness code.
