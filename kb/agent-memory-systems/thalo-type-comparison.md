...
Thalo's `synthesis` is a saved query + prompt pair for repeatable synthesis operations. We have no equivalent. See the [main Thalo note](./reviews/thalo.md) for analysis — powerful map-reduce pattern but we lack the use case.
...
- Decision to split validation into script (deterministic) + skill (judgment) — see [deterministic-validation-should-be-a-script](../notes/deterministic-validation-should-be-a-script.md)
...
- [document-classification](../reference/available-types.md) — our type system being compared
