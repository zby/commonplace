...
Six non-filesystem-first memory systems: **[Mem0](https://github.com/mem0ai/mem0)** (vector-first fact store with LLM-judged CRUD), **[Graphiti](https://github.com/getzep/graphiti)** (bi-temporal knowledge graph on Neo4j), **[Cognee](./reviews/cognee.md)** (pipeline-first graph+vector poly-store), **[Letta/MemGPT](https://github.com/letta-ai/letta)** (agent-self-managed three-tier memory hierarchy), **[A-MEM](https://arxiv.org/abs/2502.12110)** (Zettelkasten-inspired in-memory note network with memory evolution), **[AgeMem](./lightweight/agemem.md)** (RL-trained unified LTM/STM management with six learnable memory operations).
...
| Cognee | [Optional temporal_cognify](https://github.com/topoteretes/cognee); no invalidation model |
...
**A meta-observation:** the six dimensions of this review span storage, retrieval, and learning — which [suggests memory is not a separable niche](../notes/agent-memory-is-a-crosscutting-concern-not-a-separable-niche.md) but a crosscutting concern. Systems that self-identify as "memory systems" are actually making different bets across context engineering (retrieval/activation), learning theory (extraction/promotion), and action capacity (what memory serves). The review's structure already reflects this: its most consequential finding (the agency model) is a learning question, not a storage question.
