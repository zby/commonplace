...
Both are agent-operated markdown knowledge systems with types, links, skills, maintenance loops, and promotion paths — and both are *systems* in the same sense, since natural-language instructions execute on LLMs in each. The structural differences are placement, not kind:

- **Codification placement.** GBrain codifies the write path (extraction, consolidation, ranking as TypeScript with embedded LLM calls) and leaves consumption governed by natural-language instructions (skills advise the host agent). Commonplace inverts this: writing and navigation follow natural-language instructions (conventions, COLLECTION contracts), while checking is codified (validators, schemas, review ledger). In shorthand: code writes and natural-language instructions govern reads, versus natural-language instructions governing writes and code checking them.
