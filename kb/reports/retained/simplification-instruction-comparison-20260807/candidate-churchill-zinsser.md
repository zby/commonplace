...
>
> Compounding can occur without reflection, and reflection alone does not produce it. Reflection may instead offer control. It can make the theories and machinery behind improvement visible and revisable, including how the system defines evidence, problems, and possible changes. Each episode still needs an objective and comparison rule that the candidate cannot change. Commonplace tests whether retained natural-language theory can provide this control across heterogeneous changes without retraining model weights. It can install and reuse revisions, but has not shown compounding through its theory layer.

## Compounding is the payoff

Suppose an agent maintains a deployment policy that its runtime loads. A deployment fails because a credential expires on a Tuesday. The agent mistakes the date for the cause and installs “Never deploy on Tuesdays”; later runs obey. The change is objective-directed and operative, but wrong. Calling the process self-improving describes its objective, not the result of every update.

A correct policy that reduced failures would be a retained gain. Suppose the system instead installed a causal-evidence check, then used it to diagnose another failure and install a better policy. The second improvement would depend on the check. This is *compounding*: one improvement's benefit helps produce another. The feedback need not be exponential or continue indefinitely.

A system can retain many useful task rules while each new rule remains just as hard to find, evaluate, and install. Unless it reinvests the time or resources those rules save, they improve task performance without changing how it produces the next improvement: [improvements accumulate without compounding](../notes/improvements-can-accumulate-without-compounding.md).

By contrast, a validator that measurably lowers the cost of a later beneficial revision is local evidence of compounding. A task-facing gain can also feed back indirectly: it frees capacity, an allocation mechanism directs that capacity to improvement work, and a later episode uses it to produce an improvement. That path must be shown, not assumed.

The metric that accepts a change tells us whether the change met its target, not whether it made the next revision more productive. The [later-episode protocol](../notes/compounding-is-tested-in-later-improvement-not-by-the-accepting-metric.md) therefore asks whether the retained benefit made a later revision cheaper, broader, more reliable, or less human-dependent. It also requires a causal trace between the two episodes.
...
| **Revision surface** | Which behavior-shaping artifacts and relations could this path revise? Which did it revise, which stayed fixed, and could the path still be used afterward? |
...
Consider a harness optimizer that can rewrite a system prompt and three tool descriptions, while a fixed test suite determines which edits are promoted. The prompt and tool descriptions lie inside its revision surface; the suite controls promotion but remains outside it. If the suite rewards the wrong proxy, the optimizer can pursue that proxy but cannot repair the selection criterion.
...
[Opaque retained learning can also compound](../notes/accumulation-counts-dependence-through-the-retained-result.md). Reflection adds addressability: agents can [name, criticize, revise, or retire](../notes/reflection-buys-addressability.md) individual commitments. [Behavioral authority](../notes/definitions/behavioral-authority.md) maps how retained artifacts shape operation. Addressability gives agents more control, but does not cause compounding; a later improvement must still depend on a represented change.

A candidate cannot set the standard by which it is judged. Each improvement episode needs an [objective](../notes/self-improvement-is-relative-to-a-declared-objective.md) and comparison rule fixed before evaluation. Changing that standard is a separate revision requiring separate authority. Reflection can expose more of the system to revision, but cannot make a change its own judge.

The system's current [decomposition](../notes/learning-inside-a-fixed-decomposition-inherits-its-mistakes.md) determines what it can notice and change: what counts as evidence, which failures it can represent, and which revisions it can propose. A loop may optimize every named component yet have no way to identify a missing responsibility or a bad division among components.

The strongest form of this control also lets the system revise its map of what shapes behavior. If experience reveals a missing authority path or a bad division of responsibilities, the system can update the map and use it in later audits. The new map must remain open to inspection and revision. This does not guarantee that the system will find every omission.

## Evidence from reported systems

The table shows what each of six recent systems changed, what evidence shows later use, and what remained outside the reported revision path. All six revise readable artifacts. They form a comparative case set, not a representative survey or a full account of any system's capabilities.

Here, *supplied* means that researchers provided an element and the reported revision path left it fixed. This is not necessarily a defect: an evaluator may be an experimental control or safety boundary. The table records the fixed boundary, not whether it was warranted. The [detailed evidence inventory](../notes/evidence/six-reported-self-improvement-paths-expose-bounded-redesign-surfaces.md) documents editability, installation, and later dependence.
...
Among these six systems, HyperAgents comes closest to a later-episode test. Transferred evolved hyperagents, with their agent-modifying components frozen, generated stronger agents for an unseen math-grading task than the initial hyperagent did. Each hyperagent bundles prompts, insight files, and code, so the result does not isolate the causal changes. Continued evolution from transferred rather than fresh hyperagents showed no significant advantage. The experiment shows that retained changes improved one later agent-generation episode, not that the feedback recurred.
...
A theoretical [Gödel machine](../notes/goedel-machines-are-a-proof-governed-case-of-self-modification.md) can rewrite even the machinery that chooses later rewrites, but only after proving that the candidate is better than continuing. This excludes beneficial changes it cannot prove. The machine therefore has a broad formal revision surface, but no empirical evidence of compounding. Commonplace studies a narrower surface under fallible semantic judgment.

## Commonplace as a human-inclusive testbed

The six systems look narrower partly because the comparison excludes their research teams. [Commonplace's boundary](../reference/commonplace-declared-frame.md) includes maintainers, so revision-surface breadth is not directly comparable: those teams could also redesign their systems.

Commonplace does not claim merely that humans can edit it. Its claim is that retained procedures and artifacts make human–agent redesign an explicit operating path. Installed changes remain available for later use and revision.

Commonplace bets that linked natural-language theory can be both [interpreted and retained](../notes/theory-mediated-self-improvement-needs-interpretation-and-retention.md). An LLM can apply it to heterogeneous evidence while its claims, assumptions, and scopes remain separately revisable. Because notes are [routed individually](../notes/agent-context-is-constrained-by-soft-degradation-not-hard-token-limits.md), agents can load selected theory instead of the whole layer, limiting competition for context. Storing theory in notes also separates it from Python strings and executable lineages. The hypothesis is that these theories can change how later work is decomposed, not merely supply matching rules.
...
First, the [objective-level ablation](../reference/proposals/ablation-baselines-for-the-declared-objective.md) would compare the full framework with variants that remove or replace parts of it. This would test whether Commonplace earns its total lifecycle cost.

Second, a later-episode experiment would compare Commonplace with frozen-artifact and simpler-memory variants. It must trace how a later revision depends on an earlier benefit, directly or through resources that were freed and deliberately reinvested. Tasks should require heterogeneous changes so that local rule reuse cannot easily stand in for theory-mediated reinterpretation. Outside operators—or maintainers assigned to conditions before any episode—would help separate the artifacts' effects from an experienced maintainer's memory.
