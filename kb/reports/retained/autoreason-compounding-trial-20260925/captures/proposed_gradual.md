...
The requirements form a ladder. A study can reach some rungs and stop:

1. **Accepted.** The change passes its own check.
2. **Retained.** The change is still present when later work happens.
3. **Taken up.** A later improvement episode uses it directly or consumes resources it freed.
4. **Displaced gain.** That later episode does better on a measure other than the accepting metric.
5. **Attributed.** A baseline shows the gain shrinks or disappears without the earlier change.
6. **Sustained.** Rungs 3–5 hold across successive episodes.

Rungs 3–5 together are local evidence of compounding. Rung 6 is evidence that it is sustained.
...
The [Harness Updating Is Not Harness Benefit study](../sources/harness-updating-is-not-harness-benefit.ingest.md) separates producing a persistent update from a task-solving agent benefiting from it. This supplies a finer measurement ladder for the early rungs: update production → persistent retention → artifact loading → judged procedural match → task benefit. A missing earlier stage can explain a missing benefit. Observing a later stage does not by itself show that the loaded artifact caused the behavior or gain.
