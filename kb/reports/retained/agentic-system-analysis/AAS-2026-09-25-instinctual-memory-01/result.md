...
description: 'mem whole artifact with partial host loop: sourced fact extraction,
  ...
  fidelity limits'
run-id: AAS-2026-09-25-instinctual-memory-01
system: mem
run-date: '2026-09-25'
...
memory-comparison:
  axes:
    behavioral_authority:
      assessment: known
      basis: afforded
      note: Facts provide knowledge and preferences/instruction through named external
        agent channels; deployed activation is uninspected. Metadata enforces admission
        and exclusions, validates references, routes by project, and affects lexical
        ranking through corpus frequencies. Instruction force in external AGENTS.md
        consumers is afforded.
      records:
      - BAP-1
      - BAP-2
      - RTE-4
    ...
    curation_operations:
      assessment: known
      basis: wired
      note: Durable statements condense retained traces; near-duplicate culling compares
        retained facts; corrections/upserts revise entries; supersession and suppression
        withdraw reliance while preserving history; hard erasure forgets content and
        temporal eligibility removes expired content from current search. No supported
        promotion or new-claim synthesis route.
      records:
      - RTE-2
      - RTE-3
      - RTE-7
    ...
    distilled_form:
      assessment: known
      basis: wired
      note: Extracted prose statements and evidence coexist with operative typed identity/status/provenance
        fields consumed by later selection. No trace-trained parameters are written.
      records:
      - OBJ-2
      - RTE-2
      values:
    ...
    faithfulness_tested:
      assessment: known
      basis: wired
      note: Within the inspected source-only boundary, evaluation and static tests
        test retrieval/ranking contracts, not retained executions demonstrating a
        host answer's dependence on recalled facts. This is not a claim about undisclosed
        deployments.
      records:
      - RTE-8
      - ABS-1
    ...
    learning_scope:
      assessment: known
      basis: wired
      note: Lasting facts target later tasks; global preferences cross projects and
        project facts use project identities with local/shared stores. No task-local
        extraction route is established; task titles and session labels do not supply
        one.
      records:
      - RTE-2
      - RTE-5
    ...
    learning_timing:
      assessment: known
      basis: wired
      note: Explicit consolidation is a post-retention batch pass; hook sync stages
        it after session end and a backlog threshold. No per-turn online extraction
        is wired. Both describe the same rules/LLM fact route under different invocation
        schedules.
      records:
      - RTE-2
      - RTE-5
    ...
    lineage:
      assessment: known
      basis: wired
      note: Caller-authored facts/tasks, imported traces/documents, automatically
        trace-extracted facts, and deterministic projections/control records all occur.
        Memory-file extraction belongs to imported material plus compilation, not
        automatically to trace learning.
      records:
      - RTE-1
      - RTE-2
      - RTE-3
    ...
    read_back_direction:
      assessment: known
      basis: afforded
      note: Requested tool/CLI reads and task reads are pull; hooks automatically
        select retained facts for a host agent. Managed-file later loading is afforded,
        while writeback generation is explicitly requested.
      records:
      - RTE-4
      - RTE-5
      - RTE-6
    ...
    read_back_signal:
      assessment: known
      basis: afforded
      note: Session start selects pref_user and caps twenty preferences; prompt supply
        selects by project identity and meaningful word overlap, at most six facts.
        Managed-file loading affords coarse supply of its retained block. JEV/local
        judgments serve requested searches, not the automatic hook selector.
      records:
      - RTE-5
      - RTE-6
      values:
    ...
    representational_form:
      assessment: known
      basis: wired
      note: Statements, source text and task titles are natural language; operative
        IDs, roles, statuses, provenance, versions and checkpoint/control fields have
        program-defined semantics. Static opaque models are outside the accumulated-memory
        boundary.
      records:
      - OBJ-1
      - OBJ-2
    ...
    storage_substrate:
      assessment: known
      basis: wired
      note: Journal, intents, tasks and managed output use files; entity facts and
        operational state use the Git memory branch. No user-memory vector or model-weight
    ...
    trace_learning:
      assessment: known
      basis: wired
      note: Automatic rules/LLM transformation of journaled user/session material
        yields durable facts with later tool, hook and writeback consumers. This does
        not establish host obedience, improvement, or conjectural learning.
      records:
      - RTE-1
      - RTE-2
      - RTE-4
    ...
    trace_source:
      assessment: known
      basis: wired
      note: User messages from host-session logs and generic imported event records
        can feed extraction; calendar/voice Note events also reach the LLM branch.
        Tool-role records remain raw recall and do not qualify through the shipped
        extraction filter; no outcome-annotated trajectories are required.
      records:
      - RTE-1
      - RTE-2
      values:
    ...
    write_agency:
      assessment: known
      basis: wired
      note: Humans or host agents explicitly author changes/tasks; machine extraction,
        projections and maintenance operate automatically, including when manually
        invoked. Session-end sync is an automatic trigger.
      records:
      - RTE-1
      - RTE-2
      - RTE-3
      ...
      - automatic
      - manual
  scope: Accumulated journal content and metadata; derived or caller-authored entity
    facts, active-body/index projections, checkpoints, dispositions, controls, publication
    receipts/intents; managed writeback; versioned task file; their acquisition, maintenance,
    and later-consumer routes. Static program/skill prompts and pretrained/provider
    model assets are components, not accumulated memory; transient search/HTTP result
    caches are delivery state.
---
...
|---|---|---|---|---|---|---|---|
| SRC-1 | Git | `https://github.com/jasonkneen/instinctual-memory` | 6acb13dc35765bf5ccfc87e445dd09c480f1c28a | implementation | CLI/operations, extraction/admission, publication and delivery, memory specialist paths | `src/main.rs`; `src/ops.rs`; `src/consolidate.rs`; `src/change.rs`; `src/repo.rs`; `src/validate.rs`; `src/hook.rs`; `src/http_serve.rs`; `src/mcp.rs`; `src/evaluation.rs`; `src/search/rerank.rs`; `src/journal.rs`; `src/entity.rs`; `src/fact.rs`; `src/index.rs`; `src/checkpoint.rs`; `src/controls.rs`; `src/ingest.rs`; `src/search/lexical.rs`; `src/search/journal.rs`; `src/search/laya.rs`; `src/jev.rs`; `src/cli.rs`; `src/shell.rs`; `src/tidy.rs`; `src/erasure.rs`; `tests/recorded_search.rs` | deployed store/host/provider outcomes uninspected |
| SRC-2 | Git | `https://github.com/jasonkneen/instinctual-memory` | 6acb13dc35765bf5ccfc87e445dd09c480f1c28a | doctrine/design | README and shipped skill/prompt instructions | `README.md`; `skills/mem/SKILL.md`; `src/consolidate.rs:576-598` | declaration is not execution evidence |
...
>                 ));
> --- `src/repo.rs` @ `6acb13dc35765bf5ccfc87e445dd09c480f1c28a`
...
> };
> --- `src/tidy.rs` @ `6acb13dc35765bf5ccfc87e445dd09c480f1c28a`
...
> --- `src/http_serve.rs` @ `6acb13dc35765bf5ccfc87e445dd09c480f1c28a`
...
> --- `README.md` @ `6acb13dc35765bf5ccfc87e445dd09c480f1c28a`
...
1. Boundary: SRC-1/SRC-2 and the named routes above; external host decisions, provider internals and live truth state excluded. CLM-1 claims curation and validated durable retention. No observed candidate/store or controlled model-behavior experiment supplied.

2. Objects: OBJ-1 messages/files carry user assertions/instructions with unverified underlying truth. OBJ-2 fact statements can describe projects/people or encode policy; occurrence evidence and active status differ from factual warrant. OBJ-3 control records express operational disposition and progress, not independent truth. OBJ-4 delivery reshapes/selects these contents; a context channel may grant instruction force without new warrant. Schemas/sources are addressable fields, while rationale survives only when present in retained evidence/statement or optional reason; no theory-history guarantee follows.
