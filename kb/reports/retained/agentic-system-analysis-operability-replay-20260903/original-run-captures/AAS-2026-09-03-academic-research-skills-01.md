...
description: "Complete code-grounded analysis of Academic Research Skills at commit 94436237913091d4739870159d241660527e8338, bounded as a response-only complete artifact with a partial external loop"
run-id: AAS-2026-09-03-academic-research-skills-01
...
This is the canonical result for run AAS-2026-09-03-academic-research-skills-01. The frozen source register is SRCREG-v1-94436237913091d4739870159d241660527e8338. The reconciled canonical register is CANON-v2-94436237913091d4739870159d241660527e8338. The two primary lens packets were AAS-2026-09-03-academic-research-skills-01-MEM-P1 and AAS-2026-09-03-academic-research-skills-01-EPI-P1; the conflict-only correction packet was AAS-2026-09-03-academic-research-skills-01-EPI-P2. The caller authorized a compact durable analysis at `kb/agentic-systems/academic-research-skills.md`; it is a publication distilled from this canonical response result, not the canonical result itself.
...
Included are the four skills; sixteen command entry files; thirty-nine prompt roles, three of which are plugin-exposed agents; plugin manifests and hooks; deterministic scripts and schemas; the ordinary ten-stage workflow; direct-mode and alternate-install routes; the Material Passport, ledgers, and caches; citation and package checks; and the optional cross-model transport. Repository-side CI and tests were inspected only when they established implementation shape, never as observed operation.
...
| CMP-6 | Thirty-nine role prompts, including three plugin-exposed agents | Natural-language Markdown in top-level agents and skill-local agent directories | Dispatch conclusion status: claimed; operation conclusion status: uninspected. SRC-1 agents/research_architect_agent.md:1-18 and peers; SRC-2 academic-pipeline/SKILL.md:209-220 |
...
OBJ-12 is split from the original OBJ-11 family because it has a different consumer and force: it is forbidden from model, checkpoint, gate, verdict, and pipeline-transition inputs.
...
RTE-4, RTE-6, RTE-7, RTE-8, and RTE-9 are runtime route families. Their epistemically different functions are split under suffixed annotations in the epistemic lens; those suffixes do not mint parallel route IDs.
...
**Trigger evidence:** OBJ-5, OBJ-6, OBJ-11, OBJ-12; RTE-1, RTE-6, RTE-8; BAP-6, BAP-10, BAP-12. **Inspected boundary:** frozen plugin tree and its declared external host contract. **Depth:** full. The Passport, verification cache, update-check cache, and decision/activity ledgers create several distinct persistence and later-consumption paths, so a brief retained-state check would not separate actual read-back from static shipped context.
...
| RTE-1 | Only the update-check cache is use-derived read-back; command/agent announcement text is static | Valid cache younger than 24 hours becomes at most one additionalContext reminder | Context presence: claimed. Shipped wiring: wired. Operation: uninspected. Activation: wired. Causal effect: uninspected. | ABS-7: SessionStart does not read the Passport or restore workflow state |
...
The source-native mechanism is exact-key or explicit-hash retrieval, not relevance search. Commonplace's read-back distinction fits the cache and intended Passport routes because accumulated material returns to a later action; it does not fit the as-shipped prompt corpus. No source establishes model-context presence, behavioral activation by a model, token savings, protocol adherence, or causal research-quality effects.
...
Both P1 workers echoed exact run, lens, packet, revision, source-register, and canonical-register headers. The EPI-P2 correction echoed the revised header and touched only RTE-4, CLM-2, CLM-3, OBJ-3, OBJ-7, and OBJ-11. All accepted returns stayed inside the frozen commit, used sparse overlays, and made no publication decision.

Central registrations were: EPI-BAP-1 → BAP-10; EPI-BAP-2 → BAP-11; the activity renderer → BAP-12; MEM-ABS-1 and EPI-ABS-1 merged → ABS-6; MEM-ABS-2 → ABS-7; EPI-ABS-2 → ABS-8; and the post-terminal activity-store facet split from OBJ-11 → OBJ-12. Epistemically heterogeneous facets of OBJ-2, OBJ-4, OBJ-6 through OBJ-8, and OBJ-10 remain attached as explicit amendments, while RTE-4, RTE-6 through RTE-9 use functional suffixes. These registrations produced CANON-v2 without changing the source register.

Independent convergence occurred on three findings: the citation cache is executable read-back but not evidence of scholarly effect; the Passport is a detailed prompt/schema protocol without an executable in-tree resume loader; and deterministic checks license only their formal target, never manuscript or empirical truth.
...
Cross-lens ownership checks passed: the runtime account owns endpoints and progression; the memory lens owns read-back and activation distinctions; the epistemic lens owns transformations, checks, warrant, acceptance, and epistemic/operational authority. No curation label was used as warrant, and no behavioral path was treated as epistemic authority. Legacy review projection check is inapplicable because detection was not detected and no legacy review was invoked.
...
| Optional audit extensions were not exhaustively traced | CMP-8 and non-material optional modes | Only extensions needed for the declared question | Claims about every optional audit/mode | A separately scoped feature analysis |

## Verification and blockers

### Semantic verification

Passed. One run ID, boundary, revision, source register, target class, and evidence tier are used throughout. Canonical IDs resolve and proposal mappings are recorded. Conclusion-status fields use only absent, inapplicable, uninspected, claimed, afforded, wired, observed, or causally supported; no field uses implemented as a conclusion status. Epistemic architectural status and observed candidate state remain separate. Ordinary, alternate, and four forcing routes are covered. Guarantee owner, enforcement point, strength, paths, and external contract are explicit. Both full lenses ran, their headers matched, and EPI-P2 corrected the only in-flight conflict dependency. Retention is not read-back; context presence is not activation; operation is not causality; curation/use is not warrant/acceptance; and operational continuation is not PASS. No source-native mechanism was replaced by Commonplace vocabulary.

### Deterministic validation

Exact temporary response artifact /tmp/AAS-2026-09-03-academic-research-skills-01.md: PASS under `commonplace-validate --full` from the Commonplace repository root after the publication-disposition correction. Durable analysis `kb/agentic-systems/academic-research-skills.md` and its `README.md` navigation entry: PASS clean under `commonplace-validate --full`.
