# Multistage write: analyse external epistemic architecture

- **Immutable run key:** `kb/instructions/analyse-external-system-epistemic-architecture.md`
- **Current intended target:** `kb/instructions/analyse-external-system-epistemic-architecture.md`
- **Mode:** new write
- **Collection:** `kb/instructions/`
- **Type:** `kb/types/instruction.md`
- **Acceptance review:** required — this is a behavior-changing review instruction with substantive epistemic distinctions
- **Experiment retention:** required — the user asked to inspect two sub-agent applications of the candidate instruction

## Inputs

- `kb/work/epistemic-architectures/`
- focused Commonplace epistemic and authority notes listed in `brief.md`
- `kb/agent-memory-systems/types/agent-memory-system-review.md`
- `kb/instructions/write-agent-memory-system-review/SKILL.md`
- `kb/instructions/COLLECTION.md`
- `kb/types/instruction.md`

## Checklist

- [x] `brief.md`
- [x] `reconstruction.md`
- [x] `claim-disposition.md`
- [x] `claim-skeleton.md`
- [x] `draft.md`
- [x] `arc-trial.md`
- [x] `gbrain-trial.md`
- [x] `trial-assessment.md`
- [x] `post-trial-draft.md`
- [x] `audit.md`
- [x] `candidate.md`
- [x] `acceptance.md`
- [x] promotion

## Human decisions and blockers

- None. The user authorized writing the instruction and testing it through sub-agents on ARC skill and one already reviewed memory architecture with non-trivial transformations.
- GBrain is the held-out memory-system case: it is code-grounded, already reviewed, implements automatic `synthesize` and trace-learning routes, and was not one of the six epistemic-architecture workshop cases.

## Pending handoffs

- Conditional wiring into `write-agent-memory-system-review` and its type contract is not part of this target. The trials show that the instruction adds knowledge-generation analysis beyond the current memory-review schema, but automatic invocation remains a separately authorized design change.

## Outcome

- Promoted `kb/instructions/analyse-external-system-epistemic-architecture.md` after acceptance review.
- The same pre-repair draft ran cold on ARC and held-out GBrain without clarification and preserved implementation, doctrine, reported-operation, observed-run, and causal-evidence boundaries.
- ARC exposed narrow checked prediction, history-fit, plan-continuation, and replay routes. GBrain exposed direct retention of unaccepted syntheses alongside benchmark-gated non-truth-apt skill adaptation.
- Trial and audit findings produced explicit material-route scoping, separate route function and architectural status, separate observed-candidate state, indeterminate dispositions, post-acceptance-only lifecycle integration, and distinct epistemic, operational, and behavioral authority fields.
