---
description: General instruction for fixing review warnings. Reads prose and semantic reviews for a note, applies fixes using judgment, classifies each fix by strategy from the taxonomy, and reports results.
---
...
1. Read whichever reviews exist for the note:
   - `reviews/{note-stem}.prose-review.md`
   - `reviews/{note-stem}.semantic-review.md`
2. Extract all WARN-level findings. If there are none, report "no WARNs" and stop.
3. Read the target note in full.
...
- **Don't update review files.** Reviews are regenerated by sweeps.
