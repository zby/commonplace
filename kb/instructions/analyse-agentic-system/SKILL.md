...
description: "Use when asked to analyse, review, or refresh the architecture of an external agent runtime, orchestration system, agent operating layer, or narrower model-dependent operational mechanism from inspectable sources."
...
Do not produce product rankings, generic adoption advice, a universal taxonomy or maturity ladder for agentic systems, or any claim beyond the declared evidence boundary. This skill owns orchestration of the run inside the caller's existing authority: source preparation, lens scoping, lens execution, reconciliation, the logical result, verification, and reporting. It does not itself authorize source acquisition or refresh, mutation of a supplied checkout, file publication, or retained report creation. The agent executing this skill is the orchestrator referred to below; lens workers execute inside its ownership and never establish their own boundary or publication.
...
6. If no coherent system or subsystem boundary can be stated, treat the run as blocked: report the blocker and stop.
...
4. Write the synthesis as an evidence basis and boundary, an architectural characterization and claimed work, a runtime map, only the discriminating mechanism sections this target needs, a scenario-relative assessment, and concrete changes that would alter the assessment. Organize the mechanism account around the operational progression — scheduling, context, state and action, memory return where applicable, truth-apt and warrant routes where applicable, and governing controls — not as concatenated lens reports. Preserve affordance-versus-wiring-versus-operation and evidence-layer limits inside the synthesis. Mention a lens's thinness only where it bounds conclusions. Do not assign a system-wide epistemic grade.
5. In the assessment, distinguish a supported mechanism for a named work story, a responsibility deliberately externalized through a named contract, a tradeoff, a gap relative to claimed work, an unknown, and a misleading implication. Do not treat a non-runtime mechanism as a deficient runtime or turn the assessment into a total ranking.

### 9. Emit and route the result

Produce one complete logical run result whether the physical form is a structured response, one file, or a package. File creation is conditional on the authority and retention rules below; a response-only result is complete.

Required logical records, in order:

1. run/result identity;
...
- Give the run result one canonical identity, with IDs resolvable across all physical parts.
...
- No file publication is a blocker only when the caller requested a file result and no authorized target contract can represent it. Publishable limitations include doc-only evidence, inaccessible components, no observed run, no causal experiment, trigger evidence too thin to resolve, and conflicting evidence — each naming its scope and prevented conclusion. Result blockers include missing logical records, ID collisions, unsupported material claims, and failed applicable validation.

### 10. Verify and report

1. Verify: source anchors; every conclusion status is one of the eight values listed in step 3, and no record carries `implemented` as a conclusion status; unique, resolving IDs; one target classification, boundary, and revision across all records; ordinary and material alternate routes covered; each warranted forcing case completed or bounded, with a rationale when none is warranted; load-bearing guarantees name their owner, enforcement point, strength, paths, and external contract; both lens scoping records present; both lens outputs present, each meeting the brief-output floor; prevented conclusions stated for every thin, negative, or unresolved finding; shared-route ownership respected; no forbidden evidence upgrades.
...
3. Run the deterministic validation required by each chosen existing target contract. Until a dedicated run-result contract exists, use applicable generic validation plus the semantic checklist above. When no authorized file target applies, record `no deterministic validation applicable` for the response alongside the semantic checklist result and treat that as complete verification. Do not change schemas or parsers, and do not adopt an unrelated contract, to manufacture a validation path.
4. Report: result identity and response or authorized locations; target classification; boundary, revision, and tier; both lens scoping records and the depth each lens ran at; retention disposition for any files; limitations; and blockers.
...
- The synthesis is organized around the system, contains no system-wide epistemic grade, and the emitted run result contains all eleven logical records or an explicit blocker report. Any file output follows an authorized collection and retention contract.
