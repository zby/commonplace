...
- `{model-partition}` — review model partition, for example `claude-sonnet-5` or `codex`. Default to the designated review model's partition (see below).
...
The partition is fixed at selection, before any worker runs, so the orchestrator must choose it up front. **Default review model policy**: gate reviews run on the cheapest adequate model — currently `claude-sonnet-5` — regardless of the orchestrator's own model. Launch each worker with the model explicitly overridden to the designated review model and select with the partition `build_model_partition` maps it to (`claude-sonnet-5`; the registry is `MODEL_PARTITION_REGISTRY` in `src/commonplace/review/review_model.py`). No per-gate exceptions are currently declared; if evidence shows a gate under-catching on the default model, that gate's criterion file may declare a stronger required partition, and selection then splits by partition. The worker still reports the model it actually ran, and the orchestrator finalizes with that exact reported model; if the reported model maps to a different partition than the job's, the launch override did not hold — re-run under the correct partition rather than forcing the finalize.
