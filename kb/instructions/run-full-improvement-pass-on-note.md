...
**Collection/type fit:** <FITS | MISPLACED — for MISPLACED: the register mismatch, the collection/type contract it fails, and the fitting collection; a MISPLACED note takes Disposition `rehome`>
...
- Do not reframe a wrong-collection note into native-sounding prose and keep it. A register misfit is a `rehome`, judged before reframe; reframe fixes a wrong-shape title, not a wrong-collection artifact.
