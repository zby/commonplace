...
## Disposition
**keep | keep (reframe) | delete | merge into `<target-path>`** — <for delete/merge: one-line rationale naming the source finding; for a keep reframe: "reframe — title overreaches (<how: e.g. equivalence, wrong direction, universal>), warranted claim is <weaker claim>"; for plain keep: "no note-level finding" or "contribution undetermined; retained intent or authorial choice required">
