...
This workflow supplies `types/snapshot.md` as the type, a global type specified in [snapshot.md](../../types/snapshot.md). Open that spec and verify from its own frontmatter that it is a type spec before determining metadata. Stop if it is missing or invalid.
