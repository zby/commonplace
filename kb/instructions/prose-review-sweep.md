...
Run the prose gate bundle across top-level notes in `kb/notes/`. Selection is gate-based: the selector emits stale `(note, gate)` pairs, then execution groups them by note and lens.

## Steps

### 1. Inventory

Run `uv run scripts/gate_selector.py prose-review --json` to get the stale gate queue.

The selector checks missing recorded gate reviews, gate-file hash changes, and watched-region hash changes for each prose gate.
...
- the specific gate files under `kb/instructions/review-gates/prose/`
