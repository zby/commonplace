...
Minimal. Plain instructions need `description` and `type: kb/types/instruction.md`. Promoted skills add skill-specific fields (`name`, `allowed-tools`, `context`, `model`) in their `SKILL.md`. Review gates use `type: kb/types/review-gate.md` and the gate-specific fields documented in that type spec; see `REVIEW-SYSTEM.md` for runtime concepts.
