---
description: Complexity review — flags notes where presentation complexity exceeds idea complexity. Catches inflation that passes prose review because each sentence reads fine individually.
---
...
A read-only review that asks: is this note more complex than its idea warrants? A note can have clean prose, correct content, and valid structure while still being inflated — too many sections, too many framings, too many connections for what is fundamentally a simple insight.
...
```
=== COMPLEXITY REVIEW: {note-filename} ===
