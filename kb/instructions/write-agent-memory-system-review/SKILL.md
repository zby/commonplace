...
12. **Run semantic QA.** Run the live-agent procedure from `kb/instructions/run-review-batches-on-note.md` on the new review note using the `semantic` bundle: create the jobs and prompts with `commonplace-create-review-jobs`, delegate each returned job to a sub-agent, then ingest each sentinel-bracketed output with `commonplace-ingest-bundle-output`. Treat it as a read-only QA loop: extract findings, fix clearly valid issues, and leave uncertain findings for the final report rather than forcing a rewrite.
