...
The research target is a factory that learns from what it produces. The definition has two layers. The causal layer is **experience-responsive retention**: relative to a declared learner boundary, production experience—failures, surprises, evaluations, corrections, delayed consequences—causes a retained change to the factory's reusable production machinery, and later production or factory development depends on that change. Retention is boundary-relative because the same episode can be retention by a human-inclusive composite while remaining externally supplied engineering for the narrower technical subsystem inside it; what counts is what determined the update, not who typed the result. [Factory-level learning](../notes/factory-learning-retains-experience-in-reusable-machinery.md) is retention that improves the factory. Improvement is objective-relative, so learning carries a second index besides the boundary: the objective the retained change made the factory better against.
