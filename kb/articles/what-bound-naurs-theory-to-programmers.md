...
Separating execution from formulated criteria reopens a question Naur's argument had closed without answering it. [*Theory building inside the system*](./theory-building-inside-the-system.md) records, function by function, which parts of theory building the traced episodes warrant as the machinery's and which stayed with a maintainer. [*When systems learn theories about themselves*](./when-systems-learn-theories-about-themselves.md) takes up the reflective case. [*Continual learning outside the weights*](./continual-learning-outside-the-weights.md) applies the interpreter-plus-artifact possibility to a second learning substrate.
