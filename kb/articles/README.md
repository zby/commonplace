...
- [Theory building inside the system](./theory-building-inside-the-system.md) — given an interpreter that can apply retained theory, says what makes the theory operative rather than documentary, how it becomes learned state, which functions of theory building have moved from human to computational actors in one system and where that stopped, and what formalization moves rather than removes.
...
- [Moving revision decisions into the automatic system](./moving-revision-decisions-into-the-automatic-system.md) — treats human-inclusive self-revision as the cheap case, states the six things a revision decision needs represented before machinery can make it, and reads Commonplace's six-path audit for which decisions could move and the two that cannot yet.
