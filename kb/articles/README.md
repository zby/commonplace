...
- [Bootstrapping an Autonomous Theory Builder](./bootstrapping-an-autonomous-theory-builder.md) — currently a rewrite plan, not an article. The replacement will claim that starting from a working human-agent system, with people supplying the functions not yet automated, is what makes the automation bet tractable, and will cover the transfer of roles to computation one class at a time, reflection on the builder's own machinery, readiness and stop conditions, and the price paid in warrant. The superseded draft stays in git history.
