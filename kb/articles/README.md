...
- [The reachability conjecture: the LLM stays fixed, the software house learns](./reachability-conjecture-the-llm-stays-fixed-the-software-house-learns.md) — an automated software house capable of open-ended coherent change is reachable with fixed current LLMs by training the software and notes around them; why a program theory is needed and why the composite could hold one; the training path and its seed; and the four obligations a constructive witness must meet.
