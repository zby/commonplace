...
- [Bootstrapping a Fully Automated Learner with Commonplace](./bootstrapping-an-autonomous-theory-builder.md) — develops the lead's first arrangement. A fully automated learner need not be built in one step: start from a partially automated system in which operator and machinery learn together, retain what is learned in the system, apply it reflectively to the learning machinery, move functions from the operator to the machinery one at a time with the residue recorded, and let the learner build the software that operationalizes its knowledge. Commonplace is the implementation.
