...
The second benefit is legibility. Both weight and artifact changes can have rationales, logs, and evaluations, but an artifact places the operative content itself in a readable diff that can be revised directly. Behavioral evaluation tests its consequences, which still depend on the model, context, and retrieval.

A third conjectured benefit is [sample efficiency](../notes/theory-mediated-self-improvement-needs-interpretation-and-retention.md). The wall-clock explanation illustrates the claim: if it captures the cause, it can prevent a failure in a different time-dependent test, while "retry this test" cannot. An explanation that survives such a task shift may adapt the system from fewer new observations than retraining or fine-tuning would require.
...
A reflective agent needs controls that do not rely on the same text they judge. In Commonplace, [typed artifacts](../notes/why-notes-have-types.md) state what kind of claim each artifact may make. Mechanically decidable rules run as code. Semantic [review gates](../reference/README-REVIEW-SYSTEM.md) assess a candidate against a fixed criterion in a fresh context rather than continuing the conversation that produced it. Maintained indexes support retrieval.
...
Guarding writes does not decide what is worth retaining. For reusable explanatory claims, Commonplace selects for [explanatory reach](../notes/first-principles-reasoning-selects-for-explanatory-reach-over.md): explaining why something works can support its application beyond the episode that produced it. The supporting episodes remain with the theory so later evidence can overturn it, and review tests [whether the claimed reach is genuine](../notes/definitions/reach-assessment.md). Other artifacts, such as commitments and procedures, have different contracts.
...
- What system boundary is being assessed, and which of its own instructions, code, memory, or other behavior-shaping structures does the retained artifact represent and change?
- What independently specifiable objective does the evidence bear on?
- Where do candidate changes come from?
- Where can the loop reject a candidate?
- Which artifact carries an accepted change into the next run, and how does that run consume it?

The last three questions are the loop's [search, evaluation, and retention functions](../notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md). The first two establish the self-reference and evidence-responsiveness that make the pathway reflective and self-improving.
