...
Start with an episode from the system this article describes. An index page in a knowledge base promised to list every note carrying its tag. It also had to stay short, because index pages are loaded into the agent's context, where space is scarce. The two demands pull against each other, and nothing enforced either: staying complete made the page grow as notes accumulated, and by the time anyone measured, it was far past its context budget — and too large for anyone to still check that it was complete.

The repair went further than trimming the page: a recorded decision turned the completeness promise into a machine-checked mark and put size gates on the page type — a warning as a page approaches the cap, a failure beyond it — so the next page outgrowing its completeness gets flagged before the promise breaks. Bringing the pages into compliance then caught a member the documented search recipe had missed, and the recipe was corrected as well. Operation revised the system's definition, and the revised definition changed operation. The whole trail is in the commit history.
...
Across the list, each system either retains without governing, or justifies its changes through a channel — proof, fitness, benchmark — that admits only what the channel can measure. Our approach is to create a single operative self-representation from text artifacts — natural language and code — and to use LLMs to reason about it and change it. Reasoning in natural language widens the set of changes that can be proposed, criticized, and adopted; the cost is that such judgment needs verification structure outside itself, which much of this article is about.
...
The division of labor in that episode: automation did the checking — the validator enforced the mark and caught the missed member. The maintainer noticed the strain, initiated the fix, and accepted the result, with an agent drafting the candidate change in between. [Which functions are human, joint, or computational](../notes/methodological-and-computational-closure-track-different-changes.md) is read per decision, not per system.
...
## The bitter lesson binds production, not form
...
The common reading of the lesson is that code and prompts are hand-designed and will therefore be dissolved by learning at scale, which can only happen in weights. But the lesson's own contrast is between leveraging human knowledge and leveraging computation through search and learning — a claim about how behavior-determining content is produced, not about the form in which it is kept: [the lesson selects production methods, not representational forms](../notes/the-bitter-lesson-selects-production-methods-not-representational.md). Nothing in that contrast forbids learning whose products are text artifacts. Our conjecture is that a loop of this kind can scale: search over theories, instructions, tests, schemas, and programs; evaluate candidates; retain the selected ones as addressable text artifacts; and later revise or retire them. In that architecture the artifacts would be products and working state of learning — no more contrary to the lesson than learned weights are.

What the objection does constrain is how the structure is produced: hand-designing it does not scale, so the project must move toward search and learning over its own artifacts. Early fragments of that loop already run: prompt optimizers, evolutionary code search, automated harness engineering — though none yet manages a large, interdependent corpus over time.
...
The remaining question is empirical, and the declared objective exists to answer it: does an evolving, addressable layer of text artifacts improve adaptation, reliability, governance, or the objective itself, compared with stronger base models and simpler memory? Self-hosting generates the evidence; it does not settle the comparison. Early external evidence exists: [a July 2026 preprint](https://arxiv.org/abs/2607.19592) reports gains from a curated knowledge base as the only improving object, surviving transfer to a different model family — though [what improves there](../sources/knowledge-centric-self-improvement-2607.19592.ingest.md) is task knowledge, not the learner's own organization.
...
The rest of the defense portfolio — which artifacts deserve codification, [whether external structure recurs as deployments move to harder tasks](../notes/scaling-absorbs-scaffolding-at-fixed-difficulty-not-at-the-frontier.md), where explanatory reach fits — is [catalogued separately](../notes/the-bitter-lesson-defense-portfolio-has-one-load-bearing-member.md); none of it is a prerequisite for this article's claim.
...
The [self-improving-systems cluster](../notes/self-improving-systems-README.md) maps the theory behind the questions, and [the repository](https://github.com/zby/commonplace) contains the framework itself, developed in the open with its own methodology.
