---
description: "Defines reflective self-improvement and develops a functional decomposition of its loop, the omitted/frozen distinction across recent systems, verification-bounded migration, a five-question test, and the bitter-lesson constraint"
...
> **TL;DR.** Agents that rewrite their own prompts, memory, skills, and harness code now exist. Read against a functional decomposition of the improvement loop — search, evaluation, operative retention — each of the recent systems leaves some function either *omitted* (nothing performs it) or *frozen* (performed by machinery the loop cannot revise); the placements are mostly inherited rather than argued, and reported benchmark gains cannot reveal which condition holds. A loop therefore needs a theory of its own functions retained as governed, revisable artifacts. Diagnosis must distinguish omitted functions from frozen ones because they require different repairs, and decisions should move from humans into computation only as far as verification warrants. The loop can start before autonomy — Commonplace runs it human-inclusively, and applying its own decomposition to the recent systems has already forced revisions of that decomposition. Not yet shown: sustained compounding, autonomous semantic evaluation, or advantage over stronger models and simpler memory.
...
> An omitted function needs something to perform it. A frozen function needs either a governed path for revision or an explicit reason to remain fixed.

The freeze shows in the loops themselves. Read against the loop's [functional decomposition](../notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md), [each leaves a function either omitted — nothing performs it — or frozen outside the loop's own update space](../notes/an-omitted-loop-function-and-a-frozen-one-need-different-repairs.md): an edit nothing could refuse, or a gate the loop it governs cannot revise. Only the Darwin Gödel Machine defends any of its placements, and the two conditions need different repairs that reported benchmark gains cannot tell apart. Which frozen functions can move into the loop depends on what the loop can verify. This rule connects the functional decomposition to the omitted/frozen distinction, and the resulting map remains revisable: applying it to these systems exposed retirement as a function for which the decomposition has no slot, while the Darwin Gödel Machine's defended freezes split frozen functions into protective and affordable ones.

The closest comparison to Commonplace is the [reviewed Exo version](../agentic-systems/exo.md) (commit `ef4cfe05`, 2026-07-29): it changes both natural-language and executable artifacts under versioning, deliberate removal, and rebuild-gated activation, but memory facts lack declared scope, provenance, and review state, and durability requires no independent semantic review. Commonplace begins with that governance problem and provides far fewer runtime capabilities.
