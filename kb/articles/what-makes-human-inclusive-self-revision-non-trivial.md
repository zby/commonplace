...
Imagine a constitutional order whose authoritative text is kept in a writable file. Someone with access can change the file, but that alone does not constitute an amendment. A formal amendment acquires authority through the roles and procedures the order recognizes, enters the authoritative record, and affects practice when institutions apply it. If the order permits changes to its amendment rule, the same process can replace part of its own revision machinery.

The people who propose, approve, interpret, and apply the amendment act within the order through recognized roles. The order need not encode their private reasoning.

Commonplace's human-inclusive claim has the same shape. Its declared boundary includes designated maintainers, while the systems compared in [the reflective self-improvement article](./reflective-self-improvement.md) draw their reported boundaries around agent loops. Including their research teams would make those systems broadly editable too. Raw editability is therefore not the claim.

The audit tested this hypothesis:

> Commonplace gives its human–agent operator a reusable path for revising any repository-defined artifact or relation through which behavioral authority or revision governance is exercised. The path supports identifying and criticizing the incumbent, developing a successor, warranting the transition, installing it in a live authority path, and later revising what it installs.
...
*Meta* remains a role within an episode, not a permanent layer. Repetition belongs to the revision process: each episode can apply the same addressability requirement to the authority-bearing arrangement left by the previous one.

The [six-path audit](../notes/evidence/six-commonplace-paths-establish-broad-addressability-not-completeness.md) found broad, path-relative addressability and operative reuse of natural-language and symbolic revision machinery. It examined representative rather than exhaustive paths, so it did not establish complete behavioral-authority coverage. The repository also did not represent generic maintainer admission, and retained evidence showed requested or recorded model identities diverging from actual execution. The first gap concerns governance; the second, operative realization.

The declared substrate boundary sets the affordance's outer limit, and each authority path is assessed separately. The affordance does not by itself establish compounding. The main article asks whether retained benefits [help produce later improvements](../notes/improvements-can-accumulate-without-compounding.md), directly or through reinvested savings, and whether that feedback persists across episodes.

## Why editability is not enough

Including humans makes a weak reflection claim easy: a maintainer can model the system and act on that model. A stronger claim requires a [self-representation to affect operation](../notes/definitions/reflective-system.md). Its practical benefit is [addressability](../notes/reflection-buys-addressability.md): a represented commitment can be inspected, criticized, selectively revised, and reused.
...
Together, these obligations define the system-level affordance. The narrower [repeatable-path test](../notes/a-repeatable-operative-path-keeps-a-redesign-class-open-to-revision.md) traces representation, determination, admission, installation, dependence, and continuity for one redesign class.
...
The successor need not be a one-for-one replacement artifact. A better arrangement may narrow a rule, split one concept into several, reroute its useful work, or remove an artifact whose role is no longer warranted. What matters is comparison with continuing the incumbent arrangement.
...
Commonplace combines general theory, specialized methods, and operative artifacts rather than one universal revision algorithm. The audit found parts of the general affordance in this human–agent environment.
...
Agents and maintainers operate through contracts, skills, review criteria, definitions, and code that they can also revise. Their use gives these artifacts behavioral authority. This structural self-application has the [metacircular-interpreter shape](../notes/llm-executed-methodologies-are-metacircular-interpreters.md): the rules are artifacts in the system they govern. When those rules do not settle the next choice, live theory and human judgment provide the fallback. Retaining the result in operative artifacts turns the intervention into part of the system's revision machinery rather than an undocumented action outside its description.
...
- [ADR 042](../reference/adr/042-register-becomes-a-default-profile-under-open-ended-text-contracts.md) replaced the claimed exhaustive three-register taxonomy after a worked dialectical collection supplied a counterexample. The successor kept the useful theoretical, descriptive, and prescriptive profiles inside an open text-contract model. It changed the definition and root vocabulary and later made the article collection's editorial profile possible.
...
The audit examined global goals, the explanatory-reach criterion, tag-README validation, the revision lifecycle, model bindings, and maintainer admission. Goals, contracts, criteria, procedures, validator rules, and model requests were inspectable and selectively revisable. In the validator and lifecycle cases, installed machinery was also reused and revised again. This is stronger than repository writability and weaker than complete addressability.
...
Model binding exposes a different gap. The repository makes the requested model, alias, and freshness partition addressable. But retained evidence shows requested or recorded identity diverging from actual execution. This is an operative-realization gap, not an addressability failure of the request.

A gate can change its target cohort, or a validator its target type and invocation trigger, while consumer, channel, and force stay fixed. The current three-part decomposition therefore does not fully identify an authority path. The live [decomposition proposal](../reference/proposals/revise-behavioral-authority-decomposition.md) now owns the choice between adding applicability as a fourth field and treating it as a required qualifier. Authorization, runtime realization, and dependency closure remain separate questions.

The audit covers six paths within [Commonplace's declared frame](../reference/commonplace-declared-frame.md). Provider weights, inference infrastructure, and hosting lie outside that frame. Wider coverage may reveal another barrier. Even complete coverage would not establish compounding, which still requires evidence that a retained benefit helped produce a later improvement.
