...
- **Requirements live in theory.** Transferable requirements are claims — they belong in `kb/notes/` and are cited from here via a `rests-on` edge. The proposal inlines only system-specific constraints: stats, precedents, integration boundaries.
