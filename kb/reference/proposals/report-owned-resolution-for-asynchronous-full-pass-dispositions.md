...
It assumes, and does not respecify, the [artifact-version substrate](./artifact-version-substrate-for-pinned-operation-inputs.md): packet-owned capture of the exact text an operation read under an unchanged logical identity, comparison of that capture against current state, and a guard that refuses a report-driven transition when they differ. This proposal is that substrate's first consumer and supplies one target-owned response to a changed input — `superseded`. Where the two overlap, the substrate proposal is authoritative on capture, comparison, and guarding; this one is authoritative on what a disposition means and who resolves it.
...
`source` and `merge_target` are repository-relative logical identifiers. They are code-valued fields, not Markdown links, so they do not assert graph relationships. `source_capture` and `merge_target_capture` are packet-relative `.txt` paths. Each capture is authoritative for this pass, immutable, and verified against its recorded SHA-256 whenever loaded. The report and its captures are one retention unit; review snapshot retention is irrelevant to their lifetime.

A merge disposition requires `merge_target`, `merge_target_capture`, `merge_target_title`, and `merge_target_sha256`; other dispositions require all four to be null. Step 7 treats a synthesized merge as provisional: the orchestrator captures the target's current text, supplies it and the logical target path back to synthesis, and finalizes the recommendation only after confirming the rationale against that pinned target. `merge_target_title` records the pinned target's H1 so the recommendation reads standalone once the packet outlives its method reports. The report body records the source finding, standalone rationale, what a replacement would need, and any unresolved routed attention that must survive loss of those reports.

The orchestrator writes `source_capture` before running the pass and treats that text as the pass input. It does not reconstruct the capture later from the then-current note or depend on a review snapshot with the same path and hash. Report resolution may mutate the report's state; a capture, once written, is never rewritten.
...
- **Accepted deletion:** selectors no longer enumerate the absent source. Its old review rows and snapshots follow ordinary review pruning. The packet owns the assessed text needed for its resolution record, so review cleanup has no full-pass retention obligation.
...
- **Substrate dependency.** This design is unimplementable without the narrow pinning seams the substrate proposal specifies — review-job applicability and prompt preparation read the live note today, and direct methods do not all accept supplied content. If the substrate is not adopted, a full-pass packet can still record hashes, but its methods will disagree about what they assessed whenever the note is edited mid-pass.
- **Concurrent source edits.** Pinned input makes reviewers internally consistent even when the live note changes, but it cannot license applying an edit plan to different text. The pre-application hash check is a hard stop, not an acknowledgement path or automatic merge. The live edit is preserved.
...
1. A pending delete or merge leaves the source note byte-identical; while its version guard still matches, its initial review baselines remain fresh. Its packet retains and hash-verifies the exact assessed text.
...
5. A live-source mismatch prevents both automatic application of a `keep` packet and execution of an old delete/merge recommendation; a live-target mismatch also prevents an old merge. The consumer preserves the live edit and can show each mismatch against the corresponding packet capture.
...
9. Validation rejects incomplete or inconsistent capture and merge-target fields, a missing capture, a capture whose content does not match its recorded hash, incomplete terminal-resolution metadata, invalid resolution authority, and disagreement between structured report state and the rendered Resolution section.
10. Report cleanup cannot silently delete pending or still-load-bearing rejected resolutions or their packet-owned captures.
11. The local persistence boundary covers the packet and its captures as one retention unit and is tested across sessions in one working copy.
...
- [Artifact-version substrate for pinned operation inputs](./artifact-version-substrate-for-pinned-operation-inputs.md) — part-of: supplies the packet-owned capture, comparison, diff, and guard contract this design consumes
