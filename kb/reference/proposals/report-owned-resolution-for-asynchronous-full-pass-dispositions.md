...
It assumes, and does not respecify, the [artifact-version substrate](./artifact-version-substrate-for-pinned-operation-inputs.md): capture of the exact bytes an operation read under an unchanged logical identity, comparison of a capture against current state, and a guard that refuses a transition when they differ. This proposal is that substrate's first consumer and supplies one target-owned response to a changed input — `superseded`. Where the two overlap, the substrate proposal is authoritative on capture, comparison, and guarding; this one is authoritative on what a disposition means and who resolves it.

## Current state (as of 2026-07-13)

The operative [full-improvement-pass instruction](../../instructions/run-full-improvement-pass-on-note.md) runs critique and semantic review against the initial note, records a note-level `keep`, `delete`, or `merge` Disposition in its packet, and handles a non-`keep` result by inserting a `TODO(full-pass)` blockquote into the note. It then skips the closing cycle.

That short circuit conflicts with the shipped [full-improvement-pass closure](../full-improvement-pass-closure.md): the marker changes final note bytes after the initial freshness baselines were written, but no closing review re-pins them. Semantic gates watch body content, and critique effectively watches the whole note, so trivial-change acknowledgement cannot carry the initial evidence across the insertion.

The marker has no lifecycle consumer. No selector, validator, sweep, or fix instruction finds it, and a repeated pass can insert another marker. A merge target written as a Markdown link is also parsed as a real graph edge and checked for link health even though it represents a proposed maintenance operation rather than a knowledge relationship.
...
source_sha256: <sha256 of the note assessed by the pass>
pass_id: <pass-id>
disposition: keep | delete | merge
merge_target: null | kb/notes/target.md
merge_target_title: null | <title from the pinned merge target>
...
All stored paths are repository-relative identifiers. They are code-valued fields, not Markdown links, so they do not assert graph relationships. A `(path, sha256)` pair names a capture in the shipped snapshot store — the packet does not own a copy of the note, so it needs no snapshot-file field, no capture validator, and no rule that a sibling file stay byte-identical. A pass may still write a derived, human-readable copy of a capture into its directory for inspection, but that copy is explicitly not the authority and nothing resolves against it.

A merge disposition requires `merge_target`, `merge_target_title`, and `merge_target_sha256`; other dispositions require all three to be null. Step 7 treats a synthesized merge as provisional: the orchestrator captures the target's current text, supplies it and the logical target path back to synthesis, and finalizes the recommendation only after confirming the rationale against that pinned target. `merge_target_title` records the pinned target's H1 so the recommendation reads standalone once the packet outlives its method reports. The report body records the source finding, standalone rationale, what a replacement would need, and any unresolved routed attention that must survive loss of those reports.

The orchestrator captures the source before running the pass and treats that text as the pass input. It does not reconstruct the capture later from the then-current note, and it records the capture it created rather than assuming that any snapshot sharing the same path is the same input. Report resolution may mutate the report's state; a capture, once written, is never rewritten.

### What this workflow pins

The capture, comparison, diff, and guard mechanics are the [artifact-version substrate](./artifact-version-substrate-for-pinned-operation-inputs.md)'s, including the input-boundary seams each method needs and the invariant that a capture path is never substituted for the logical path. This proposal fixes only which artifacts a full pass pins and what its guards protect.

A pass pins two artifacts, both under their unchanged logical identities:

- **The assessed note** (`source` / `source_sha256`). Every method — compression, critique, composition friction, semantic review, connection discovery, and step 7 synthesis — reads that captured text, and none performs a live-note read. The logical path stays `source`, so collection rules, type context, relative-link bases, report naming, and the note side of every freshness key still resolve from `kb/notes/example.md` rather than from wherever the capture is stored.
- **The merge target, when one is proposed** (`merge_target` / `merge_target_sha256` / `merge_target_title`). Step 7 treats a synthesized merge as provisional: the orchestrator captures the target's current text, supplies it and the logical target path back to synthesis, and finalizes the recommendation only against that pinned text. A later live-target read never substitutes for it.

Pinning both is what makes an asynchronous merge recommendation checkable later: a recommendation is a claim about a *pair* of artifacts, so a guard over the source alone would let the target drift out from under it unnoticed.

The live note may change concurrently without changing what the pass assessed. Such a change makes the resulting freshness baseline immediately stale, which is correct. Before applying a `keep` packet, step 8 must compare the live note to `source_sha256` and abort without editing if they differ. Before resolving a `delete`, the asynchronous consumer performs the same comparison. Before resolving a `merge`, it compares both the source and target to their recorded hashes. Any mismatch follows the supersession rules below; a missing or relocated target is a mismatch, not permission to find a substitute implicitly.
...
| `superseded` | The source bytes changed enough that the recommendation no longer applies without reassessment. |

The actor verifies `source_sha256` before acting and, for a merge, also verifies `merge_target_sha256`. A mismatch is explained with a diff from the corresponding retained snapshot to the current artifact; it does not silently reject or accept the recommendation. The actor resolves it as `superseded` or starts a new pass. An accepted operation is recorded only after the filesystem change succeeds and its resulting paths are verified. If a pending report names a source that is already absent, the consumer must ask for reconciliation rather than infer acceptance; deletion, relocation, and partial merge failure are observationally ambiguous. The retained snapshot may show the removed content, but it cannot prove whether deletion, relocation, or a partial merge was intended. The same reconciliation rule applies when a merge target is absent or has moved.

Rejection is a human judgment over one recommendation against its guarded inputs: the assessed-note snapshot and, for a merge, the target snapshot. A later pass over the same bytes may run when explicitly requested, but its synthesis must load the prior rejection and may not repeat the same disposition without materially new evidence. Review workers do not receive the prior resolution, since it would bias the independent assays; only the orchestrator sees it during reconciliation. A changed guarded-input hash removes that constraint while retaining the earlier decision as history.
...
- **Pending:** the initial critique and semantic baselines remain fresh only while the live source still matches `source_sha256`. A mismatch stales them normally and supersedes the recommendation rather than acknowledging the change.
...
- **Accepted deletion:** selectors no longer enumerate the absent source. Its old review rows are harmless historical storage and may be garbage-collected by a separate policy if accumulation becomes material — with one new constraint, since packets now resolve their captures through the snapshot store: a future pruning policy must not remove a snapshot that a pending or rejected packet still guards against. Reusing the store instead of copying it moves that liveness requirement into the DB rather than eliminating it.
...
- **Where captured text lives.** Delegated to the [artifact-version substrate](./artifact-version-substrate-for-pinned-operation-inputs.md), whose settled default reuses the shipped snapshot store rather than giving each packet its own capture file. The packet therefore records `(path, sha256)` and resolves the text through that store. The cost this proposal accepts: a packet is only diffable where the review database is. The benefit: no capture files, no capture validator, and no cleanup rule binding a packet to files it owns.
...
- **Substrate dependency.** This design is unimplementable without the pinning seams the substrate proposal specifies — review-job capture and prompt rendering read the live note today. If the substrate is not adopted, a full-pass packet can still record hashes, but its methods will disagree about what they assessed whenever the note is edited mid-pass, and the guard will protect a recommendation nobody can reconstruct.
- **Concurrent source edits.** Pinned input makes reviewers internally consistent even when the live note changes, but it cannot license applying an edit plan to different bytes. The pre-application hash check is a hard stop, not an acknowledgement path.
...
1. A pending delete or merge leaves the source note byte-identical; while its version guard still matches, its initial review baselines remain fresh. Its packet retains the exact assessed bytes.
2. Every method and the synthesis step consume the pass's pinned inputs through the [substrate contract](./artifact-version-substrate-for-pinned-operation-inputs.md) — satisfying its adoption criteria 2 through 4 — with no intervening live read; a proposed merge is finalized only against the pinned target.
3. The pending selector finds the report without reading note bodies or treating target paths as graph edges.
4. A repeated pass over the same guarded inputs returns the pending report rather than running new assays.
5. A live-source mismatch prevents both automatic application of a `keep` packet and execution of an old delete/merge recommendation; a live-target mismatch also prevents an old merge. The consumer can show each mismatch against the corresponding retained snapshot.
6. Accepted delete and accepted merge record resolution and verify resulting paths; merge edits stale only the surviving target's relevant reviews.
7. Rejection requires matching guarded inputs, keeps the note and its baselines unchanged at resolution, records user authority and a rationale, and prevents unsupported repetition of the same recommendation during later synthesis; an opposite-direction merge can be recorded as `alternative-applied`.
8. The report resolves `type: kb/reports/types/full-pass-report.md`; ignore rules and report cleanup preserve `kb/reports/types/` while applying resolution-aware retention only to instances under `kb/reports/full-pass/`.
9. Validation rejects incomplete or inconsistent merge-target fields, a capture the store cannot resolve for a recorded `(path, sha256)`, incomplete terminal-resolution metadata, invalid resolution authority, and disagreement between structured report state and the rendered Resolution section.
10. Report cleanup cannot silently delete pending or still-load-bearing rejected resolutions, and no snapshot-pruning policy may remove a capture a pending or rejected packet still resolves against.
11. The local persistence boundary covers the packet and the captures it resolves, and is tested across sessions in one working copy.
...
- [Artifact-version substrate for pinned operation inputs](./artifact-version-substrate-for-pinned-operation-inputs.md) — part-of: supplies the capture, comparison, diff, and guard operations this design consumes; adopting this proposal without it leaves the guards unimplementable
