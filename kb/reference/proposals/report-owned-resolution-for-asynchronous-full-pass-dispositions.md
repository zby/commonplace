...
It assumes, and does not respecify, the [packet-owned capture proposal](./artifact-version-substrate-for-pinned-operation-inputs.md): capture of an operation's start state, comparison of that capture against current state, and a guard that refuses a report-driven transition when they differ. This proposal is that design's first consumer and supplies one target-owned response to a changed input — `superseded`. Where the two overlap, the capture proposal is authoritative on capture, comparison, and guarding; this one is authoritative on what a disposition means and who resolves it.
...
The capture, comparison, diff, and guard mechanics are the [packet-owned capture proposal](./artifact-version-substrate-for-pinned-operation-inputs.md)'s. A capture path is never substituted for the logical path or passed to an assessment method. This proposal fixes which artifacts a full pass guards and what a failed guard means.
...
6. A live-source mismatch prevents both automatic application of a `keep` packet and execution of an old delete/merge recommendation; a live-target mismatch also prevents an old merge. The consumer preserves the live edit and can show each mismatch against the corresponding packet capture.
...
8. Rejection requires matching guarded inputs, keeps the note unchanged, records user authority and a rationale, and prevents unsupported repetition of the same recommendation during later synthesis; an opposite-direction merge can be recorded as `alternative-applied`.
...
- [Packet-owned captures for guarded operation transitions](./artifact-version-substrate-for-pinned-operation-inputs.md) — part-of: supplies the capture, comparison, diff, and guard contract this design consumes
