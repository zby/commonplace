...
- **Instruction versus command consumer.** The settled initial default is an instruction that lists, inspects, and resolves the small number of reports. The one exception is the version guard itself: the [packet-owned capture proposal](./packet-owned-captures-for-guarded-operation-transitions.md) requires it to be a deterministic command, because a comparison an agent is merely instructed to perform is the step whose omission silently reinstates the defect this design exists to prevent. Selection and resolution remain prose; the check that gates them does not. Promoting the rest of the consumer to a command becomes warranted when exact transition validation, concurrent actors, or report volume makes prose orchestration unreliable.
