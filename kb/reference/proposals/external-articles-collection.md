...
- Whether note→article lineage is a first-class link label (for example `adapted-from` with a source-side `Adapted into:` footer, which needs the source collection's authorization) or lives only on the article side as a source-notes list.
