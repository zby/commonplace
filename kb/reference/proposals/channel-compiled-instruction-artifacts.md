...
That is runtime parameterisation wearing prose clothing. The [adopted claim](../../notes/generate-instructions-at-build-time.md) rejects it in placeholder form for exactly the reasons that apply here — interpretation overhead on every use across every substitution site, with occasional silent misreads — and states the canonical form of a skill as standalone, with the templating step producing that form rather than leaking into the runtime artifact. A prose branch is a substitution site whose resolver is the LLM.
...
- Eleven promoted skills carry 28 shell fences, every one labelled `bash` or `sh` and none `powershell`. Two contain constructs that cannot run under PowerShell: a `test -d … && echo … || echo …` block with `2>/dev/null` in the health-check skill, and `xargs -r` in the connect skill, whose own prose flags the `-r` guard as load-bearing.
