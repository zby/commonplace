...
- [Collections and types](../collections-and-types.md#authoring-composition) describes the shipped design as read-time composition of three ordinary files, plus a document's optional write brief read through its validated `brief:` pointer (ADR 092). There is no resolver command or generated write-context packet.
...
- Write briefs were adopted with a validated sidecar pointer read by `cp-skill-write`, not through an assembler ([ADR 092](../adr/092-write-briefs-are-optional-sidecars-named-by-a-validated-pointer.md)). A brief could later become a named assembler role without changing the brief or its association, but briefs are not a reason to adopt this proposal.
