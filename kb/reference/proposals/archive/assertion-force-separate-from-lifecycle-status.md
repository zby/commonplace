...
- [A framework rule is a first principle only if no rival preserves its boundary invariants](../../../notes/framework-rule-is-a-first-principle-without-boundary-preserving-rival.md) — grounds: names status/lifecycle enum values as a demoting choice, not an inherited constraint, the premise the cube analysis builds on
