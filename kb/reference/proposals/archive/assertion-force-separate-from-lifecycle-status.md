...
- [A framework rule is an inherited constraint only if no rival preserves its boundary invariants](../../../notes/a-framework-rule-is-inherited-only-without-a-boundary-preserving-rival.md) — grounds: names status/lifecycle enum values as a demoting choice, not an inherited constraint, the premise the cube analysis builds on
