...
> **Archived** (see [archive README](./README.md)). Adopted by [ADR 057](../../adr/057-articles-use-an-editorial-profile-and-excluded-drafts.md). The live article rules now reside in [the article collection contract](../../../articles/COLLECTION.md) and the [publication procedure](../../../instructions/publish-an-article.md); [ADR 063](../../adr/063-all-article-drafts-circulate-behind-a-banner.md) governs draft circulation. [ADR 069](../../adr/069-collection-contract-bundles-become-one-time-prototypes.md) later retired the shared editorial-profile model. The first article remains a near-complete draft until separately authorized for publication. ADR 057 records the original choices and consequences; what remains here is the worked case's dated design texture.
...
- Writable collections are `kb/notes/` (theoretical profile), `kb/reference/` (descriptive), `kb/instructions/` (prescriptive), `kb/agent-memory-systems/` and `kb/agentic-systems/` (descriptive external-system coverage), `kb/sources/` (captured material), and `kb/work/` (workshop). Each is authored for an agent's bounded context. The three default profiles plus one promoted profile all assume that in-project consumer; new profiles are promoted only after a worked case ([ADR 042](../../adr/042-register-becomes-a-default-profile-under-open-ended-text-contracts.md)).
...
- An editorial/expository profile written and exercised in the worked case, then promoted to the profile catalogue at `kb/reference/text-contract-profiles.md` under the [ADR 042](../../adr/042-register-becomes-a-default-profile-under-open-ended-text-contracts.md) guard.
...
- **First post: reflective self-improving systems** — the KB's home territory. Distil what the [self-improving-systems cluster](../../../tags/self-improving-systems-README.md) establishes into one dated post for researchers and builders: what it means for a system to operate on its own methodology, and what Commonplace's worked experience says about making that loop governable. Draft it directly in the collection root with `status: draft` — the contract, type, and validation bind from the first commit, and the rendered badge marks the state.
...
- [Text contract](../../definitions/collection.md#text-contract) — defined-in: the binding local collection-contract concept this proposal applies to articles.
- [ADR 042: Register becomes a default profile under open-ended text contracts](../../adr/042-register-becomes-a-default-profile-under-open-ended-text-contracts.md) — see-also: the historical open, worked-case-gated profile model under which this proposal sought promotion.
