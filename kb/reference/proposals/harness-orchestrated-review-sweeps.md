...
5. **Finalize** — `commonplace-finalize-review-job --review-job-id ...` per job: strictly parse, persist all pair results, append acceptance events, and refresh inspection artifacts (deterministic, Python). Missing or malformed pair output fails the whole job.
...
- **Orchestration recipes are unversioned.** A thread pool in the package is tested code; a workflow script re-derived per run is not. Parse failures fail safely without advancing freshness, but recipe drift is invisible until a run misbehaves.
