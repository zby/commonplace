...
- [Stale indexes reduce discovery when they suppress fallback search](../../notes/stale-indexes-reduce-discovery-when-they-suppress-fallback-search.md) — rests-on: a marked-but-incomplete head can suppress the more complete sweep, which is why the marks must be machine-enforced or omitted
...
- [index-curation-adds-orientation-that-generation-cannot-produce](../../notes/index-curation-adds-orientation-that-generation-cannot-produce.md) — rests-on: the groupings and phrases are the durable value that survives a mark's drop
