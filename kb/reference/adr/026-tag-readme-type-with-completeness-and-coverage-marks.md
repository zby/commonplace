...
- [stale-indexes-are-worse-than-no-indexes](../../notes/stale-indexes-are-worse-than-no-indexes.md) — rests-on: the marked-but-incomplete head is the catastrophic state, which is why the marks must be machine-enforced or not exist
