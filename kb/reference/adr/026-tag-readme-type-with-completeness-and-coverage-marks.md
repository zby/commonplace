...
- A `complete` enumeration claim is worthless unless machine-enforced — a careful agent would re-run the by-tag `rg` sweep anyway, so an unenforced claim saves no work. This is the problem in [stale indexes reduce discovery when they suppress fallback search](../../notes/indexes-lower-recall-when-they-suppress-retrieval-that-would-find-more.md) in its sharpest form: a marked-but-incomplete head tells exhaustive consumers to stop looking while members are missing.
...
- [Stale indexes reduce discovery when they suppress fallback search](../../notes/indexes-lower-recall-when-they-suppress-retrieval-that-would-find-more.md) — rests-on: a marked-but-incomplete head can suppress the more complete sweep, which is why the marks must be machine-enforced or omitted
