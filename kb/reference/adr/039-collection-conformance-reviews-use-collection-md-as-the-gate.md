...
# 039-Collection-conformance reviews use COLLECTION.md as the gate
...
- **The gate id is the virtual `collection/{path}` lens; the persisted identity is the COLLECTION.md repo path.** `{path}` is the collection directory relative to `kb/`, so nested namespace collections stay unambiguous: `collection/notes` names `kb/notes/COLLECTION.md`, `collection/commonplace/notes` names `kb/commonplace/notes/COLLECTION.md`. Acceptance stays path-keyed; the shorthand is derived for display and CLI input.
