...
> **Partially superseded by [ADR 069](./069-collection-contract-bundles-become-one-time-prototypes.md):** the closed register taxonomy remains retired and local text contracts remain authoritative, but adopted profiles and their promotion catalogue are replaced by one-time-copy collection prototypes with no inheritance, conformance, or synchronization.

> **Definition placement superseded by [ADR 071](./071-text-contract-is-part-of-the-collection-definition.md):** this record preserves the rename and the then-current theory-definition placement; the collection definition now owns the surviving text-contract vocabulary.
...
1. **Retire "register."** Rename to **text contract** (the requirement — what a `COLLECTION.md` states) and **profile** (the library entry — a named, proven bundle). The definition note then at `kb/notes/definitions/register.md` is renamed to the theory-side `text-contract` definition and revised to define both terms in place of the single register concept. `AGENTS.md`'s vocabulary entry and each collection's `COLLECTION.md` header get the same mechanical rename. The concepts doing real analytical work — the theoretical profile's formulation constraint, the dialectical profile's mandatory evidentiality — carry over unchanged; only the taxonomy label retires.
...
Mechanical execution of items 1–3 shipped at the time: the definition note was renamed to `text-contract` and revised, the profile catalogue at `kb/reference/text-contract-profiles.md` was added, the foundation note was revised in place, and `AGENTS.md`'s vocabulary entry and routing table were updated. ADR 069 later retired the profile catalogue, and ADR 071 moved the surviving definition into the [collection definition](../definitions/collection.md#text-contract). The originating proposal was fully implemented, not merely superseded by a different design, and was removed rather than kept as a stub; this ADR remains its complete historical record, including the surface-decomposition mapping above, which the proposal asked to be recorded here on adoption.
...
- [Collection and text contract](../definitions/collection.md) — implemented-by: current owner of the surviving vocabulary this decision introduced
- [ADR 071: Text contract is part of the collection definition](./071-text-contract-is-part-of-the-collection-definition.md) — superseded-by: changes only the definition's collection and canonical owner
- [Artifact classification separates content kind, lineage, and authority](../../notes/artifact-classification-separates-content-kind-lineage-and-authority.md) — see-also: current successor to the foundation note whose in-place revision Decision 3 records
...
- [A universal knowledge framework demotes content taxonomies to defaults](../../notes/a-universal-knowledge-framework-demotes-content-taxonomies-to-defaults.md) — rests-on: the transferable claim this ADR instantiates for registers
