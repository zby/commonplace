...
> **Partially superseded by [ADR 069](./069-collection-contract-bundles-become-one-time-prototypes.md):** adopted profiles and their promotion catalogue are replaced by one-time-copy collection prototypes; the register retirement and local text-contract authority stand.

> **Definition placement superseded by [ADR 071](./071-text-contract-is-part-of-the-collection-definition.md):** the collection definition now owns the surviving text-contract vocabulary.
...
This ADR adopts four locked decisions:

1. **Retire "register."** Rename to **text contract** (the requirement — what a `COLLECTION.md` states) and **profile** (the library entry — a named, proven bundle). The concepts doing real analytical work — the theoretical profile's formulation constraint, the dialectical profile's mandatory evidentiality — carry over unchanged; only the taxonomy label retires.
...
- The rename spans several surfaces (definition note and its backlinks, `AGENTS.md` vocabulary and routing table, each `COLLECTION.md` header, the foundation note), so a missed surface leaves a stale term in a binding position.
...
- The decomposition of a text contract across system surfaces — `COLLECTION.md` for subtree-wide clauses, global type specs for shape-wide clauses, collection-local type specs for shape-that-only-exists-here clauses, traits for opt-in clauses (recorded above) — is unaffected in substance by this ADR. The transferable substrate behind it was promoted independently of the register/profile rename, in [directory placement is total, frontmatter classification is partial](../../notes/directory-placement-is-total-frontmatter-classification-is-partial.md).
