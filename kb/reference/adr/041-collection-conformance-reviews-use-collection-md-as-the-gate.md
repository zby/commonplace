...
The asymmetry is not just a coverage gap; it distorts contract design. When the type surface is the only natural-language contract with enforcement, clauses migrate to where the machinery is rather than where their scope says they belong — a convention that binds *everything in a collection* gets pushed into a type spec so that something checks it. Symmetric enforcement is what lets the collection/type split be decided by scope (what set of artifacts a clause binds) instead of by which surface happens to have teeth.

The snapshot table is role-neutral ([ADR 032](./032-review-freshness-uses-db-snapshots-not-git.md)), so any repo document can sit on the gate side with no storage change; ADR 038's consequences name `COLLECTION.md`-as-gate as the generalization path.
...
- **Pairs derive from note location, not the gate catalog.** One pair per note is derived from its position in the collection tree; the selector accepts the whole collection cohort or one collection's cohort alongside catalog gate ids and the `type` lens. A note outside any collection has no pair; a `COLLECTION.md` is contract, not content, and never pairs with itself.
- **The gate id is the virtual `collection/{path}` lens; the persisted identity is the COLLECTION.md repo path.** `{path}` is the collection directory relative to `kb/`, so collections under a non-collection namespace stay unambiguous. Collections do not nest — a `COLLECTION.md` inside another collection is invalid and the validator rejects it — so in a valid repo the nearest contract above a note is the only one. Acceptance stays path-keyed; the shorthand is derived for display and input.
- **Everything downstream of pair derivation is the existing machinery, unchanged**, exactly as in ADR 038: a `COLLECTION.md` edit flips `gate-changed` for exactly the notes in that collection, a note edit flips `note-changed`, and the existing ack command re-pins the current contract snapshot for trivial edits. No new selector reason, no new acceptance semantics, no storage change.
- **A mechanical wrapper renders the contract as a gate, referencing rather than embedding it**, as ADR 038's wrapper does for type specs: judge whether the note follows the collection's authoring conventions (placement, title and description conventions, quality goal, outbound linking rules), structural checks excluded as the validator's job, with an authored `## Review` section of the `COLLECTION.md` — where the freshness hash sees it — treated as the operative test.
...
- [factored dependency pairs for review freshness](../proposals/factored-dependency-pairs-for-review-freshness.md) — part-of: the proposal whose COLLECTION.md-as-gate item this ADR adopts
