...
1. The live-agent surface is single-note (`create-review-runs`) and per-run all-or-nothing (`ingest-bundle-output`), while the batch machinery (run preparation, pair prompt rendering, salvage finalization) existed only as library internals with no CLI.
...
1. **Batch-granular endpoints.** `commonplace.review.batch` plus two CLIs expose the executor's batch machinery to external executors:
   ...
   The salvage finalization is shared code (`executor.finalize_runs_from_parsed`), so subprocess and external execution cannot drift. The single-note live-agent flow (`create-review-runs`/`ingest-bundle-output`) remains all-or-nothing per returned run, but uses the same pair parser and artifact manifest contract.
