...
> **Current terminology:** [ADR 069](./069-collection-contract-bundles-become-one-time-prototypes.md) replaced the dialectical/evidential profile with a creation-time prototype; this validator remains generic and no collection inherits it.
...
Downstream KBs running the shipped validator receive the generic check with no collection-specific configuration; using the experimental dialectical/evidential prototype is neither required nor a source of inheritance.
...
**An open architectural question this decision surfaces rather than settles.** The dividing line in validation is *not* frontmatter versus body — the schema already validates the body. The parse model hands the schema the body, headings, links, and body dates, and several type schemas assert required headings. Anything about a note's own text that the parse model exposes is expressible declaratively, and is type-owned.
...
That class has no design. It has no shared model of a *positioned* element, no shared severity policy, and no owner. Sharing the code-fence primitive removes the immediate divergence without supplying one, and the shared parse remains lossy for it: it exposes links as a set of URLs without positions, which suffices for link health but not for proximity pairing, so the quote checker still carries its own link extraction. The gap belongs to the kb-graph-loader workshop — and it is the same gap, since a referential check *is* a graph edge being resolved.
