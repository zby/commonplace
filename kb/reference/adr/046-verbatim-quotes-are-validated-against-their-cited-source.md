...
**An open architectural question this decision surfaces rather than settles.** Validation now has two distinct check classes. Frontmatter checks are schema-driven, type-owned, and declarative. **Body-content checks — link health and verbatim quotes — are neither**: they are hand-written regex passes over prose, with no shared model of "an element inside a note," no shared severity policy, and no declared owner. Sharing the code-fence primitive removes the immediate divergence but does not give the class a design. The quote checker still carries its own link regex, because the shared parser yields link *URLs* without positions and proximity pairing needs offsets — so the shared parse is lossy for this consumer. That is a real gap, not a tidy-up: it belongs to the [kb-graph-loader](../../work/kb-graph-loader/README.md) question of whether validation, indexes, and review targeting should share one loaded note model.
