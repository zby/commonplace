...
The operator set these constraints for the choice:
...
The library stays in the installed package, and projects read it there. `commonplace-init` writes only small, machine-specific, uncommitted files that point to it.
...
- *Review identity.* A criterion file outside the repository, in the installed library, is identified relative to the library root, such as `commonplace:instructions/review-gates/prose/source-residue.md`, with no version or absolute path. A criterion file inside the repository keeps its repository-relative path; in the source checkout, where the library is the repository's own `kb/`, that covers the library too, so the checkout's baselines keep their identities. The identity's form decides where it resolves, with no fallback. In installed projects, init retires once the baselines recorded under the old copy's criteria; historical results keep their original identities.
- *Bare global type names.* A global type is named by its bare name (`type: note`) and resolves to `types/<name>.md` under the library root. Collection-local and project-shared types keep the path form (`./`, `../`, or `kb/…`). The form alone selects the resolver, with no fallback, so a bare name cannot be shadowed. Framework validation rules are keyed by bare name. A local schema that builds on a global schema refers to it as `commonplace:types/<name>.schema.yaml`, resolved in the library, so every artifact validates exactly as before. Gate `requires_type:` values name global types the same way.
...
**Library preparation at build time.** The package does not ship the raw trees. A build hook (`hatch_build.py`) stages a copy in which each link leaving the library points to the ingest's canonical source URL or, for any other repository file, to its GitHub URL; a link that resolves to nothing fails the build. The sdist and the wheel both carry the prepared copy.

**Migration.** Rerunning init in an existing project compares each old framework copy (`kb/commonplace/`, global types under `kb/types/`, copied `cp-skill-*` directories) with the installed version. A matching file is removed or replaced by a stub. A differing file is kept and listed for the operator, because it may carry a local change and the old copies have no version marker. Init never traverses or deletes through a symlink. It rewrites the project's frontmatter pointers to global types to bare names, and schema references to global schemas to `commonplace:types/...`, unless the project has its own file at that path, and lists every file it rewrote. It retires baselines recorded under the old copy's criteria, and it lists the skill directories whose old copies it turned into stubs, with the command to untrack them should version control still track them; it does not run git itself ([ADR 039](./039-tool-visibility-is-package-owned-and-git-is-never-invoked.md)).

## Considered alternatives

Each option below lost to a constraint or to evidence from the framework-delivery workshop's probes; the workshop's records remain in git history.
...
**Left open.** Whether a stale stub or an uncovered root makes commands stop or only warn. Stopping would not close the stale-read gap (see Consequences), so commands warn. Also open: where a project records its accepted Commonplace version range; a `[tool.commonplace]` table in `pyproject.toml` suits Python projects but not others.
...
**Separate changes.** A recorded PEP 440 version range per project, which would fill ADR 021's missing version marker, remains useful in any layout and is decided separately.
...
**Where the decision stops applying.** The evidence is probe revision 4 on Linux only. Claude Code and Codex passed every case, including the silent stale read after an install-mode switch. They differed once: in a clone without init, the Codex agent did not stop but answered from a nearby source checkout. The `AGENTS.md` template rule above answers that; it is untested. Replies from other harnesses (Gemini CLI, OpenCode, Cursor, Goose, GitHub Copilot, enterprise harnesses) to the shared probe were still pending at adoption. Untested and outside the decision's evidence: Windows and macOS, the router skill with a full-size index, upgrades during a running session, centrally managed settings, and skills whose harness reads metadata beyond name and description from the skill directory. The design assumes seven harness properties (H1–H7): a user-level uv tool install, something that loads standing instructions into every session (the pointer line names no machine path or project, so a user-level or administrator-set prompt serves when `AGENTS.md` is not loaded), file reads outside the project, following an instruction to read a file by absolute path, project-level Agent Skills discovery (skills layer only), a shell (commands only), and sub-agents with a fresh context (skills that need workers). A harness that lacks H1, H3, or H4 is outside this decision and needs an operator decision of its own. In a harness without sub-agents, sub-agents are emulated, so users of such harnesses can start using Commonplace (operator decision, 2026-09-24). The emulation is designed per harness from probe results, at the cost of weaker or slower separation and possibly a step for the user; adopting one requires `cp-skill-ingest` to accept it, since its rules allow only a harness-provided sub-agent today. Revisit the stubs when the required harnesses can discover skills from a configurable location on every platform; the stubs and their checks can then be deleted.
