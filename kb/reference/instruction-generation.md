...
An `.envrc` generated by an older Commonplace release is outside the current manifest, so re-running init preserves it. Init reports an exact legacy generated file as removable only after the operator verifies the user-level tool in fresh processes; it flags edited `.envrc` files for manual review.
