---
description: Commonplace's current build-time instruction generation flow — scaffold trees, template substitution, the `commonplace-init` entry point, and the specific generated artifacts
type: note
tags: [architecture]
status: current
---
...
How commonplace currently instantiates build-time generation over runtime parameterisation. This is a current-state description of the scaffold, template substitution, and install entry point. For the general argument that build-time generation beats runtime variables, see [generate instructions at build time](../notes/generate-instructions-at-build-time.md).

## The entry point

`commonplace-init` (`src/commonplace/cli/init_project.py`) is the single build/install step. It creates the project directory structure, copies scaffold trees verbatim, and resolves a small set of templates with per-project values. Everything a new commonplace project needs is produced by this one command.
...
**Scaffold trees** (from `SCAFFOLD_TREES`) — copied verbatim from `src/commonplace/scaffold/`:

- `kb/instructions/` — skill definitions and procedural guidance
- `kb/types/` — global base types (`note`, `text`)
- `kb/reference/` — the reference collection's type definitions and starter README (once the reference scaffold lands per move-plan sub-effort 1)
...
- `src/commonplace/assets/qmd-collections.yml` → `qmd-collections.yml` with `/PATH/TO/COMMONPLACE/` resolved to the actual root path
...
- The live commonplace `AGENTS.md` (what you're reading from) — the template targets practitioner projects; this repo's own control-plane file is authored directly
- The `mkdocs.yml` nav — updates to reflect new reference pages are manual
...
- [generate-instructions-at-build-time](../notes/generate-instructions-at-build-time.md) — theory: the general argument that build-time generation beats runtime parameterisation
...
- [architecture](./architecture.md) — current-state: where the generation pipeline sits inside the broader repo layout
- [control-plane-goals](./control-plane-goals.md) — current-state: how the generated `AGENTS.md.template` carries the `## KB Goals` section for practitioners to fill in
