...
- **[cp-skill-compile-collections](../../.claude/skills/cp-skill-compile-collections/SKILL.md)** — the skill that reads every `COLLECTION.md`, distills the registers, quality goals, and outbound linking tables into the topology document, and writes it to `kb/reports/collection-topology.md`. Run it when a `COLLECTION.md` changes or before a connection sweep.
- **[cp-skill-connect](../../.claude/skills/cp-skill-connect/SKILL.md)** — the skill that loads the topology document during discovery so it can pick relationship labels appropriate to the source/target register pair without re-reading every collection's conventions.
