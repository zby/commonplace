...
- `--all-gates` selects every applicable review criterion in place of naming ids/bundles (mutually exclusive with them): all catalog gates plus each typed note's type-conformance pair. The flag means the same thing in every review command. Like every selector flag it only *chooses* pairs — the selected `(note, gate)` pairs still run through create-jobs → review → finalize; it is not a one-shot "run all gates" command
