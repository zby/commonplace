...
A tag-README may declare one validator-enforced frontmatter mark (ADR 026, amended by [ADR 090](./adr/090-one-completeness-mark-reaches-members-in-one-hop.md)): `complete: true` — every artifact carrying the tag is linked from the head or carries a tag whose head is linked from it, so the head reaches every member in one hop and a reader can skip the by-tag `rg` recipe below for that tag. It ranges over the participating collections declared in [`kb/tags/COLLECTION.md`](../tags/COLLECTION.md) ([ADR 089](./adr/089-tags-are-one-namespace-per-kb-with-heads-in-kb-tags.md)). It is an accelerator, never load-bearing: scoped `rg` always recovers membership regardless of any mark — full semantics in the [`tag-readme` type spec](../types/tag-readme.md).
