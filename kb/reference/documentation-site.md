...
`properdocs.yml` also carries a `redirect_maps` block (preserves external URLs across note renames; written by `commonplace-relocate-note`) and `exclude_docs`. The exclusion keeps generated reports, the ignored `kb/sources/.snapshots/` cache, and a few workshop fixtures out of the build. Ingest analyses under `kb/sources/` still render; their frontmatter retains source identity and the exact local-snapshot checksum without republishing the captured page. Content-collection `dir-index.md` and per-tag listing pages are generated at build time only (ADR 025), covered in [storage-architecture.md](./storage-architecture.md).
