...
- [Commonplace agent memory gap plan](./commonplace-agent-memory-gap-plan.md) *(note)* - Plan for closing Commonplace's current agent-memory gaps: session traces, candidates, cue activation, behavioral evaluation, lifecycle, source alignment, import, ranking, and authority
...
- [Link vocabulary and linking approach](./link-vocabulary.md) *(note)* - Commonplace's linking approach: collection-owned outbound rules, reader-need labels, articulation tests, connect reports, and the shared label catalogue for COLLECTION.md authors
- [Navigation](./navigation.md) *(note)* - How agents navigate a Commonplace KB using control-plane pointers, rg, titles/descriptions, indexes, links, connect reports, and possible future search layers
...
- [Scenario architecture](./scenario-architecture.md) *(note)* - Scenario-derived shipped architecture — single KB root with a library/user split under kb/commonplace/, package-provided commands, promoted skills, and a measurable scenario decomposition
