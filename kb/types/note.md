...
[text](./text.md) files have implicit `status: seedling`. When a text file gains frontmatter (promotion to `note`), status should be set explicitly. `/cp-skill-convert` promotes structure (`text` → `note`) while preserving provisionality by setting `status: seedling`, not `status: current`. Human review flips the status to `current`.
