...
Every Markdown artifact in this collection other than `COLLECTION.md`, `README.md`, and `text.md` is a type spec and carries `type: type-spec`. The root [`type-spec.md`](./type-spec.md) is self-referential. `COLLECTION.md` governs local authoring and routing, `README.md` is curated navigation, `text.md` documents the implicit no-frontmatter artifact type, and schema files are symbolic sidecars rather than Markdown artifacts.
