...
- Optional `requires_trait` or `requires_type` — narrow the gate to a subset of artifacts that carry the given trait or type. A `requires_type` value is written like a `type:` value, such as `types/definition.md`.
