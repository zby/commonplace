...
def _scope_roots(file_path: Path, repo_root: Path) -> list[Path]:
    try:
    ...
    if len(parts) >= 3 and parts[0] == "kb" and parts[1] == "work":
        scopes.append(repo_root / "kb" / "work" / parts[2])
    if len(parts) >= 2 and parts[0] == "kb":
        scopes.append(repo_root / "kb" / parts[1])
    scopes.append(repo_root)
    return scopes


def _definition_path(type_name: str, file_path: Path, repo_root: Path) -> Path | None:
    for scope_root in _scope_roots(file_path, repo_root):
...
def _resolve_known_type(type_name: str, file_path: Path, repo_root: Path, seen: set[str]) -> TypeProfile:
    if type_name in seen:
        chain = " -> ".join([*sorted(seen), type_name])
        raise ValueError(f"cyclic type inheritance: {chain}")

    definition_path = _definition_path(type_name, file_path, repo_root)
    if definition_path is None:
        ...
        return _resolve_known_type("note", file_path, repo_root, seen | {type_name})
    ...
    if type_name == "text":
        ...
        base_profile = _resolve_known_type(base, file_path, repo_root, seen | {type_name})
    elif type_name == "note":
        base_profile = TypeProfile(resolved_type="note", definition_path=definition_path)
    else:
        base_profile = _resolve_known_type("note", file_path, repo_root, seen | {type_name})
...
def resolve_type(
    ...
    *,
    ...
    return _resolve_known_type(type_name, file_path, repo_root, set())
