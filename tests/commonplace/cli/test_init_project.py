...
def test_installation_warnings_report_exact_legacy_envrc(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (tmp_path / ".envrc").write_bytes(LEGACY_GENERATED_ENVRC)
    ...
    assert lines
    assert any("exact .envrc generated by an older" in line for line in lines)
    assert any("review and remove it" in line for line in lines)


def test_installation_warnings_recognize_windows_legacy_envrc(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (tmp_path / ".envrc").write_bytes(
        LEGACY_GENERATED_ENVRC.replace(b"\n", b"\r\n")
    )
    monkeypatch.setattr(init_project_module, "_installed_command_names", lambda: ())
    monkeypatch.setattr(init_project_module, "_uv_tool_bin", lambda: None)
...
def test_installation_warnings_preserve_edited_legacy_envrc(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    (tmp_path / ".envrc").write_text(
        'export PATH="$PWD/.venv/bin:$PATH"\nexport PROJECT_SETTING=1\n',
        encoding="utf-8",
    )
    monkeypatch.setattr(init_project_module, "_installed_command_names", lambda: ())
    monkeypatch.setattr(init_project_module, "_uv_tool_bin", lambda: None)
    ...
    assert any("edited .envrc" in line for line in lines)
    assert any("will not alter edited files" in line for line in lines)
...
def test_main_emits_command_installation_diagnostics(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    (tmp_path / ".envrc").write_bytes(LEGACY_GENERATED_ENVRC)
    monkeypatch.setattr(init_project_module, "_installed_command_names", lambda: ())
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert "Command installation diagnostics:" in captured.out
    assert "exact .envrc generated by an older" in captured.out
