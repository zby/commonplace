...
class TestGateIdPlumbing:
    def test_collection_shorthand_resolves_to_collection_md(self, tmp_path: Path) -> None:
        ...
        assert normalize_gate_path(tmp_path, "collection/notes") == "kb/notes/COLLECTION.md"

    def test_collection_md_repo_path_normalizes(self, tmp_path: Path) -> None:
        build_fixture(tmp_path)
        assert normalize_gate_path(tmp_path, "kb/notes/COLLECTION.md") == "kb/notes/COLLECTION.md"

    def test_gate_id_for_collection_md_path_uses_collection_lens(self, tmp_path: Path) -> None:
        build_fixture(tmp_path)
        assert gate_id_for_path(tmp_path, "kb/notes/COLLECTION.md") == "collection/notes"

    def test_stored_collection_md_path_renders_collection_lens_without_fs(self) -> None:
        assert gate_id_from_stored_path("kb/notes/COLLECTION.md") == "collection/notes"
        assert gate_id_from_stored_path("kb/commonplace/notes/COLLECTION.md") == "collection/commonplace/notes"
        assert gate_id_from_stored_path("kb/instructions/review-gates/prose/source-residue.md") == (
    ...
    def test_missing_collection_contract_errors(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(FileNotFoundError, match="collection/nonexistent"):
            resolve_collection_gate_id(tmp_path, "collection/nonexistent")

    def test_collection_gate_id_rejects_traversal(self, tmp_path: Path) -> None:
        with pytest.raises(ValueError, match="invalid collection gate id"):
            resolve_collection_gate_id(tmp_path, "collection/../secrets")
        with pytest.raises(ValueError, match="invalid collection gate id"):
            resolve_collection_gate_id(tmp_path, "collection/")

    def test_non_collection_paths_are_not_collection_gate_paths(self) -> None:
        assert not is_collection_md_gate_path("kb/COLLECTION.md")
        assert not is_collection_md_gate_path("kb/notes/types/COLLECTION.md")
        assert not is_collection_md_gate_path("kb/notes/plain.md")
        assert not is_collection_md_gate_path("docs/COLLECTION.md")
        assert is_collection_md_gate_path("kb/notes/COLLECTION.md")
        assert is_collection_md_gate_path("kb/commonplace/notes/COLLECTION.md")
...
class TestSelectorCollectionPairs:
    def test_collection_request_derives_pair_from_note_location(self, tmp_path: Path) -> None:
        ...
        assert [(s.note_path, s.gate_path, s.gate_id, s.reason) for s in stale] == [
    ...
    def test_collection_path_request_filters_to_cohort(self, tmp_path: Path) -> None:
        ...
        stale = review_target_selector.select_stale_gates(
            tmp_path,
            model=TEST_MODEL,
            gate_ids=["collection/reference"],
            current_only=True,
        )
        assert [(s.note_path, s.gate_id) for s in stale] == [
            ("kb/reference/doc.md", "collection/reference"),
        ]

    def test_fresh_collection_pair_is_not_selected(self, tmp_path: Path) -> None:
        build_fixture(tmp_path)
        seed_acceptance(tmp_path, note_path="kb/notes/plain.md", gate_path="kb/notes/COLLECTION.md")
        stale = review_target_selector.select_stale_gates(
            tmp_path,
            model=TEST_MODEL,
            gate_ids=["collection"],
    ...
    def test_collection_md_edit_marks_cohort_gate_changed(self, tmp_path: Path) -> None:
        fixture = build_fixture(tmp_path)
        seed_acceptance(tmp_path, note_path="kb/notes/plain.md", gate_path="kb/notes/COLLECTION.md")
        seed_acceptance(tmp_path, note_path="kb/reference/doc.md", gate_path="kb/reference/COLLECTION.md")
        ...
        assert [(s.note_path, s.gate_id, s.reason) for s in stale] == [
            ("kb/notes/plain.md", "collection/notes", "gate-changed"),
        ]
    ...
    def test_note_edit_marks_collection_pair_note_changed_with_diff(self, tmp_path: Path) -> None:
        ...
        assert [(s.gate_id, s.reason) for s in stale] == [("collection/notes", "note-changed")]
...
""",
    ...
    def test_requested_mode_emits_collection_pairs(self, tmp_path: Path) -> None:
        build_fixture(tmp_path)
        requested = review_target_selector.select_requested_gates(
            tmp_path,
            gate_ids=["collection"],
            current_only=True,
        )
        assert [(s.note_path, s.gate_id, s.reason) for s in requested] == [
...
class TestAckCollectionPair:
    def test_ack_after_collection_md_edit_repins_current_contract(self, tmp_path: Path) -> None:
        ...
        assert [s.reason for s in stale_before] == ["gate-changed"]
...
class TestPromptWrapper:
        ...
        prompt = render_pairs_prompt(
            notes=[
                NoteReviewTarget(
                    ...
                    review_job_id=1,
                    gate_paths=("kb/notes/COLLECTION.md",),
    ...
    def test_collection_md_gate_needs_no_gate_text(self) -> None:
        prompt = render_pairs_prompt(
            notes=[
                NoteReviewTarget(
                    note_path="kb/notes/plain.md",
        ...
        prompt = render_pairs_prompt(
            notes=[
                NoteReviewTarget(
                    note_path="kb/notes/plain.md",
                    review_job_id=1,
                    gate_paths=("kb/instructions/review-gates/prose/source-residue.md",),
                    note_text="# Plain\n\nBody.",
                )
            ],
...
class TestCreateJobsForCollectionPairs:
    def test_selector_json_feeds_create_review_jobs(self, tmp_path: Path) -> None:
        ...
        assert [(pair["gate_path"], pair["gate_id"]) for pair in pairs] == [
        ...
        selector_json = json.dumps(
            {
                ...
                "targets": [
                    {
                        "note_path": "kb/notes/plain.md",
                        "gate_path": "kb/reference/COLLECTION.md",
                        "gate_id": "collection/reference",
                        "reason": "requested",
                    }
                ],
            }
