...
class TestGateIdPlumbing:
    def test_type_shorthand_resolves_to_global_type_spec(self, tmp_path: Path) -> None:
        ...
        assert normalize_gate_path(tmp_path, "type/definition") == "kb/types/definition.md"

    def test_type_spec_repo_path_normalizes(self, tmp_path: Path) -> None:
        build_fixture(tmp_path)
        assert normalize_gate_path(tmp_path, "kb/types/definition.md") == "kb/types/definition.md"

    def test_gate_id_for_type_spec_path_uses_type_lens(self, tmp_path: Path) -> None:
        build_fixture(tmp_path)
        assert gate_id_for_path(tmp_path, "kb/types/definition.md") == "type/definition"

    def test_stored_type_spec_path_renders_type_lens_without_fs(self) -> None:
        assert gate_id_from_stored_path("kb/types/definition.md") == "type/definition"
        assert gate_id_from_stored_path("kb/notes/types/structured-claim.md") == "type/structured-claim"
    ...
    def test_collection_local_type_resolves_when_unique(self, tmp_path: Path) -> None:
        ...
        assert resolve_type_gate_id(tmp_path, "type/structured-claim") == "kb/notes/types/structured-claim.md"
    ...
    def test_global_type_spec_wins_over_collection_local(self, tmp_path: Path) -> None:
        ...
        assert resolve_type_gate_id(tmp_path, "type/definition") == "kb/types/definition.md"
    ...
    def test_ambiguous_collection_local_type_errors(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValueError, match="ambiguous"):
            resolve_type_gate_id(tmp_path, "type/adr")

    def test_missing_type_spec_errors(self, tmp_path: Path) -> None:
        (tmp_path / "kb").mkdir()
        with pytest.raises(FileNotFoundError, match="type/nonexistent"):
            resolve_type_gate_id(tmp_path, "type/nonexistent")

    def test_type_gate_id_rejects_nested_names(self, tmp_path: Path) -> None:
        with pytest.raises(ValueError, match="invalid type gate id"):
            resolve_type_gate_id(tmp_path, "type/nested/name")

    def test_review_gate_catalog_paths_are_not_type_spec_paths(self) -> None:
        assert not is_type_spec_gate_path("kb/instructions/review-gates/types/sneaky.md")
        assert not is_type_spec_gate_path("kb/types/definition.schema.yaml")
        assert is_type_spec_gate_path("kb/types/definition.md")
...
class TestSelectorTypePairs:
    ...
    def test_type_pair_supports_model_agnostic_missing_review(self, tmp_path: Path) -> None:
        ...
        assert [(s.gate_id, s.reason) for s in stale] == [("type/definition", "missing-review")]

    def test_fresh_type_pair_is_not_selected(self, tmp_path: Path) -> None:
        build_fixture(tmp_path)
        seed_acceptance(tmp_path, note_path="kb/notes/definition.md", gate_path="kb/types/definition.md")
        stale = review_target_selector.select_stale_gates(
            tmp_path,
            model=TEST_MODEL,
            gate_ids=["type"],
    ...
    def test_type_spec_edit_marks_cohort_gate_changed(self, tmp_path: Path) -> None:
        fixture = build_fixture(tmp_path)
        seed_acceptance(tmp_path, note_path="kb/notes/definition.md", gate_path="kb/types/definition.md")
        seed_acceptance(tmp_path, note_path="kb/notes/plain.md", gate_path="kb/types/note.md")
        ...
        assert [(s.note_path, s.gate_id, s.reason) for s in stale] == [
            ("kb/notes/definition.md", "type/definition", "gate-changed"),
        ]
    ...
    def test_note_edit_marks_type_pair_note_changed_with_diff(self, tmp_path: Path) -> None:
        ...
        assert [(s.gate_id, s.reason) for s in stale] == [("type/definition", "note-changed")]
    ...
    def test_mixed_catalog_and_type_requests(self, tmp_path: Path) -> None:
        ...
        stale = review_target_selector.select_stale_gates(
            tmp_path,
            model=TEST_MODEL,
            gate_ids=["prose/source-residue", "type"],
            note_filter=["kb/notes/plain.md"],
        )
...
class TestPromptWrapper:
    def test_type_spec_gate_is_referenced_not_embedded(self) -> None:
        prompt = render_pairs_prompt(
            ...
            gate_texts={"kb/types/definition.md": "# Definition\n\n## Authoring Instructions\n\nSharpen the term."},
            result_kind="verdict",
        )
        ...
        selector_json = json.dumps(
            {
                ...
                "targets": [
                    {
                        "note_path": "kb/notes/plain.md",
                        "gate_path": "kb/types/definition.md",
                        "gate_id": "type/definition",
                        "reason": "requested",
                    }
                ],
            }
