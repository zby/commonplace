...
from tests.commonplace.review.pair_helpers import accept_pair, criterion_path_for_test
...
def _insert_completed_job(
    ...
    *,
    ...
    return job_id, {
        criterion_id: pair_ids_by_path[criterion_path_for_test(criterion_id)]
        for criterion_id in criterion_ids
    }
...
def test_inline_prune_keeps_bundled_job_until_all_pairs_are_superseded(tmp_path: Path) -> None:
    ...
    with review_db.connect(db_path) as conn:
        ...
        residue_job_id, residue_pairs = _insert_completed_job(
            conn,
            note_path="kb/notes/mixed.md",
        ...
        superseded_residue = accept_pair(
            conn,
            review_pair_id=residue_pairs["prose/source-residue"],
            note_path="kb/notes/mixed.md",
            criterion_id="prose/source-residue",
            model_partition=MODEL_PARTITION,
        ...
        first_deleted_jobs = review_db.prune_superseded_freshness_baselines(
            conn, [superseded_residue]
        )
        ...
        deleted_residue_pair = conn.execute(
            "SELECT review_pair_id FROM review_pairs WHERE review_pair_id = ?",
    ...
    assert deleted_residue_pair is None
    assert second_deleted_jobs == {bundled_job_id}
    assert review_job_ids == {residue_job_id, grounding_job_id}
