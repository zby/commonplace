...
The rendered-site homepage is [kb/index.md](./kb/index.md). Start with [deploy-time learning](./kb/notes/deploy-time-learning-is-the-missing-middle.md). The foundational vocabulary proceeds from [actionable methodology](./kb/notes/definitions/actionable-methodology.md), through [reflective system](./kb/notes/definitions/reflective-system.md), to [graded reflective coverage across representational forms](./kb/notes/reflective-coverage-is-graded-across-representational-forms.md), [the proposal-selection improvement loop](./kb/notes/a-proposal-selection-loop-requires-search-evaluation-and-retention.md), and [closure under recommendations](./kb/notes/a-methodology-governs-its-own-extension-only-as-far-as-it-settles.md). The organizing thesis: [an improvement loop closes by converting improvised decisions into governed machinery](./kb/notes/an-improvement-loop-closes-by-converting-improvised-decisions.md); [Commonplace as a reflective system](./kb/reference/commonplace-as-a-reflective-system.md) is the observed instance.

On structure: a wiki accretes it. Commonplace hardens only what an inherited constraint forces and leaves the rest collection-local and replaceable, [since task-fitted structure costs cross-task reuse](./kb/notes/task-fitted-structure-costs-cross-task-reuse.md).
