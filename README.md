...
**Research on reflective self-improvement, running as one.**

An agent can read its own source — the prompts that steer it, the instruction files it obeys, the code of its validators — so a system can improve itself by analyzing and rewriting its own definition. That is **[reflective self-improvement](https://zby.github.io/commonplace/articles/reflective-self-improvement/)**, and Commonplace is a framework for doing it systematically: an agent-operated knowledge base whose contracts, validation, and review gates aim the loop at theories with [explanatory reach](https://zby.github.io/commonplace/notes/first-principles-reasoning-selects-for-explanatory-reach-over/) rather than a pile of retained lessons. This repository is the first deployment — the methodology lives here as a Commonplace knowledge base and improves through it.

Behind that claim is a growing body of research on how to build the most powerful agentic systems. The bet is simple: an LLM can consume a theory and act on it, and can even write the code that theory calls for — so an _actionable_ theory of how to build such systems is itself a way of building them. Commonplace aims to be that theory: one closed under its own recommendations, telling an agent when to reason from prose, when to freeze knowledge into durable code, and how to verify what it produces.
...
LLM agents follow the methodology the notes lay out to maintain the wiki those notes live in. The content is AI-generated throughout — a human directs the inquiry; agents (Claude, ChatGPT, and others) draft, connect, and maintain the notes.
...
The methodology notes live under `kb/notes/`; the rendered-site homepage is [kb/index.md](./kb/index.md). The short outward statement of the whole position is the article [Reflective self-improvement](https://zby.github.io/commonplace/articles/reflective-self-improvement/); for the theory, start with [deploy-time learning](./kb/notes/deploy-time-learning-is-the-missing-middle.md). The foundational vocabulary proceeds from [actionable methodology](./kb/notes/definitions/actionable-methodology.md), through [reflective system](./kb/notes/definitions/reflective-system.md), to [graded reflective coverage across representational forms](./kb/notes/reflective-coverage-is-graded-across-representational-forms.md), [governed adaptation](./kb/notes/governed-adaptation-requires-search-evaluation-and-retention.md) — search, evaluation, and operative retention — and [closure under recommendations](./kb/notes/a-methodology-governs-its-own-extension-only-as-far-as-it-settles.md). Each property can hold without the others; [Commonplace as a reflective system](./kb/reference/commonplace-as-a-reflective-system.md) applies them to this repository.

On structure: a wiki accretes it, because structure is cheap to add and free to leave. Commonplace hardens only what an inherited constraint forces and leaves the rest collection-local and replaceable, [since task-fitted structure costs cross-task reuse](./kb/notes/task-fitted-structure-costs-cross-task-reuse.md).
