...
- **Conjectural learning** — learning in which formulated tentative theories are operative in the learning system: a theory guides decisions through what it says, it is criticized for what it says, and the result of that criticism improves the system's capacity for future action. Exercising the process without that improvement is an attempt to learn. Addressability and retaining the theory rather than reconstructing it are conjectured benefits, and fixed weights are a study condition; none is a condition of the term. Replaced the KB's earlier *theory refinement* and *learning by theory refinement* on 2026-09-21. See `kb/notes/definitions/conjectural-learning.md`.
...
- **Externally tested theory builder** — a theory builder whose evidence interface supplies an external falsifier, an external objective, and an outcome level independent of its own evaluators, so outcome comparison can proceed without first settling its internal warrant. Commonplace producing a KB for consuming projects is the arrangement the research program tests; a software house is a different arrangement with the same interface, kept as a companion approach for comparison, not as the main path. See `kb/notes/definitions/externally-tested-theory-builder.md`.
...
- **Theory builder** — the complete persistent system responsible for developing and revising addressable tentative theories about the subjects it investigates, including every person or component in an internal theory-building role; identified by continuity of responsibility and lineage, not by any component, and assessed against a declared boundary, seed, and evidence interface. *Reflective* and *autonomous* are its two independent conditions. See `kb/notes/definitions/theory-builder.md`.
...
- **YAGNI** — don't implement features that aren't needed yet. If you identify a gap, write it down instead of implementing it: a system feature or design gap becomes a design proposal in `kb/reference/proposals/` (see its README for the contract); a transferable insight becomes a note in `kb/notes/`; finished theory the KB has not decided to import into its vocabulary (a framework, a distinction, new terms, a change to an adopted definition) becomes a theory proposal in `kb/notes/proposals/` (see its README for the contract).
