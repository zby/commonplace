...
- **Acceptance** — the current snapshot-pinned freshness baseline for one `(note, criterion, model partition)` pair. It records completed evidence, not global approval; a fresh report is not “handled.” See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
- **Assay** — any snapshot-anchored LLM evaluation executed through the review job pipeline. Verdict-kind gates decide; report-kind assays complete with reusable analysis and no decision. See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
...
- **Criterion** — the instruction text applied to a note in an assay. It occupies the persisted `gate_path` side of a review pair; a gate is a bounded verdict-kind criterion, while critique is a report-kind criterion. See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
...
- **Gate** — a bounded, verdict-kind assay criterion. `gate_path` remains the persisted name for the criterion side of every review pair, including report assays. See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
...
- **Result kind** — the persisted pair protocol: `verdict` completes with a decision; `report` completes without one. `REPORT` is a completion marker, not a fifth decision. See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
