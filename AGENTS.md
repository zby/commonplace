...
- **Criterion** — the instruction text applied to a note in an assay. It occupies the persisted `gate_path` side of a review pair; a gate is a closed-ended, verdict-kind criterion, while critique is an open-ended, report-kind criterion. See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
...
- **Gate** — a closed-ended, verdict-kind assay criterion. `gate_path` remains the persisted name for the criterion side of every review pair, including report assays. See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
