...
4. Run `python3 skills/validate/validate_notes.py <note-path>` to check structure, frontmatter, and links.
...
| Review a note or run a review sweep | `kb/instructions/REVIEW-SYSTEM.md` | Experimental opt-in operational path — use only when the task is explicitly about review, triage, ack, or sweep work |
| Fix review warnings | `kb/instructions/FIX-SYSTEM.md` | Complement to the review system — applies fixes from warn findings |
...
Use `python3 skills/validate/validate_notes.py all` for batch validation audits.
...
- The task is explicit review work (single-note review, triage, ack, or sweep) -> read `kb/instructions/REVIEW-SYSTEM.md` and follow the referenced instruction instead of folding review into the default write path.
- The task is fixing review warnings -> read `kb/instructions/FIX-SYSTEM.md` and follow the referenced instruction instead of improvising fixes.
