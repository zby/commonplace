...
- **Externally tested theory builder** — a theory builder whose evidence interface supplies an external falsifier, an external objective, and an outcome level independent of its own evaluators, so outcome comparison can proceed without first settling its internal warrant; a software house is the paradigm arrangement, and Commonplace producing a KB for consuming projects is the first arrangement the research program tests. See `kb/notes/definitions/externally-tested-theory-builder.md`.
