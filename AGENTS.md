...
- **Freshness baseline** — the current snapshot-pinned applicability boundary for one `(note, criterion, model partition)` key. It preserves an evidence review pair while tracking the latest note and criterion snapshots through which that evidence remains fresh; it is not endorsement or global approval. See `kb/reference/README-REVIEW-SYSTEM.md#concepts`.
