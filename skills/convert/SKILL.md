...
- `traits` is always `[]` — trait assignment is semantic work, done later by the `validate` skill or human review.
- `tags` is always `[]` — tag assignment is semantic work, done later by the `connect` skill or human review.
...
The filename should match the title — whether the title itself is good is a semantic question for the `validate` skill.
...
Next steps:
- Run the `connect` skill on `new-filename.md` — find connections
- Run the `validate` skill on `new-filename.md` — check quality
...
Next steps:
- Run the `validate` skill on `filename.md` — check quality
- Review and set status: current when endorsed
===
```
