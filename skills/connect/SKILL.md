...
2. **Check note type.** If the target has no frontmatter (a `text` file), run the `convert` skill on it first to add frontmatter with `status: seedling`.
...
**Orphan discovery:** If you encounter unconnected notes during search, flag them. Do not attempt to connect them inside the same run — that is separate connection work.
