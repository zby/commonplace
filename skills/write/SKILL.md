...
Run the `validate` skill on the new file after writing. If validation finds structural issues, fix them before stopping.

7. Finish by prompting for connection work.
Tell the user where the file was written and suggest running the `connect` skill on the new file as the next step.
