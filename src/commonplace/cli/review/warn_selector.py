...
def main(argv: list[str] | None = None, *, cwd: Path | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Select current actionable findings from effective warn reviews "
            "and report stale warn pairs separately."
        )
    )
    ...
    notes, stale_pairs = scan_reviews(repo_root, note_filter, db_path=db_path)
    if not notes and not stale_pairs:
        print("[]" if args.json else "No warn findings found.")
        return 0

    if args.json:
        print(render_json(notes, stale_pairs))
    else:
        print(render_grouped(notes, stale_pairs))
    return 0
