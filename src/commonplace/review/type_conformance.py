...
`type/{name}` lens; the persisted criterion identity is the type-spec repo path.
"""
...
def resolve_type_criterion_id(repo_root: Path, criterion_id: str) -> str:
    ...
    candidates = sorted(
        ...
        if path.is_file() and is_type_spec_criterion_path(path.relative_to(repo_root).as_posix())
    )
    if not candidates:
        raise FileNotFoundError(f"type spec not found for gate id: {criterion_id}")
    if len(candidates) > 1:
        rendered = ", ".join(path.relative_to(repo_root).as_posix() for path in candidates)
        raise ValueError(f"type gate id is ambiguous across collections: {criterion_id} ({rendered})")
